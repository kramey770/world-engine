...
const creationTools: Array<{
...
}> = [
  {
    ...
    idleDescription: "Add a new settlement using Azgaar's native placement rules.",
  },
  {
    id: "marker",
    label: "Marker",
    instruction: "Click the map to place a point of interest.",
    idleDescription: "Place a marker using Azgaar's native map interaction.",
  },
]
  ...
  useEffect(() => {
    function handleEngineMessage(event: MessageEvent) {
      ...
      if (event.data.type === "creation:mode") {
        setCreationState(event.data.active ? { tool: event.data.tool, active: true } : null)
      }
        ...
        {activeCategory === "Layers" ? (
          ...
          <section className="absolute left-3 top-3 z-10 w-[min(21rem,calc(100%-1.5rem))] rounded-xl border border-border/80 bg-card/95 p-4 shadow-xl backdrop-blur-xl sm:left-5 sm:top-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-sm font-semibold text-foreground">Create on the map</h2>
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                  Start a focused creation tool, then place the new entity directly on the map.
                </p>
              </div>
            </div>
            {creationTools.map((tool) => {
              ...
              className={`mt-4 w-full rounded-lg border px-3 py-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-wait disabled:opacity-60 ${
                isActive
                  ? "border-primary/50 bg-primary/10"
                  : "border-border/70 bg-background/60 hover:border-primary/30 hover:bg-muted"
              }`}
            >
              <span className="block text-xs font-semibold text-foreground">
                  {isActive ? `Place ${tool.label.toLowerCase()} on map` : tool.label}
              </span>
              <span className="mt-1 block text-[10px] leading-snug text-muted-foreground">
                  {isActive ? tool.instruction : tool.idleDescription}
              </span>
              </button>
              )
            })}
            ...
            <p className="mt-4 border-t border-border/70 pt-3 text-[11px] leading-relaxed text-muted-foreground/80">
              {creationState
                ? "Press Escape or choose the active tool again to cancel."
