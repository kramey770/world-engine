      ...
      <div className="flex h-full min-h-44 flex-col" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
        <div className="flex items-start justify-between">
          <div>
            ...
            <h2 className="mt-1 font-serif text-2xl text-white">World pulse</h2>
          </div>
          <Network className="size-4 text-sky-200/70" />
        </div>

        <div className="relative mt-5 flex h-16 items-end gap-1 border-l border-b border-sky-200/25 px-2 pb-1">
          {mode.items.map((item, index) => <span key={item.label} className="flex-1 bg-gradient-to-t from-sky-200/70 to-sky-200/20" style={{ height: `${28 + index * 12}px` }} title={`${item.label}: ${item.value}`} />)}
          <span className="absolute inset-x-1 top-2 h-px bg-emerald-200/40" />
        </div>
