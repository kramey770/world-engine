    ...
    <HubBox definition={definition} onOpen={onOpen}>
      <div className="relative flex min-h-52 flex-1 flex-col justify-end overflow-hidden" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
        ...
        <div className="relative z-[1] max-w-[78%]">
          <div className="hub-kicker text-sky-200"><FileText className="size-3.5" /> {item?.eyebrow ?? definition.eyebrow}</div>
          <h2 className="mt-3 font-serif text-4xl leading-[.92] tracking-tight text-white">{item?.title ?? "Writing studio"}</h2>
          <p className="mt-3 max-w-sm font-serif text-base leading-relaxed text-white/65">{item?.summary ?? "The current draft, chapter, and scene work are waiting to be opened here."}</p>
