...
export function ItemCanonRecord({ itemId, onCancel, onCreated, className }: { itemId: string | null; onCancel?: () => void; onCreated?: (id: string) => void; className?: string }) {
  const { getItem, updateItem, addItem, deleteItem } = useItemCanon()
  ...
  return <div className={cn("flex min-h-0 flex-col", className)}>
    <div className="min-h-0 flex-1 overflow-y-auto">
      ...
      {mode === "view" && item ? <div className="flex flex-col gap-6 p-4">
        ...
        <button type="button" onClick={() => { deleteItem(item.id); onCancel?.() }} className="inline-flex h-9 w-full items-center justify-center gap-2 rounded-lg border border-destructive/30 text-sm text-destructive hover:bg-destructive/10"><Trash2 className="size-3.5" />Delete Item</button>
