  ...
  return (
    <div className={cn("absolute inset-0", className)} onClick={onClick}>
      <button
        ...
        className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-2 bg-background/65 text-sm font-medium text-foreground opacity-0 backdrop-blur-[1px] transition-opacity hover:opacity-100 focus-visible:opacity-100"
        aria-label={label}
      >
        {value ? <Upload className="size-6" /> : <ImagePlus className="size-8" />}
        <span>{value ? "Replace image or icon" : "Import image or icon"}</span>
      </button>
      {value && (
        <button
          type="button"
