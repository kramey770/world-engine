...
async function showSupporters() {
	...
	alertMessage.innerHTML =
		`<ul style='column-count: ${columns}; column-gap: 2em'>` +
...
function updateOutputToFollowInput(ev) {
	...
	// specific cases
	if (id === "manorsInput")
		return (manorsOutput.value = value == 100001 ? "auto" : value);
...
function fitMapToScreen() {
	...
	const zoomMin = rn(
		Math.max(svgWidth / graphWidth, svgHeight / graphHeight),
		3,
	);
...
function changeEmblemShape(emblemShape) {
	...
	const shapePath =
		window.EmblemRenderer && EmblemRenderer.shieldPaths[emblemShape];
	...
	pack.provinces.forEach((province) => {
		if (!province.i || province.removed || !province.coa || province.coa.custom)
			return;
		const culture = pack.cells.culture[province.center];
		const newShield =
			specificShape || Emblems.getShield(culture, province.state);
	...
	pack.burgs.forEach((burg) => {
		...
		const newShield =
			specificShape || Emblems.getShield(burg.culture, burg.state);
...
function changeStatesNumber(value) {
	...
	if (style.labels.groups.capital)
		style.labels.groups.capital["font-size"] = `${capitalSize}%`;
	if (style.labels.groups.states)
		style.labels.groups.states["font-size"] = `${stateSize}%`;
	d3.select("#labels")
		.select("[data-group='capital']")
		.attr("font-size", `${capitalSize}%`);
	d3.select("#labels")
		.select("[data-group='states']")
		.attr("font-size", `${stateSize}%`);
}
...
function changeZoomExtent(value) {
	if (+zoomExtentMin.value > +zoomExtentMax.value) {
		[zoomExtentMin.value, zoomExtentMax.value] = [
			zoomExtentMax.value,
			zoomExtentMin.value,
		];
	}
...
function applyStoredOptions() {
	...
	if (heightmapId) {
		const name =
			heightmapTemplates[heightmapId]?.name ||
			precreatedHeightmaps[heightmapId]?.name ||
			heightmapId;
		applyOption(ensureEl("templateInput"), heightmapId, name);
	}

	if (stored("distanceUnit"))
		applyOption(distanceUnitInput, stored("distanceUnit"));
	...
	for (let i = 0; i < localStorage.length; i++) {
		...
		const input =
			document.getElementById(key + "Input") || document.getElementById(key);
	...
	if (stored("temperatureEquator"))
		options.temperatureEquator = +stored("temperatureEquator");
	if (stored("temperatureNorthPole"))
		options.temperatureNorthPole = +stored("temperatureNorthPole");
	if (stored("temperatureSouthPole"))
		options.temperatureSouthPole = +stored("temperatureSouthPole");
...
function randomizeOptions() {
	...
	if (randomize || !stored("statesNumber"))
		statesNumber.value = gauss(18, 5, 2, 30);
	if (randomize || !stored("provincesRatio"))
		provincesRatio.value = gauss(20, 10, 20, 100);
	if (randomize || !stored("manors")) {
		manorsInput.value = 100001;
		manorsOutput.value = "auto";
	}
	if (randomize || !stored("religionsNumber"))
		religionsNumber.value = gauss(6, 3, 2, 10);
	if (randomize || !stored("sizeVariety"))
		sizeVariety.value = gauss(4, 2, 0, 10, 1);
	if (randomize || !stored("growthRate"))
		growthRate.value = rn(1 + Math.random(), 1);
	if (randomize || !stored("cultures"))
		culturesInput.value = culturesOutput.value = gauss(12, 3, 5, 30);
	if (randomize || !stored("culturesSet")) randomizeCultureSet();

	// 'Configure World' settings
	if (randomize || !stored("temperatureEquator"))
		options.temperatureEquator = gauss(25, 7, 20, 35, 0);
	if (randomize || !stored("temperatureNorthPole"))
		options.temperatureNorthPole = gauss(-25, 7, -40, 10, 0);
	if (randomize || !stored("temperatureSouthPole"))
		options.temperatureSouthPole = gauss(-15, 7, -40, 10, 0);
	if (randomize || !stored("prec")) options.prec = gauss(100, 40, 5, 500);

	// 'Units Editor' settings
	const US = navigator.language === "en-US";
	if (randomize || !stored("distanceScale"))
		distanceScale = distanceScaleInput.value = gauss(3, 1, 1, 5);
...
function setRendering(value) {
	...
	if (value === "optimizeSpeed") {
		...
		if (pack.cells && d3.select("#statesHalo").selectAll("*").size() === 0)
			Layers.draw("states");
	}
}
...
function generateEra() {
	...
	if (!stored("era"))
		eraInput.value =
			Names.getBaseShort(P(0.7) ? 1 : rand(Names.nameBases.length)) + " Era";
...
function updateTilesOptions() {
	...
	function getRowLabel(row) {
		const first =
			row >= alphabet.length
				? alphabet[Math.floor(row / alphabet.length) - 1]
				: "";
	...
	for (let y = 0, row = 0; y + tileH <= graphHeight; y += tileH, row++) {
		for (let x = 0, column = 1; x + tileW <= graphWidth; x += tileW, column++) {
			...
			labels.push(
				`<text x=${x + tileW / 2} y=${y + tileH / 2}>${getRowLabel(row)}${column}</text>`,
			);
		}
	}
...
function changeViewMode(event) {
	...
	if (!pressed && button.id !== "viewStandard")
		window.Controllers.View3d.open(button.id);
