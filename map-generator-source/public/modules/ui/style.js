...
function selectStyleElement() {
	...
	if (
		...
		const defaultGroupSelector =
			styleElement === "terrs" ? "#landHeights" : "g";
		if (styleElement === "labels") {
			const selected =
				group && el.select(`[data-group="${CSS.escape(group)}"]`);
			el =
				selected && selected.size()
					? selected
					: el.select(defaultGroupSelector);
		} else {
			el =
				group && el.select("#" + group).size()
					? el.select("#" + group)
					: el.select(defaultGroupSelector);
		}
	}
	...
	if (styleElement === "terrs") {
		...
		styleHeightmapRenderOceanOption.style.display =
	...
	if (styleElement === "compass") {
		...
		const tr = parseTransform(
			d3.select("#compass").select("use").attr("transform"),
		);
	...
	if (styleElement === "population") {
		...
		stylePopulationRuralStrokeInput.value =
			stylePopulationRuralStrokeOutput.value = d3
				.select("#population")
				.select("#rural")
				.attr("stroke");
		stylePopulationUrbanStrokeInput.value =
			stylePopulationUrbanStrokeOutput.value = d3
				.select("#population")
				.select("#urban")
				.attr("stroke");
		styleStrokeWidth.style.display = "block";
	...
	if (styleElement === "regions") {
		...
		styleStatesBodyOpacity.value =
			d3.select("#statesBody").attr("opacity") || 1;
		styleStatesBodyFilter.value = d3.select("#statesBody").attr("filter") || "";
		styleStatesHaloWidth.value =
			d3.select("#statesHalo").attr("data-width") || 10;
		styleStatesHaloOpacity.value =
			d3.select("#statesHalo").attr("opacity") || 1;
	...
	if (styleElement === "ocean") {
		...
		styleOceanFill.value = styleOceanFillOutput.value = d3
			.select("#oceanLayers")
			.select("#oceanBase")
			.attr("fill");
		styleOceanPattern.value = ensureEl("oceanicPattern").getAttribute("href");
		styleOceanPatternOpacity.value =
			ensureEl("oceanicPattern").getAttribute("opacity") || 1;
	...
	if (styleElement === "emblems") {
		...
		emblemsStateSizeInput.value =
			d3.select("#emblems").select("#stateEmblems").attr("data-size") || 1;
		emblemsProvinceSizeInput.value =
			d3.select("#emblems").select("#provinceEmblems").attr("data-size") || 1;
		emblemsBurgSizeInput.value =
			d3.select("#emblems").select("#burgEmblems").attr("data-size") || 1;
	...
	if (
		...
		if (styleElement === "labels") {
			options.labels.groups.forEach((group) => {
				const groupElement = ensureEl("labels").querySelector(
					`[data-group="${CSS.escape(group.name)}"]`,
				);
				const count = groupElement?.childElementCount || 0;
				styleGroupSelect.options.add(
					new Option(`${group.name} (${count})`, group.name, false, false),
				);
			});
	...
	if (styleElement === "scaleBar") {
		...
		if (scaleBarBack.size()) {
			...
			styleScaleBarBackgroundFill.value =
				styleScaleBarBackgroundFillOutput.value = scaleBarBack.attr("fill");
			styleScaleBarBackgroundStroke.value =
				styleScaleBarBackgroundStrokeOutput.value = scaleBarBack.attr("stroke");
			styleScaleBarBackgroundStrokeWidth.value =
			...
			styleScaleBarBackgroundPaddingRight.value =
				scaleBarBack.attr("data-right");
			styleScaleBarBackgroundPaddingBottom.value =
...
function getEl() {
	...
	if (el === "labels")
		return map.select("#labels").select(`[data-group="${CSS.escape(g)}"]`);
...
styleFilterInput.addEventListener("change", function () {
	if (styleGroupSelect.value === "ocean")
		return d3.select("#oceanLayers").attr("filter", this.value);
...
function updateTextureSelectValue(href) {
	const isAdded = Array.from(styleTextureInput.options).some(
		(option) => option.value === href,
	);
	if (isAdded) {
...
openCreateHeightmapSchemeButton.addEventListener("click", function () {
	...
	this.dataset.stops = scheme.startsWith("#")
		...
		: (() =>
				[0, 0.25, 0.5, 0.75, 1]
					.map(heightmapColorSchemes[scheme])
					.map(toHEX)
					.join(","))();
	...
	function renderStops() {
		...
		Array.from(container.querySelectorAll("button.add")).forEach(
			(button, index) =>
				(button.onclick = () => {
					const middleColor = d3.interpolateRgb(
						stops[index],
						stops[index + 1],
					)(0.5);
	...
	function handleCreate() {
		...
		if (stops in heightmapColorSchemes)
			return tip("This scheme already exists", false, "error");
...
styleLegendColItems.addEventListener("input", (e) => {
	d3.select("#legend")
		.select("#legendBox")
		.attr("data-columns", e.target.value);
	Layers.draw("legend");
});
...
styleLegendOpacity.addEventListener("input", (e) => {
	d3.select("#legend")
		.select("#legendBox")
		.attr("fill-opacity", e.target.value);
});
...
function changeFontSize(el, size) {
	...
	const scaleSize =
		styleElementSelect.value === "coordinates"
			? rn(size / scale ** 0.8, 2)
			: size;
...
styleMarketsIcon.addEventListener("click", function () {
	window.Controllers.IconSelector.open(
		d3.select("#markets").attr("data-icon") || "⚖️",
		(value) => {
...
function textureProvideURL() {
	...
	$("#alert").dialog({
		...
		buttons: {
			Apply: function () {
				if (!textureURL.value)
					return tip("Please provide a valid URL", false, "error");
...
styleVignettePreset.addEventListener("change", function () {
	...
	if (vignette) {
		...
		styleFillInput.value = styleFillOutput.value =
			vignette.getAttribute("fill");
...
styleScaleBar.addEventListener("input", (event) => {
	...
	if (id === "styleScaleBarSize")
		d3.select("#scaleBar").attr("data-bar-size", value);
	else if (id === "styleScaleBarFontSize")
		d3.select("#scaleBar").attr("font-size", value);
	else if (id === "styleScaleBarPositionX")
		d3.select("#scaleBar").attr("data-x", value);
	else if (id === "styleScaleBarPositionY")
		d3.select("#scaleBar").attr("data-y", value);
	else if (id === "styleScaleBarLabel")
		d3.select("#scaleBar").attr("data-label", value);
