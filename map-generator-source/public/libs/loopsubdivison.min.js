...
(() => {
	...
	function modify(bufferGeometry, iterations = 1, params = {}) {
		if (arguments.length > 3)
			console.warn(
				`modify() now uses a parameter object. See readme for more info!`,
			);
		if (typeof params !== "object") params = {};
		if (params.split === undefined) params.split = !0;
		if (params.uvSmooth === undefined) params.uvSmooth = !1;
		if (params.preserveEdges === undefined) params.preserveEdges = !1;
		if (params.flatOnly === undefined) params.flatOnly = !1;
		...
		for (let i = 0; i < iterations; i++) {
			const currentTriangles = modifiedGeometry.attributes.position.count / 3;
	...
	window.loopSubdivision.modify = modify;
	function edgeSplit(geometry) {
		...
		for (const attributeName in morphAttributes) {
			...
			for (let i = 0, l = morphAttribute.length; i < l; i++) {
				...
				const floatArray = splitAttribute(morphAttribute[i], attributeName, !0);
				array.push(
		...
		existing.dispose();
		return split;
		function splitAttribute(attribute, attributeName, morph = !1) {
			...
			let groupStart, groupMaterial;
			let index = 0;
			let skipped = 0;
			const step = attribute.itemSize;
			for (let i = 0; i < vertexCount; i += 3) {
				if (!triangleExist[i / 3]) {
					skipped += 3;
					continue;
	...
	function smooth(geometry, params = {}) {
		...
		for (const attributeName in morphAttributes) {
			...
			for (let i = 0, l = morphAttribute.length; i < l; i++) {
				...
				const flatAttribute = flatAttribute(
					morphAttribute[i],
		...
		function subdivideAttribute(
			...
			const arrayLength =
				flatGeometry.attributes.position.count * flatAttribute.itemSize;
						...
						if (neighbors) {
							if (params.preserveEdges) {
								...
								for (const edgeHash of edgeSet) {
									if (flatOpposites[edgeHash].length % 2 !== 0) hasPair = !1;
								}
								if (!hasPair) continue;
							}
							...
							const beta =
								(1 / k) *
								(5 / 8 - (3 / 8 + (1 / 4) * Math.cos((2 * Math.PI) / k)) ** 2);
							const startWeight = 1.0 - beta * k;
							_vertex[v].multiplyScalar(startWeight);
	...
	const _positionShift = 10 ** POSITION_DECIMALS;
	...
	function verifyGeometry(geometry) {
		if (geometry === undefined) {
			console.warn(`window.loopSubdivision: Geometry provided is undefined`);
			return !1;
		}
		if (!geometry.isBufferGeometry) {
			console.warn(
				`window.loopSubdivision: Geometry provided is not 'BufferGeometry' type`,
			);
			return !1;
		}
		if (geometry.attributes.position === undefined) {
			console.warn(
				`window.loopSubdivision: Geometry provided missing required 'position' attribute`,
			);
			return !1;
		}
