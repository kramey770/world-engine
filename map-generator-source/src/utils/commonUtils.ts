...
export const parseError = (error: Error): string => {
	...
	const stringify = (e: Error) =>
		isFirefox ? `${e.toString()} ${e.stack}` : e.stack || e.toString();
	const errorString = getErrorChain(error)
		.map(stringify)
		.join("<br>Caused by: ");
	const regex =
...
export const getBase64 = (
	...
	xhr.onload = () => {
		...
		if (
			...
			ERROR &&
				console.error(
					`Cannot load image ${url}: status ${xhr.status}, type ${blob?.type || "unknown"}`,
				);
...
export const speak = (text: string): void => {
	...
	if (voices.length) {
		const voiceId = Number(
			(document.getElementById("speakerVoice") as HTMLSelectElement).value,
		);
