...
export const convertTemperature = (
	...
	const scale =
		targetScale ||
		(ensureEl<HTMLSelectElement>("temperatureScale")
			.value as TemperatureScale) ||
		"°C";
...
export function getFriendlyPrecipitation(
	cellId: number,
	pack: PackedGraph,
	grid: any,
): string {
...
export function getFriendlyHeight(
	[x, y]: [number, number],
	pack: PackedGraph,
	grid: any,
): string {
...
export function getCellPopulation(
	cellId: number,
	pack: PackedGraph,
): [number, number] {
	...
	const urban = burgId
		? (pack.burgs[burgId].population || 0) * populationRate * urbanization
		: 0;
	return [rural, urban];
}
