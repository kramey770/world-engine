...
function buildList(
	...
	return {
		html: `<${tag}>${contents.map((content) => `<li>${content}</li>`).join("")}</${tag}>`,
		next: index,
	};
}
...
function buildTable(lines: string[], start: number, html: string[]): number {
	...
	while (index < lines.length && lines[index].includes("|")) {
		...
		rows.push(
			`<tr>${values.map((value, column) => cell(value, column, "td")).join("")}</tr>`,
		);
		index++;
	}

	const head = headers
		.map((header, column) => cell(header, column, "th"))
		.join("");
	html.push(
