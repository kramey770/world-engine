...
function initialize(): void {
	for (const lockEl of Array.from(
		document.querySelectorAll<HTMLElement>("[data-locked]"),
	)) {
		...
		lockEl.addEventListener("click", () => {
			const ids = lockEl.dataset.ids
				? lockEl.dataset.ids.split(",")
				: [lockEl.id.slice(5)];
