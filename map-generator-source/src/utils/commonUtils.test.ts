...
describe("parseError", () => {
	...
	it("should report the causes the error was wrapped over", () => {
		...
		const wrapped = new Error('Generation Pipeline failed at step "rivers"', {
			cause: original,
		});
	...
	it("should not follow a cause that is not an error", () => {
		expect(
			parseError(new Error("boom", { cause: "just a string" })).includes(
				"Caused by:",
			),
		).toBe(false);
	});
...
describe("getBase64", () => {
	...
	const call = () =>
		new Promise<string | ArrayBuffer | null>((resolve) =>
			getBase64("https://example.com/img.png", resolve),
		);
	...
	it("returns null for a non-2xx response instead of inlining the error page", async () => {
		next = {
			status: 404,
			blob: new Blob(["<!DOCTYPE html><h1>404</h1>"], { type: "text/html" }),
		};
