...
export const findPath = (
	...
	const useHeuristic =
		typeof goal === "number" && goal >= 0 && cellsP[goal] !== undefined;
...
export const parsePathPoints = (path: string): Point[] => {
	...
	for (const match of path.matchAll(
		/([AaCcHhLlMmQqSsTtVvZz])([^AaCcHhLlMmQqSsTtVvZz]*)/g,
	)) {
		const command = match[1];
		const type = command.toUpperCase();
		const relative = command !== type;
		const values =
			match[2].match(/[-+]?(?:\d*\.?\d+)(?:e[-+]?\d+)?/gi)?.map(Number) || [];
...
export const meander = (
	...
	for (let i = 0; i <= lastStep; i++, step++) {
		...
		if (isWaterCell && (isWaterCell[i] || isWaterCell[i + 1]))
			meanderVal *= WATER_MEANDER_SCALE;
...
function relaxAcuteAngles(points: Point[], anchorIndices: number[]): void {
	...
	for (let iter = 0; iter < RELAX_ITERATIONS; iter++) {
		...
		for (let i = 1; i < n - 1; i++) {
			...
			const before =
				acuteCost(at, i - 1) + acuteCost(at, i) + acuteCost(at, i + 1);
			const after =
				acuteCost(withFlip, i - 1) +
				acuteCost(withFlip, i) +
				acuteCost(withFlip, i + 1);
