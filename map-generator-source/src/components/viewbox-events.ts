...
export function applyDefaultViewboxEvents(): void {
	...
	select<SVGGElement, unknown>("#legend").call(
		drag<SVGGElement, unknown>().on("start", dragLegendBox),
	);
}
...
const GRAND_EDITORS: Record<string, Opener> = {
	emblems: (target) =>
		Controllers.EmblemsEditor.open(undefined, undefined, undefined, target),
	...
	goodsBurgs: (_target, parent) =>
		Controllers.ProductionOverview.open(Number(parent.dataset.id)),
	...
	markets: (target, parent) => {
		if (target.tagName !== "path")
			Controllers.MarketOverview.open(Number(parent.dataset.id));
	},
};
...
function onClick(event: MouseEvent): void {
	...
	const open =
		PARENT_EDITORS[parent.id] ||
		GRAND_EDITORS[grand.id] ||
		GREAT_EDITORS[great.id];
	open?.(target, parent);
}
