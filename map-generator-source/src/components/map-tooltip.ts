...
export function showNotes(event: Event): void {
	...
	const burg = target.closest<HTMLElement>(
		"[data-label-type='burg'][data-id], #burgIcons [data-id]",
	);
	const id = burg
...
export function showMapTooltip(
	...
	const path = (
		event.composedPath ? event.composedPath() : getComposedPath(target)
	) as HTMLElement[];
	...
	const elementTip = getElementTip({
		group,
		subgroup,
		target,
		event,
...
function getElementTip({
	...
	if (target.closest("#labels [data-label-type]"))
		return `${text}. Click to edit the label`;

	if (group === "armies")
		return `${(parent as SVGElement & { dataset: DOMStringMap }).dataset.name}. Click to edit`;

	if (group === "emblems" && target.tagName === "use")
		return getEmblemTip(target, parent, event);
	...
	if (group === "routes") {
		...
		if (route)
			return route.name
				? `${route.name}. Click to edit the Route`
				: "Click to edit the Route";
		return undefined;
	}
	...
	if (group === "markers")
		return "Click to edit the Marker. Hold Shift to not close the assosiated note";

	if (group === "ruler")
		return findEl("measurersEditor")
			? "Drag the measurer or its points to edit"
			: "Click to open the Measurers Editor";
...
function getEmblemTip(
	...
	const name =
		"fullName" in element ? element.fullName || element.name : element.name;
...
function getGoodsTip(target: SVGElement, cellId: number): string | undefined {
	...
	const formatProduction = (produced: Record<string, number>) =>
		Object.entries(produced)
			...
			.map(([goodId, amount]) => {
				const name = (
					Goods.get(Number(goodId))?.name || "unknown"
				).toLowerCase();
	...
	if (target.closest("#goodsIcons")) {
		const good = Goods.get(
			Number(target.closest<SVGElement>("[data-i]")?.dataset.i),
		);
		return `${good?.name} bonus resource. Click to open Goods Editor and select displayed goods`;
	}

	if (target.closest("#goodsCells")) {
		const produced = Production.getCellProduction(
			cellId,
			Goods.getBiomesProduction(),
		);
...
function showLayerTip(
	...
	if (Layers.isOn("precipitation") && isLand)
		return void tip(
			`Annual Precipitation: ${getFriendlyPrecipitation(cellId, pack, grid)}`,
		);
	if (Layers.isOn("population")) return void tip(getPopulationTip(cellId));
	if (Layers.isOn("temperature"))
		return void tip(
			`Temperature: ${convertTemperature(grid.cells.temp[gridCellId])}`,
		);
	...
	if (
		cells.state[cellId] &&
		(Layers.isOn("provinces") || Layers.isOn("states"))
	) {
		const stateId = cells.state[cellId];
		const provinceId = cells.province[cellId];
		const province = provinceId
			? `${pack.provinces[provinceId].fullName}, `
			: "";
