...
import {
	drawPrecipitation,
	removePrecipitation,
} from "@/renderers/draw-precipitation";
...
export class LayersRegistry<Id extends string = string> {
	...
	constructor(private layers: Layer<Id>[]) {
		for (const layer of layers)
			if (layer.params.permanent) this.active.add(layer.id);
	}
	...
	hide(...ids: Id[]): void {
		const activeLayers = ids.filter(
			(id) => this.active.has(id) && !this.get(id).params.permanent,
		);
		...
		const hidden = this.layers
			.filter(
				(layer) =>
					...
					!known.includes(layer.id) &&
					this.active.has(layer.id),
			)
			.map((layer) => layer.id);
	...
	draw(...ids: Id[]): void {
		for (const layer of this.layers) {
			if (ids.includes(layer.id) && this.active.has(layer.id))
				layer.params.draw?.(layer);
		}
	}
	...
	get state(): LayersState {
		return {
			...
			active: this.layers
				.filter((layer) => this.active.has(layer.id) && !layer.params.permanent)
				.map((layer) => layer.id),
		};
	}
	...
	restore({ order, active }: LayersState): void {
		...
		this.active = new Set(
			this.layers
				.filter((layer) => layer.params.permanent || active.includes(layer.id))
				.map((layer) => layer.id),
		);
		this.init();
		this.emit();
...
const mapLayers = [
	...
	new Layer({
		id: "lakes",
		parent: "viewbox",
		children: ["freshwater", "salt", "sinkhole", "frozen", "lava", "dry"].map(
			(id) => ({ id, tag: "g" }),
		),
		keepContent: true,
		draw: drawLakes,
	...
	new Layer({
		...
		parent: "viewbox",
		children: [
			{ id: "compassRose", tag: "use", attrs: { href: "#defs-compass-rose" } },
		],
	}),
	...
	new Layer({
		id: "borders",
		parent: "viewbox",
		children: ["stateBorders", "provinceBorders"].map((id) => ({
			id,
			tag: "g",
		})),
		draw: drawBorders,
	...
	new Layer({
		...
		children: ["roads", "trails", "searoutes", "airroutes", "traderoutes"].map(
			(id) => ({ id, tag: "g" }),
		),
		draw: drawRoutes,
		erase: removeRoutes,
	...
	new Layer({
		id: "goods",
		parent: "viewbox",
		children: ["goodsCells", "goodsIcons", "goodsBurgs"].map((id) => ({
			id,
			tag: "g",
		})),
		draw: drawGoods,
	...
	new Layer({
		id: "emblems",
		parent: "viewbox",
		children: ["burgEmblems", "provinceEmblems", "stateEmblems"].map((id) => ({
			id,
			tag: "g",
		})),
		draw: drawEmblems,
