...
describe("hidden columns persistence", () => {
	...
	it("round-trips the hidden set", () => {
		...
		expect(loadHiddenColumns("burgsOverview", COLUMNS)).toEqual(
			new Set(["treasury"]),
		);
	});
	...
	it("ignores legacy stored visibility", () => {
		...
		const columns = [...COLUMNS, { key: "type", label: "Type", hidden: true }];
		expect(loadHiddenColumns("statesEditor", columns)).toEqual(
			new Set(["type"]),
		);
		expect(localStorage.getItem("columnsHidden:statesEditor")).toBe(
			'["population"]',
		);
	});
	...
	it("defaults to mobileHidden columns when nothing is stored and MOBILE is true", () => {
		(globalThis as Record<string, unknown>).MOBILE = true;
		expect(loadHiddenColumns("burgsOverview", MOBILE_COLUMNS)).toEqual(
			new Set(["population"]),
		);
	});
	...
	it("combines hidden and mobileHidden defaults on mobile", () => {
		...
		const columns = [
			...MOBILE_COLUMNS,
			{ key: "type", label: "Type", hidden: true },
		];
		expect(loadHiddenColumns("burgsOverview", columns)).toEqual(
			new Set(["population", "type"]),
		);
	});
	...
	it("restores the default visibility after a saved override", () => {
		...
		expect(restoreDefaultColumnVisibility("statesEditor", columns)).toEqual(
			new Set(["type"]),
		);
		expect(loadHiddenColumns("statesEditor", columns)).toEqual(
			new Set(["type"]),
	...
	it("inverts configurable column visibility and persists it", () => {
		...
		expect(invertColumnVisibility("statesEditor", columns)).toEqual(
			new Set(["population", "type"]),
		);
		expect(loadHiddenColumns("statesEditor", columns)).toEqual(
			new Set(["population", "type"]),
...
describe("buildTracks", () => {
	...
	it("drops the tracks of hidden columns", () => {
		expect(buildTracks(COLUMNS, new Set(["group", "length"]))).toBe(
			"1.4em 8em 4em",
		);
	});
...
describe("getLastVisibleIndex", () => {
	it("returns the index of the last column that is not hidden", () => {
		expect(getLastVisibleIndex(COLUMNS, new Set(["length", "actions"]))).toBe(
			2,
		);
	});
...
describe("renderEditorHeader", () => {
	...
	it("wraps the cells in a header div with the given id", () => {
		expect(
			html.startsWith('<div id="routesOverviewHeader" class="header">'),
		).toBe(true);
	...
	it("marks sortable columns and their sort type", () => {
		expect(
			html.includes(
				'data-col="name" class="sortable alphabetically" data-sortby="name"',
			),
		).toBe(true);
	});
	...
	it("leaves structural columns unsortable", () => {
		const locateCell = html.slice(
			html.indexOf('data-col="locate"'),
			html.indexOf('data-col="name"'),
		);
	...
	it("applies the initial sort icon to the default-sorted column", () => {
		expect(
			html.includes(
				'data-col="length" class="sortable icon-sort-number-down" data-sortby="length"',
			),
		).toBe(true);
	});
	...
	it("anchors the button to the last initially visible column", () => {
		...
		const nameCell = anchored.slice(
			anchored.indexOf('data-col="name"'),
			anchored.indexOf('data-col="extra"'),
		);
		expect(nameCell.includes('id="exampleColumnsButton"')).toBe(true);
		expect(
			anchored
				.slice(anchored.indexOf('data-col="extra"'))
				.includes('id="exampleColumnsButton"'),
		).toBe(false);
	});
});
