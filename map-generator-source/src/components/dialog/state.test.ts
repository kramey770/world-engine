...
describe("dialog state", () => {
	it("creates filters once per dialog", async () => {
		...
		const filters = state.get("burgsOverview", "filters", () => ({
			search: "",
			stateId: -1,
		}));
		filters.search = "port";
		...
		expect(
			state.get("burgsOverview", "filters", () => ({
				search: "new default",
				stateId: 1,
			})),
		).toEqual(filters);
		expect(
			state.get("statesEditor", "filters", () => ({ search: "" })),
		).toEqual({ search: "" });
	});
	...
	it("persists filter mutations and sorting", async () => {
		...
		filters.search = "port";
		state.set("burgsOverview", "filters", filters);
		state.set("burgsOverview", "sorting", {
			sortBy: "name",
			alphabetically: true,
			direction: 1,
		});
	...
	it("restores state in a new session", async () => {
		...
		const state = await loadState();
		const filters = state.get("goodsEditor", "filters", () => ({
	...
	it("persists array filters as plain JSON", async () => {
		...
		expect(
			JSON.parse(localStorage.getItem("fmg-dialog-state")!).goodsEditor.filters
				.visibleTags,
		).toEqual(["food"]);
	});
	...
	it("falls back per field when stored values are malformed", async () => {
		...
		expect(
			state.get("burgsOverview", "sorting", () => ({
				sortBy: "name",
				alphabetically: true,
				direction: 1,
			})),
	...
	it("removes one section without affecting the rest of the dialog state", async () => {
		...
		state.set("statesEditor", "sorting", {
			sortBy: "name",
			alphabetically: true,
			direction: 1,
		});
		...
		expect(
			JSON.parse(localStorage.getItem("fmg-dialog-state")!).statesEditor,
		).toEqual({
	...
	it("is cleared only through explicit application-data cleanup", async () => {
		...
		expect(
			state.get("burgsOverview", "filters", () => ({ search: "" })),
		).toEqual({ search: "" });
