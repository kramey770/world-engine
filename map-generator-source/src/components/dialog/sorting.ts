...
export function applySortingByHeader(
	dialogId: string,
	headerContainerId = `${dialogId}Header`,
): void {
	...
	for (const header of Array.from(
		container.querySelectorAll<HTMLElement>(".sortable"),
	)) {
...
function toggleSortIcon(dialogId: string, header: HTMLElement): void {
	...
	for (const sortable of Array.from(
		headers.querySelectorAll<HTMLElement>("div.sortable"),
	)) {
...
export function applySorting(headers: HTMLElement): void {
	...
	lines
		.sort((a, b) => {
			const aValue = alphabetically
				? a.dataset[sortBy]
				: Number(a.dataset[sortBy]);
			const bValue = alphabetically
				? b.dataset[sortBy]
...
function restoreSortState(dialogId: string, headers: HTMLElement): void {
	const sort = dialogState.get(dialogId, "sorting", () =>
		getActiveSort(headers),
	);
	if (!sort) return;

	const header = Array.from(
		headers.querySelectorAll<HTMLElement>(".sortable"),
	).find((cell) => cell.dataset.sortby === sort.sortBy);
	if (!header) {
...
export function sortData<T>(
	...
	return data.sort((a, b) => {
		...
		if (sort.alphabetically) {
			...
			return (
				(aString > bString ? 1 : aString < bString ? -1 : 0) * sort.direction
			);
		}
...
export function sortDataByColumns<T>(
	dialogId: string,
	data: T[],
