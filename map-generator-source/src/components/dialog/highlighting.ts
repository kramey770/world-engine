...
export function applyLineHighlighting(
	...
	const highlight = debounce((event: MouseEvent | TouchEvent) => {
		...
		const lineId =
			cellId === undefined
				? undefined
				: resolveLineId({ target: event.target, cellId });
...
function highlightLine(containerId: string, lineId: number): void {
	...
	const line = Array.from(
		container.querySelectorAll<HTMLElement>("div[data-id]"),
	).find((element) => element.dataset.id === String(lineId));
	if (!line) return;
...
function unhighlightLines(containerId: string): void {
	const container = findEl(containerId);
	if (!container) return;
	for (const line of Array.from(container.getElementsByClassName("hovered")))
