...
export function renderEditorPagination(
	...
	if (!nav) {
		...
		nav.style.cssText =
			"margin-left: auto; display: inline-flex; gap: 0.3em; align-items: center;";
	...
	nav
		.querySelector<HTMLElement>(".editorPagePrev")
		?.addEventListener("click", () => onGoto(view.page - 1));
	nav
		.querySelector<HTMLElement>(".editorPageNext")
		?.addEventListener("click", () => onGoto(view.page + 1));
	nav
		.querySelector<HTMLInputElement>(".editorPageInput")
	...
	const cells = columns.map((column, index) => {
		...
		if (column.sortBy) {
			...
			if (column.defaultSort) {
				...
				classes.push(
					`icon-sort-${type}-${column.defaultSort === "desc" ? "down" : "up"}`,
				);
			}
		}
		const tip =
			column.tip ??
			(column.sortBy && column.label ? `Click to sort by ${column.label}` : "");
		const attributes = [
			`data-col="${column.key}"`,
...
export function loadHiddenColumns(
	...
	const saved = dialogState.get<ColumnVisibilityState | null>(
		dialogId,
		"columns",
		() => null,
	);
	...
	for (const key of saved?.hidden ?? [])
		if (configurable.has(key)) hidden.add(key);
...
export function restoreDefaultColumnVisibility(
	dialogId: string,
	columns: EditorColumn[],
): Set<string> {
...
export function invertColumnVisibility(
	dialogId: string,
	columns: EditorColumn[],
): Set<string> {
	...
	const inverted = new Set(
		columns
			.filter((column) => !column.permanent && !hidden.has(column.key))
			.map((column) => column.key),
	);
...
const dialogColumnsRegistry = new Map<
	string,
	{ columns: EditorColumn[]; modeHidden: Set<string> }
>();
...
function bindColumnsPicker({
    ...
    `;
      ...
      <div style="display: flex; justify-content: flex-end; gap: 0.2em; margin-top: 0.5em;">
		...
		popup
			.querySelector(`#${popupId}RestoreDefaults`)
		...
		const positionPopup = () => {
			...
			const fitsBelow =
				rect.bottom + 2 + popupHeight <= window.innerHeight - margin;
			const fitsAbove = rect.top - 2 - popupHeight >= margin;
			const top =
				fitsBelow || !fitsAbove ? rect.bottom + 2 : rect.top - popupHeight - 2;
