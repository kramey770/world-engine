...
export function applyZoomBehavior(): void {
	select<SVGSVGElement, unknown>("#map").call(
		zoomBehavior.on("zoom", onZoom).on("end", handleZoomEnd),
	);
}
	...
	if (didScaleChange) {
		...
		if (options.labels.resizeOnZoom) {
			const fontSize = Math.max(
				Math.round(((100 + 100 / scale) / 2) * 100) / 100,
				1,
			);
...
function invokeActiveZooming(): void {
	const isOptimized =
		ensureEl<HTMLSelectElement>("shapeRendering").value === "optimizeSpeed";
...
function zoomTo(x: number, y: number, z = 8, duration = 2000): void {
	const transform = zoomIdentity
		.translate(x * -z + svgWidth / 2, y * -z + svgHeight / 2)
		.scale(z);
...
export function setTranslateExtent(
	x0: number,
	y0: number,
	x1: number,
	y1: number,
