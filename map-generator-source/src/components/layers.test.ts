...
const groupIds = (parent = "viewbox") =>
	Array.from(document.getElementById(parent)!.children, (node) => node.id);
const displayOf = (elementId: string) =>
...
describe("init", () => {
	...
	it("adopts an existing group and reorders it to match registration order", () => {
		document.getElementById("viewbox")!.innerHTML =
			/* html */ `<g id="b-el"><circle id="kept" /></g>`;
		registry(
			new Layer({ id: "a", element: "a-el", parent: "viewbox" }),
			new Layer({ id: "b", element: "b-el", parent: "viewbox" }),
		);
	...
	it("creates declared children and applies attrs, appending to the declared parent", () => {
		...
		expect(Array.from(group.children, (node) => node.id)).toEqual([
			"one",
			"two",
		]);
	});
	...
	it("creates children that are not groups, with their attributes", () => {
		...
		expect(
			Array.from(document.getElementById("a-el")!.children, (node) => node.id),
		).toEqual(["one", "rose"]);
	...
	it("adopts a child that is already in the svg instead of duplicating it", () => {
		document.getElementById("viewbox")!.innerHTML =
			/* html */ `<g id="a-el"><use id="rose" href="#kept" /></g>`;
		registry(
			new Layer({
				id: "a",
				element: "a-el",
				parent: "viewbox",
				children: [{ id: "rose", tag: "use", attrs: {} }],
			}),
		);
...
describe("show and hide", () => {
	const setup = () => {
		...
		const erase = vi.fn();
		registry(
			new Layer({ id: "a", element: "a-el", parent: "viewbox", draw, erase }),
		);
		return { draw, erase };
	};
...
describe("erase", () => {
	it("clears emblem content while preserving its parent groups", () => {
		...
		document.getElementById("stateEmblems")!.innerHTML =
			/* html */ `<use data-i="1" />`;
		...
		expect(
			Array.from(
				document.getElementById("emblems")!.children,
				(node) => node.id,
			),
		).toEqual(["burgEmblems", "provinceEmblems", "stateEmblems"]);
	...
	it("clears declared children and removes everything else by default", () => {
		...
		document
			.getElementById("a-el")!
			.insertAdjacentHTML("beforeend", /* html */ `<g id="dropped"></g>`);
	...
	it("keeps a declared child that is not a group, content and all", () => {
		...
		document
			.getElementById("a-el")!
			.insertAdjacentHTML("beforeend", /* html */ `<circle id="dropped" />`);

		Layers.hide("a");
		Layers.eraseAll();

		expect(document.getElementById("dropped")).toBeNull();
		expect(document.getElementById("rose")!.getAttribute("href")).toBe(
			"#defs-rose",
	...
	it("eraseAll drops the content of every viewbox layer, on or off, keepContent included", () => {
		...
		Layers.show("a");
		document.getElementById("a-el")!.innerHTML =
			/* html */ `<circle id="dropped-a" />`;
		document.getElementById("kept")!.innerHTML =
			/* html */ `<circle id="dropped-b" />`;
		document.getElementById("chrome-el")!.innerHTML =
			/* html */ `<circle id="chrome-content" />`;
...
describe("setActive", () => {
	it("turns on the listed layers, turns off the rest and preserves permanent layers", () => {
		...
		expect([Layers.isOn("a"), Layers.isOn("b"), Layers.isOn("s")]).toEqual([
			false,
			true,
			true,
		]);
...
describe("move", () => {
	const register = () => {
		registry(
			...["a", "b", "c"].map(
				(id) => new Layer({ id, element: `${id}-el`, parent: "viewbox" }),
			),
		);
	};
	...
	it("keeps a layer among its own parent's layers when the successor is in another parent", () => {
		...
		expect(Layers.all.map((layer) => layer.id)).toEqual([
			"b",
			"a",
			"bar",
			"top",
