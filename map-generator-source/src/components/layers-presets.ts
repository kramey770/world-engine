...
function restoreCustomPresets(): void {
	const stored: Record<string, string[]> | null = JSON.parse(
		localStorage.getItem("presets") || "null",
	);
	if (!stored) return;
	...
	for (const name in stored) {
		...
		if (!DEFAULT_PRESETS[name])
			ensureEl<HTMLSelectElement>("layersPreset").add(new Option(name, name));
	}
}
...
export function applyLayersPreset(): void {
	const stored =
		...
		ensureEl<HTMLSelectElement>("layersPreset").value;
...
export function applyURLLayers(params: URLSearchParams): void {
	...
	if (layersParam) {
		...
		if (ids.length) {
			...
			ERROR &&
				console.error(
					`URL param layers="${layersParam}" has no valid layer ids`,
				);
		}
		return;
	}
...
function findPresetName(param: string): string | undefined {
	...
	const byKey = Object.keys(presets).find(
		(key) => key.toLowerCase() === needle,
	);
	if (byKey) return byKey;

	const option = Array.from(
		ensureEl<HTMLSelectElement>("layersPreset").options,
	).find((option) => option.text.toLowerCase() === needle);
...
function setPresetName(name: string): void {
	...
	ensureEl("removePresetButton").style.display = DEFAULT_PRESETS[name]
		? "none"
		: "inline-block";
...
function savePreset(): void {
	confirmationDialog({
		...
		onConfirm: () => {
			...
			ensureEl<HTMLSelectElement>("layersPreset").add(
				new Option(name, name, false, true),
			);
...
function removePreset(): void {
	...
	ensureEl<HTMLSelectElement>("layersPreset").options.remove(
		Array.from(ensureEl<HTMLSelectElement>("layersPreset").options).findIndex(
...
function highlightCurrentPreset(): void {
	...
	const current = Object.keys(presets).find(
		(name) => asSet(presets[name]) === active,
	);

	ensureEl<HTMLSelectElement>("layersPreset").value = current ?? "custom";
	ensureEl("removePresetButton").style.display =
		current && !DEFAULT_PRESETS[current] ? "inline-block" : "none";
	ensureEl("savePresetButton").style.display = current
		? "none"
		: "inline-block";
}

ensureEl<HTMLSelectElement>("layersPreset").addEventListener(
	"change",
	(event) => {
