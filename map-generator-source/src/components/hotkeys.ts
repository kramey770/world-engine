...
function handleKeyup(event: KeyboardEvent): void {
	...
	else if (code === "KeyO" && findEl("canvas3d"))
		Controllers.View3d.toggleOptions();
	...
	else if ((shift || altShift) && code === "KeyH")
		Controllers.HeightmapEditor.open();
	else if ((shift || altShift) && code === "KeyB")
		Controllers.BiomesEditor.open();
	else if ((shift || altShift) && code === "KeyS")
		Controllers.StatesEditor.open();
	else if ((shift || altShift) && code === "KeyP")
		Controllers.ProvincesEditor.open();
	else if ((shift || altShift) && code === "KeyD")
		Controllers.DiplomacyEditor.open();
	else if ((shift || altShift) && code === "KeyL")
		Controllers.LabelsOverview.open();
	else if ((shift || altShift) && code === "KeyC")
		Controllers.CulturesEditor.open();
	else if ((shift || altShift) && code === "KeyN")
		Controllers.NamesbaseEditor.open();
	else if ((shift || altShift) && code === "KeyZ")
		Controllers.ZonesEditor.open();
	else if ((shift || altShift) && code === "KeyR")
		Controllers.ReligionsEditor.open();
	else if ((shift || altShift) && code === "KeyY")
		Controllers.EmblemsEditor.openDefault();
	else if ((shift || altShift) && code === "KeyQ")
		Controllers.UnitsEditor.open();
	else if ((shift || altShift) && code === "KeyO")
		Controllers.NotesEditor.open();
	else if ((shift || altShift) && code === "KeyA")
		Controllers.ChartsOverview.open();
	else if ((shift || altShift) && code === "KeyT")
		Controllers.BurgsOverview.open();
	else if ((shift || altShift) && code === "KeyU")
		Controllers.RoutesOverview.open();
	else if ((shift || altShift) && code === "KeyV")
		Controllers.RiversOverview.open();
	else if ((shift || altShift) && code === "KeyM")
		Controllers.MilitaryOverview.open();
	else if ((shift || altShift) && code === "KeyK")
		Controllers.MarkersOverview.open();
	else if ((shift || altShift) && code === "KeyE") Controllers.CellInfo.open();
	else if ((shift || altShift) && code === "KeyG")
		Controllers.GoodsEditor.open();
	else if ((shift || altShift) && code === "Equal")
		Controllers.MeasurersEditor.open();
...
function handleSizeChange(key: string): void {
	...
	brush =
		brushIds
			.map((id) => findEl<HTMLInputElement>(id))
...
function handleBracketSizeChange(code: string): boolean {
	const isHeightmapBrushPressed = Boolean(
		findEl("brushesButtons")?.querySelector("button.pressed"),
	);
...
function removeElementOnKey(): void {
	const fastDelete = Array.from(
		document.querySelectorAll<HTMLElement>("[role='dialog'] .fastDelete"),
	).find((dialog) => dialog.style.display !== "none");
	if (fastDelete) fastDelete.click();

	const visibleDialogs = Array.from(
		document.querySelectorAll<HTMLElement>("[role='dialog']"),
	).filter((dialog) => dialog.style.display !== "none");
