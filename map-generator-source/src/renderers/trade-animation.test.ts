...
describe("getPathCost", () => {
	it("returns 1 for a sea-route segment", () => {
		globalThis.pack = makePack({ 0: { 1: 7 } }, [
			{ i: 7, group: "searoutes" },
		]) as any;
	...
	it("returns 5 for a roads segment", () => {
		globalThis.pack = makePack({ 0: { 1: 3 } }, [
			{ i: 3, group: "roads" },
		]) as any;
	...
	it("returns 5 for a trails segment", () => {
		globalThis.pack = makePack({ 0: { 1: 4 } }, [
			{ i: 4, group: "trails" },
		]) as any;
		expect(ta.getPathCost(0, 1)).toBe(5);
	});
...
describe("getDealBatches", () => {
	...
	it("skips deals whose market cannot be resolved", () => {
		const deals: any[] = [
			{ seller: 2, sellerType: "burg", buyer: 99, buyerType: "market" },
		];
...
describe("getPath", () => {
	...
	it("returns points and a land segment when a roads route connects the burgs", () => {
		...
		globalThis.pack = makePack({ 0: { 1: 0 }, 1: { 0: 0 } }, [
			{ i: 0, group: "roads" },
		]) as any;
		const result = ta.getPath({
			id: "1-2",
			deals: [],
			startBurgId: 1,
			endBurgId: 2,
...
describe("trigger", () => {
	...
	it("clears animations and returns when the layer is disabled", () => {
		...
		ta.trigger([
			{ id: "1-2", deals: [], startBurgId: 1, endBurgId: 2, type: "local" },
		]);
