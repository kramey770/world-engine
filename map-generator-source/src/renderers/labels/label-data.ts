...
function getMegalopolisLabels(): { composites: LabelData[] } {
	...
	for (const mega of megas.values()) {
		...
		for (const member of mega.members) {
			if (!member.capital)
				megalopolisMemberMinZoom.set(member.i, MEGALOPOLIS_SPLIT_ZOOM);
		}
		composites.push({
...
function buildStateLabel(state: State): LabelData | undefined {
	...
	const fitted =
		customPath || isPlainText(state.label) ? null : fitStateLabel(state, group);
...
function buildRouteLabel(route: Route): LabelData | undefined {
	...
	const defaultPath = isPlainText(route.label)
		? undefined
		: formatPathPoints(route.points);
	return {
		...route.label,
...
function formatPathPoints(pointLike: number[][]): Point[] {
	...
	if (trimmed.length && trimmed.at(0)![0] > trimmed.at(-1)![0])
		trimmed.reverse();
	return trimmed;
}

const LABEL_PATH_POINTS_RADIUS = 12;
function trimAroundCenter(
	points: Point[],
	radius = LABEL_PATH_POINTS_RADIUS,
): Point[] {
