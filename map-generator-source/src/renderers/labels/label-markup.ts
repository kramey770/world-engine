...
export function createLabelElements(label: LabelData, document: Document) {
	...
	if (label.pathPoints?.length) {
		...
		if (label.letterSpacing !== undefined)
			textPath.setAttribute("letter-spacing", `${label.letterSpacing}px`);
	...
	if (label.letterSpacing)
		text.setAttribute("letter-spacing", `${label.letterSpacing}px`);
...
function appendText(
	parent: SVGTextElement | SVGTextPathElement,
	value: string,
	x: string,
): void {
	...
	lines.forEach((line, index) => {
		const tspan = parent.ownerDocument.createElementNS(
			"http://www.w3.org/2000/svg",
			"tspan",
		);
