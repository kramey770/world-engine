...
export function getVisibleLabels(): LabelData[] {
	...
	const visibleGroups = new Set(
		options.labels.groups
			.filter((group) => isGroupVisible({ group, bounds }))
			.map(({ name }) => name),
	);
	return [...scene.values()].filter(
		(label) => visibleGroups.has(label.group) && isLabelVisible(bounds, label),
	);
}
...
function materializeLabel(
	...
	if (
		!groupOptions ||
		!isGroupVisible({ group: groupOptions, bounds: context.bounds })
	)
		return;
...
function reconcileLabels(context: ViewportRenderContext): void {
	...
	for (const group of options.labels.groups)
		reconcileGroup(labels, textPaths, group.name, context);
}
...
function reconcileGroup(
	...
	const group = labels.querySelector<SVGGElement>(
		`#${CSS.escape(`labels-${groupName}`)}`,
	);
	const groupOptions = options.labels.groups.find(
	...
	const visibleLabels = isVisible
		? (labelsByGroup.get(groupName) || []).filter((label) =>
				isLabelVisible(context.bounds, label),
			)
		: [];
...
function isLabelVisible(
	...
	if (!options.labels.showAll) {
		if (label.minZoom !== undefined && bounds.scale < label.minZoom)
			return false;
		if (label.maxZoom !== undefined && bounds.scale >= label.maxZoom)
			return false;
	}
