...
export function fitStateLabel(
	state: State,
	group: string,
): { pathPoints: Point[]; text: string; fontSize: number } {
	const mode =
		options.labels.groups.find((option) => option.name === group)?.mode ||
		"auto";
	...
	if (!cellsNumber)
		return {
			pathPoints: [],
			text: mode === "short" ? state.name : fullName,
			fontSize: 100,
		};
	...
	const basePath = getRegionLabelPath(
		state.i,
		pack.cells.state,
		pole,
		cellsNumber,
	...
	const fitLines = (lines: string[], fixedFontSize?: number) => {
		const textWidth =
			Math.max(...lines.map(estimateTextWidth), 1) * baseFontSize;
		const spacingWidth =
			Math.max(...lines.map((line) => Math.max([...line].length - 1, 0))) *
			letterSpacing;
		...
		const initialFontSize =
			fixedFontSize ??
			minmax(getFontSize(basePath.length), MIN_FONT_SIZE, MAX_FONT_SIZE);
		const offset =
			baseFontSize * (initialFontSize / 100) * lines.length * LINE_HALF_HEIGHT;
		let fittedPath = getRegionLabelPath(
			state.i,
			pack.cells.state,
			pole,
			cellsNumber,
		...
		if (!fittedPath.length && initialFontSize > MIN_FONT_SIZE) {
			const minOffset =
				baseFontSize * (MIN_FONT_SIZE / 100) * lines.length * LINE_HALF_HEIGHT;
			fittedPath = getRegionLabelPath(
				state.i,
				pack.cells.state,
				pole,
				cellsNumber,
		...
		const fontSize =
			fixedFontSize ?? minmax(rn(fitFontSize), MIN_FONT_SIZE, MAX_FONT_SIZE);
		const requiredPathLength =
			((textWidth * (fontSize / 100) + spacingWidth) / PATH_USAGE) *
			PATH_EXTENSION;
		const pathPoints =
			fontSize < 100
				? extendPathShoulders(fittedPath.pathPoints, requiredPathLength)
				: fittedPath.pathPoints;
	...
	if (state.label?.text) {
		selected = fitLines(
			state.label.text.split("|"),
			state.label.fontSize ?? 100,
		);
	} else if (mode === "short") {
		...
		const fullLabel =
			oneLine.fitFontSize >= twoLine.fitFontSize * ONE_LINE_GAIN
				? oneLine
				: twoLine;

		if (
			mode === "full" ||
			fullLabel.fitFontSize >= MIN_FULL_NAME_SIZE ||
			state.name === fullName
		) {
			selected = fullLabel;
		} else {
			...
			selected =
				shortLabel.fitFontSize >= fullLabel.fitFontSize * SHORT_NAME_GAIN
					? shortLabel
					: fullLabel;
		}
	}
...
function extendPathShoulders(
	...
	if (!shoulderLength || shoulderLength * 2 >= requiredLength)
		return pathPoints;
