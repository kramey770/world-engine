...
export function drawPrecipitation(): void {
	...
	const cellsNumberModifier =
		(+ensureEl<HTMLInputElement>("pointsInput").dataset.cells! / 10000) ** 0.25;
	const data = Array.from(cells.i as ArrayLike<number>).filter(
		(i) => cells.h[i] >= 20 && cells.prec[i],
	);
	const getRadius = (precipitation: number) =>
		rn(Math.sqrt(precipitation / 4) / cellsNumberModifier, 2);
...
function drawWindDirections(): void {
	...
	const wind = select(ensureEl<SVGGElement>("prec"))
		.append("g")
		.attr("id", "wind");
	...
	for (let tier = 0; tier < 6; tier++) {
		if (westerly.length > 1) {
			...
			if (west.length > 3)
				addArrow(20, getMeanY(west[0][0], west[west.length - 1][0]), "\u21C9");
		}
		...
		if (easterly.length > 1) {
			...
			if (east.length > 3)
				addArrow(
					graphWidth - 52,
					getMeanY(east[0][0], east[east.length - 1][0]),
					"\u21C7",
				);
		}
	}
