...
vi.mock("@/components/layers", () => ({
	Layers: { isOn: () => mocks.layerOn },
}));

vi.mock("@/renderers/emblems/renderer", () => ({
	EmblemRenderer: mocks.emblemRenderer,
}));
vi.mock("@/renderers/viewport/viewport-renderer", async (importOriginal) => {
	const actual =
		await importOriginal<
			typeof import("@/renderers/viewport/viewport-renderer")
		>();
	return {
		...actual,
		ViewportLayers: {
...
describe("viewport emblem rendering", () => {
	it("retries a full draw when an emblem is added during the deferred collision pass", () => {
		...
		expect(
			document.querySelector("#provinceEmblems use[data-i='1']"),
		).not.toBeNull();
	});
	...
	it("only materializes and renders definitions for emblems inside the viewport", () => {
		...
		expect(mocks.emblemRenderer.trigger).toHaveBeenCalledWith(
			"stateCOA1",
			pack.states[1].coa,
		);
		...
		const pannedUses =
			document.querySelectorAll<SVGUseElement>("#stateEmblems use");
	...
	it("shows out-of-range emblem groups only when showAll is enabled", () => {
		...
		renderViewport();
		expect(document.getElementById("stateEmblems")?.classList).toContain(
			"hidden",
		);
		...
		renderViewport();
		expect(document.getElementById("stateEmblems")?.classList).not.toContain(
			"hidden",
		);
	...
	it("keeps an edited emblem scene and DOM in sync", () => {
		drawEmblems();
		expect(
			document.querySelector("#stateEmblems use[data-i='1']"),
		).not.toBeNull();
	...
	it("frees the definition once the entity itself loses its coat of arms", () => {
		...
		pack.states[1] = {
			i: 1,
			removed: true,
		} as unknown as (typeof pack.states)[number];
		renderViewport();
	...
	it("removes definitions for invalid entities during reconciliation", () => {
		document.querySelector("#coas")!.innerHTML =
			'<g id="stateCOA1"></g><g id="provinceCOA1"></g>';
		pack.provinces.push({
			i: 1,
			center: 1,
			coa: { shield: "heater", t1: "gules" },
	...
	it("frees the shields panning left behind, sparing the ones still on screen", () => {
		...
		coas.insertAdjacentHTML(
			"beforeend",
			'<g id="stateCOA1"></g><g id="stateCOA2"></g>',
		);
		// stateCOA2 sits outside the viewport, so only an open dialog keeps it alive
		document.body.insertAdjacentHTML(
			"beforeend",
			'<svg id="dialog"><use href="#stateCOA2"></use></svg>',
		);
	...
	it("keeps the shields on hand while the view is showing them", () => {
		...
		for (let i = 0; i < 250; i++) {
			coas.insertAdjacentHTML("beforeend", `<g id="burgCOA${i}"></g>`);
			document
				.getElementById("burgEmblems")!
				.insertAdjacentHTML("beforeend", `<use href="#burgCOA${i}"></use>`);
		}
	...
	it("resolves export definitions from pack data when a viewport scene is not ready", async () => {
		pack.states.push({
			...
			center: 1,
			coa: { shield: "heater", t1: "gules" },
		} as (typeof pack.states)[number]);
		document
			.querySelector("#stateEmblems")!
			.insertAdjacentHTML("beforeend", '<use data-i="3" />');
	...
	it("persists the moved position on the COA belonging to the dragged use", () => {
		...
		const restored = document.querySelector<SVGUseElement>(
			"#stateEmblems use[data-i='2']",
		)!;
		const fontSize = Number(
			document.querySelector("#stateEmblems")!.getAttribute("font-size"),
		);
	...
	it("persists an edited size on the COA across viewport rematerialization", () => {
		...
		expect(
			document
				.querySelector("#stateEmblems use[data-i='1']")
				?.getAttribute("width"),
		).toBe("2.5em");
