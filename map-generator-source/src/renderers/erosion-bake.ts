...
function getBakeSize(bakeResolution: number): [number, number] {
	...
	const short = Math.max(
		64,
		Math.round(long * (aspect <= 1 ? aspect : 1 / aspect)),
	);
...
function buildCoastTexture(bakeW: number, bakeH: number) {
	...
	for (const river of pack.rivers || []) {
		...
		const points =
			river.points && river.points.length === river.cells.length
				? river.points
				: null;
		try {
			...
			riverCtx.fill(
				new Path2D(
					Rivers.getRiverPath(meandered, river.widthFactor, river.sourceWidth),
				),
			);
			...
			for (let pointIndex = 1; pointIndex < meandered.length; pointIndex++) {
				...
				riverCtx.beginPath();
				riverCtx.moveTo(
					meandered[pointIndex - 1][0],
					meandered[pointIndex - 1][1],
				);
	...
	for (const feature of pack.features) {
		...
		const surface = Math.round(
			Math.max(feature.height || SEA_LEVEL, SEA_LEVEL),
		);
		const path = new Path2D(Coastline.getFeaturePath(feature));
		surfaceCtx.fillStyle =
			surfaceCtx.strokeStyle = `rgb(${surface},${surface},${surface})`;
...
function buildRiverCanvas(bakeW: number, bakeH: number) {
	...
	for (const river of pack.rivers) {
		...
		for (let k = 0; k < river.cells.length; k++) {
			...
			const offset =
				river.widthFactor * (lengthWidth + fluxWidth) +
				(river.sourceWidth || 0);
...
function runErosionPass(
	...
	bakeH: number,
	textures: {
		height: THREEType.DataTexture;
		coast: THREEType.DataTexture;
		rivers: THREEType.Texture;
	},
) {
...
function enforceDownhillCourses(bakeResult: ErosionBakeResult) {
	...
	for (const river of pack.rivers) {
		...
		for (let k = 0; k < course.length; k++) {
			...
			const segmentTexels = Math.hypot(
				(x - prevX) * scaleX,
				(y - prevY) * scaleY,
			);
...
export async function bake(
	renderer: THREEType.WebGLRenderer,
	params: BakeParams,
): Promise<ErosionBakeResult | null> {
	...
	try {
		...
		const result: ErosionBakeResult = {
			key,
			heights,
			pixels,
			coast: coast.data,
