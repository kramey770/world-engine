...
const clampToRendererLimit = (value: number) => {
	...
	return maxTextureSize
		? Math.min(clampTextureResolution(value), maxTextureSize)
		: clampTextureResolution(value);
};
...
const setResolutionScale = (scale: number) => {
	...
	options.threeD.resolution = resolutionScaleToGlobeMultiplier(
		options.threeD.resolutionScale,
	);
	redraw();
};
...
function syncErosionUI() {
	...
	const section = document.getElementById(
		"options3dErosionSection",
	) as HTMLElement | null;
	if (section)
		section.style.display = options.threeD.erosion ? "block" : "none";

	const subdivide = document.getElementById(
		"options3dSubdivide",
	) as HTMLInputElement | null;
...
const setTimeOfDay = (presetName: string) => {
	...
	if (options.threeD.extendedWater)
		setColors(preset.skyColor, preset.waterColor);
};
...
async function newMesh(canvas: HTMLCanvasElement) {
	...
	spotLight.position.set(
		options.threeD.sun.x,
		options.threeD.sun.y,
		options.threeD.sun.z,
	);
	...
	options.threeD.resolutionScale = clampToRendererLimit(
		options.threeD.resolutionScale,
	);
	options.threeD.resolution = resolutionScaleToGlobeMultiplier(
		options.threeD.resolutionScale,
...
async function createTextLabel({
	...
	setFont();
	context2d.canvas.width = Math.max(
		1,
		...lines.map((line) => Math.ceil(context2d.measureText(line).width)),
	);
...
async function createLabels() {
	...
	function getBurgLabelOptions(burg: Burg) {
		const groupStyle = getGroupStyle({
			name: burg.label?.group || burg.group || "burg",
			type: "burg",
		});
		const size = Number.parseFloat(String(groupStyle["font-size"]));
		const letterSpacing =
			Number(burg?.label?.letterSpacing ?? groupStyle["letter-spacing"]) || 0;
	...
	function getStateLabelOptions(state: State) {
		const groupStyle = getGroupStyle({
			name: state.label?.group || "state",
			type: "state",
		});
		const size = Number.parseFloat(String(groupStyle["font-size"]));
		const letterSpacing =
			Number(state?.label?.letterSpacing ?? groupStyle["letter-spacing"]) || 0;
	...
	function getIconGeometry(groupName: string, iconSize: number) {
		...
		if (!iconGeometries[key]) {
			iconGeometries[key] = new Three.CylinderGeometry(
				iconSize * 2,
				iconSize * 2,
				iconSize,
				16,
	...
	function getLineMaterial(groupName: string, iconColor: string) {
		if (!lineMaterials[groupName]) {
			lineMaterials[groupName] = new Three.LineBasicMaterial({
				color: iconColor,
			});
		}
	...
	for (let i = 1; i < pack.burgs.length; i++) {
		...
		if (Layers.isOn("labels") && !burg.label?.hidden) {
			const burgSprite = await createTextLabel({
		...
		if (Layers.isOn("burgIcons")) {
			...
			const points = [
				new Three.Vector3(x, lineStart, z),
				new Three.Vector3(x, lineEnd, z),
			];
...
async function createMesh(
	...
	const useSatellite = Boolean(
		options.threeD.satellite &&
			!options.threeD.isGlobe &&
			!options.threeD.wireframe,
	);
	...
	if ((options.threeD.erosion || useSatellite) && !options.threeD.isGlobe) {
		const baseBakeResolution =
			options.threeD.erosionDetail >= 2048
				? 4096
				: options.threeD.erosionDetail > 512
					? 2048
					: 1024;
		const satelliteBakeResolution =
			options.threeD.resolutionScale >= 8192
				? 8192
				: options.threeD.resolutionScale >= 4096
					? 2048
					: 1024;
		const desiredBakeResolution = useSatellite
			? Math.max(baseBakeResolution, satelliteBakeResolution)
			: baseBakeResolution;
		const maxBakeResolution = Math.min(
		...
		if (!bakeResult && options.threeD.erosion) {
			...
			window.tip(
				"Eroded terrain is not supported on this device",
				false,
				"warn",
				4000,
			);
	...
	if (erosionBakeActive) {
		...
		const segX =
			width >= height
				? segLong
				: Math.max(2, Math.round((segLong * width) / height));
		const segY =
			width >= height
				? Math.max(2, Math.round((segLong * height) / width))
				: segLong;
		...
		if (options.threeD.subdivide) {
			...
			const smoothGeometry = (window as any).loopSubdivision.modify(
				geometry,
				1,
				subdivideParams,
			);
...
function getMeshHeight(i: number) {
	...
	if (height < 20) {
		...
		waterCellId =
			grid.cells.c[i].find((c: number) => grid.cells.h[c] < 20) ?? null;
	}
	...
	if (waterCellId !== null) {
		...
		const waterHeight =
			feature.type === "lake" && feature.height ? feature.height : 20;
...
function extendWater(width: number, height: number) {
	...
	waterMaterial = new Three.MeshBasicMaterial({
		color: options.threeD.waterColor,
	});
...
async function update3dTexture() {
	...
	if (
		...
		const satelliteTexture = generateSatelliteTexture(
			Renderer,
			erosionBakeData,
			{
				scale: options.threeD.scale,
...
async function OrbitControls(
	camera: THREE.Camera,
	domElement: HTMLElement,
): Promise<Controls | false> {
	if ((Three as any).OrbitControls)
		return new (Three as any).OrbitControls(camera, domElement);
		...
		script.onload = () =>
			resolve(
				(Three as any).OrbitControls
					? new (Three as any).OrbitControls(camera, domElement)
					: false,
			);
...
async function MapControls(
	...
	if ((Three as any).MapControls)
		return new (Three as any).MapControls(camera, domElement);
	...
	return new Promise((resolve) => {
		...
		script.onload = () =>
			resolve(
				(Three as any).MapControls
					? new (Three as any).MapControls(camera, domElement)
					: false,
			);
		script.onerror = () => resolve(false);
	});
...
async function updateGlobeTexure(addMesh?: boolean) {
	...
	img2.src = await Services.ExportMap.getMapURL("mesh", {
		noScaleBar: true,
		fullMap: true,
		noVignette: true,
	});
...
function OBJExporter(): any {
	...
	return new Promise((resolve) => {
		...
		script.onload = () =>
			resolve(
				(Three as any).OBJExporter ? new (Three as any).OBJExporter() : false,
			);
		script.onerror = () => resolve(false);
	});
}
