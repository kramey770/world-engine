...
export function drawLegend(name: string, data: LegendItem[]): void {
	...
	const boxStyle = (attr: string, inputId: string) =>
		box?.getAttribute(attr) ?? styleInput(inputId);
...
export function redrawLegend(): void {
	...
	const data: LegendItem[] = (legend.attr("data") || "")
		.split("|")
		.map((line) => line.split(","));
...
export function fitLegendBox(): void {
	...
	const px = Number.isNaN(Number(legend.attr("data-x")))
		? 0.99
		: Number(legend.attr("data-x")) / 100;
	const py = Number.isNaN(Number(legend.attr("data-y")))
		? 0.93
		: Number(legend.attr("data-y")) / 100;
...
const getBBox = (legend: ReturnType<typeof getLegend>) =>
	(legend.node() as SVGGElement).getBBox();
