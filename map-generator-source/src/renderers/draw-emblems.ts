...
function getEmblemSize(type: EmblemType, count: number): number {
	const { extent, min, max, expected, countDivisor, deficitDivisor } =
		SIZING[type];
	const startSize = minmax((graphHeight + graphWidth) / extent, min, max);
	const countMod =
		1 + count / countDivisor - (expected - count) / deficitDivisor;
...
export function drawEmblems(): void {
	...
	for (const type of TYPES)
		sizes[type] = getEmblemSize(type, valid[type].length);
	...
	timeout(() => {
		...
		for (const type of TYPES)
			ensureEl(GROUPS[type]).setAttribute("font-size", String(sizes[type]));
		for (const type of TYPES) {
 ...
 */
export function redrawEmblems(
	entities: Iterable<readonly [EmblemType, number]>,
): void {
...
export function subscribeToEmblemReconciliation(
	listener: () => void,
): () => void {
 ...
 */
export async function renderEmblemDefinitions(
	root: ParentNode,
): Promise<() => void> {
	const uses = Array.from(
		root.querySelectorAll<SVGUseElement>("#emblems use[data-i]"),
	);
...
function releaseTransientDefinitions(ids: string[]): void {
	...
	const materialized = new Set(
		Array.from(
			document.querySelectorAll<SVGUseElement>("#emblems use[data-i]"),
			(use) => use.getAttribute("href"),
		),
	);
	for (const id of ids)
		if (!materialized.has(`#${id}`)) EmblemRenderer.remove(id);
}
...
function reconcileEmblems(context: ViewportRenderContext): void {
	...
	for (const type of TYPES) {
		...
		for (const stored of scene.values()) {
			...
			const item = isCurrentNode(type, entity, stored)
				? stored
				: getNode(type, entity);
		...
		const materialized = new Map(
			Array.from(
				group.querySelectorAll<SVGUseElement>(":scope > use[data-i]"),
			).map((use) => [use.dataset.i!, use]),
		);
...
function evictDefinitionSlack(shown: number): void {
	...
	if (coas && coas.childElementCount > Math.max(DEFINITION_SLACK, shown * 2))
		evictUnreferencedDefinitions();
}
...
function materialize(
	...
	let use = existing;
	if (!use) {
		use = group.ownerDocument.createElementNS(
...
function getNode(
	type: EmblemType,
	entity: Burg | Province | State,
): EmblemData {
	const emblem = entity.coa;
	if (!emblem)
		throw new Error(`Cannot render ${getId(type, entity.i)} without a COA`);
...
function isCurrentNode(
	...
	const [poleX, poleY] = getPole(type, entity);
	return (
		(entity.coa.x !== undefined || stored.poleX === poleX) &&
		(entity.coa.y !== undefined || stored.poleY === poleY)
	);
}
...
function isVisible(
	{ x, y, shift }: EmblemData,
	{ bounds }: ViewportRenderContext,
): boolean {
	return (
		x + shift >= bounds.x0 &&
		x - shift <= bounds.x1 &&
