...
export function readBurgLabelStyles(
	root: ParentNode = document,
): Record<string, GroupStyle> {
	const out: Record<string, GroupStyle> = {};
	const burgGroups = options.labels.groups.filter(
		(group) => group.type === "burg",
	);
