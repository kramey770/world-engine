...
function getMeasurerStyle(): MeasurerStyle {
	...
	const strokeWidth =
		Number(ruler?.getAttribute("stroke-width")) || DEFAULT_STROKE_WIDTH;
	const fontSize =
		Number(ruler?.getAttribute("font-size")) || DEFAULT_FONT_SIZE;
	// an empty attribute means "no dashes"; only a missing one falls back to the default
	const dasharray =
		ruler?.getAttribute("stroke-dasharray") ?? DEFAULT_DASHARRAY;
...
export function drawMeasurers(): void {
	...
	for (const measurer of pack.measurers)
		RENDERERS[measurer.type](measurer, style);
}

const RENDERERS: Record<
	MeasurerType,
	(measurer: Measurer, style: MeasurerStyle) => void
> = {
	Ruler: renderRuler,
...
function renderRuler(
	measurer: Measurer,
	{ strokeWidth, dasharray, fontSize }: MeasurerStyle,
): void {
	...
	el.append("text")
		...
		.attr("x", x)
		.attr("y", y)
		.text(getDistance(length));
}

function renderPathMeasurer(
	measurer: Measurer,
	{ strokeWidth, dasharray, fontSize }: MeasurerStyle,
): void {
