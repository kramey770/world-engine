...
export function drawCoordinates(): void {
	...
	const { lonT, lonW, lonE, latN, latS } = mapCoordinates as Required<
	...
	coordinates.attr(
		"font-size",
		Math.max(rn(desiredSize / scale ** 0.8, 2), 0.1),
	);
	...
	const corner = point.matrixTransform(
		ensureEl<SVGGElement>("viewbox").getScreenCTM()!.inverse(),
	);
	...
	const labels = graticule.lines().map((line) => {
		...
		const [x, y] = isLatitude
			? [rn(corner.x, 2), rn(position[1], 2)]
			: [rn(position[0], 2), rn(corner.y, 2)];
