...
const burgIconsRenderer = (): void => {
	...
	for (const m of megas.values())
		for (const b of m.members) megaMemberIds.add(b.i);
	...
	for (const { name } of options.burgs.groups) {
		...
		const composites = [...megas.values()].filter(
			(m) => m.anchor.group === name,
		);
		if (composites.length)
			iconsGroup.innerHTML = compositesMarkup(iconsGroup, composites);
	}
...
function compositesMarkup(
	iconsGroup: SVGGElement,
	composites: Megalopolis[],
): string {
...
function renderVisibleIcons(context: ViewportRenderContext): void {
	...
	for (const { name } of options.burgs.groups) {
		const iconsGroup = root.querySelector<SVGGElement>(
			`#burgIcons > g#${CSS.escape(name)}`,
		);
		if (!iconsGroup) continue;

		const minZoom =
			options.labels.groups.find((group) => group.name === name)?.zoom?.min ??
			groupMinZoom(name);
		const gatePassed =
			unbounded || options.labels.showAll || bounds.scale >= minZoom;
		...
		const missing = visible.filter(
			(b) => !iconsGroup.querySelector(`:scope > #burg${b.i}`),
		);
		if (missing.length) {
		...
		const portGroup = root.querySelector<SVGGElement>(
			`#anchors > g#${CSS.escape(name)}`,
		);
		...
		const missingPorts = ports.filter(
			(b) => !portGroup.querySelector(`:scope > #anchor${b.i}`),
		);
...
export const removeBurgIcons = (): void => {
	for (const icon of Array.from(
		document.querySelectorAll("#icons use, #icons circle"),
	))
		icon.remove();
};
...
function createIconGroups(): void {
	...
	const sortedGroups = [...options.burgs.groups].sort(
		(a, b) => a.order - b.order,
	);
