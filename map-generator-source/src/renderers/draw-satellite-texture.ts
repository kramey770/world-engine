...
export function getSatelliteBiomeData(
	...
	return {
		color: [customColor.r / 255, customColor.g / 255, customColor.b / 255] as [
			number,
			number,
			number,
		],
...
export function generateSatelliteTexture(
	...
	try {
		...
		fieldTexture = new THREE.DataTexture(
			bakeResult.pixels,
			cols,
			rows,
			THREE.RGBAFormat,
		...
		coastTexture = new THREE.DataTexture(
			bakeResult.coast,
			cols,
			rows,
			THREE.RGBAFormat,
			...
			uniforms: {
				...
				uSlopeScale: {
					value: new THREE.Vector2(
						worldPerHeight / (graphWidth / cols),
						worldPerHeight / (graphHeight / rows),
					),
				},
...
export function generateRiverFlowTexture(): THREEType.Texture {
	...
	const steepness = (drop: number) =>
		Math.min(Math.max((drop - 0.6) / 2.4, 0), 1);
		...
		try {
			...
			for (let pointIndex = 1; pointIndex < meandered.length; pointIndex++) {
				...
				for (let s = 0; s < subs; s++) {
					...
					const drop =
						Math.max(0, heightAt(ax, ay, 82) - heightAt(bx, by, 82)) / subLen;
