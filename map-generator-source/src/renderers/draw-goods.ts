...
export function drawGoods() {
	...
	const visible = new Set(
		pack.goods.filter((good) => good.visible).map((good) => good.i),
	);
...
export function encodeCellFill(goodId: number, normalized: number): number {
	const bucket = Math.min(
		CELL_BUCKETS - 1,
		Math.floor(normalized * CELL_BUCKETS),
	);
...
export function cellFillColor(
	key: number,
	getGoodColor: (goodId: number) => string,
): string {
...
export function capVisible<T extends { x: number; y: number }>(
	...
	for (const item of items) {
		if (
			item.x < bounds.x0 - pad ||
			item.x > bounds.x1 + pad ||
			item.y < bounds.y0 - pad ||
			item.y > bounds.y1 + pad
		)
			continue;
		result.push(item);
...
function buildGoodsCellsContent(displayedGoods: Set<number>): string {
	...
	for (const [cellId, total] of totals) {
		keys[cellId] = encodeCellFill(
			dominant.get(cellId)!,
			normalize(total, 0, maxTotal),
		);
	}

	const isolines = getIsolines(pack, (cellId) => keys[cellId] || null, {
		fill: true,
	});
...
function buildPlateItems(displayedGoods: Set<number>): SceneItem[] {
	...
	const plateIcon =
		+select("#goods").select("#goodsBurgs").attr("data-size") || PLATE_ICON;
	...
	for (const burg of pack.burgs) {
		...
		const contentWidth =
			entries.reduce((sum, e) => sum + e.width, 0) +
			plateEntryGap * (entries.length - 1);
