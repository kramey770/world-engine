...
describe("paint overlay", () => {
	it("updates only the cells passed to it", () => {
		openPaintOverlay();
		updatePaintOverlay(graph, [
			{ cell: 0, values: [{ id: 1, color: "#ff0000" }] },
		]);

		const firstPolygon = document.querySelector<SVGPolygonElement>(
			'polygon[data-cell="0"]',
		)!;

		updatePaintOverlay(graph, [
			{ cell: 1, values: [{ id: 2, color: "#0000ff" }] },
		]);
	...
	it("replaces only the requested cell preview", () => {
		...
		const previous = document.querySelector<SVGPolygonElement>(
			'polygon[data-cell="0"]',
		)!;

		updatePaintOverlay(graph, [
			{ cell: 0, values: [{ id: 2, color: "#0000ff" }] },
		]);

		const current = document.querySelector<SVGPolygonElement>(
			'polygon[data-cell="0"]',
		)!;
