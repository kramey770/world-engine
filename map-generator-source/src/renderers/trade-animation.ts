...
export class TradeAnimationModule {
	...
	private topUp(): void {
		...
		const target =
			options.trade.animation.concurrent ?? DEFAULT_OPTIONS.concurrent;
	...
	private spawnOne(batches: TradeBatch[]): boolean {
		...
		while (true) {
			const enabledBatches =
				type === "both"
					? batches
					: batches.filter((batch) => batch.type === type);
	...
	getPath(batch: TradeBatch): TradePath | null {
		...
		const path =
			!startBurg || !endBurg
				? null
				: this.findRoutePath(startBurg.cell, endBurg.cell);
	...
	findRoutePath(startCell: number, endCell: number) {
		...
		while (queue.length) {
			...
			if (cell === endCell)
				return this.buildPathResult(stateId, prevCellArr, prevStateArr);
	...
	private buildPathResult(
		...
		for (let i = 1; i < cells.length - 1; i++) {
			...
			const edgePoints = this.extractEdgePoints(
				fromCell,
				toCell,
				pack.cells.routes[fromCell]?.[toCell],
				routeById,
			);
			...
			for (; k < edgePoints.length; k++)
				currentPoints.push([edgePoints[k][0], edgePoints[k][1]]);
		}
		segments.push({ type: currentType, points: currentPoints });
	...
	getDealBatches(deals: Deal[]): TradeBatch[] {
		...
		for (const deal of deals) {
			...
			const type =
				deal.sellerType === "market" && deal.buyerType === "market"
					? "global"
					: "local";
