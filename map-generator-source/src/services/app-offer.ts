...
const RELEASES_API =
	"https://api.github.com/repos/Azgaar/Fantasy-Map-Generator/releases/latest";
const RELEASES_PAGE =
	"https://github.com/Azgaar/Fantasy-Map-Generator/releases/latest";
...
async function detectTarget(): Promise<string | undefined> {
	...
	try {
		const { architecture } =
			(await agent?.getHighEntropyValues?.(["architecture"])) ?? {};
...
async function loadRelease(): Promise<Release | undefined> {
	try {
		...
		return {
			...
			assets: assets.map(({ name, browser_download_url, size }: Asset) => ({
				name,
...
function renderDownloads(release: Release, target: string | undefined): string {
	const find = (suffix: string) =>
		release.assets.find(({ name }) => name.endsWith(suffix));
	...
	const others = DOWNLOADS.filter(({ suffix }) => suffix !== target)
		...
		.map(
			({ label, asset }) =>
				`<a href="${asset!.browser_download_url}" target="_blank">${label}</a>`,
		)
		.join(" &middot; ");
	...
	const rest = others
		? `<p>${primary ? "Other systems" : "Downloads"}: ${others}.</p>`
		: "";
	return `${main}${rest}`;
}
