...
async function saveMap(method: SaveMethod): Promise<void> {
	if (customization)
		return tip(
			"Map cannot be saved in EDIT mode, please complete the edit and retry",
			false,
			"error",
		);
	closeDialogs("#alert");
...
function prepareMapData(): string {
	...
	const fonts = JSON.stringify(
		getUsedFonts(ensureEl("map") as Element as SVGSVGElement),
	);
	...
	for (const group of Array.from(cloneEl.querySelectorAll("#emblems > g")))
		group.innerHTML = "";
	...
	const { spacing, cellsX, cellsY, boundary, points, features, cellsDesired } =
		grid;
	...
	const customGoodIcons = Array.from(
		goodIconsEl.querySelectorAll('[id^="good-custom-"]') || [],
	)
		.map((el) => el.outerHTML)
		.join("")
	...
	const namesData = Names.nameBases
		.map((b, i) => {
			const names =
				defaultNameBases[i] && defaultNameBases[i].b === b.b ? "" : b.b;
