...
export async function resolveVersionConflicts(
	...
	const isOlderThan = (tagVersion: string) =>
		compareVersions(mapVersion, tagVersion).isOlder;
	...
	if (isOlderThan("1.139.0")) {
		...
		const [colorData = "", habitabilityData = "", nameData = ""] =
			data[3].split("|");
	...
	if (isOlderThan("1.142.0")) {
		...
		for (const f of pack.features) {
			if (f?.type === "lake" && !f.shoreline)
				f.shoreline = Lakes.defineShoreline(f);
		}
	}
	...
	if (isOlderThan("1.6.0")) {
		...
		for (const f of pack.features) {
			...
			const heights = pack.cells.c[f.firstCell]
	...
	if (isOlderThan("1.63.0")) {
		...
		if (oceanicPattern && !oceanicPattern.getAttribute("opacity"))
	...
	if (isOlderThan("1.65.0")) {
		...
		const defaultWidthFactor = rn(
			1 / (Number(pointsInput.dataset.cells) / 10000) ** 0.25,
			2,
		);
	...
	if (isOlderThan("1.7.0")) {
		...
		if (defs && markersGroup) {
			const markerElements =
				markersGroup.querySelectorAll<SVGUseElement>("use");
			...
			pack.markers = Array.from(markerElements).map((el, i) => {
				...
				const px = text
					? Number(text.getAttribute("font-size")?.replace("px", ""))
					: NaN;
				const dx = text
					? Number(text.getAttribute("x")?.replace("%", ""))
					: NaN;
				const dy = text
					? Number(text.getAttribute("y")?.replace("%", ""))
					: NaN;
	...
	if (isOlderThan("1.72.0")) {
		...
		const storedStyles = Object.keys(localStorage).filter((key) =>
	...
	if (isOlderThan("1.73.0")) {
		...
		const zones = Array.from(
			document.querySelectorAll<SVGGElement>("#zones > g"),
		);
		zones.forEach((zone) => {
	...
	if (isOlderThan("1.84.0")) {
		...
		if (!grid.cellsDesired)
			grid.cellsDesired = rn(
				(graphWidth * graphHeight) / grid.spacing ** 2,
				-3,
			);
	}
	...
	if (isOlderThan("1.88.0")) {
		...
		pack.states.forEach(({ coa }) => {
			if (coa && typeof coa === "object" && coa.shield === "state")
				delete coa.shield;
		});
	}
	...
	if (isOlderThan("1.91.0")) {
		...
		pack.states.forEach((state) => {
			if ((state.coa as unknown) === "custom")
				state.coa = { custom: true } as typeof state.coa;
		});
		pack.provinces.forEach((province) => {
			if ((province.coa as unknown) === "custom")
				province.coa = { custom: true } as typeof province.coa;
		});
		pack.burgs.forEach((burg) => {
			if ((burg.coa as unknown) === "custom")
				burg.coa = { custom: true } as typeof burg.coa;
		});
	...
	if (isOlderThan("1.94.0")) {
		...
		if (textureImage.size()) {
			...
			const href =
				textureImage.attr("xlink:href") ||
				textureImage.attr("href") ||
				textureImage.attr("src");
	...
	if (isOlderThan("1.96.0")) {
		...
		const curveTypes: Record<number, string> = {
			0: "curveBasisClosed",
			1: "curveLinear",
			2: "curveStep",
		};
		const curve =
			curveTypes[+select("#terrs").attr("curve")] || "curveBasisClosed";
	...
	if (isOlderThan("1.99.0")) {
		...
		for (const g of document.querySelectorAll("#viewbox > #routes > g")) {
			...
			for (const node of g.querySelectorAll<SVGPathElement>("path")) {
				...
				const feature =
					secondCellId === undefined ? undefined : pack.cells.f[secondCellId];
				...
				pack.routes.push({
					...
					feature,
					points,
				} as unknown as (typeof pack.routes)[number]);
			}
		}
		...
		select("#zones")
			...
			.each(function () {
				...
				pack.zones.push({
					...
					cells,
					color,
				} as unknown as (typeof pack.zones)[number]);
			});
	...
	if (isOlderThan("1.111.0")) {
		...
		if (!pack.ice.length) {
			...
			if (iceGroup) {
				...
				iceGroup
					.querySelectorAll<SVGPolygonElement>("polygon[type='iceShield']")
					.forEach((polygon) => {
						// Parse points string "x1,y1 x2,y2 x3,y3 ..." into array [[x1,y1], [x2,y2], ...]
						const points = [...polygon.points].map((svgPoint) => [
							svgPoint.x,
							svgPoint.y,
						]);
				...
				// Migrate icebergs
				iceGroup
					.querySelectorAll<SVGPolygonElement>("polygon:not([type])")
					.forEach((polygon) => {
	...
	if (isOlderThan("1.127.0")) {
		...
		if (pack.goods?.length && !pack.goods.some((good) => good.visible))
			pack.goods[0].visible = true;
	}
	...
	if (isOlderThan("1.138.0")) {
		...
		const MEASURER_TYPES = [
			"Ruler",
			"Opisometer",
			"RouteOpisometer",
			"Planimeter",
		];
		const isMeasurerType = (type: string): type is MeasurerType =>
			MEASURER_TYPES.includes(type);
	...
	if (isOlderThan("1.139.0")) {
		...
		if (
			terrs.attr("opacity") !== null ||
			terrs.attr("filter") !== null ||
			terrs.attr("scheme") !== null
		) {
			terrs
				.attr("opacity", null)
				.attr("filter", null)
	...
	if (isOlderThan("1.140.0")) {
		...
		const legacyStateMode =
			"stateLabelsMode" in options ? options.stateLabelsMode : undefined;
		const stateMode: LabelNameMode =
			legacyStateMode === "short" || legacyStateMode === "full"
				? legacyStateMode
				: "auto";
		...
		const burgGroups = Array.from(
			document.querySelectorAll<SVGGElement>("#burgLabels > g"),
		);
		for (const burgGroup of burgGroups) {
			const name = claimName(burgGroup.id);
			const oldStyle = deriveLabelsStyle(burgGroup);
			const zoom = deriveZoomExtent(
				Number.parseFloat(oldStyle["font-size"] as string),
			);

			options.labels.groups.push({
				name,
				type: "burg",
				isDefault: burgGroup.id === "towns",
				zoom,
			});
			style.labels.groups[name] = oldStyle;
		}

		const migratedBurgStyle = burgGroups.length
			? style.labels.groups[burgGroups[0].id]
			: undefined;
		const migratedBurgGroupIds = new Set(
			burgGroups.map((burgGroup) => burgGroup.id),
		);
		for (const burgGroup of options.burgs.groups) {
			...
			style.labels.groups[name] = migratedBurgStyle
				? { ...migratedBurgStyle }
				: getGroupStyle({ name, type: "burg" });
		}

		if (
			options.labels.groups.every((group) => !group.isDefault) &&
			options.labels.groups[0]
		)
		...
		for (const burg of pack.burgs) {
			const { labelDx, labelDy } = burg as {
				labelDx?: number;
				labelDy?: number;
			};
			if (labelDx || labelDy)
				burg.label = { ...burg.label, dx: labelDx || 0, dy: labelDy || 0 };
		...
		const provinceGroup = document.querySelector<SVGGElement>(
			"#provs #provinceLabels",
		);
		if (provs && provinceGroup) {
			...
			style.labels.groups.province = getGroupStyle({
				name: "province",
				type: "province",
			});
		}
		...
		pack.addedLabels = [];
		const addedGroups = Array.from(
			labels.querySelectorAll<SVGGElement>(
				":scope > g:not(#states):not(#burgLabels)",
			),
		);
		for (const addedGroup of addedGroups) {
			...
			for (const textEl of addedGroup.querySelectorAll<SVGTextElement>(
				":scope > text",
			)) {
				...
				if (label?.text && label.pathPoints?.length) {
					const [x, y] =
						label.pathPoints[Math.floor(label.pathPoints.length / 2)];
					const addedLabel = AddedLabels.add({
						x,
						y,
		...
		if (stateGroup) {
			...
			options.labels.groups.push({
				...Labels.getFallbackGroup("state"),
				mode: stateMode,
			});
			style.labels.groups.state = getGroupStyle({
				name: "state",
				type: "state",
			});
		}
		...
		for (const textEl of document.querySelectorAll<SVGTextElement>(
			...
			if (pathEl)
				state.label = getPathLabel({
					textEl,
					pathEl,
					names: [state.name, state.fullName],
				});
		}
		...
		function deriveZoomExtent(fontSize: number) {
			...
			return {
				min: inRange(rn(24 / fontSize - 1, 1)),
				max: inRange(rn(240 / fontSize - 1, 1)),
			};
		}
		...
		function getPathLabel({
			...
			const pathPoints = pathEl
				? parsePathPoints(pathEl.getAttribute("d") || "")
				: null;
			...
			if (textPath) {
				...
				const fontSize = Number.parseFloat(
					textPath.getAttribute("font-size") || "",
				);
				if (Number.isFinite(fontSize) && fontSize !== 100)
					label.fontSize = fontSize;
				const letterSpacing = Number.parseFloat(
					textPath.getAttribute("letter-spacing") || "",
				);
				if (letterSpacing && Number.isFinite(letterSpacing))
	...
	if (isOlderThan("1.142.0")) {
		...
		if (terrainEl) {
			...
			for (const attribute of ["set", "size", "density"])
				terrainEl.removeAttribute(attribute);
	...
	if (isOlderThan("1.144.0")) {
		...
		for (const group of options.labels?.groups ?? []) {
			const layer =
				group.layerDependency && LAYER_ID_MAP[group.layerDependency];
			if (layer) group.layerDependency = layer;
		}

		const storedPresets: Record<string, string[]> | null = safeParseJSON(
		...
		if (storedPresets) {
			...
			localStorage.setItem(
				"presets",
				JSON.stringify(Object.fromEntries(remapped)),
			);
		}
		...
		function recoverLayersState(): LayersState {
			...
			if (foggingContainer)
				foggingContainer.replaceWith(...(fogging ? [fogging] : []));
			...
			const has = (id: string, selector: string) =>
				Boolean(findEl(id)?.querySelector(selector));
			const shown = (id: string) =>
				findEl(id) && findEl(id)?.style.display !== "none";
			...
			const positions = new Map(
				Array.from(
					ensureEl("map").querySelectorAll("#viewbox > *, #map > g"),
			...
			const order = Layers.all
				...
				.sort(
					(a, b) => positions.get(a.elementId)! - positions.get(b.elementId)!,
				)
				.map((layer) => layer.id);
	...
	if (isOlderThan("1.145.0")) {
		...
		for (const layer of Layers.all) {
			for (const group of document.querySelectorAll<SVGGElement>(
				`#map g#${layer.elementId}`,
			)) {
				groups.add(group);
				for (const child of group.querySelectorAll<SVGGElement>("g[id]"))
					groups.add(child);
			}
		}
		...
		for (const sameId of groupsById.values()) {
			const keep =
				sameId.find((group) => !isEmpty(group)) ??
				(declared.has(sameId[0].id) ? sameId[0] : undefined);
	...
	if (isOlderThan("1.146.0")) {
		...
		for (const use of Array.from(
			document.querySelectorAll(
				"#coastline > g > use[data-f], #lakes > g > use[data-f]",
			),
		)) {
		...
		function recoverMovedVertices(): GraphOverrides | null {
			...
			for (const layerId of FILL_LAYERS) {
				const paths = Array.from(
					findEl(layerId)?.querySelectorAll("path") || [],
				);
			...
			INFO &&
				console.info(
					`Recovered ${Object.keys(moved).length} moved vertices from the map svg`,
				);
			...
			for (let resolved = true; resolved; ) {
				...
				for (let index = 0; index < ids.length; index++) {
					...
					const candidates = vertices.v[prev].filter(
						(vertexId) =>
							vertices.v[next].includes(vertexId) &&
							!inChain.has(String(vertices.p[vertexId])),
					);
					...
					const vertexId = candidates.sort(
						(a, b) =>
							distance(vertices.p[a], point) - distance(vertices.p[b], point),
					)[0];
    ...
    `;
		...
		function restoreMissingDefTemp(): void {
			...
			const restore = (node: Element, parent: Element) => {
				...
				for (const child of Array.from(node.children))
					if (child.id) restore(child, existing);
			};
			...
			if (restored.length)
				WARN &&
					console.warn(
						"[Auto-update] Restored missing svg defs:",
						restored.join(", "),
					);
		}
		...
		async function restoreLayerStyles(): Promise<void> {
			const [, preset] = await (window as any).getStylePreset(
				localStorage.getItem("presetStyle") || "default",
			);

			for (const layer of Layers.all) {
				restoreGroupStyle(
					layer.elementId,
					preset[`#${layer.elementId}`],
					layer.params.attrs,
				);
				...
				for (const child of layer.children) {
					const style =
						preset[`#${child.id}`] ||
						preset[`#${layer.elementId} > #${child.id}`];
		...
		function restoreGroupStyle(
			...
			if (!style || group?.tagName !== "g" || !isBareGroup(group, declared))
				return;

			for (const [name, value] of Object.entries(style)) {
				if (value === null || value === "null") continue;
				if (id === "terrain" && ["set", "size", "density"].includes(name))
					continue;
				group.setAttribute(name, String(value));
			}
		}

		function isBareGroup(
			group: Element,
			declared: Record<string, string> = {},
		): boolean {
			const ignored = new Set(["id", "style", ...Object.keys(declared)]);
			return Array.from(group.attributes).every((attribute) =>
				ignored.has(attribute.name),
			);
		}
	}
}
