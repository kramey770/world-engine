...
async function exportToSvg(): Promise<void> {
	...
	try {
		...
		tip(
			`SVG export failed: ${(error as Error)?.message || "Unknown error"}`,
			true,
			"error",
			5000,
		);
...
async function exportToPng(): Promise<void> {
	...
	try {
		...
		const resolution =
			ensureEl<HTMLInputElement>("pngResolutionInput").valueAsNumber;
		const link = document.createElement("a");
		...
		const blob = await new Promise<Blob>((resolve, reject) => {
			...
			img.onerror = () =>
				reject(new Error("Cannot load map image for PNG export"));
			img.src = url;
		});
		...
		ERROR && console.error(error);
		tip(
			`PNG export failed: ${(error as Error)?.message || "Unknown error"}`,
			true,
			"error",
			5000,
		);
...
async function exportToJpeg(): Promise<void> {
	...
	try {
		...
		const blob = await new Promise<Blob>((resolve, reject) => {
			...
			img.onerror = () =>
				reject(new Error("Cannot load map image for JPEG export"));
			img.src = url;
		});
		...
		ERROR && console.error(error);
		tip(
			`JPEG export failed: ${(error as Error)?.message || "Unknown error"}`,
			true,
			"error",
			5000,
		);
...
async function exportToPngTiles(): Promise<void> {
	...
	zip
		...
		.catch((error: Error) => {
			...
			tip(
				`PNG tiles export failed: ${error?.message || "Unknown error"}`,
				true,
				"error",
				5000,
			);
...
async function getMapURL(
	...
	const cloneEmblems =
		cloneEl.getElementById("emblems")?.querySelectorAll("use") ?? [];
	...
	if (cloneEl.getElementById("burgIcons")) {
		...
		for (const group of Array.from(groups)) {
			if (!group.dataset.icon || cloneDefs.querySelector(group.dataset.icon))
				continue;
	...
	// add goods icons
	if (
		cloneEl.getElementById("goodsIcons") ||
		cloneEl.getElementById("goodsBurgs")
	) {
		const uniqueIcons = new Set<string>();
		const goodsUseElements = cloneEl.querySelectorAll(
			"#goodsIcons use, #goodsBurgs use",
		);
...

	{
		...
		const externalMarkerImages = cloneEl.querySelectorAll<SVGImageElement>(
			'#markers image[href]:not([href=""])',
		);
		const imageHrefs = Array.from(externalMarkerImages).map((img) =>
			...
			await new Promise<void>((resolve) => {
				getBase64(url, (base64) => {
					externalMarkerImages.forEach((img) => {
						if (typeof base64 === "string" && img.getAttribute("href") === url)
...

	{
		...
		const externalRegimentImages = cloneEl.querySelectorAll<SVGImageElement>(
			'#armies image[href]:not([href=""])',
		);
		const imageHrefs = Array.from(externalRegimentImages).map((img) =>
			img.getAttribute("href"),
		);
	...
	const hatchingFills = unique(
		Array.from(hatchingUsers).map((el) => el.getAttribute("fill")),
	);
...
export function flattenSymbolReferences(svg: SVGSVGElement): void {
	...
	svg.querySelectorAll("use").forEach((use) => {
		...
		const symbol = svg.querySelector<SVGSymbolElement>(
			`symbol[id="${href.slice(1)}"]`,
		);
		if (!symbol) return;
		...
		const width =
			resolveLength(use.getAttribute("width")) ??
			resolveLength(symbol.getAttribute("width")) ??
			vw;
		const height =
			resolveLength(use.getAttribute("height")) ??
			resolveLength(symbol.getAttribute("height")) ??
			vh;
		...
		use.setAttribute(
			"transform",
			existing ? `${transform} ${existing}` : transform,
		);
...
function removeUnusedElements(clone: MapSelection): void {
	if (!select("#terrain").selectAll("use").size())
...
function updateMeshCells(clone: MapSelection): void {
	...
	const data = renderOcean
		? cellIds
		: cellIds.filter((i) => grid.cells.h[i] >= 20);
	const scheme = getColorScheme(
		select("#terrs").select("#landHeights").attr("scheme"),
	);
...
function inlineStyle(clone: MapSelection): void {
	...
	clone
		.selectAll<SVGElement, unknown>("g, #ruler *, #scaleBar > text")
		.each(function () {
...
function saveGeoJsonCells(): void {
	...
	const json: { type: string; features: unknown[] } = {
	...
	const getHeight = (i: number) =>
		parseInt(getFriendlyHeight(cells.p[i], pack, grid), 10);
...
function saveGeoJsonMarkers(): void {
	const features = pack.markers.map((marker) => {
		const { i, type, icon, x, y, size, fill, stroke } =
			marker as typeof marker & {
				size?: number;
				fill?: string;
				stroke?: string;
			};
