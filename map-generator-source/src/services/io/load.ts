...
async function quickLoad(): Promise<void> {
	...
	if (blob) loadMapPrompt(blob);
		...
		tip(
			"No map stored. Save map to browser storage first",
			true,
			"error",
			2000,
		);
...
async function createSharableDropboxLink(): Promise<void> {
	const mapFile = (
		document.querySelector("#loadFromDropbox select") as HTMLSelectElement
	).value;
	...
	try {
		...
		const directLink = previewLink.replace(
			"www.dropbox.com",
...
async function uncompress(
	...
	try {
		const uncompressedStream = new Blob([compressedData])
			.stream()
...
async function parseLoadedData(
	...
	try {
		...
		customization = 0;
		if (ensureEl("customizationMenu").offsetParent)
...

		{
			...
			if (settings[17])
				options.temperatureNorthPole = options.temperatureSouthPole =
					+settings[17];
			...
			if (settings[26])
				ensureEl<HTMLInputElement>("growthRate").value = settings[26];
		}
		...
		pack.provinces = data[30]
			? JSON.parse(data[30])
			: ([0] as unknown as typeof pack.provinces);
		...
		if (data[31]) {
			...
			namesDL.forEach((d, i) => {
				...
				if (!e.length) return;
				const b =
					e[5].split(",").length > 2 || !Names.nameBases[i]
						? e[5]
						: Names.nameBases[i].b;
				Names.nameBases[i] = {
					name: e[0],
		...
		if (data[45]) {
			...
			if (goodIconsDefs)
				goodIconsDefs.insertAdjacentHTML("beforeend", data[45]);
		}
		...
		if (heightmapColorSchemes) {
			...
			if (oceanScheme && !(oceanScheme in heightmapColorSchemes))
				addCustomColorScheme(oceanScheme);
			...
			if (landScheme && !(landScheme in heightmapColorSchemes))
				addCustomColorScheme(landScheme);
		}
		...
		// data integrity checks
		{
			...
			const featureVerticesMismatch = pack.features.some((f) =>
				f?.vertices?.some((vertex) => !vertices.p[vertex]),
			);

			if (cellsMismatch || featureVerticesMismatch) {
				const message =
					"[Data integrity] Striping issue detected. To fix try to edit the heightmap in ERASE mode";
			...
			const invalidProvinces = [...new Set(cells.province)].filter(
				(p) =>
					p &&
					(!pack.provinces[p] ||
						(pack.provinces[p] as { removed?: boolean }).removed),
			);
			invalidProvinces.forEach((p) => {
				...
				ERROR &&
					console.error(
						"[Data integrity] Invalid province",
						p,
						"is assigned to cells",
						invalidCells,
					);
			...
			invalidCultures.forEach((c) => {
				...
				ERROR &&
					console.error(
						"[Data integrity] Invalid culture",
						c,
						"is assigned to cells",
						invalidCells,
					);
			...
			invalidReligions.forEach((r) => {
				...
				ERROR &&
					console.error(
						"[Data integrity] Invalid religion",
						r,
						"is assigned to cells",
						invalidCells,
					);
			...
			invalidFeatures.forEach((f) => {
				...
				ERROR &&
					console.error(
						"[Data integrity] Invalid feature",
						f,
						"is assigned to cells",
						invalidCells,
					);
			...
			const invalidBurgs = [...new Set(cells.burg)].filter(
				(burgId) =>
					burgId && (!pack.burgs[burgId] || pack.burgs[burgId].removed),
			);
			...
			pack.burgs.forEach((burg) => {
				if (typeof burg.capital === "boolean")
					burg.capital = Number(burg.capital);

				if (!burg.i && burg.lock) {
					ERROR &&
						console.error(
							`[Data integrity] Burg 0 is marked as locked, removing the status`,
						);
					delete burg.lock;
					return;
				}
				...
				if (burg.removed && burg.lock) {
					ERROR &&
						console.error(
							`[Data integrity] Removed burg ${burg.i} is marked as locked. Unlocking the burg`,
						);
					delete burg.lock;
					return;
				}
				...
				if (
					...
					ERROR &&
						console.error(
							`[Data integrity] Burg ${burg.i} is missing cell info or coordinates. Removing the burg`,
						);
					burg.removed = true;
				}
				...
				if (burg.state === undefined) {
					ERROR &&
						console.error("[Data integrity] Burg", burg.i, "has no state data");
					burg.state = 0;
				}
			});
			...
			pack.states.forEach((state) => {
				...
				const stateBurgs = pack.burgs.filter(
					(b) => b.state === state.i && !b.removed,
				);
				...
				if (capitalBurgs.length > 1) {
					const message = `[Data integrity] State ${state.i} has multiple capitals (${capitalBurgs
						...
						.join(
							", ",
						)}) assigned. Keeping the first as capital and moving others`;
				...
				if (state.i && stateBurgs.length && !capitalBurgs.length) {
					ERROR &&
						console.error(
							`[Data integrity] State ${state.i} has no capital. Making the first burg capital`,
						);
			...
			pack.routes.forEach((route) => {
				if (!route.points || route.points.length < 2) {
					ERROR &&
						console.error(
							`[Data integrity] Route ${route.i} has less than 2 points. Removing the route`,
						);
					Routes.remove(route);
				}
			});
			...
			for (const from in pack.cells.routes) {
				...
				for (const to in value) {
					...
					if (!route) {
						ERROR &&
							console.error(
								`[Data integrity] Route ${routeId} from ${from} to ${to} is missing. Removing the route`,
							);
					...
					if (markerIds[marker.i]) {
						ERROR &&
							console.error(
								...
								marker.i,
								"has non-unique id. Changing to",
								nextId,
							);

						const domElements = document.querySelectorAll<HTMLElement>(
							`#marker${marker.i}`,
						);
						if (domElements[1]) domElements[1].id = `marker${nextId}`; // rename 2nd dom element

						const noteElements = notes.filter(
		...
		WARN &&
			console.warn(
				`TOTAL: ${rn((performance.now() - uploadTimeStart) / 1000, 2)}s`,
			);
		logStats();
