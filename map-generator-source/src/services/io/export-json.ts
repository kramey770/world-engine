...
function exportToJson(type: ExportJsonType): void {
	if (customization) {
		tip(
			"Data cannot be exported when edit mode is active, please exit the mode and retry",
			false,
			"error",
		);
		return;
...
function getMapInfo() {
	return {
		version: VERSION,
		description:
			"Azgaar's Fantasy Map Generator output: azgaar.github.io/Fantasy-map-generator",
...
function getGridCellsData() {
	...
	const gridData = {
		...
		vertices: Array.from(grid.vertices.p).map(
			(_: unknown, vertexId: number) => ({
				i: vertexId,
