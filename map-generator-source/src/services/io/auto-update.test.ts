...
beforeEach(() => {
	...
	(
		globalThis as typeof globalThis & {
			getStylePreset: () => Promise<[string, object]>;
		}
	).getStylePreset = async () => ["default", {}];
});
...
describe("v1.144 layer id migration", () => {
	...
	it("maps exceptional legacy toggle ids and preserves unknown dependencies", () => {
		...
		expect(
			options.labels?.groups.map((group) => group.layerDependency),
		).toEqual(["heightmap", "markets", "burgIcons", "scaleBar", "customLayer"]);
	...
	it("rewrites already-current presets to themselves, so a repeated pass is harmless", () => {
		localStorage.setItem(
			"presets",
			JSON.stringify({ mine: ["biomes", "states"] }),
		);
...
describe("v1.145 svg layer cleanup", () => {
	...
	it("keeps an empty layer group that is the only one with its id", () => {
      ...
      <g id="texture" data-href="./images/textures/marble-big.jpg" mask="url(#land)" style="display: none;"></g>
		...
		expect(document.querySelector("#texture")?.getAttribute("data-href")).toBe(
...
describe("v1.145.2 moved vertices recovery", () => {
	...
	const withStates = (
		d: string,
	) => /* html */ `<svg id="map"><g id="viewbox"><g id="regions"><g id="statesBody">
...
describe("v1.146 rendering groups", () => {
	...
	it("leaves current maps alone", () => {
		...
		expect(pack.features.slice(1).every((feature) => !feature.subtype)).toBe(
			true,
		);
	});
});
...
describe("v1.140 label group migration", () => {
	...
	function setupLegacyMap() {
      ...
      <g id="viewbox">
		...
		globalThis.style = {
			labels: { groups: {} },
		} as unknown as typeof globalThis.style;
		globalThis.pack = {
			features: [],
	...
	it("derives zoom bounds the Label Groups editor accepts", () => {
		...
		const outOfRange = options.labels.groups
			.filter(
				({ zoom }) =>
					(zoom.min !== null && zoom.min < 0.01) ||
					(zoom.max !== null && zoom.max < 0.01),
			)
		...
		expect(
			options.labels.groups.find(({ name }) => name === "state")?.zoom,
		).toEqual({ min: null, max: 2.4 });
		expect(
			options.labels.groups.find(({ name }) => name === "towns")?.zoom,
		).toEqual({ min: 7, max: 79 });
	});
	...
	it("renames a legacy group that claims a name the migration reserves", () => {
		...
		expect(
			options.labels.groups.find(({ name }) => name === "river")?.type,
		).toBe("river");
		expect(
			options.labels.groups.find(({ name }) => name === "river2")?.type,
		).toBe("burg");
...
describe("missing svg defs", () => {
	const getDeftempIds = () =>
		Array.from(document.querySelectorAll("#deftemp > *"), (node) => node.id);
	...
	it("restores every defs element index.html declares", () => {
		...
		const restored = Array.from(
			document.querySelectorAll("#map defs [id]"),
			(node) => node.id,
		);
