...
const setToken = (provider: string, key: string) =>
	localStorage.setItem(lSKey(provider), key);
const getToken = (provider: string) => localStorage.getItem(lSKey(provider));
const setRefreshToken = (provider: string, key: string) =>
	localStorage.setItem(`${lSKey(provider)}-refresh`, key);
const getRefreshToken = (provider: string) =>
	localStorage.getItem(`${lSKey(provider)}-refresh`);
const setTokenExpiry = (provider: string, ts: number) =>
	localStorage.setItem(`${lSKey(provider)}-expires`, String(ts));
const getTokenExpiry = (provider: string) =>
	Number(localStorage.getItem(`${lSKey(provider)}-expires`)) || undefined;
...
const dropbox: DropboxProvider = {
	...
	async save(fileName, contents) {
		const resp = await this.call("filesUpload", {
			path: `/${fileName}`,
			contents,
		});
	...
	async list() {
		...
		const filesData: CloudFile[] = resp.result.entries.map(
			({ name, client_modified, size, path_lower }: any) => ({
				name,
	...
	auth() {
		...
		if (isElectron()) {
			const message =
				"Dropbox is not available in the Desktop App, use the browser version to sync maps";
		...
		if (!this.authWindow) {
			const message =
				"Dropbox sign-in window was blocked. Allow popups for this site and retry";
			this.returnError(message);
			return Promise.reject(new Error(message));
		}
	...
	async setDropBoxToken({ token, refreshToken, expiresAt }) {
		DEBUG.cloud &&
			console.info("Access token:", token, "refresh token:", refreshToken);
...
export const CloudStorage = {
	save: (fileName: string, contents: string) =>
		dropbox.save(fileName, contents),
