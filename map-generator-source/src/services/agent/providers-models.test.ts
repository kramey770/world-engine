...
describe("filterChatModels", () => {
	it("keeps chat models and drops audio, image and embedding variants for OpenAI", () => {
		...
		expect(filterChatModels("openai", ids)).toEqual([
			"gpt-5.6-luna",
			"o4-mini",
		]);
	});
	...
	it("drops embeddings, moderation and OCR models for Mistral", () => {
		...
		expect(filterChatModels("mistral", ids)).toEqual([
			"mistral-small-latest",
			"codestral-latest",
		]);
	});
	...
	it("keeps everything for local servers", () => {
		expect(filterChatModels("local", ["llama3.2", "qwen2.5-coder:7b"])).toEqual(
...
describe("listModels", () => {
	...
	it("caches what it fetched", async () => {
		...
		expect(cachedModels("deepseek")).toEqual([
			"deepseek-chat",
			"deepseek-reasoner",
		]);
	});
...
describe("mergeModels", () => {
	it("keeps curated models first and appends new discovered ones without duplicates", () => {
		expect(
			mergeModels(["qwen-flash", "qwen-plus"], ["qwen-plus", "qwen-max"]),
		).toEqual(["qwen-flash", "qwen-plus", "qwen-max"]);
	});
});
