...
const FILTERS: Partial<
	Record<ProviderSpec["id"], { include?: RegExp; exclude?: RegExp }>
> = {
	anthropic: { include: /^claude/ },
	openai: {
		include: /^(gpt|o\d)/,
		exclude:
			/audio|realtime|image|tts|embed|whisper|moderation|transcribe|dall/,
	},
	mistral: { exclude: /embed|moderation|ocr|voxtral|transcribe/ },
	qwen: {
		include: /^qwen/,
		exclude: /embed|ocr|audio|tts|asr|image|video|omni|vl|mt/,
	},
...
export function filterChatModels(
	...
	return ids.filter(
		(id) =>
			(!filter.include || filter.include.test(id)) && !filter.exclude?.test(id),
	);
}
...
export async function listModels(
	...
	const models = filterChatModels(
		providerId,
		await fetchModelIds(providerId, key),
	);
...
async function fetchModelIds(
	...
	const response = await fetch(modelsUrl(providerId), {
		headers: authHeaders(providerId, key),
	});
	if (!response.ok)
...
function modelsUrl(providerId: ProviderSpec["id"]): string {
	...
	if (providerId === "local") {
		const base = (
			localStorage.getItem(LOCAL_URL_STORAGE) || DEFAULT_LOCAL_URL
		).replace(/\/+$/, "");
		return `${base}/models`;
	}
...
export function cacheModels(
	...
	try {
		localStorage.setItem(
			`fmg-ai-models-${providerId}`,
			JSON.stringify({ time: Date.now(), models }),
		);
	} catch {
...
export function cachedModels(providerId: ProviderSpec["id"]): string[] {
	try {
		const stored = JSON.parse(
			localStorage.getItem(`fmg-ai-models-${providerId}`) ?? "null",
		);
...
export function mergeModels(curated: string[], discovered: string[]): string[] {
	return [
		...curated,
		...discovered.filter((model) => !curated.includes(model)),
	];
}
