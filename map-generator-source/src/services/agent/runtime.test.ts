...
suite("runScript", () => {
	...
	test("sees page globals and supports top-level await", async () => {
		...
		const result = await runScript(
			"const value = await Promise.resolve(answerForTest); return value",
		);
	...
	test("captures console output", async () => {
		const result = await runScript(
			"console.log('checked', 3); console.warn('careful'); return null",
		);
...
suite("serialize", () => {
	...
	it("labels functions and undefined", () => {
		...
		expect(serialize({ draw: function drawStates() {} })).toContain(
			"[Function drawStates]",
		);
	});
...
suite("describe", () => {
	...
	it("falls back to describing the string when it is not an expression", () => {
		expect(describe("not an expression!!")).toEqual({
			type: "string",
	...
	it("lists prototype methods of class instances", () => {
		...
		const described = describe(new StatesModule()) as {
			type: string;
			methods: string[];
		};
	...
	it("samples typed arrays", () => {
		expect(describe(new Uint16Array([5, 6, 7]))).toEqual({
			type: "Uint16Array",
			length: 3,
			sample: [5, 6, 7],
		});
