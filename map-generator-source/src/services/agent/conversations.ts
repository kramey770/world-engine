...
export function touch(conversation: Conversation): void {
	const firstQuestion = conversation.entries.find(
		(entry) => entry.kind === "message" && entry.role === "user",
	);
	if (firstQuestion?.kind === "message")
		conversation.title = shorten(firstQuestion.text);
...
const shorten = (text: string): string =>
	text.length > TITLE_LENGTH
		? `${text.slice(0, TITLE_LENGTH).trimEnd()}…`
		: text;
...
function load(): Conversation[] {
	try {
		...
		return Array.isArray(stored)
			? stored.filter((item) => item?.id && Array.isArray(item.entries))
			: [];
	} catch {
		return [];
	}
