...
function inspect(value: unknown): Record<string, unknown> {
	...
	if (ArrayBuffer.isView(value)) {
		...
		return {
			type: typed.constructor.name,
			length: typed.length,
			sample: Array.from(typed.slice(0, 5)),
		};
	}
	...
	if (Array.isArray(value)) {
		return {
			type: "Array",
			length: value.length,
			sample: value.slice(0, 3).map(summarize),
		};
	}
...
export function serialize(value: unknown, limit = MAX_RESULT_CHARS): string {
	...
	const replacer = (_key: string, item: unknown): unknown => {
		...
		if (item instanceof Set)
			return { Set: [...item].slice(0, MAX_ARRAY_ITEMS) };
		if (item instanceof Map)
			return { Map: [...item.entries()].slice(0, MAX_ARRAY_ITEMS) };
		...
		if (item && typeof item === "object") {
			...
			if (Array.isArray(item) && item.length > MAX_ARRAY_ITEMS) {
				return [
					...item.slice(0, MAX_ARRAY_ITEMS),
					`… ${item.length - MAX_ARRAY_ITEMS} more items`,
				];
			}
		}
