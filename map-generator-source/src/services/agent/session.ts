	...
	input_schema: {
		...
		properties: {
			code: {
				type: "string",
				description:
					"JavaScript to execute. Use `return` to produce the result.",
			},
		},
		required: ["code"],
	},
...
export function createSession(getConfig: () => SessionConfig) {
	...
	async function ask(
		...
		try {
			...
			handlers.onText(
				`(stopped after ${MAX_ITERATIONS} script steps — ask again to continue)`,
			);
		} finally {
			controller = null;
			handlers.onStatus("");
...
export function trimHistory(messages: Message[]): void {
	...
	results
		.slice(0, Math.max(0, results.length - KEEP_DETAILED_RESULTS))
		.forEach((block) => {
