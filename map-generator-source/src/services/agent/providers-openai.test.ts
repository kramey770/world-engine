			...
			tool_calls: [
				{
					id: "call_1",
					type: "function",
					function: { name: "run", arguments: '{"code":"return 1;"}' },
				},
			],
		});
	});
...
describe("toChatMessages content null", () => {
	it("sends null content when an assistant turn has only tool calls", () => {
		const messages: Message[] = [
			{
				role: "assistant",
				content: [{ type: "tool_use", id: "c", name: "run", input: {} }],
			},
		];
		const [, assistant] = toChatMessages(system, messages);
			...
			choices: [
				{
					message: {
						...
						tool_calls: [
							{
								id: "call_9",
								function: { name: "run", arguments: '{"code":"return 2;"}' },
							},
						],
					},
			...
			choices: [
				{
					message: {
						tool_calls: [
							{ id: "c", function: { name: "run", arguments: "{broken" } },
						],
					},
					finish_reason: "tool_calls",
				},
