...
export function compareVersions(
	...
	let [major1, minor1, patch1] = version1!.split(".").map(Number) as [
		number,
		number,
		number,
	];
	let [major2, minor2, patch2] = version2!.split(".").map(Number) as [
		number,
		number,
		number,
	];
...
function announceVersion(): void {
	if (parseMapVersion(VERSION) !== VERSION)
		alert("versioning: Invalid format or parsing function");
	...
	if (
		compareVersions(storedVersion, VERSION, {
			major: true,
			minor: true,
			patch: false,
		}).isOlder
		...
		localStorage.setItem("version", VERSION);
		tip(
			`Updated to v${VERSION}. Reload the page if you get errors`,
			true,
			"success",
			6000,
		);
