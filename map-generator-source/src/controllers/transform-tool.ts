...
function addListeners(): void {
	...
	ensureEl<HTMLInputElement>("transformPointsInput").oninput =
		handlePointsInput;
...
async function loadPreview(): Promise<void> {
	...
	img.onload = () => {
		...
		$canvas
			.getContext("2d")
			?.drawImage(img, 0, 0, width * SCALE, height * SCALE);
	};
}
...
function handleInput(): void {
	...
	const EXP = 1.0965;
	const scale = rn(
		EXP ** +ensureEl<HTMLInputElement>("transformScaleInput").value,
		2,
	); // [0.1, 10]x
...
function handleMousemove(e: MouseEvent): void {
	...
	ensureEl<HTMLInputElement>("transformShiftX").value = String(
		Math.round(mouseX + e.clientX / previewScale),
	);
	ensureEl<HTMLInputElement>("transformShiftY").value = String(
		Math.round(mouseY + e.clientY / previewScale),
	);
	handleInput();
}
...
function transformMap(): void {
	...
	const transformPointsValue = ensureEl<HTMLInputElement>(
		"transformPointsInput",
	).value;
	const globalPointsValue = ensureEl<HTMLInputElement>("pointsInput").value;
	if (transformPointsValue !== globalPointsValue)
		changeCellsDensity(transformPointsValue);
...
function getProjection(): [
	...
	const shiftY = +ensureEl<HTMLInputElement>("transformShiftY").value;
	const angle =
		(+ensureEl<HTMLInputElement>("transformAngleInput").value / 180) * Math.PI;
