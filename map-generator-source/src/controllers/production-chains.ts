...
function buildLayout(goods: Good[]): LayoutData {
	...
	for (const component of components) {
		...
		graphEdges.push(
			...createComponentEdges(componentEdges, componentNodesById),
		);
...
function renderComponentSeparators(
	componentBands: ComponentBand[],
	offsetY: number,
	svgWidth: number,
): string {
	...
	nodeSelection.each(function () {
		...
		group.addEventListener("mouseenter", () => {
			applyChainVisibility(
				edgeSelection,
				nodeSelection,
				getDirectedChainIds(nodeId, layout.edges),
			);
		});
...
function renderGraph(layout: LayoutData): string {
	...
	const separators = renderComponentSeparators(
