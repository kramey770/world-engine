...
const marketsTable = initEditorTable<MarketRow>({
	getData: getMarketsData,
	onUpdate: renderMarketsPage,
});
...
function renderDialog(): void {
	...
	ensureEl("marketsOverviewRefresh").addEventListener(
		"click",
		marketsTable.refresh,
	);
	ensureEl("marketsOverviewExport").addEventListener(
		"click",
		downloadMarketsCsv,
	);
	ensureEl("marketsOverviewCompare").addEventListener("click", () =>
		Controllers.ComparePrices.open(),
	);
	ensureEl("marketsOverviewPercentage").addEventListener(
		"click",
	...
	ensureEl("marketsRegenerateProduction").addEventListener(
		"click",
...
function renderMarketsPage(view: TableView<MarketRow>): void {
	...
	if (!view.all.length) {
		...
		renderEditorPagination(
			ensureEl("marketsOverviewFooter"),
			view,
			marketsTable.goto,
		);
		return;
	...
	const format = (type: keyof typeof totals, value: number, price = false) =>
		percentage
			? `${rn(totals[type] ? (value / totals[type]) * 100 : 0, 2)}%`
			: price
				? formatPrice(rn(value))
				: value;
...
function renderMarketRow(
	row: MarketRow,
	format: (
		type: keyof Omit<MarketRow, "market" | "name" | "owner">,
		value: number,
		price?: boolean,
	) => string | number,
): string {
...
function openPaintEditor(): void {
	...
	void Controllers.PaintEditor.open({
		...
		items: [
			...
			...getMarketsData().map((row) => ({
				id: row.market.i,
				name: row.name,
				color: row.market.color,
			})),
		],
		dontOverrideControl: true,
		getValue: (cell) => pack.cells.market[cell],
		filterCell: (_cell, _currentMarket, nextMarket) =>
			nextMarket === 0 || Boolean(Markets.get(nextMarket)),
...
function renderNoMarketRow(
	format: (
		type: "cells" | "burgs" | "stock" | "sales" | "buys" | "value",
		value: number,
	) => string | number = (_type, value) => value,
): string {
...
function enterAddMarketMode(): void {
	...
	tip(
		"Click on a burg on the map to create a new market there. Hold Shift to add multiple",
		true,
	);
	select<SVGElement, unknown>("#viewbox")
		.style("cursor", "crosshair")
		.on("click", addMarketOnClick);
}
...
function addMarketOnClick(this: SVGElement, ev: MouseEvent): void {
	...
	if (!burgId) {
		tip(
			"Click on a burg to create a new market — no burg found here",
			false,
			"error",
		);
		return;
...
function getMarketTotalStock(market: Market): number {
	return Object.values(market.goods).reduce(
		(sum, g) => sum + (g.stock || 0),
		0,
	);
}
...
function getMarketBurgs(marketId: number): number {
	const marketArr = pack.cells.market;
	if (!marketArr) return 0;
	return (pack.burgs as Burg[]).filter(
		(b) => b.i && !b.removed && marketArr[b.cell] === marketId,
	).length;
}
...
function getMarketFinancials(market: Market): {
	...
	const stockValue = Object.values(market.goods).reduce(
		(sum, g) => sum + (g.stock || 0) * (g.price || 0),
		0,
	);
...
function regenerateMarkets() {
	confirmationDialog({
      ...
      <label style="display:flex; align-items:center; gap:.4em; margin-top:.6em;">
		...
		confirm: "Regenerate",
		onConfirm: () => {
			const regenProduction = ensureEl<HTMLInputElement>(
				"marketsRegenerateProductionToggle",
			).checked;
			Markets.regenerate();
			if (regenProduction) {
