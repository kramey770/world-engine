...
const getProvincePopulation = (province: Province) =>
	rn(
		province.rural! * populationRate +
			province.urban! * populationRate * urbanization,
	);
const columns: EditorColumn<Province>[] = [
	...
	},
	{
		...
		width: "7em",
		sortBy: (province) =>
			province.burg ? pack.burgs[province.burg]?.name || "" : "",
		sortType: "alpha",
	},
	{
		key: "state",
	...
	},
	{
		...
		label: "Population",
		width: "6em",
		sortBy: getProvincePopulation,
	},
	{ key: "actions", width: "5.4em", permanent: true, align: "right" },
];
const provincesTable = initEditorTable<Province>({
	getData: getProvincesData,
	onUpdate: renderProvincesPage,
});
...
function renderDialog(): void {
	...
	applyLineHighlighting(
		"provincesEditor",
		({ cellId }) => pack.cells.province[cellId],
	);

	ensureEl("provincesEditorRefresh").addEventListener(
		"click",
		refreshProvincesEditor,
	);
	ensureEl("provincesEditStyle").addEventListener("click", () =>
		editStyle("provs"),
	);
	...
	ensureEl("provincesPercentage").addEventListener(
		"click",
	...
	ensureEl("provincesRelease").addEventListener(
		"click",
	...
	ensureEl("provincesBodySection").addEventListener("click", (ev: Event) => {
		...
		else if (cl.contains("icon-dot-circled"))
			void Controllers.BurgsOverview.open({ stateId });
...
function updateFilter(): void {
	...
	if (
		...
		!pack.states.some((s) => s.i === filterState.stateId && !s.removed)
	) {
		filterState.stateId = -1;
	}
...
function getProvincesData(): Province[] {
	const provinces = pack.provinces.filter(
		(province) => province.i && !province.removed,
	);
	const filtered =
		filterState.stateId === -1
			? provinces
			: provinces.filter((province) => province.state === filterState.stateId);
...
function renderProvincesPage(view: TableView<Province>): void {
	...
	const lines = view.rows
		.map((p) => {
			...
			const focused = select<SVGElement, unknown>("#deftemp")
				.select(`#fog #focusProvince${p.i}`)
				.size();
	...
	ensureEl("provincesFooterArea").innerHTML = view.all.length
		? si(totals.area / view.all.length) + unit
		: `0${unit}`;
	ensureEl("provincesFooterPopulation").innerHTML = view.all.length
		? si(totals.population / view.all.length)
		: "0";
	ensureEl("provincesFooterArea").dataset.area = String(totals.area);
	ensureEl("provincesFooterPopulation").dataset.population = String(
		totals.population,
	);
	renderEditorPagination(
		ensureEl("provincesFooter"),
		view,
		provincesTable.goto,
	);
...
function provinceHighlightOn(event: Event): void {
	...
	const el = ensureEl("provincesBodySection").querySelector(
		`div[data-id='${province}']`,
	);
...
function declareProvinceIndependence(
	provinceId: number,
	...
	if (!burgId) {
		tip(
			"Cannot declare independence of a province without burg",
			false,
			"error",
		);
		return;
...
function changePopulation(province: number): void {
	...
	if (!cells.length) {
		tip(
			"Province does not have any cells, cannot change population",
			false,
			"error",
		);
		return;
		...
		if (Number.isFinite(urbanChange) && urbanChange !== 1) {
			p.burgs!.forEach((b) => {
				pack.burgs[b].population = rn(
					(pack.burgs[b].population ?? 0) * urbanChange,
					4,
				);
			});
		}
...
function toggleFog(p: number, cl: DOMTokenList): void {
	const path = select<SVGGElement, unknown>("#provs")
		.select(`#province${p}`)
		.attr("d");
	...
	$("#alert").dialog({
		...
		buttons: {
			Remove: function (this: HTMLElement) {
				...
				const g = select<SVGGElement, unknown>("#provs").select(
					"#provincesBody",
				);
...
function renderNameEditor(): void {
	...
	ensureEl("provinceNameEditorShortCulture").addEventListener(
		"click",
		regenerateShortNameCulture,
	);
	ensureEl("provinceNameEditorShortRandom").addEventListener(
		"click",
	...
	ensureEl("provinceNameEditorAddForm").addEventListener(
		"click",
		addCustomForm,
	);
	ensureEl("provinceNameEditorFullRegenerate").addEventListener(
		"click",
		regenerateFullName,
	);
...
function regenerateFullName(): void {
	...
	const form = ensureEl<HTMLSelectElement>(
		"provinceNameEditorSelectForm",
	).value;
...
function applyNameChange(p: Province): void {
	...
	p.formName = ensureEl<HTMLSelectElement>(
		"provinceNameEditorSelectForm",
	).value;
...
function togglePercentageMode(): void {
	const body = ensureEl("provincesBodySection");
	body.dataset.type =
		body.dataset.type === "absolute" ? "percentage" : "absolute";
...
function openPaintEditor(): void {
	...
	void Controllers.PaintEditor.open({
		...
		filterCell: (cell, currentProvince, nextProvince) => {
			...
			if (pack.cells.state[cell] !== pack.provinces[nextProvince].state)
				return false;
			if (!currentProvince || cell !== pack.provinces[currentProvince].center)
				return true;
			tip(
...
function addProvince(this: SVGElement, event: any): void {
	...
	if (cells.h[center] < 20) {
		tip(
			"You cannot place province into the water. Please click on a land cell",
			false,
			"error",
		);
		return;
	...
	if (oldProvince && provinces[oldProvince].center === center) {
		tip(
			"The cell is already a center of a different province. Select other cell",
			false,
			"error",
		);
		return;
	...
	dialogState.set(dialogId, "filters", filterState);
	ensureEl<HTMLSelectElement>("provincesFilterState").value = String(
		filterState.stateId,
	);
	provincesTable.reset();
}
...
function openProvinceMergeDialog(): void {
	...
	if (selectedState === -1) {
		alertMessage.innerHTML =
			"Please select a specific state from the filter to merge provinces within that state.";
		$("#alert").dialog({
	...
	const provincesToMerge = pack.provinces.filter(
		(p) => p.i && !p.removed && p.state === selectedState,
	);
	if (provincesToMerge.length < 2) {
		alertMessage.innerHTML =
			"Not enough provinces in the selected state to merge.";
		$("#alert").dialog({
			title: "Merge Provinces",
			buttons: {
				OK: function (this: HTMLElement) {
		...
		buttons: {
			Merge: function (this: HTMLElement) {
				const formData = new FormData(
					ensureEl<HTMLFormElement>("mergeProvincesForm"),
				);
            ...
            <p>The following provinces will be <strong>removed</strong>: ${provincesToMergeIds
							.map(
								(provinceId) =>
									`${emblem(provinceId)}${pack.provinces[provinceId].name}`,
							)
							.join(", ")}.</p>
...
function highlightProvinceOnMergeHover(event: Event): void {
	...
	if (!province) return;
	const d = select<SVGGElement, unknown>("#provs")
		.select(`#province${province}`)
		.attr("d");
	if (!d) return;
