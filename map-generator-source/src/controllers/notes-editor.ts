...
function open(id?: string, name?: string): void {
	...
	if (notesList.length || id) {
		...
		notesName.value = "";
		notesLegend.innerHTML =
			"No notes added. Click on an element (e.g. label or marker) and add a free text note";
	}
...
function renderDialog(): void {
	...
	ensureEl<HTMLSelectElement>("notesSelect").addEventListener(
		"change",
		changeElement,
	);
	ensureEl<HTMLInputElement>("notesName").addEventListener("input", changeName);
	ensureEl("notesNameSpeak").addEventListener("click", () =>
		speak(ensureEl<HTMLInputElement>("notesName").value),
	);
	...
	ensureEl("notesUpload").addEventListener("click", () =>
		ensureEl("legendsToLoad").click(),
	);
	ensureEl<HTMLInputElement>("legendsToLoad").addEventListener(
		"change",
...
async function initEditor(): Promise<void> {
	if (!window.tinymce) {
		const url =
			"https://azgaar.github.io/Fantasy-Map-Generator/libs/tinymce/tinymce.min.js";
		try {
	...
	tinymce._setBaseUrl(
		"https://azgaar.github.io/Fantasy-Map-Generator/libs/tinymce",
	);
	tinymce.init({
		license_key: "gpl",
...
function updateLegend(): void {
	...
	note.legend = activeEditor
		? activeEditor.getContent()
		: ensureEl("notesLegend").innerHTML;
	updateNotesBox(note);
}
...
function triggerNotesRemove(): void {
	confirmationDialog({
		title: "Remove note",
		message:
			"Are you sure you want to remove the selected note? There is no way to undo this action",
		confirm: "Remove",
		onConfirm: removeSelectedNote,
	});
}
