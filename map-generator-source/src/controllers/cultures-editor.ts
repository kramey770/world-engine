...
const columns: EditorColumn<Culture>[] = [
	...
	},
	{
		...
		defaultSort: "desc",
		sortBy: (culture) =>
			(culture.rural || 0) * populationRate +
			(culture.urban || 0) * populationRate * urbanization,
	},
	{
		key: "emblems",
		label: "Emblems",
...
function renderDialog(): void {
	...
	ensureEl("culturesEditorRefresh").addEventListener(
		"click",
	...
	ensureEl("culturesEditStyle").addEventListener("click", () =>
		editStyle("cults"),
	);
	ensureEl("culturesLegend").addEventListener("click", toggleLegend);
	ensureEl("culturesPercentage").addEventListener(
		"click",
		togglePercentageMode,
	);
	ensureEl("culturesHeirarchy").addEventListener("click", showHierarchy);
	ensureEl("culturesRecalculate").addEventListener("click", () =>
		recalculateCultures(true),
	);
	ensureEl("culturesManually").addEventListener("click", openPaintEditor);
	ensureEl("culturesEditNamesBase").addEventListener("click", () =>
		Controllers.NamesbaseEditor.open(),
	);
	...
	ensureEl("culturesImport").addEventListener("click", () =>
		ensureEl("culturesCSVToLoad").click(),
	);
...
function culturesEditorAddLines(view: TableView<Culture>): void {
	...
	for (const c of view.all) {
		...
		totalPopulation += rn(
			(c.rural ?? 0) * populationRate +
				(c.urban ?? 0) * populationRate * urbanization,
		);
	}
	...
	ensureEl("culturesFooterCultures").innerHTML = String(
		pack.cultures.filter((c) => c.i && !c.removed).length,
	);
	ensureEl("culturesFooterCells").innerHTML = String(
		(pack.cells.h as unknown as number[]).filter((h) => h >= 20).length,
	);
	...
	ensureEl("culturesFooterPopulation").dataset.population =
	...
	ensureEl("culturesBody")
		...
		.forEach(
			($el) => void $el.addEventListener("click", cultureRegenerateName),
		);
	ensureEl("culturesBody")
		.querySelectorAll("div > input.cultureExpan")
		.forEach(
			($el) => void $el.addEventListener("change", cultureChangeExpansionism),
		);
	ensureEl("culturesBody")
	...
	ensureEl("culturesBody")
		...
		.forEach(
			($el) => void $el.addEventListener("change", cultureChangeEmblemsShape),
		);
	ensureEl("culturesBody")
	...
	ensureEl("culturesBody")
		...
		.forEach(
			($el) => void $el.addEventListener("click", cultureRegenerateBurgs),
		);
	ensureEl("culturesBody")
		.querySelectorAll("div > span.icon-target")
		.forEach(
			($el) => void $el.addEventListener("click", cultureHighlightElement),
		);
	ensureEl("culturesBody")
...
function getBaseOptions(base: number): string {
	...
	if (!Names.nameBases[base])
		options += `<option selected value="${base}">removed</option>`; // in case namesbase was removed
	return options;
}
...
function getShapeOptions(isDiversiform: boolean, selected: string): string {
	...
	const shapes = Object.keys(Emblems.shields.types).flatMap((type) =>
		Object.keys(Emblems.shields[type]),
	);
	const options = shapes.map(
		(shape) =>
			`<option ${shape === selected ? "selected" : ""} value="${shape}">${capitalize(shape)}</option>`,
	);
...
function cultureRegenerateName(this: HTMLElement): void {
	...
	if (!Names.nameBases[base]) {
		tip(
			"Namesbase is not defined, please select a valid namesbase",
			false,
			"error",
			5000,
		);
	...
	(this.parentNode as HTMLElement).querySelector<HTMLInputElement>(
		"input.cultureName",
	)!.value = name;
...
function changePopulation(this: HTMLElement): void {
	...
	if (!culture.cells) {
		tip(
			"Culture does not have any cells, cannot change population",
			false,
			"error",
		);
		return;
...
function applyPopulationChange(
	...
	if (Number.isFinite(ruralChange) && ruralChange !== 1) {
		const cells = (pack.cells.i as unknown as number[]).filter(
			(i) => pack.cells.culture[i] === culture,
		);
		cells.forEach((i) => {
...
function cultureRegenerateBurgs(this: HTMLElement): void {
	...
	const cultureBurgs = pack.burgs.filter(
		(b) => b.culture === cultureId && !b.removed && !b.lock,
	);
	...
	Layers.draw("labels");
	tip(
		`Names for ${cultureBurgs.length} burgs are regenerated`,
		false,
		"success",
	);
}
...
function removeCulture(cultureId: number): void {
	...
	cultures
		...
		.forEach((c: any) => {
			c.origins = (c.origins ?? []).filter(
				(origin: number) => origin !== cultureId,
			);
...
function cultureHighlightElement(this: HTMLElement): void {
	const cultureId = +(this.closest(".states") as HTMLElement).dataset.id!;
	highlightElement(
		select("#cults").select(`#culture${cultureId}`).node() as Element,
		4,
	);
}
...
function cultureRemovePrompt(this: HTMLElement): void {
	...
	confirmationDialog({
		...
		message:
			"Are you sure you want to remove the culture? <br>This action cannot be reverted",
		confirm: "Remove",
...
function togglePercentageMode(): void {
	if (ensureEl("culturesBody").dataset.type === "absolute") {
		...
		const totalPopulation = +ensureEl("culturesFooterPopulation").dataset
			.population!;
		...
		ensureEl("culturesBody")
			...
			.forEach((el) => {
				...
				el.querySelector<HTMLElement>(".cultureCells")!.innerText =
					`${rn((+cells! / totalCells) * 100)}%`;
				el.querySelector<HTMLElement>(".cultureArea")!.innerText =
...
async function showHierarchy(): Promise<void> {
	...
	const getDescription = (culture: any) => {
		...
		const populationText =
			population > 0 ? `${si(rn(population))} people` : "Extinct";
...
function applyCulturePaint(changes: ReadonlyMap<number, number>): void {
	for (const [cell, culture] of changes) {
		...
		if (pack.cells.burg[cell])
			pack.burgs[pack.cells.burg[cell]].culture = culture;
	}
	if (changes.size) {
		Layers.draw("cultures");
...
function exitAddCultureMode(): void {
	...
	if (culturesAdd.classList.contains("pressed"))
		culturesAdd.classList.remove("pressed");
}
...
function addCulture(this: SVGElement, event: MouseEvent): void {
	...
	if (pack.cells.h[center] < 20) {
		tip(
			"You cannot place culture center into the water. Please click on a land cell",
			false,
			"error",
		);
		return;
	...
	if (occupied) {
		tip(
			"This cell is already a culture center. Please select a different cell",
			false,
			"error",
		);
		return;
...
async function uploadCulturesData(this: HTMLInputElement): Promise<void> {
	...
	const populated = cells.pop
		.map((c: number, i: number) => (c ? i : null))
		.filter((c: number | null) => c);
	...
	for (const culture of data) {
		...
		culture.origins = current.i
			? restoreOrigins(culture.origins || "")
			: [null];
		current.shield = shapes.includes(culture.emblemsShape)
			? culture.emblemsShape
			: "heater";
		current.base = Names.nameBases.findIndex(
			(n) => n.name === culture.namesbase,
		); // can be -1 if namesbase is not found
