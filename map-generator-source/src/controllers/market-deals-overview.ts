...
function open(marketId: number): void {
	...
	if (!(["all", "local", "global"] as string[]).includes(filterState.scope))
		filterState.scope = "all";
...
function renderDialog(): void {
	...
	ensureEl("marketDealsRefresh").addEventListener(
		"click",
		marketDealsTable.refresh,
	);
	...
	ensureEl("marketDealsBody").addEventListener("click", (ev) => {
		...
		const dealId = el
			.closest<HTMLElement>(".marketDealParty")
			?.closest<HTMLElement>(".marketDeal")?.dataset.id;
	...
	ensureEl("marketDealsFilter").addEventListener("change", (ev) => {
		filterState.scope = (ev.target as HTMLSelectElement)
			.value as typeof filterState.scope;
		dialogState.set(dialogId, "filters", filterState);
		marketDealsTable.reset();
	});
}
...
function getFilteredMarketDeals(): Deal[] {
	...
	const deals = allDeals.filter((deal) => {
		...
		return filterState.scope === "local"
			? counterparty.type === "burg"
			: counterparty.type === "market";
	});
...
function renderMarketDealsPage(view: TableView<Deal>): void {
	...
	const netFlow = view.all.reduce(
		(total, deal) => total + getDealNet(deal, activeMarketId),
		0,
	);
	...
	renderEditorPagination(
		ensureEl("marketDealsFooter"),
		view,
		marketDealsTable.goto,
	);
	updateDialog(dialogId, { width: "fit-content", position });
}

export function getMarketDeals(
	deals: readonly Deal[],
	marketId: number,
): Deal[] {
	return deals.filter(
...
function getParty(deal: Deal): Burg | null {
	...
	const burgId =
		counterparty.type === "burg"
			? counterparty.id
			: Markets.get(counterparty.id)?.centerBurgId;
