...
vi.mock("@/components/dialog/dialog-helpers", async (importOriginal) => ({
	...(await importOriginal<
		typeof import("@/components/dialog/dialog-helpers")
	>()),
	closeDialogs: vi.fn(),
}));

const getOptions = (
	overrides: Partial<PaintEditorOptions> = {},
): PaintEditorOptions => ({
	title: "Paint states",
...
async function dragBrush(): Promise<void> {
	...
	eventView.dispatchEvent(
		mouseEvent("mouseup", { bubbles: true, button: 0, clientX: 2, clientY: 2 }),
	);
	...
	window.$ = vi.fn((target: string | HTMLElement) => {
		...
		const dialog = vi.fn((command: unknown, key?: string, value?: unknown) => {
			...
			if (typeof command === "object") {
				...
				dialogOptions.set(element, { ...options, [key!]: value });
...
describe("PaintEditor", () => {
	it("reopens its destroyed parent through the close callback", () => {
		const onClose = vi.fn(() => {
			document
				...
				.insertAdjacentHTML(
					"beforeend",
					'<div id="parentDialog" class="dialog"></div>',
				);
		});
	...
	it("owns stroke history", async () => {
		...
		expect([
			...(onApply.mock.calls[0][0] as ReadonlyMap<number, number>),
		]).toEqual([]);
	});
	...
	it("supports overlapping values without delegating state management", async () => {
		...
		const changes = onApply.mock.calls[0][0] as ReadonlyMap<
			number,
			readonly number[]
		>;
	...
	it("enforces the universal zero-only protection toggle", async () => {
		const onApply = vi.fn();
		PaintEditor.open(
			getOptions({ dontOverrideControl: true, getValue: () => 1, onApply }),
		);
		const itemSelect = document.getElementById(
			"paintEditorSelect",
		) as HTMLSelectElement;
		itemSelect.value = "2";
		itemSelect.dispatchEvent(new Event("change"));
		const protect = document.getElementById(
			"paintEditorDontOverride",
		) as HTMLInputElement;
		expect(protect.parentElement?.textContent).toContain(
	...
	it("uses the -1 item to remove all overlapping values", async () => {
		...
		const landOnly = document.getElementById(
			"paintEditorLandOnly",
		) as HTMLInputElement;
		...
		expect(
			document
				.querySelector("#paintEditorOverlay polygon")
				?.getAttribute("fill"),
		).toBe("#ffffff");
		document.getElementById("paintEditorApply")?.click();
	...
	it("uses the default brush radius", () => {
		...
		expect(
			(
				document.getElementById("paintEditorBrush") as HTMLInputElement
			).getAttribute("value"),
		).toBe("12");
	});
});
