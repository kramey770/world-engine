...
function open(id: string): void {
	...
	selectedRiver = select<SVGElement, unknown>(`#${id}`).on(
		"click",
		addControlPoint,
	);
...
function renderDialog(): void {
	...
	ensureEl("riverCreateSelectingCells").addEventListener(
		"click",
		openRiverCreator,
	);
	ensureEl("riverEditStyle").addEventListener("click", openRiverStyle);
	ensureEl("riverElevationProfile").addEventListener(
		"click",
		showRiverElevationProfile,
	);
	...
	ensureEl("riverNameSpeak").addEventListener("click", () =>
		speak(ensureEl<HTMLInputElement>("riverName").value),
	);
...
function updateRiverData(): void {
	...
	const sortedRivers = pack.rivers
		.slice()
		.sort((a: River, b: River) => (a.name > b.name ? 1 : -1));
	...
	ensureEl<HTMLInputElement>("riverBasin").value = pack.rivers.find(
		(river: River) => river.i === r.basin,
	)!.name;
...
function updateRiverLength(river: River): void {
	river.length = rn(
		(selectedRiver.node() as SVGGeometryElement).getTotalLength() / 2,
		2,
	);
...
function generateNameCulture(): void {
	const r = getRiver();
	r.name = ensureEl<HTMLInputElement>("riverName").value = Rivers.getName(
		r.mouth,
	);
}
...
function generateNameRandom(): void {
	const r = getRiver();
	if (r)
		r.name = ensureEl<HTMLInputElement>("riverName").value = Names.getBase(
...
function showRiverElevationProfile(): void {
	const points = (
		select("#controlPoints").selectAll("*").data() as Point[]
	).map(([x, y]) => Pack.findCell(x, y)!);
	const river = getRiver();
...
function removeRiver(): void {
	alertMessage.innerHTML =
		"Are you sure you want to remove the river and all its tributaries";
	$("#alert").dialog({
		resizable: false,
		width: "22em",
