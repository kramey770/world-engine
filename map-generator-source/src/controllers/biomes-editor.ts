...
function renderDialog(): void {
	...
	ensureEl("biomesEditorRefresh").addEventListener(
		"click",
		refreshBiomesEditor,
	);
	ensureEl("biomesEditStyle").addEventListener("click", () =>
		editStyle("biomes"),
	);
	...
	applyLineHighlighting(
		dialogId,
		({ cellId }) => cellId && pack.cells.biome[cellId],
	);
...
type BiomeStatisticsSource = {
	...
	cells: Pick<
		PackedGraph["cells"],
		"i" | "h" | "biome" | "area" | "pop" | "burg"
	>;
	burgs: Array<{ population?: number }>;
};

export function collectBiomeStatistics(
	source: BiomeStatisticsSource = pack,
): BiomeStatistics[] {
	const { cells } = source;
	const statistics = source.biomes.map(() => ({
		cells: 0,
		area: 0,
		rural: 0,
		urban: 0,
...
function biomesEditorAddLines(
	view: TableView<Biome>,
	statistics: BiomeStatistics[],
): void {
	...
	for (const biome of view.all) {
		...
		totalPopulation += rn(
			statistic.rural * populationRate +
				statistic.urban * populationRate * urbanization,
		);
	}
	const totalMapArea = getArea(sum(pack.cells.area));
	ensureEl("biomesFooterBiomes").innerHTML = String(view.all.length);
	ensureEl("biomesFooterCells").innerHTML = String(
	...
	ensureEl("biomesFooterPopulation").dataset.population =
...
function togglePercentageMode(): void {
	...
	if (body.dataset.type === "absolute") {
		...
		const totalPopulation = +ensureEl("biomesFooterPopulation").dataset
			.population!;

		body.querySelectorAll<HTMLElement>(":scope > div").forEach((el) => {
			el.querySelector(".biomeCells")!.innerHTML =
				`${rn((+el.dataset.cells! / totalCells) * 100)}%`;
			el.querySelector(".biomeArea")!.innerHTML =
				`${rn((+el.dataset.area! / totalArea) * 100)}%`;
			el.querySelector(".biomePopulation")!.innerHTML =
				`${rn((+el.dataset.population! / totalPopulation) * 100)}%`;
		});
...
export function removeCustomBiome(
	biomes: Biome[],
	cellBiomes: ArrayLike<number>,
	biomeId: number,
): boolean {
...
function addCustomBiome(): void {
	...
	if (!biome) {
		tip(
			"Maximum number of biomes reached (255), data cleansing is required",
			false,
			"error",
		);
		return;
