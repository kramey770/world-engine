...
function open(onApply: (distribution: string) => void, initialExpression = "") {
	const groups: DistCondition[][] = parseExpression(initialExpression) ?? [
	...
	function updateOutput() {
		...
		ensureEl<HTMLElement>("distCellCount").textContent =
			countMatchingCells(expr) || "";
		ensureEl<HTMLElement>("distHumanPreview").textContent =
			interpretDistribution(expr);
	}
	...
	function openFeatureTypePicker(cond: DistCondition, onApplied: () => void) {
		...
		$(pickerEl).dialog({
			...
			buttons: {
				...
				Apply: function () {
					cond.typeValues = entries
						.filter((e) => e.cb.checked)
						.map((e) => e.value);
					onApplied();
					$(this).dialog("close");
				},
		...
		if (def.paramType === "none") {
			...
			const refreshBiomeSummary = () => {
				summary.textContent = cond.biomeIds.length
					? cond.biomeIds
							.map((id) => pack.biomes[id]?.name || `Biome ${id}`)
							.join(", ")
					: "none";
			};
			refreshBiomeSummary();
			...
			const refreshShoreSummary = () => {
				if (!cond.shoreValues.length) {
					...
					summary.textContent = cond.shoreValues
						.map((v) => LABELS[v] ?? v)
						.join(", ");
				}
			};
			...
			const refreshTypeSummary = () => {
				summary.textContent = cond.typeValues.length
					? cond.typeValues.join(", ")
					: "none";
			};
			refreshTypeSummary();
			const editBtn = document.createElement("button");
...
function interpretDistribution(dist: string): string {
	...
	return dist
		.replace(/biome\(([^)]+)\)/g, (_, args) => {
			const names = args
				.split(",")
				.map((a: string) => pack.biomes[parseInt(a.trim(), 10)]?.name);
			return names.length === 1 ? names[0] : `${list(names)}`;
		})
		.replace(
			/minHeight\((-?\d+(?:\.\d+)?)\)/g,
			(_, h) => `min height ${getHeight(+h)}`,
		)
		.replace(
			/maxHeight\((-?\d+(?:\.\d+)?)\)/g,
			(_, h) => `max height ${getHeight(+h)}`,
		)
		.replace(
			/minTemp\((-?\d+(?:\.\d+)?)\)/g,
			(_, t) => `min temp ${convertTemperature(+t)}`,
		)
		.replace(
			/maxTemp\((-?\d+(?:\.\d+)?)\)/g,
			(_, t) => `max temp ${convertTemperature(+t)}`,
		)
		.replace(/shore\(([^)]+)\)/g, (_, args) =>
			args
				...
				.map(
					(a: string) =>
						SHORE_OPTIONS.find((opt) => opt.value === a.trim())?.label ||
						a.trim(),
				)
				.join("/"),
		)
