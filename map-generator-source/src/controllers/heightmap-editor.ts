...
} from "@/services/io/heightmap-draft";
...
function open(options?: { mode?: string; tool?: string }): void {
	filterState = dialogState.get(
		dialogId,
		"filters",
		(): FilterState => ({ cellType: "all" }),
	);
	if (!(["all", "land", "water"] as string[]).includes(filterState.cellType))
		filterState.cellType = "all";
...
function renderTemplateEditor(): void {
	...
	ensureEl("templateUndo").addEventListener("click", () =>
		restoreHistory(edits.n - 1),
	);
	ensureEl("templateRedo").addEventListener("click", () =>
		restoreHistory(edits.n + 1),
	);
	ensureEl("templateSave").addEventListener("click", downloadTemplate);
	ensureEl("templateLoad").addEventListener("click", () =>
		ensureEl("templateToLoad").click(),
	);
...
function renderImageConverter(): void {
	...
	select("#imageConverterPalette")
		...
		.style("background-color", (i: number) =>
			color(1 - (i < 20 ? i - 5 : i) / 100),
		)
		.style("width", (i: number) => (i < 40 || i > 68 ? ".2em" : ".1em"))
		.on("touchmove mousemove", showPalleteHeight)
		.on("click", assignHeight);

	ensureEl("convertImageLoad").addEventListener("click", () =>
	...
	ensureEl<HTMLInputElement>("imageToLoad").onchange = () =>
		loadImage.call(ensureEl<HTMLInputElement>("imageToLoad"));
	...
	ensureEl("convertAutoFMG").addEventListener("click", () =>
		autoAssing("scheme"),
	);
	ensureEl("convertColorsButton").addEventListener(
		"click",
		setConvertColorsNumber,
	);
	...
	ensureEl<HTMLInputElement>("convertOverlay").addEventListener(
		"input",
		function (this: HTMLInputElement) {
			setOverlayOpacity(+this.value);
		},
	);
	ensureEl<HTMLInputElement>("convertOverlayNumber").addEventListener(
		"input",
		function (this: HTMLInputElement) {
			setOverlayOpacity(+this.value);
		},
...
function addToolbarListeners(): void {
	...
	ensureEl("heightmapPreview").addEventListener(
		"click",
	...
	ensureEl("loadHeightmapDraft").addEventListener("click", () =>
		ensureEl("heightmapDraftToLoad").click(),
	);
	ensureEl("heightmapDraftToLoad").addEventListener(
		"change",
		function (this: HTMLInputElement) {
...
function saveDraft(): void {
	...
	tip(
		"Heightmap draft is saved. Load it later via the Load Draft button to continue painting",
		true,
		"success",
		6000,
	);
...
function loadDraft(text: string): void {
	...
	confirmationDialog({
		...
		message:
			"Loading a draft will replace the current heightmap. Are you sure you want to proceed?",
		confirm: "Load",
...
function applyDraft(draft: HeightmapDraft): void {
	...
	let newGrid: GridGraph;
	try {
		newGrid = Grid.generate(draft.seed, draft.graphWidth, draft.graphHeight);
	} finally {
	...
	if (newGrid.points.length !== draft.heights.length) {
		tip(
			"Heightmap draft does not match its grid and cannot be loaded",
			false,
			"error",
			6000,
		);
	...
	const densityValue = Object.keys(cellsDensityMap).find(
		(key) => cellsDensityMap[+key] === draft.cellsDesired,
	);
...
function enterHeightmapEditMode(mode: string, tool?: string): void {
	...
	closeDialogs();
	tip(
		'Heightmap edit mode is active. Click on "Exit Customization" to finalize the heightmap',
		true,
	);
	...
	if (mode === "erase") {
		...
		select<SVGElement, unknown>("#deftemp")
			...
			.remove();
		select<SVGElement, unknown>("#viewbox")
			.selectAll("#coastline use, #lakes path, #oceanLayers path")
			.remove();
		filterState.cellType = "all";
	}
	...
	ensureEl("applyTemplate").style.display =
		mode === "erase" ? "inline-block" : "none";
	ensureEl("convertImage").style.display =
		mode === "erase" ? "inline-block" : "none";
	ensureEl("saveHeightmapDraft").style.display =
		mode === "erase" ? "inline-block" : "none";
	ensureEl("loadHeightmapDraft").style.display =
		mode === "erase" ? "inline-block" : "none";

	// hide erosion checkbox if mode is Keep
	ensureEl("allowErosionBox").style.display =
		mode === "keep" ? "none" : "inline-block";
...
function moveCursor(this: SVGElement, event: any): void {
	...
	ensureEl("heightmapInfoHeight").innerHTML =
		`${grid.cells.h[cell]} (${getFriendlyHeight(grid.cells.h[cell])})`;
	...
	const pressed =
		findEl("brushesButtons")?.querySelector<HTMLElement>("button.pressed");
	if (!pressed) return;
	...
	moveCircle(
		x,
		y,
		ensureEl<HTMLInputElement>("heightmapBrushRadius").valueAsNumber,
	);
}
...
async function finalizeHeightmap(): Promise<void> {
	if (
		...
		tip(
			"Insufficient land area. There should be at least 200 land cells!",
			false,
			"error",
		);
		return;
	...
	if (
		ensureEl("options").querySelector<HTMLElement>(".tab > button.active")!
			.id === "toolsTab"
	) {
	...
	try {
		...
		tip(
			`Failed to apply the edited heightmap: ${(error as Error).message}`,
			false,
			"error",
			6000,
		);
...
function restoreRiskedData(): void {
	...
	for (const p of pack.provinces) {
		...
		const provCells = pack.cells.i.filter(
			(i) => pack.cells.province[i] === p.i,
		);
		if (!provCells.length) {
			const state = p.state;
...
function updateHeightmap(): void {
	...
	const changed = grid.cells.h.reduce(
		(s: number, h: number, i: number) => (h !== prev[i] ? s + 1 : s),
		0,
	);
	...
	const cellTypeFilter =
		findEl<HTMLSelectElement>("cellTypeFilter")?.value ?? filterState.cellType;
...
function mockHeightmap(): void {
	...
	const data = ensureEl<HTMLInputElement>("renderOcean").checked
		? cellIds
		: cellIds.filter((i) => grid.cells.h[i] >= 20);
...
function mockHeightmapSelection(selection: number[]): void {
	...
	selection.forEach((i) => {
		let cell: any = select<SVGElement, unknown>("#viewbox")
			.select("#heights")
			.select(`#cell${i}`);
...
function updateStatistics(): void {
	const landCells = grid.cells.h.reduce(
		(s: number, h: number) => (h >= 20 ? s + 1 : s),
		0,
	);
	ensureEl("landmassCounter").innerText =
		`${landCells} (${rn((landCells / grid.cells.i.length) * 100)}%)`;
...
function placeLinearFeature(this: SVGElement, event: any): void {
	...
	const randomness =
		ensureEl<HTMLInputElement>("heightmapLineRandomness").valueAsNumber / 200;
...
function applyFillBrush(this: SVGElement, event: any): void {
	...
	if (cellTypeFilter === "water") {
		tip(
			"Fill brush is not available with 'only water cells' filter",
			false,
			"error",
		);
		return;
	...
	const { selection, reachedBorder } = collectFillSelection(
		start,
		isWaterFill,
		startHeight,
	);
	...
	if (isWaterFill && reachedBorder) {
		tip(
			"Selected water area is open to map border and is not enclosed",
			false,
			"error",
		);
		return;
...
function collectFillSelection(
	...
	while (stack.length) {
		...
		const matches = isWaterFill
			? heights[cell] < 20
			: heights[cell] === targetHeight;
		if (!matches) continue;
...
function applyConeToSelection(
	selection: number[],
	isWaterFill: boolean,
	targetHeight: number,
): number[] {
	const power =
		ensureEl<HTMLInputElement>("heightmapBrushPower").valueAsNumber * 10;
	const { h: heights, c: neighbors, i: cells } = grid.cells;
	...
	let head = 0;
	selection.forEach((cell) => {
		const isEdgeCell = neighbors[cell].some(
			(next: number) => !inSelection[next],
		);
...
function changeHeightForSelection(selection: number[], start: number): void {
	...
	const limit = (v: number): number =>
		minmax(v, land ? 20 : 0, ocean ? 19 : 100);
	const heights = grid.cells.h;

	const brush = document.querySelector<HTMLElement>(
		"#brushesButtons > button.pressed",
	)!.id;
	if (brush === "brushRaise") {
		selection.forEach((i) => {
		...
		selection.forEach((i, d) => {
			heights[i] = limit(
				heights[i] + interpolate(d / Math.max(selection.length - 1, 1)),
			);
		});
		...
		selection.forEach((i) => {
			heights[i] = rn(
				((mean(
					grid.cells.c[i]
						.filter((c: number) =>
							land ? heights[c] >= 20 : ocean ? heights[c] < 20 : true,
						)
		...
		selection.forEach((i) => {
			heights[i] =
				heights[i] < 15
					? heights[i]
					: limit(heights[i] + power / 1.6 - Math.random() * power);
		});
	}
...
function cellTypeFilterChange(): void {
	...
	if (
		...
		ensureEl("heightmapEditMode").innerHTML === "keep"
	) {
...
function disruptAllHeights(): void {
	grid.cells.h = grid.cells.h.map((h: number) =>
		h < 15 ? h : lim(h + 2.5 - Math.random() * 4),
	);
	updateHeightmap();
}
...
function startFromScratch(): void {
	...
	if (!someHeights) {
		tip(
			"Heightmap is already cleared, please do not click twice if not required",
			false,
			"error",
		);
		return;
...
function addStep(
	...
	$body.insertAdjacentHTML(
		"beforeend",
		getStepHTML(type, count, dist, arg4, arg5),
	);

	const $elDist = $body.querySelector<HTMLSelectElement>(
		"div:last-child > span > .templateDist",
	);
...
function selectTemplate(e: Event): void {
	...
	alertMessage.innerHTML =
		"Are you sure you want to select a different template? All changes will be lost.";
	$("#alert").dialog({
		resizable: false,
...
function executeTemplate(): void {
	const steps = ensureEl("templateBody").querySelectorAll<HTMLElement>(
		"#templateBody > div",
	);
	...
	for (const step of steps) {
		...
		const count =
			step.querySelector<HTMLInputElement>(".templateCount")?.value || "";
		const height =
			step.querySelector<HTMLInputElement>(".templateHeight")?.value || "";
		const dist =
			step.querySelector<HTMLSelectElement>(".templateDist")?.value || "";
...
function downloadTemplate(): void {
	...
	for (const s of Array.from(steps)) {
		...
		const count =
			s.querySelector<HTMLInputElement>(".templateCount")?.value || "0";
		const arg3 =
...
function openImageConverter(): void {
	...
	clearMainTip();
	tip(
		"Image Converter is opened. Upload image and assign height value for each color",
		false,
		"warn",
	); // main tip
...
function showPalleteHeight(this: HTMLElement): void {
	...
	const former = ensureEl("imageConverterPalette").querySelector<HTMLElement>(
		".hoveredColor",
	);
...
function heightsFromImage(count: number): void {
	...
	sampleCanvas
		.getContext("2d")!
		.drawImage(sourceImage, 0, 0, grid.cellsX, grid.cellsY);
	...
	select<SVGElement, unknown>("#viewbox")
		...
		.attr(
			"fill",
			(d: number) =>
				`rgb(${data[d * 4]}, ${data[d * 4 + 1]}, ${data[d * 4 + 2]})`,
		)
		.on("click", mapClicked);

	const colors: string[] = pallete.map(
		(p: number[]) => `rgb(${p[0]}, ${p[1]}, ${p[2]})`,
	);
...
function mapClicked(this: SVGElement): void {
	...
	const palleteColor = ensureEl("imageConverter").querySelector<HTMLElement>(
		`div[data-color="${fill}"]`,
	);
	palleteColor?.click();
}
...
function colorClicked(this: HTMLElement): void {
	...
	const selectedColor =
		ensureEl("imageConverter").querySelector("div.selectedColor");
	if (selectedColor) selectedColor.classList.remove("selectedColor");
	const hoveredColor = ensureEl("imageConverterPalette").querySelector(
		"div.hoveredColor",
	);
	if (hoveredColor) hoveredColor.classList.remove("hoveredColor");
	ensureEl("colorsSelectValue").innerHTML = ensureEl(
		"colorsSelectFriendly",
	).innerHTML = "0";
	...
	if (this.dataset.height) {
		...
		ensureEl("imageConverterPalette")
			.querySelector(`div[data-color="${height}"]`)
...
function assignHeight(this: HTMLElement): void {
	...
	const selectedColor =
		ensureEl("imageConverter").querySelector<HTMLElement>("div.selectedColor")!;
	...
	if (
		(selectedColor.parentNode as HTMLElement).id === "colorsUnassignedContainer"
	) {
		ensureEl("colorsAssignedContainer").appendChild(selectedColor);
		ensureEl("colorsAssigned").style.display = "block";

		ensureEl("colorsUnassignedNumber").innerHTML = String(
			ensureEl("colorsUnassignedContainer").childElementCount - 2,
		);
		ensureEl("colorsAssignedNumber").innerHTML = String(
			ensureEl("colorsAssignedContainer").childElementCount - 2,
		);
	}
}
...
function autoAssing(type: string): void {
	...
	let unassigned =
		colorsUnassignedContainer.querySelectorAll<HTMLElement>("div");
	if (!unassigned.length) {
		heightsFromImage(+ensureEl<HTMLInputElement>("convertColors").value);
		unassigned = colorsUnassignedContainer.querySelectorAll<HTMLElement>("div");
		if (!unassigned.length) {
			tip(
				"No unassigned colors. Please load an image and click the button again",
				false,
				"error",
			);
			return;
	...
	const getHeightByScheme = (clr: string): number => {
		...
		const hue = hsl(clr).h;
		const closest = hues.reduce((prev, curr) =>
	...
	unassigned.forEach((el) => {
		...
		const height =
			type === "hue"
				...
				: type === "lum"
					? getHeightByLum(clr)
					: getHeightByScheme(clr);
		const colorTo = color(
			1 - (height < 20 ? (height - 5) / 100 : height / 100),
		);
		select<SVGElement, unknown>("#viewbox")
			.select("#heights")
			.selectAll(`polygon[fill='${clr}']`)
	...
	Array.from(colorsAssignedContainer.children)
		.sort(
			(a, b) =>
				+(a as HTMLElement).dataset.height! -
				+(b as HTMLElement).dataset.height!,
		)
		.forEach((line) => {
	...
	ensureEl("colorsAssignedNumber").innerHTML = String(
		colorsAssignedContainer.childElementCount - 2,
	);
}
...
function setOverlayOpacity(v: number): void {
	ensureEl<HTMLInputElement>("convertOverlay").value =
		ensureEl<HTMLInputElement>("convertOverlayNumber").value = String(v);
...
function toggleHeightmapPreview(): void {
	...
	preview.addEventListener("mouseover", () =>
		tip("Heightmap preview. Click to download a screen-sized image"),
	);
