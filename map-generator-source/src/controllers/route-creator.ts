...
function renderDialog(): void {
	...
	ensureEl("routeCreatorGroupSelect").addEventListener("change", () =>
		drawRoute(creatorPoints),
	);
	ensureEl("routeCreatorGroupEdit").addEventListener(
		"click",
		() => void Controllers.RouteGroupsEditor.open(),
	);
	ensureEl("routeCreatorComplete").addEventListener("click", completeCreation);
	ensureEl("routeCreatorCancel").addEventListener("click", () =>
...
function onBodyClick(ev: Event): void {
	...
	if (target.classList.contains("icon-trash-empty"))
		removePoint((target.parentNode as HTMLElement).dataset.point!);
}
...
function removePoint(pointString: string): void {
	...
	drawRoute(creatorPoints);
	ensureEl("routeCreatorBody")
		.querySelector(`[data-point='${pointString}']`)
		?.remove();
}
