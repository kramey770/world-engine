...
function open(): void {
	cleanup();
	renderDialog();
	select<SVGGElement, unknown>("#viewbox").on(
...
function updateFields(point: Point, cellId: number, gridCellId: number): void {
	...
	set(
		"infoArea",
		cells.area[cellId]
			? `${si(getArea(cells.area[cellId]))} ${getAreaUnit()}`
			: "n/a",
	);
	...
	set(
		"infoPrec",
		cells.h[cellId] >= 20
			? getFriendlyPrecipitation(cellId, pack, grid)
			: "n/a",
	);
	set(
		"infoRiver",
		cells.h[cellId] >= 20 && cells.r[cellId]
			? getRiverInfo(cells.r[cellId])
			: "no",
	);
	set("infoState", getStateInfo(cellId));
	set(
		"infoProvince",
		getNamedInfo(pack.provinces, cells.province[cellId], "fullName"),
	);
	set(
		"infoCulture",
		getNamedInfo(pack.cultures, cells.culture[cellId], "name"),
	);
	set(
		"infoReligion",
		getNamedInfo(pack.religions, cells.religion[cellId], "name"),
	);
	set("infoPopulation", getFriendlyPopulation(cellId));
	set("infoBurg", getNamedInfo(pack.burgs, cells.burg[cellId], "name"));
	set(
		"infoFeature",
		featureId ? `${feature.subtype || feature.type} (${featureId})` : "n/a",
	);
	...
	set(
		"infoCellProduction",
		listProduction(
			Production.getCellProduction(cellId, Goods.getBiomesProduction()),
		),
	);
	...
	set(
		"infoBurgProduction",
		burgId
			? listProduction(Production.getBurgProduction(pack.burgs[burgId]))
			: "n/a",
	);
}
...
function getNamedInfo<T, K extends keyof T>(
	elements: T[],
	id: number,
	key: K,
): string {
...
function getStateInfo(cellId: number): string {
	...
	return stateId
		? `${pack.states[stateId].fullName} (${stateId})`
		: "neutral lands (0)";
}
...
function getMarketInfo(marketId: number | undefined): string {
	...
	return centerBurg
		? `${centerBurg.name} market (${marketId})`
		: `market ${marketId}`;
}
...
function listProduction(produced: Record<string, number>): string {
	...
	return entries
		.map(
			([goodId, amount]) =>
				`${Goods.get(Number(goodId))?.name || goodId}: ${rn(amount, 2)}`,
		)
		.join(", ");
}
...
function getElevation(feature: Feature, h: number): string {
	...
	if (feature.border)
		return `0 ${ensureEl<HTMLSelectElement>("heightUnit").value}`; // ocean: 0
	if (feature.type === "lake")
		return `${getHeight(feature.height)} (${feature.height})`; // lake: set on river generation
	return "n/a";
}
...
function getDepth(feature: Feature, [x, y]: Point): string {
	if (feature.land)
		return `0 ${ensureEl<HTMLSelectElement>("heightUnit").value}`; // land: 0

	const gridH = grid.cells.h[Grid.findCell(x, y)];
	// lake: difference between surface and bottom
	if (feature.type === "lake")
		return getHeight(gridH === 19 ? feature.height / 2 : gridH, true);
