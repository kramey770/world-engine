...
function renderDialog(): void {
	...
	ensureEl("marketOverviewRefresh").addEventListener(
		"click",
	...
	ensureEl("marketOverviewRelocate").addEventListener(
		"click",
		toggleRelocateMarket,
	);
	ensureEl("marketOverviewName").addEventListener("input", onRenameInput);
	ensureEl("marketOverviewNameReset").addEventListener(
		"click",
		resetMarketName,
	);
}
...
function refreshNameInput(market: Market): void {
	...
	input.placeholder =
		pack.burgs[market.centerBurgId]?.name || `Market ${market.i}`;
}
...
function onRenameInput(this: HTMLInputElement): void {
	...
	$("#marketOverview").dialog(
		"option",
		"title",
		`Market Stock: ${Markets.getName(market)}`,
	);
}
...
function renderMarketOverviewPage(view: TableView<MarketGoodRow>): void {
	...
	ensureEl("marketOverviewGoodsBody").innerHTML =
		lines.join("") || "No market goods available";
	...
	renderEditorPagination(
		ensureEl("marketOverviewSummary"),
		view,
		marketOverviewTable.goto,
	);
...
function toggleRelocateMarket(): void {
	...
	if (button.classList.contains("pressed")) {
		select<SVGGElement, unknown>("#viewbox")
			...
			.on("click", relocateMarketOnClick);
...
function relocateMarketOnClick(this: SVGGElement, event: MouseEvent): void {
	...
	if (!burgId || !burg || burg.removed) {
		tip(
			"No valid burg in this cell. Click on a cell with a burg",
			false,
			"error",
		);
		return;
...
function closeMarketOverview() {
	if (ensureEl("marketOverviewRelocate").classList.contains("pressed"))
		toggleRelocateMarket();
