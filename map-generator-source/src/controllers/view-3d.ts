...
const loadRenderer = (): Promise<Renderer> =>
	renderer
		...
		: import("@/renderers/view-3d-renderer").then((m) => (renderer = m));

// --- renderer bridge ---------------------------------------------------------
const create = (canvas: HTMLCanvasElement, type = "viewMesh") =>
	loadRenderer().then((m) => m.create(canvas, type));
...
const setSunColor = (color: string) =>
	loadRenderer().then((m) => m.setSunColor(color));
const setScale = (scale: number) =>
	loadRenderer().then((m) => m.setScale(scale));
const setResolutionScale = (scale: number) =>
	loadRenderer().then((m) => m.setResolutionScale(scale));
const setLightness = (intensity: number) =>
	loadRenderer().then((m) => m.setLightness(intensity));
const setSun = (x: number, y: number, z?: number) =>
	loadRenderer().then((m) => m.setSun(x, y, z));
const setRotation = (speed: number) =>
	loadRenderer().then((m) => m.setRotation(speed));
const toggleLabels = () => loadRenderer().then((m) => m.toggleLabels());
const toggleSubdivision = () =>
	loadRenderer().then((m) => m.toggle3dSubdivision());
const toggleErosion = () => loadRenderer().then((m) => m.toggleErosion());
const setErosionStrength = (value: number) =>
	loadRenderer().then((m) => m.setErosionStrength(value));
const setErosionRiverDepth = (value: number) =>
	loadRenderer().then((m) => m.setErosionRiverDepth(value));
const setErosionDetail = (value: number) =>
	loadRenderer().then((m) => m.setErosionDetail(value));
const setErosionOctaves = (value: number) =>
	loadRenderer().then((m) => m.setErosionOctaves(value));
...
const setResolution = (resolution: number) =>
	loadRenderer().then((m) => m.setResolution(resolution));
const setColors = (sky: string, water: string) =>
	loadRenderer().then((m) => m.setColors(sky, water));
const setTimeOfDay = (presetName: string) =>
	loadRenderer().then((m) => m.setTimeOfDay(presetName));
...
function renderOptionsDialog(): void {
	...
	ensureEl("options3dSave").addEventListener(
		"click",
		() => void saveScreenshot(),
	);
	ensureEl("options3dOBJSave").addEventListener("click", () => void saveOBJ());
	ensureEl("options3dScaleRange").addEventListener(
		"input",
		onChangeHeightScale,
	);
	ensureEl("options3dScaleNumber").addEventListener(
		"change",
		onChangeHeightScale,
	);
	ensureEl("options3dLightnessRange").addEventListener(
		"input",
		onChangeLightness,
	);
	ensureEl("options3dLightnessNumber").addEventListener(
		"change",
		onChangeLightness,
	);
	...
	ensureEl("options3dMeshSkinResolution").addEventListener(
		"change",
		onChangeResolutionScale,
	);
	ensureEl("options3dMeshRotationRange").addEventListener(
		"input",
		onChangeRotation,
	);
	ensureEl("options3dMeshRotationNumber").addEventListener(
		"change",
		onChangeRotation,
	);
	ensureEl("options3dGlobeRotationRange").addEventListener(
		"input",
		onChangeRotation,
	);
	ensureEl("options3dGlobeRotationNumber").addEventListener(
		"change",
		onChangeRotation,
	);
	ensureEl("options3dMeshLabels3d").addEventListener(
		"change",
		() => void toggleLabels(),
	);
	...
	ensureEl("options3dGlobeResolution").addEventListener(
		"change",
		onChangeResolution,
	);
	ensureEl("options3dMeshWireframeMode").addEventListener(
		"change",
		() => void toggleWireframe(),
	);
	ensureEl("options3dSunColor").addEventListener("input", onChangeSunColor);
	ensureEl("options3dSubdivide").addEventListener(
		"change",
	...
	ensureEl("options3dErosionDetail").addEventListener(
		"change",
		onChangeErosionDetail,
	);
	// "change" instead of "input": every value change triggers a GPU re-bake
	ensureEl("options3dErosionStrengthRange").addEventListener(
		"change",
		onChangeErosionStrength,
	);
	ensureEl("options3dErosionStrengthNumber").addEventListener(
		"change",
		onChangeErosionStrength,
	);
	ensureEl("options3dErosionRiverDepthRange").addEventListener(
		"change",
		onChangeErosionRiverDepth,
	);
	ensureEl("options3dErosionRiverDepthNumber").addEventListener(
		"change",
		onChangeErosionRiverDepth,
	);
	ensureEl("options3dErosionOctaves").addEventListener(
		"change",
		onChangeErosionOctaves,
	);
...
function updateValues(): void {
	...
	const globe =
		(document.getElementById("canvas3d") as HTMLElement | null)?.dataset
			.type === "viewGlobe";
	...
	ensureEl("options3dColorSection").style.display = o.extendedWater
		? "block"
		: "none";
	...
	ensureEl<HTMLInputElement>("options3dSubdivide").disabled = Boolean(
		o.erosion,
	);
	ensureEl<HTMLInputElement>("options3dErosion").checked = Boolean(o.erosion);
	ensureEl("options3dErosionSection").style.display = o.erosion
		? "block"
		: "none";
	...
	ensureEl<HTMLInputElement>("options3dSatellite").checked = Boolean(
		o.satellite,
	);
...
function onChangeRotation(this: HTMLInputElement): void {
	const sibling = (this.nextElementSibling ||
		this.previousElementSibling) as HTMLInputElement | null;
...
function onToggleErosion(): void {
	...
	ensureEl("options3dErosionSection").style.display = enabled
		? "block"
		: "none";
