...
function open(id: string): void {
	...
	selectedRoute = select<SVGElement, unknown>(`#${id}`).on(
		"click",
		addControlPoint,
	);
...
function renderDialog(): void {
	...
	ensureEl("routeCreateSelectingCells").addEventListener(
		"click",
		showCreationDialog,
	);
	ensureEl("routeSplit").addEventListener("click", togglePressed);
	ensureEl("routeJoin").addEventListener("click", openJoinRoutesDialog);
	ensureEl("routeElevationProfile").addEventListener(
		"click",
		showRouteElevationProfile,
	);
	...
	ensureEl("routeNameSpeak").addEventListener("click", () =>
		speak(ensureEl<HTMLInputElement>("routeName").value),
	);
...
function updateRouteData(route: Route): void {
	...
	select("#routes")
		...
		.each(function () {
			routeGroup.options.add(
				new Option(this.id, this.id, false, this.id === route.group),
			);
		});

	updateRouteLength(route);

	const isWaterRoute = route.points.some(
		([_x, _y, cellId]) => pack.cells.h[cellId] < 20,
	);
	ensureEl("routeElevationProfile").style.display = isWaterRoute
		? "none"
		: "inline-block";
}
...
function addControlPoint(this: any, event: any): void {
	...
	const index = getSegmentId(
		route.points as [number, number][],
	...
	if (isNewCell) {
		...
		if (!prev)
			ERROR &&
				console.error("Can't add control point to the start of the route");
		if (!next)
			ERROR && console.error("Can't add control point to the end of the route");
		if (!prev || !next) return;
...
function handleControlPointClick(this: any): void {
	...
	function removeControlPoint(controlPoint: any): void {
		const isOnlyPointInCell =
			route.points.filter((p) => p[2] === point[2]).length === 1;
...
function openJoinRoutesDialog(): void {
	...
	if (candidateRoutes.length) {
		...
		$("#alert").dialog({
			...
			buttons: {
				...
				Join: () => {
					...
					const selectedRoute = pack.routes.find(
						(r: Route) => r.i === selectedRouteId,
					) as Route;
		...
		tip(
			"No routes to join with. Route must start or end at current route's start or end cell",
			false,
			"error",
			4000,
		);
...
export function mergeRoutePoints(
	...
	if (routeEnd === joinedStart)
		return [...routePoints, ...joinedPoints.slice(1)];
	if (routeStart === joinedEnd)
		return [...joinedPoints, ...routePoints.slice(1)];
	if (routeStart === joinedStart)
		return [...[...routePoints].reverse(), ...joinedPoints.slice(1)];
	if (routeEnd === joinedEnd)
		return [...routePoints, ...[...joinedPoints].reverse().slice(1)];
	return null;
}
...
function generateName(): void {
	const route = getRoute();
	route.name = ensureEl<HTMLInputElement>("routeName").value =
		Routes.generateName(route) || UNNAMED_ROUTE;
}
...
function removeRoute(): void {
	confirmationDialog({
		title: "Remove route",
		message:
			"Are you sure you want to remove the route? <br>This action cannot be reverted",
		confirm: "Remove",
		onConfirm: () => {
