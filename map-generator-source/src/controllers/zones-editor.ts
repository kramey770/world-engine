...
const zonesTable = initEditorTable<ZoneRow>({
	getData: getZonesData,
	onUpdate: renderZonesPage,
});
...
function renderDialog(): void {
	...
	applyLineHighlighting("zonesEditor", ({ target }) => {
		...
		return zone && /^zone\d+$/.test(zone.id)
			? Number(zone.id.slice(4))
			: undefined;
	});
	...
	ensureEl("zonesEditStyle").addEventListener("click", () =>
		editStyle("zones"),
	);
	...
	body.addEventListener("click", (ev: Event) => {
		...
		else if (target.classList.contains("zonePopulation"))
		...
		else if (target.classList.contains("zoneFog"))
			toggleFog(zone, target.classList);
	});
	...
	body.addEventListener("input", (ev: Event) => {
		...
		if (target.classList.contains("zoneName"))
			changeDescription(zone, target.value);
		else if (target.classList.contains("zoneType"))
			changeType(zone, target.value);
	});
...
function getZonesData(): ZoneRow[] {
	const zones =
		filterState.type === "all"
			? pack.zones
			: pack.zones.filter((zone) => zone.type === filterState.type);
	return zones.map((zone) => {
		const area = getArea(sum(zone.cells.map((cell) => pack.cells.area[cell])));
		const rural =
			sum(zone.cells.map((cell) => pack.cells.pop[cell])) * populationRate;
		...
		const urban =
			sum(
				pack.burgs
					.filter((burg) => burg.i && !burg.removed && zoneCells.has(burg.cell))
					.map((burg) => burg.population ?? 0),
			) *
			populationRate *
			urbanization;
...
function renderZonesPage(view: TableView<ZoneRow>): void {
	...
	const lines = view.rows.map(
		({
			zone: { i, name, type, cells, color, hidden },
			area,
			rural,
			urban,
			population,
	...
	ensureEl("zonesFooterPopulation").dataset.population =
		String(totalPopulation);
	ensureEl("zonesFooterNumber").innerHTML =
		`${view.all.length} of ${pack.zones.length}`;
...
function movezone(
	_ev: unknown,
	ui: { item: ArrayLike<HTMLElement> & { index(): number } },
): void {
	const zone = pack.zones.find((z) => z.i === +ui.item[0].dataset.id!);
	if (!zone) return;
	const nextId =
		ui.item[0].nextElementSibling instanceof HTMLElement
			? +ui.item[0].nextElementSibling.dataset.id!
			: null;
	const previousId =
		ui.item[0].previousElementSibling instanceof HTMLElement
			? +ui.item[0].previousElementSibling.dataset.id!
			: null;
	pack.zones.splice(pack.zones.indexOf(zone), 1);
	const nextIndex =
		nextId === null ? -1 : pack.zones.findIndex((item) => item.i === nextId);
	const previousIndex =
		previousId === null
			? -1
			: pack.zones.findIndex((item) => item.i === previousId);
	const newIndex =
		nextIndex >= 0
			? nextIndex
			: previousIndex >= 0
...
function applyZonePaint(
	zones: readonly Zone[],
	changes: ReadonlyMap<number, readonly number[]>,
): void {
	const cellsByZone = new Map(
		zones.map((zone) => [zone.i, new Set(zone.cells)]),
	);
...
function toggleFog(zone: Zone, cl: DOMTokenList): void {
	...
	if (inactive) {
		const path = select<SVGGElement, unknown>("#zones")
			.select(`#zone${zone.i}`)
			.attr("d");
...
function changePopulation(zone: Zone): void {
	...
	if (!landCells.length) {
		tip(
			"Zone does not have any land cells, cannot change population",
			false,
			"error",
		);
		return;
	...
	const rural = rn(
		sum(landCells.map((i) => pack.cells.pop[i])) * populationRate,
	);
	const urban = rn(
		sum(burgs.map((b) => b.population ?? 0)) * populationRate * urbanization,
	);
...
function zoneRemove(zone: Zone): void {
	confirmationDialog({
		title: "Remove zone",
		message:
			"Are you sure you want to remove the zone? <br>This action cannot be reverted",
		confirm: "Remove",
		onConfirm: () => {
