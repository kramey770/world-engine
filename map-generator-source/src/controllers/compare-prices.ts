	...
	initColumnVisibility({
		...
		onUpdate: () =>
			updateDialog(dialogId, {
				width: "fit-content",
				position: { ...position, of: activeAnchor },
			}),
	});

	ensureEl("marketsGoodCompareSelect").addEventListener("change", () => {
		activeGoodId = +ensureEl<HTMLSelectElement>("marketsGoodCompareSelect")
			.value;
		comparePricesTable.reset();
	});
	ensureEl("marketsGoodCompareRefresh").addEventListener(
		"click",
		comparePricesTable.refresh,
	);
	ensureEl("marketsGoodComparePercentage").addEventListener(
		"click",
...
function renderComparePricesPage(view: TableView<Market>): void {
	...
	if (!good) {
		...
		updateFooter(0, 0);
		renderEditorPagination(
			ensureEl("marketsGoodCompareFooter"),
			view,
			comparePricesTable.goto,
		);
		return;
	}

	const totalStock = view.all.reduce(
		(total, market) => total + rn(market.goods[good.i]?.stock ?? 0, 2),
		0,
	);
	const priceSum = view.all.reduce(
		(total, market) => total + rn(market.goods[good.i]?.price ?? 0, 2),
		0,
	);
	...
	const lines = view.rows.map((market) => {
		...
		const stockText = percentage
			? totalStock
				? `${rn((stock / totalStock) * 100, 2)}%`
				: "0%"
			: String(stock);
	...
	updateFooter(
		rn(totalStock, 2),
		view.all.length ? rn(priceSum / view.all.length, 2) : 0,
	);
	renderEditorPagination(
		ensureEl("marketsGoodCompareFooter"),
		view,
...
function rebuildGoodSelect(): void {
	...
	select.innerHTML = sortedGoods
		.map(
			(g) =>
				`<option value="${g.i}" ${g.i === prev ? "selected" : ""}>${g.name}</option>`,
		)
		.join("");
