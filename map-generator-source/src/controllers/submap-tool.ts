...
function generateSubmap(): void {
	...
	const submapPointsValue =
		ensureEl<HTMLInputElement>("submapPointsInput").value;
	...
	if (ensureEl<HTMLInputElement>("submapRescaleBurgStyles").checked)
...
function rescaleBurgStyles(scale: number): void {
	for (const group of ensureEl("burgIcons").querySelectorAll<SVGGElement>(
		":scope > g",
	)) {
	...
	const burgLabelGroups = new Set(
		pack.burgs
			...
			.map((burg) => burg.label?.group || burg.group || "burg"),
	);
