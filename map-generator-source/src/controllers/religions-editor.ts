...
const columns: EditorColumn<Religion>[] = [
	...
	},
	{
		...
		width: "6em",
		sortBy: (religion) =>
			(religion.rural || 0) * populationRate +
			(religion.urban || 0) * populationRate * urbanization,
	},
	{
		key: "expansion",
		label: "Expansion",
...
function getFilteredReligions(): Religion[] {
	return pack.religions.filter(
		(r) => !r.removed && !(r.i && !r.cells && !filterState.showExtinct),
	);
}
...
function renderDialog(): void {
	...
	ensureEl("religionsEditorRefresh").addEventListener(
		"click",
	...
	ensureEl("religionsEditStyle").addEventListener("click", () =>
		editStyle("relig"),
	);
	ensureEl("religionsLegend").addEventListener("click", toggleLegend);
	ensureEl("religionsPercentage").addEventListener(
		"click",
	...
	ensureEl("religionsRecalculate").addEventListener("click", () =>
...
function religionsEditorAddLines(view: TableView<Religion>): void {
	...
	for (const r of view.all) {
		...
		totalPopulation += rn(
			(r.rural ?? 0) * populationRate +
				(r.urban ?? 0) * populationRate * urbanization,
		);
	}
	...
	ensureEl("religionsOrganized").innerHTML = String(
		validReligions.filter((r) => r.type === "Organized").length,
	);
	ensureEl("religionsHeresies").innerHTML = String(
		validReligions.filter((r) => r.type === "Heresy").length,
	);
	ensureEl("religionsCults").innerHTML = String(
		validReligions.filter((r) => r.type === "Cult").length,
	);
	ensureEl("religionsFolk").innerHTML = String(
		validReligions.filter((r) => r.type === "Folk").length,
	);
	...
	ensureEl("religionsFooterPopulation").dataset.population =
		String(totalPopulation);

	renderEditorPagination(
		ensureEl("religionsFooter"),
		view,
		religionsTable.goto,
	);
	...
	ensureEl("religionsBody")
		...
		.forEach(
			(el) => void el.addEventListener("change", religionChangeExpansionism),
		);
	ensureEl("religionsBody")
...
const religionHighlightOn = debounce((event: any) => {
	...
	const $el = ensureEl("religionsBody").querySelector(
		`div[data-id='${religionId}']`,
	);
...
function changePopulation(this: HTMLElement): void {
	...
	if (!religion.cells) {
		tip(
			"Religion does not have any cells, cannot change population",
			false,
			"error",
		);
		return;
	...
	const burgs = pack.burgs.filter(
		(b) => !b.removed && pack.cells.religion[b.cell] === religionId,
	);
	...
	function applyPopulationChange() {
		...
		if (Number.isFinite(ruralChange) && ruralChange !== 1) {
			const cells = (pack.cells.i as unknown as number[]).filter(
				(i) => pack.cells.religion[i] === religionId,
			);
			cells.forEach((i) => {
...
function religionRemovePrompt(this: HTMLElement): void {
	...
	confirmationDialog({
		...
		message:
			"Are you sure you want to remove the religion? <br>This action cannot be reverted",
		confirm: "Remove",
...
function removeReligion(religionId: number): void {
	...
	pack.religions
		...
		.forEach((r) => {
			r.origins = (r.origins ?? []).filter(
				(origin: number) => origin !== religionId,
			);
...
function togglePercentageMode(): void {
	if (ensureEl("religionsBody").dataset.type === "absolute") {
		...
		const totalPopulation = +ensureEl("religionsFooterPopulation").dataset
			.population!;
		...
		ensureEl("religionsBody")
			...
			.forEach(($el) => {
				...
				$el.querySelector<HTMLElement>(".religionArea")!.innerText =
...
function syncFilterControls(): void {
	ensureEl("religionsBody").dataset.extinct = filterState.showExtinct
		? "show"
		: "hide";
	ensureEl("religionsExtinct").classList.toggle(
		"active",
		filterState.showExtinct,
	);
}
...
function openPaintEditor(): void {
	...
	void Controllers.PaintEditor.open({
		...
		items: pack.religions
			.filter(
				(religion) => !religion.removed && (!religion.i || religion.cells),
			)
			.map((religion) => ({
				id: religion.i,
				name: religion.name,
...
function exitAddReligionMode(): void {
	...
	if (religionsAdd.classList.contains("pressed"))
		religionsAdd.classList.remove("pressed");
}
...
function addReligion(this: SVGElement, event: MouseEvent): void {
	...
	if (pack.cells.h[center] < 20) {
		tip(
			"You cannot place religion center into the water. Please click on a land cell",
			false,
			"error",
		);
		return;
	...
	const occupied = pack.religions.some(
		(r) => !r.removed && r.center === center,
	);
	if (occupied) {
		tip(
			"This cell is already a religion center. Please select a different cell",
			false,
			"error",
		);
		return;
...
function recalculateReligions(must?: boolean): void {
	if (!must && !ensureEl<HTMLInputElement>("religionsAutoChange").checked)
		return;
