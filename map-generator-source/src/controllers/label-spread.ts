	...
	const candidates =
		label.type === "burg"
			? buildBurgCandidates(
					label,
					currentCandidate,
					burgIconBounds.get(label.entityId),
				)
			: isPath
...
function getBurgLabelCandidates({
	...
	const candidates = shifts
		.map(([deltaX, deltaY]) => ({
			...
			placement: {
				dx: round(currentDx + deltaX),
				dy: round(currentDy + deltaY),
			},
			preference:
				changePenalty + Math.hypot(deltaX, deltaY) * displacementScale,
		}))
		.sort((first, second) => first.preference - second.preference)
		.filter(
			(candidate) =>
				candidate.placement.dx !== current.placement.dx ||
				candidate.placement.dy !== current.placement.dy,
		);
...
function getBurgChangePenalty(bounds: LabelBounds): number {
	...
	return (
		BASE_BURG_PLACEMENT_CHANGE_PENALTY *
		(screenHeight / BASE_BURG_SCREEN_HEIGHT) ** 2
	);
}
...
function buildPointCandidates(
	...
	const step = Math.max(
		...
		toMapUnits(MINIMUM_POINT_STEP_SCREEN),
	);
	const map = mapBounds();
	...
	for (const multiplier of POINT_STEP_MULTIPLIERS) {
		...
		const preference =
			POINT_PLACEMENT_CHANGE_PENALTY +
			multiplier ** 2 * POINT_DISPLACEMENT_WEIGHT;
...
function buildPathCandidates(
	...
	const isCurrentValid =
		currentMeasurement.upright && fitsPath(currentMeasurement, currentOffset);
	const candidates: LabelPlacementCandidate[] = [
		{
			...current,
			preference:
				getPathStartOffsetPreference(currentOffset) +
				(isCurrentValid ? 0 : INVALID_PLACEMENT_PENALTY),
		},
	];

	const offsets = getPathStartOffsetCandidates(currentOffset).filter(
		(offset) => offset !== currentOffset,
	);
	for (const [startOffset, measurement] of sandbox.measurePathOffsets(
		label,
		offsets,
	)) {
		...
		candidates.push({
			...
			preference:
				PATH_PLACEMENT_CHANGE_PENALTY +
				getPathStartOffsetPreference(startOffset),
		});
	}
	return candidates;
}
...
function getPathStartOffsetCandidates(currentOffset: number): number[] {
	...
	const offsets = Array.from(
		{
			length:
				(MAXIMUM_START_OFFSET - MINIMUM_START_OFFSET) / START_OFFSET_STEP + 1,
		},
		(_, index) => MINIMUM_START_OFFSET + index * START_OFFSET_STEP,
	);
	return [
		clampedCurrent,
		...offsets.filter((offset) => offset !== clampedCurrent),
	];
}
...
function isPathTextUpright(
	...
	for (let index = 1; index <= DIRECTION_SAMPLES; index++) {
		const point = path.getPointAtLength(
			start + ((end - start) * index) / DIRECTION_SAMPLES,
		);
...
function getDisplayedBurgIconBounds(): Map<number, LabelBounds> {
	const mapRect = document
		.querySelector<SVGSVGElement>("#map")
	...
	const icons = document.querySelectorAll<SVGGraphicsElement>(
		"#burgIcons use[data-id], #anchors use[data-id]",
	);
...
function getBurgIconObstacles(
	boundsByBurg: Map<number, LabelBounds>,
): LabelPlacementItem[] {
	return [...boundsByBurg].map(([id, bounds]) => ({
		id: `labelSpreadBurgIcon${id}`,
		obstacle: true,
		candidates: [
			{ placement: {}, bounds, collisionBounds: bounds, preference: 0 },
		],
	}));
}
...
function placementChanged(
	...
	if (
		...
		return true;
	return (
		placement.startOffset !== undefined &&
		placement.startOffset !== round(label.startOffset ?? 50)
	);
}
...
function toCandidate(
	...
	const collisionShapes = measurement.inkBounds.map((bounds) =>
		padBounds(bounds, LABEL_PADDING_SCREEN),
	);
	const burgCollisionShapes =
		label.type === "burg"
			? measurement.inkBounds.map((bounds) =>
					padBounds(bounds, BURG_LABEL_PADDING_SCREEN),
				)
			: undefined;
	return {
		placement,
		bounds: measurement.bounds,
		collisionBounds: (burgCollisionShapes ?? collisionShapes).reduce(
			unionBounds,
		),
		collisionShapes,
		burgCollisionShapes,
...
class LabelMeasurementSandbox {
	...
	constructor(labels: LabelData[]) {
		...
		const fontSize =
			renderedLabels?.getAttribute("font-size") ||
			(renderedLabels && getComputedStyle(renderedLabels).fontSize);
	...
	measure(label: LabelData): Measurement {
		...
		const textLength =
			textPath?.getComputedTextLength() ?? text.getComputedTextLength();
		const pathLength = path?.getTotalLength() ?? 0;
		const upright = path
			? isPathTextUpright(path, textLength, label.startOffset ?? 50)
			: true;
	...
	measurePathOffsets(
		...
		if (path && textPath) {
			...
			for (const startOffset of startOffsets) {
				...
				measurements.set(
					startOffset,
					this.read(text, textLength, pathLength, true),
				);
			}
		}
		text.remove();
	...
	private attach(label: LabelData) {
		...
		if (!group)
			throw new Error(`Cannot measure missing Label Group: ${label.group}`);
		const elements = createLabelElements(
			{ ...label, id: `labelSpreadMeasurement${this.counter++}` },
			document,
		);
	...
	private read(
		...
		return {
			bounds,
			inkBounds: inkBounds.length ? inkBounds : [bounds],
			textLength,
			pathLength,
			upright,
		};
	...
	private createGroup(groupName: string): SVGGElement {
		const groupOptions = options.labels.groups.find(
...
function padBounds(
	bounds: LabelBounds,
	screenPadding = LABEL_PADDING_SCREEN,
): LabelBounds {
...
function translateBoundsList(
	bounds: LabelBounds[] | undefined,
	dx: number,
	dy: number,
...
function translateCandidate(
	...
	return {
		...
		burgCollisionShapes: translateBoundsList(
			candidate.burgCollisionShapes,
			dx,
			dy,
		),
...
function getInkEnvelope(candidate: LabelPlacementCandidate): LabelBounds {
	return candidate.inkBounds?.length
		? candidate.inkBounds.reduce(unionBounds)
		: candidate.bounds;
}
...
function getOutsideArea(bounds: LabelBounds, map = mapBounds()): number {
	...
	const insideWidth = Math.max(
		Math.min(bounds.x2, map.x2) - Math.max(bounds.x1, map.x1),
		0,
	);
	const insideHeight = Math.max(
		Math.min(bounds.y2, map.y2) - Math.max(bounds.y1, map.y1),
		0,
	);
...
function optimizeLabelPlacements(
	...
	const state: SolverState = {
		...
		staticCosts: validItems.map((item) =>
			Float64Array.from(
				item.candidates,
				(candidate) =>
					getOutsideArea(candidate.bounds, bounds) * OUTSIDE_WEIGHT +
	...
	validItems.forEach(
		(item, index) =>
			void selected.set(item.id, item.candidates[state.selected[index]]),
	);
	return {
		selected,
 ...
 */
function getConflictComponents(
	state: SolverState,
	pairs: [number, number][],
): number[][] {
...
function sortByMobility(
	component: number[],
	items: LabelPlacementItem[],
): number[] {
	return component.sort((first, second) => {
		const difference =
			getMinimumMovementPreference(items[first]) -
			getMinimumMovementPreference(items[second]);
...
function getMinimumMovementPreference(item: LabelPlacementItem): number {
	...
	return Math.min(
		...item.candidates.slice(1).map((candidate) => candidate.preference),
	);
}
...
function anneal(
	...
	const movable = component.filter(
		(index) => state.items[index].candidates.length > 1,
	);
	...
	const bestSelection = Uint16Array.from(
		component,
	...
	for (
		...
		iteration++, temperature *= cooling
	) {
		if (iteration % movable.length === 0)
			conflicted = getConflictedItems(state, movable);
		const pool =
			conflicted.length && random() < CONFLICTED_ITEM_BIAS
				? conflicted
				: movable;
		const itemIndex = pool[Math.floor(random() * pool.length)];
		const previousIndex = state.selected[itemIndex];
		const nextIndex = Math.floor(
			random() * state.items[itemIndex].candidates.length,
		);
	...
	for (const otherIndex of state.interactions[itemIndex]) {
		...
		cost +=
			getPairOverlap(
				...
				candidate,
				other,
				other.candidates[state.selected[otherIndex]],
			) * OVERLAP_WEIGHT;
	}
	return cost;
}
...
function getComponentCost(state: SolverState, component: number[]): number {
	...
	for (const itemIndex of component) {
		...
		for (const otherIndex of state.interactions[itemIndex]) {
			...
			cost +=
				getSelectedPairOverlap(state, itemIndex, otherIndex) * OVERLAP_WEIGHT;
		}
	}
	return cost;
}

function getSelectedPairOverlap(
	state: SolverState,
	firstIndex: number,
	secondIndex: number,
): number {
...
function getPotentialInteractions(items: LabelPlacementItem[]): number[][] {
	...
	const ordered = items
		.map((item, index) => ({
			index,
			bounds: item.candidates
				.map((candidate) => candidate.collisionBounds)
				.reduce(unionBounds),
		}))
	...
	for (let left = 0; left < ordered.length; left++) {
		...
		for (let right = left + 1; right < ordered.length; right++) {
			...
			if (
				second.bounds.y1 >= first.bounds.y2 ||
				second.bounds.y2 <= first.bounds.y1
			)
				continue;
...
function getOverlapPairs(state: SolverState): [number, number][] {
	const ordered = state.items
		.map((item, index) => ({
			index,
			bounds: item.candidates[state.selected[index]].collisionBounds,
		}))
		.sort((first, second) => first.bounds.x1 - second.bounds.x1);
	...
	for (let left = 0; left < ordered.length; left++) {
		...
		for (let right = left + 1; right < ordered.length; right++) {
			const second = ordered[right];
			if (second.bounds.x1 >= first.bounds.x2) break;
			if (getSelectedPairOverlap(state, first.index, second.index) > 0)
...
function getPairOverlap(
	...
	if (firstItem.obstacle || secondItem.obstacle) {
		const [obstacle, label] = firstItem.obstacle
			? [first, second]
			: [second, first];
		return (
			getShapesOverlap([obstacle.bounds], label.inkBounds ?? [label.bounds]) *
			OBSTACLE_CONFLICT_WEIGHT
		);
	}
	...
	const overlap = getShapesOverlap(
		getCollisionShapes(first, isBurgToBurg),
		getCollisionShapes(second, isBurgToBurg),
	);
...
function getConflictWeight(
	first: LabelPlacementItem,
	second: LabelPlacementItem,
): number {
	if (first.kind === "burg" || second.kind === "burg")
		return BURG_CONFLICT_WEIGHT;
	if (first.kind === "path" || second.kind === "path")
		return PATH_CONFLICT_WEIGHT;
	return 1;
}
...
function getShapesOverlap(first: LabelBounds[], second: LabelBounds[]): number {
	...
	for (const firstBounds of first) {
		for (const secondBounds of second)
			overlap += getOverlapScore(firstBounds, secondBounds);
	}
	return overlap;
}
...
function getOverlapScore(first: LabelBounds, second: LabelBounds): number {
	...
	return (
		ratio *
		clamp(
			(ratio - IGNORED_OVERLAP_RATIO) /
				(FULL_OVERLAP_RATIO - IGNORED_OVERLAP_RATIO),
			0,
			1,
		)
	);
...
function getArea(bounds: LabelBounds): number {
	return (
		Math.max(bounds.x2 - bounds.x1, 0) * Math.max(bounds.y2 - bounds.y1, 0)
	);
}
