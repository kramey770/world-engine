...
function open(type: LabelType, id: number): void {
	...
	const textEl = document.querySelector<SVGTextElement>(
		`#labels text[data-label-type='${type}'][data-id='${id}']`,
	);
	if (!textEl) return;
...
function renderDialog(): void {
	...
	ensureEl("labelGroupsConfigure").addEventListener(
		"click",
	...
	ensureEl("labelTextSpeak").addEventListener("click", () =>
		speak(ensureEl<HTMLInputElement>("labelText").value),
	);
	...
	ensureEl("labelStartOffsetValue").addEventListener(
		"input",
		changeStartOffsetFromValue,
	);
	ensureEl("labelRelativeSize").addEventListener("input", changeRelativeSize);

	ensureEl("labelLetterSpacingShow").addEventListener(
		"click",
		showLetterSpacingSection,
	);
	ensureEl("labelLetterSpacingHide").addEventListener(
		"click",
		hideLetterSpacingSection,
	);
	ensureEl("labelLetterSpacingSize").addEventListener(
		"input",
...
function updateControls(): void {
	...
	const topButtonsVisible =
		!ensureEl("labelEditor").classList.contains("section-open");
	ensureEl("labelOffsetShow").style.display =
		topButtonsVisible && hasPath ? "inline-block" : "none";
	ensureEl("labelRemoveSingle").style.display =
		topButtonsVisible && label.type === "added" ? "inline-block" : "none";
	ensureEl("labelReset").style.display =
		topButtonsVisible && Labels.hasOverride(label.type, label.entityId)
			? "inline-block"
			: "none";

	const pathToggle = ensureEl("labelPathToggle");
	pathToggle.className = hasPath
...
function updateValues(): void {
	...
	ensureEl<HTMLInputElement>("labelStartOffsetValue").value =
		String(startOffset);
	ensureEl<HTMLInputElement>("labelRelativeSize").value = String(
		label.fontSize ?? 100,
	);
	ensureEl<HTMLInputElement>("labelLetterSpacingSize").value = String(
...
function hideTopButtons(): void {
	...
	document
		.querySelectorAll<HTMLElement>("#labelEditor > button")
		.forEach((el) => {
...
function changeGroup(this: HTMLSelectElement): void {
	...
	const targetType = options.labels.groups.find(
		(group) => group.name === nextGroup,
	)?.type;
	const apply = () => {
...
function changeText(): void {
	...
	if (label.type === "state")
		tip(
			"Use States Editor to change the actual state name, not just a label",
			false,
			"warn",
		);
	if (label.type === "province")
		tip(
			"Use Provinces Editor to change the actual province name, not just a label",
			false,
			"warn",
		);
}
...
const nameGenerators: Record<LabelType, (label: LabelData) => string> = {
	...
	route: (label) => {
		const points =
			pack.routes.find((route) => route.i === label.entityId)?.points ?? [];
		return Routes.generateName({ group: label.group, points }) || UNNAMED_ROUTE;
	},
};

function generateRandomName(): void {
	ensureEl<HTMLInputElement>("labelText").value =
		nameGenerators[label.type](label);
	changeText();
}
