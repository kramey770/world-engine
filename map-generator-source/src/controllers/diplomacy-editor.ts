...
const diplomacyTable = initEditorTable<State>({
	getData: () =>
		sortDataByColumns(
			dialogId,
			pack.states.filter(
				(state) => state.i && !state.removed && state.i !== selectedDiplomacyId,
			),
			columns,
		),
...
function open(): void {
	...
	if (pack.states.filter((s) => s.i && !s.removed).length < 2) {
		tip(
			"There should be at least 2 states to edit the diplomacy",
			false,
			"error",
		);
		return;
	...
	if (
		!selectedDiplomacyId ||
		!pack.states[selectedDiplomacyId] ||
		pack.states[selectedDiplomacyId].removed
	) {
		selectedDiplomacyId = pack.states.find(
...
function renderDialog(): void {
	...
	ensureEl("diplomacyEditorRefresh").addEventListener(
		"click",
		refreshDiplomacyEditor,
	);
	ensureEl("diplomacyEditStyle").addEventListener("click", () =>
		editStyle("regions"),
	);
	ensureEl("diplomacyRegenerate").addEventListener(
		"click",
		regenerateRelations,
	);
	ensureEl("diplomacyReset").addEventListener("click", resetRelations);
	ensureEl("diplomacyShowMatrix").addEventListener(
		"click",
		showRelationsMatrix,
	);
	...
	ensureEl("diplomacyBodySection").addEventListener("click", (ev) => {
		...
		if (el.closest(".changeRelations")) {
			...
			const objectId = +ensureEl(
				"diplomacyBodySection",
			).querySelector<HTMLElement>("div.Self")!.dataset.id!;
...
function showStateRelations(): void {
	const selectedLine = ensureEl(
		"diplomacyBodySection",
	).querySelector<HTMLElement>("div.Self");
	const sel = selectedLine
		? +selectedLine.dataset.id!
		: pack.states.find((s) => s.i && !s.removed)!.i;
	if (!sel) return;
	Layers.show("states");
  ...
  `;
		...
		buttons: {
			Apply: function (this: HTMLElement) {
				const formData = new FormData(
					ensureEl<HTMLFormElement>("relationsForm"),
				);
	...
	const stateCheckboxes = () =>
		document.querySelectorAll<HTMLInputElement>(
			"#stateSelectionContainer input[name='objectSelect']",
		);
...
function changeRelation(
	...
	const vassal = (): string[] => [
		`Vassalization`,
		`${subjectName} became a vassal of ${objectName}`,
	];
	const suzerain = (): string[] => [
		`Vassalization`,
		`${subjectName} vassalized ${objectName}`,
	];
	const rival = (): string[] => [
		`Rivalization`,
		`${subjectName} and ${objectName} became rivals`,
	];
	...
	const war = (): string[] => [
		`War declaration`,
		`${subjectName} declared a war on its enemy ${objectName}`,
	];
	const peace = (): string[] => {
	...
	$("#alert").dialog({
		...
		buttons: {
			Save: function (this: HTMLElement) {
				const data = this.querySelector("div")!
					.innerText.split("\n")
					.join("\r\n");
