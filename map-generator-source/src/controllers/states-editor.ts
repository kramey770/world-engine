...
import {
	bindColumnSorting,
	sortDataByColumns,
} from "@/components/dialog/sorting";
import {
	type EditorColumn,
...
const STATE_COLUMNS: EditorColumn<State>[] = [
	...
	},
	{
		...
		sortBy: (s) =>
			rn(
				(s.rural || 0) * populationRate +
					(s.urban || 0) * populationRate * urbanization,
			),
	},
	{
		key: "treasury",
...
const statesTable = initEditorTable<State>({
	getData: () =>
		sortDataByColumns(dialogId, getFilteredStatesData(), STATE_COLUMNS),
...
function renderDialog(): void {
	...
	ensureEl("statesEditorRefresh").addEventListener(
		"click",
		refreshStatesEditor,
	);
	initColumnVisibility({
	...
	ensureEl("statesEditStyle").addEventListener("click", () =>
		editStyle("regions"),
	);
	...
	ensureEl("statesRegenerateBack").addEventListener(
		"click",
		exitRegenerationMenu,
	);
	ensureEl("statesRecalculate").addEventListener("click", () =>
		recalculateStates(true),
	);
	ensureEl("statesRandomize").addEventListener(
		"click",
		randomizeStatesExpansion,
	);
	ensureEl("statesGrowthRate").addEventListener("input", () =>
		recalculateStates(false),
	);
	ensureEl("statesManually").addEventListener(
		"click",
		enterStatesManualAssignent,
	);
	ensureEl("statesManuallyState").addEventListener("change", highlightBrushRow);
	ensureEl("statesManuallyDemote").addEventListener(
		"change",
		updateDemotePickerLabel,
	);
	ensureEl("statesManuallyUndo").addEventListener(
		"click",
		undoStatesManualAssignment,
	);
	ensureEl("statesManuallyApply").addEventListener(
		"click",
		applyStatesManualAssignent,
	);
	ensureEl("statesManuallyCancel").addEventListener("click", () =>
	...
	ensureEl("statesBodySection").addEventListener("click", (event) => {
		...
		if ($element.tagName === "FILL-BOX")
			stateChangeFill($element as FillBoxElement);
		else if (classList.contains("name")) editStateName(stateId);
		else if (classList.contains("coaIcon"))
			void Controllers.EmblemsEditor.open(
				"state",
				`stateCOA${stateId}`,
				pack.states[stateId],
			);
		else if (classList.contains("icon-star-empty")) stateCapitalZoomIn(stateId);
		else if (classList.contains("icon-dot-circled"))
			Controllers.BurgsOverview.open({ stateId });
			...
			highlightElement(
				select("#regions").select(`#state${stateId}`).node() as Element,
				4,
			);
		else if (classList.contains("icon-trash-empty")) stateRemovePrompt(stateId);
		else if (
			classList.contains("icon-lock") ||
			classList.contains("icon-lock-open")
		)
	...
	ensureEl("statesBodySection").addEventListener("input", (ev) => {
		...
		if (classList.contains("stateCapital"))
			stateChangeCapitalName(state, line, $element.value);
	});
	...
	ensureEl("statesBodySection").addEventListener("change", (ev) => {
		...
		if (!line) return;
		const state = +line.dataset.id!;
		if (classList.contains("stateCulture"))
			stateChangeCulture(state, line, $element.value);
		else if (classList.contains("cultureType"))
			stateChangeType(state, line, $element.value);
		else if (classList.contains("statePower"))
			stateChangeExpansionism(state, line, $element.value);
	});
}
...
function renderStatesPage(view: TableView<State>): void {
	...
	ensureEl("statesFooterStates").innerHTML = String(
		pack.states.filter((s) => s.i && !s.removed).length,
	);
	...
	ensureEl("statesFooterPopulation").dataset.population =
...
function stateHighlightOn(event: any): void {
	...
	const interpolate = interpolateString(
		`0, ${totalLength}`,
...
function editStateName(state: number): void {
	renderNameEditor();
	const stateNameEditorCustomForm = ensureEl<HTMLInputElement>(
		"stateNameEditorCustomForm",
	);
	const stateNameEditorSelectForm = ensureEl<HTMLSelectElement>(
		"stateNameEditorSelectForm",
	);

	// reset input value and close add mode
	stateNameEditorCustomForm.value = "";
	const addModeActive =
		stateNameEditorCustomForm.style.display === "inline-block";
	if (addModeActive) {
	...
	ensureEl("stateNameEditorShortCulture").addEventListener(
		"click",
		regenerateShortNameCulture,
	);
	ensureEl("stateNameEditorShortRandom").addEventListener(
		"click",
	...
	ensureEl("stateNameEditorCustomForm").addEventListener(
		"change",
		addCustomForm,
	);
	ensureEl("stateNameEditorFullRegenerate").addEventListener(
		"click",
		regenerateFullName,
	);
	...
	function addCustomForm() {
		...
		const addModeActive =
			stateNameEditorCustomForm.style.display === "inline-block";
		stateNameEditorCustomForm.style.display = addModeActive
			? "none"
			: "inline-block";
		stateNameEditorSelectForm.style.display = addModeActive
			? "inline-block"
			: "none";
	...
	function applyNameChange(s: any) {
		...
		if (
			changed &&
			ensureEl<HTMLInputElement>("stateNameEditorUpdateLabel").checked
		) {
...
function changePopulation(stateId: number): void {
	...
	if (!state.cells) {
		tip(
			"State does not have any cells, cannot change population",
			false,
			"error",
		);
		return;
	...
	function applyPopulationChange() {
		...
		if (Number.isFinite(ruralChange) && ruralChange !== 1) {
			const cells = (pack.cells.i as unknown as number[]).filter(
				(i) => pack.cells.state[i] === stateId,
			);
			cells.forEach((i) => {
...
function openTreasuryDialog(stateId: number): void {
	...
	const pollTaxRevenue = rn(
		state.pollTax * ((state.rural || 0) + (state.urban || 0)),
		2,
	);
...
function togglePercentageMode(): void {
	if (ensureEl("statesBodySection").dataset.type === "absolute") {
		...
		const totalPopulation = +ensureEl("statesFooterPopulation").dataset
			.population!;
		const totalTreasury = pack.states.reduce(
			(sum, s) => sum + (s.treasury || 0),
			0,
		);
		const totalCells = pack.states.reduce(
			(sum, s) => sum + (s.i && !s.removed ? s.cells || 0 : 0),
			0,
		);
		...
		ensureEl("statesBodySection")
			...
			.forEach((el) => {
				...
				el.querySelector<HTMLElement>(".stateBurgs")!.innerText =
					`${rn((+burgs! / totalBurgs) * 100)}%`;
				el.querySelector<HTMLElement>(".stateCells")!.innerText =
					`${rn((+cells! / totalCells) * 100)}%`;
				el.querySelector<HTMLElement>(".stateArea")!.innerText =
					`${rn((+area! / totalArea) * 100)}%`;
				el.querySelector<HTMLElement>(".statePopulation")!.innerText =
					`${rn((+population! / totalPopulation) * 100)}%`;
				el.querySelector<HTMLElement>(".stateTreasury")!.innerText =
					`${rn((+treasury! / totalTreasury) * 100, 2)}%`;
			});
	} else {
		ensureEl("statesBodySection").dataset.type = "absolute";
		statesTable.refresh();
...
function showStatesChart(): void {
	...
	node
		...
		.style(
			"font-size",
			(d: any) => `${rn((d.r ** 0.97 * 4) / lp(d.data.name), 2)}px`,
		)
		.selectAll("tspan")
		...
		.attr(
			"dy",
			(_d: any, i: number, n: any) => `${i ? 1 : (n.length - 1) / -2}em`,
		);
...
function enterStatesManualAssignent(): void {
	...
	document
		.querySelectorAll<HTMLElement>("#statesBottom > button")
		.forEach((el) => {
			el.style.display = "none";
		});
	...
	tip(
		"Pick a state to paint with, then drag the circle. Click the map to pick the state under the cursor",
		true,
	);
...
function populateBrushStateSelect(): void {
	...
	sel.innerHTML = states
		.map(
			(s: any) =>
				`<option value="${s.i}">${s.i ? s.fullName : s.name}</option>`,
		)
		.join("");
}
...
function isDemotePickerOn(): boolean {
	return Boolean(
		(ensureEl("statesManuallyDemote") as HTMLInputElement | null)?.checked,
	);
}
...
function updateDemotePickerLabel(): void {
	...
	if (label)
		label.textContent = isDemotePickerOn() ? "Demote into: " : "Paint as: ";
}
...
function selectStateOnMapClick(this: any, event: any): void {
	...
	const assigned = select("#statesBody")
		.select("#temp")
		.select(`polygon[data-cell='${i}']`);
	const state = assigned.size()
		? +assigned.attr("data-state")
		: (pack.cells as any).state[i!];
...
function dragStateBrush(this: any, event: any): void {
	...
	event.on("drag", (dragEvent: any) => {
		...
		const found =
			r > 5 ? Pack.findAll(p[0], p[1], r) : [Pack.findCell(p[0], p[1])];
		const selection = found.filter(
			(i): i is number => i !== undefined && isLand(i, pack),
		);
...
function changeStateForSelection(selection: number[]): void {
	...
	const preventOverwrite = (
		document.getElementById("statesManuallyProtect") as HTMLInputElement | null
	)?.checked;

	selection.forEach((i) => {
		const exists = temp.select(`polygon[data-cell='${i}']`);
		const stateOld = exists.size()
			? +exists.attr("data-state")
			: pack.cells.state[i];
...
function applyStatesManualAssignent(): void {
		...
		.each(function () {
			if (this.dataset.demote) {
				...
				if (ownerId && sourceId && ownerId !== sourceId) {
					if (!demotionsByOwner.has(ownerId))
						demotionsByOwner.set(ownerId, new Set());
	...
	demotionsByOwner.forEach((sourceSet, ownerId) => {
		const ids = [...sourceSet].filter(
			(id) => pack.states[id] && !pack.states[id].removed,
		);
...
function adjustProvinces(affectedProvinces: number[]): void {
	...
	affectedProvinces.forEach((provinceId) => {
		...
		const provCells = cells.i.filter(
			(i: number) => cells.province[i] === provinceId,
		);
		const provStates = [
			...new Set(provCells.map((i: number) => cells.state[i])),
		] as number[];
	...
	redrawEmblems(
		createdProvinces.map((provinceId) => ["province", provinceId] as const),
	);

	function changeProvinceOwner(
		provinceId: number,
		newOwnerId: number,
		...
		prevOwner.provinces = prevOwner.provinces.filter(
			(province: number) => province !== provinceId,
		);
	...
	function splitProvince(
		...
		provinceStates.forEach((stateId) => {
			const stateProvinceCells = provinceCells.filter(
				(i) => cells.state[i] === stateId,
			);
			...
			if (stateProvinceCells.length < 20) {
				const closestProvince = findClosestProvince(
					provinceId,
					stateId,
					stateProvinceCells,
				);
	...
	function createProvince(
		...
		const name = nameByBurg
			? burg.name
			: oldProvince.name ||
				Names.getState(Names.getCultureShort(culture), culture);

		const formOptions = ["Zone", "Area", "Territory", "Province"];
		const formName =
			burgCell && oldProvince.formName ? oldProvince.formName : ra(formOptions);
	...
	function findClosestProvince(
		...
		const borderCell = sourceCells.find((i) =>
			cells.c[i].some((c: number) => {
				return (
					cells.state[c] === stateId &&
					cells.province[c] &&
					cells.province[c] !== provinceId
				);
			}),
		);
...
function enterAddStateMode(this: HTMLElement): void {
	...
	tip(
		"Click on the map to create a new capital or promote an existing burg",
		true,
	);
	select<SVGElement, unknown>("#viewbox")
		.style("cursor", "crosshair")
...
function addState(this: SVGElement, event: MouseEvent): void {
	...
	if (cells.h[center] < 20) {
		tip(
			"You cannot place state into the water. Please click on a land cell",
			false,
			"error",
		);
		return;
	...
	if (burgId && burgs[burgId].capital) {
		tip(
			"Existing capital cannot be selected as a new state capital! Select other cell",
			false,
			"error",
		);
		return;
	...
	const culture = cells.culture[center];
	const basename =
		center % 5 === 0 ? burgs[burgId].name : Names.getCulture(culture);
...
function exitAddStateMode(): void {
	...
	if (statesAdd.classList.contains("pressed"))
		statesAdd.classList.remove("pressed");
}
...
function openStateMergeDialog(): void {
  ...
  `;
	...
	applyLineHighlighting(
		"mergeStatesForm",
		({ cellId }) => pack.cells.state[cellId],
	);
		...
		buttons: {
			Merge: function (this: HTMLElement) {
				const formData = new FormData(
					ensureEl<HTMLFormElement>("mergeStatesForm"),
				);
...
function mergeStates(
	...
	if (!pack.states[rulingStateId].label)
		delete pack.states[rulingStateId].label;
...
function downloadStatesCsv(): void {
	...
	const data = statesTable.view().all.map((s) => {
		...
		const population = rn(
			rural * populationRate + urban * populationRate * urbanization,
		);
		return [
			s.i,
			s.name,
