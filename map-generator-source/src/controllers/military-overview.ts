...
function renderDialog(): void {
	...
	ensureEl("militaryOverviewRefresh").addEventListener(
		"click",
		refreshMilitaryOverview,
	);
	ensureEl("militaryPercentage").addEventListener(
		"click",
		togglePercentageMode,
	);
	ensureEl("militaryOptionsButton").addEventListener(
		"click",
		militaryCustomize,
	);
	ensureEl("militaryRegimentsList").addEventListener("click", () =>
		openRegimentsOverview(-1),
	);
	ensureEl("militaryOverviewRecalculate").addEventListener(
		"click",
		militaryRecalculate,
	);
	ensureEl("militaryExport").addEventListener("click", downloadMilitaryData);
	ensureEl("militaryWiki").addEventListener("click", () =>
		wiki("Military-Forces"),
	);
...
function getMilitaryColumns(): EditorColumn<MilitaryRow>[] {
	const unitColumns: EditorColumn<MilitaryRow>[] = options.military.map(
		(unit) => ({
			key: `unit:${unit.name}`,
...
function rebuildMilitaryColumns(): void {
	columns = getMilitaryColumns();
	ensureEl(`${dialogId}Header`).outerHTML = renderEditorHeader({
		dialogId,
		columns,
	});
	bindMilitaryColumns();
		...
		.map((state) => {
			const forces = Object.fromEntries(
				options.military.map((unit) => [
					unit.name,
					(state.military || []).reduce(
						(total, regiment) => total + (regiment.u[unit.name] || 0),
						0,
					),
				]),
			);
			const population = rn(
				((state.rural || 0) + (state.urban || 0) * urbanization) *
					populationRate,
			);
			const total = options.military.reduce(
				(sum, unit) => sum + (forces[unit.name] || 0) * unit.crew,
				0,
			);
			return {
				state,
		...
		(result, row) => {
			...
			for (const unit of options.military)
				result.units[unit.name] =
					(result.units[unit.name] || 0) + row.forces[unit.name];
			return result;
		},
		{ total: 0, population: 0, units: {} as Record<string, number> },
	);
	const percent = (value: number, total: number) =>
		`${rn(total ? (value / total) * 100 : 0)}%`;
...
function changeAlert(state: number, alert: number): void {
	...
	(s.military || []).forEach((r) => {
		...
		select<SVGGElement, unknown>(
			`#armies > g > g#regiment${s.i}-${r.i} > text`,
		).text(Military.getTotal(r)); // change icon text
	});
...
function updateFooter(view: TableView<MilitaryRow>): void {
	...
	ensureEl("militaryFooterForces").innerHTML = si(
		statesNumber ? total / statesNumber : 0,
	);
	...
	ensureEl("militaryFooterAlert").innerHTML = String(
		rn(
			statesNumber ? sum(view.all.map((row) => row.alert)) / statesNumber : 0,
			2,
		),
	);
}
...
function militaryCustomize(): void {
	...
	$("#militaryOptions").dialog({
		...
		open: function () {
			...
			buttons[3].addEventListener("mousemove", () =>
				tip("Close the window without saving the changes"),
			);
		},
	});
	...
	function addUnitLine(unit: MilitaryUnit): void {
		...
		const getLimitButton = (
			attr: "biomes" | "states" | "cultures" | "religions",
		): string => {
			...
			buttons: {
				...
				Apply: function () {
					const inputs = Array.from(
						alertMessage.querySelectorAll<HTMLInputElement>("input"),
					);
	...
	function applyMilitaryOptions(): void {
		...
		const names = unitLines.map((r) =>
			sanitizeId(r.querySelector("input")!.value),
		);
		...
		options.military = unitLines.map((r, i) => {
			...
			const values = elements.map((el) => {
				...
				if (type)
					return value ? value.split(",").map((v) => parseInt(v, 10)) : null;
				if ((el as HTMLInputElement).type === "number")
					return +(el as HTMLInputElement).value || 0;
				if ((el as HTMLInputElement).type === "checkbox")
					return +(el as HTMLInputElement).checked || 0;
