		...
		buttons: {
			...
			Add: () => {
				...
				ensureEl("labelGroupsBody").insertAdjacentHTML(
					"beforeend",
					createRow(group, true, 0),
				);
			},
			Restore: () => {
				const defaults = Labels.getDefaultOptions();
				ensureEl<HTMLInputElement>("labelsResizeOnZoom").checked =
					defaults.resizeOnZoom;
...
function renderDialog(): void {
	...
	ensureEl("labelGroupsBurgGroupsLink").addEventListener("click", () =>
		Controllers.BurgGroupEditor.open(),
	);
	ensureEl("labelGroupsMissing").addEventListener(
		"click",
		onMissingGroupsClick,
	);
}
...
function addMissingGroups(
	counts: Map<string, number>,
	groups: LabelGroup[],
): void {
	const definedGroups = new Set(groups.map(({ name }) => name));
	const missingGroups = [...counts.entries()]
		.filter(([name]) => !definedGroups.has(name))
		.sort();

	ensureEl("labelGroupsMissingWrapper").style.display = missingGroups.length
		? "flex"
		: "none";
...
function onMissingGroupsClick(event: Event): void {
	...
	if (button?.dataset.group)
		void Controllers.LabelsOverview.open(button.dataset.group);
}
...
function removeRow(row: HTMLTableRowElement): void {
	...
	confirmationDialog({
		...
		message:
			"Remove the group? This won't affect labels unless the changes are applied.",
		confirm: "Remove",
		onConfirm: () => {
			row.remove();
			validateForm();
...
function validateForm(): boolean {
	...
	nameInputs.forEach((input) => {
		...
		if (!isValidName)
			message =
				"Group name must start with a letter or underscore and not contain special characters";
...
function submitForm(event: Event): void {
	...
	const rows = Array.from(
		ensureEl("labelGroupsBody").children,
	) as HTMLTableRowElement[];
	if (!rows.length)
		return void tip("At least one group should be defined", false, "error");
...
function rowToGroup(row: HTMLTableRowElement): LabelGroup {
	const name = row
		.querySelector<HTMLInputElement>('[name="name"]')!
		.value.trim();
	const type = row.querySelector<HTMLSelectElement>('[name="type"]')!
		.value as LabelType;
	const active =
		row.querySelector<HTMLInputElement>('[name="active"]')!.checked;
	const mode = row.querySelector<HTMLSelectElement>('[name="mode"]')!
		.value as LabelNameMode;
	...
	const dependency = row
		.querySelector<HTMLSelectElement>('[name="dependency"]')!
		.value.trim();
