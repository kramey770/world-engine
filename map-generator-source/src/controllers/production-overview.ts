...
function open(burgId: number): void {
	...
	const isBurgSeller = (deal: Deal) =>
		deal.sellerType === "burg" && deal.seller === burgId;
	const isBurgBuyer = (deal: Deal) =>
		deal.buyerType === "burg" && deal.buyer === burgId;
	...
	const getDealTax = (deal: Deal) => {
		...
		return (
			deal.units * deal.price * States.getSalesTax(pack.burgs[deal.seller])
		);
	};
	...
	const renderDecisionCandidate = (candidate: ProductionCandidate) => {
		...
		if (candidate.isPreparation) {
			...
			const gain =
				((candidate.gainPerWorker || 0) / candidate.demandMultiplier) * workers;
	...
	const renderDecisionDetails = (
		...
		const candidatesHtml = `<ul style="margin:.2em 0 0 1.1em;padding:0">${[
			...candidates,
		]
	...
	const allRows = data.flatMap((entry) => {
		if (isMfgRecord(entry)) {
			...
			const cultureSuffix =
				cultureModifier !== 1 ? ` ${modifierBadge(cultureModifier)}` : "";
			const allInputs = mfg.recipe
				.map((item) => `${rn(item.units, 2)} ${goodDot(item.goodId)}`)
				.join(` and `);
           ...
           ${renderDataCell("", "right", styles.subtle)}
			...
			producedByGood[entry.goodId] =
				(producedByGood[entry.goodId] || 0) + entry.units;
			netInventory[entry.goodId] =
				(netInventory[entry.goodId] || 0) + entry.units;
