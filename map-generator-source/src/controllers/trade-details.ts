...
function renderDialog(): void {
	...
	ensureEl("tradeDetailsSummary").addEventListener("click", (event) => {
		const zoomEl = (event.target as HTMLElement).closest<HTMLElement>(
			"[data-zoom]",
		);
		if (!activeBatch || !zoomEl) return;
		const burgId =
			activeBatch[
				zoomEl.dataset.zoom === "start" ? "startBurgId" : "endBurgId"
			];
...
function renderTradeDetailsPage(view: TableView<TradeDetailRow>): void {
	...
	renderEditorPagination(
		ensureEl("tradeDetailsFooter"),
		view,
		tradeDetailsTable.goto,
	);
