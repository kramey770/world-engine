...
import {
	...
	panBy,
	zoomAt,
} from "@/utils/panZoomUtils";
import {
	type Burg,
	cellSlotAfterRemoval,
	groundSlotOnPlacement,
} from "../generators/burgs-generator";
...
function open(id: number | string): void {
	...
	selectedId = +id;
	selected = select<any, unknown>("#labels").select(
		`[data-label-type='burg'][data-id='${id}']`,
	);
	if (!selected.size())
		selected = select<any, unknown>("#burgIcons").select(`[data-id='${id}']`);
...
function renderDialog(): void {
	...
	ensureEl("burgNameSpeak").addEventListener("click", () =>
		speak(ensureEl<HTMLInputElement>("burgName").value),
	);
	...
	ensureEl("burgPreviewObject").addEventListener(
		"wheel",
		onPreviewWheel as EventListener,
		{ passive: false },
	);
	ensureEl("burgPreviewObject").addEventListener(
		"dblclick",
		onPreviewDoubleClick as EventListener,
	);
	ensureEl("burgPreviewObject").addEventListener(
		"pointerdown",
		onPreviewPointerDown as EventListener,
	);
	...
	ensureEl("burgEditAnchorStyle").addEventListener(
		"click",
		editGroupAnchorStyle,
	);

	ensureEl("burgEmblem").addEventListener("click", openEmblemEdit);
	ensureEl("burgSetPreviewLink").addEventListener(
		"click",
	...
	ensureEl("burgTemperatureGraph").addEventListener(
		"click",
		showTemperatureGraph,
	);
	ensureEl("burgProductionOverview").addEventListener(
		"click",
...
function updateBurgValues(): void {
	...
	const megaNote = mega
		...
		: "";
	ensureEl("burgProvinceAndState").innerHTML =
		provinceName + stateName + megaNote;
	...
	ensureEl<HTMLInputElement>("burgPopulation").value = String(
		rn(b.population! * populationRate * urbanization),
	);
	ensureEl("burgWealth").innerHTML =
		`🟡 ${rn(b.population! > 0 ? (b.product || 0) / b.population! : 0, 2)}`;
	ensureEl("burgTreasury").innerHTML = `🟡 ${rn(b.treasury || 0, 2)}`;
	ensureEl("burgEditAnchorStyle").style.display = +b.port!
		? "inline-block"
		: "none";
	...
	cultures.forEach(
		(c) =>
			void cultureSelect.options.add(
				new Option(c.name, String(c.i), false, c.i === b.culture),
			),
	);
	...
	ensureEl("burgProduction").innerHTML = getProduction(
		Production.getBurgProduction(b),
	);
...
function toggleFeature(this: HTMLElement): void {
	...
	ensureEl("burgEditAnchorStyle").style.display = burg.port
		? "inline-block"
		: "none";
...
function updateTradeRoleControl(burg: any): void {
	...
	select.options[0].text =
		!burg.tradeRoleManual && burg.tradeRole
			? `Auto (${burg.tradeRole})`
			: "Auto";
...
function toggleSkyPort(burgId: number): void {
	...
	if (burg.skyPort) {
		...
		if (burg.flying) {
			tip(
				"Cannot disable Sky Port on a flying burg — turn off Flying instead",
				false,
				"warn",
			);
			return;
...
function togglePort(burgId: number): void {
	...
	if (burg.port) {
		...
		if (haven) {
			...
			if (!portFeatureId) {
				tip(
					"No navigable water body found downstream, cannot assign port",
					false,
					"warn",
				);
				return;
...
function showStyleSection(): void {
	document
		.querySelectorAll<HTMLElement>("#burgBottom > button")
		.forEach((el) => {
...
function commitPreviewTransform(): void {
	...
	const frame =
		ensureEl("burgPreviewObject").querySelector<HTMLIFrameElement>("iframe");
	if (!frame) return;
	const { k, x, y } = previewTransform;
	previewCommittedK = k;
...
function relocateBurgOnClick(this: SVGGElement, event: any): void {
	...
	// change data
	cells.burg[burg.cell] = cellSlotAfterRemoval(
		cells.burg[burg.cell],
		burg,
		pack.burgs,
	);
	cells.burg[cellId] = groundSlotOnPlacement(
		cells.burg[cellId],
		id,
		Boolean(burg.flying),
	);
	...
	if (burg.label)
		Object.assign(burg.label, { dx: 0, dy: 0, pathPoints: undefined });
	Layers.draw("labels");
