...
import {
	type EmblemType,
	redrawEmblem,
	subscribeToEmblemReconciliation,
...
async function openDefault(): Promise<void> {
	const firstState = pack.states.find(
		(state) => state.i && !state.removed && state.coa,
	);
	const firstBurg = pack.burgs.find(
		(burg) => burg.i && !burg.removed && burg.coa,
	);
...
function open(
	...
	unsubscribeFromReconciliation =
		subscribeToEmblemReconciliation(makeEmblemsDraggable);
...
function renderDialog(): void {
	...
	ensureEl("emblemsUploadImage").onclick = () =>
		ensureEl("emblemImageToLoad").click();
	ensureEl("emblemsUploadSVG").onclick = () =>
		ensureEl("emblemSVGToLoad").click();
...
function updateElementSelectors(): void {
	...
	// set active type
	(emblemStates.parentElement as HTMLElement).className =
		type === "state" ? "active" : "";
	(emblemProvinces.parentElement as HTMLElement).className =
		type === "province" ? "active" : "";
	(emblemBurgs.parentElement as HTMLElement).className =
		type === "burg" ? "active" : "";
	...
	if (type === "state") state = el.i;
		...
		burg = el.i;
		province = pack.cells.province[el.cell!]
			? pack.provinces[pack.cells.province[el.cell!]].i
			: 0;
		state = el.state ?? 0;
	}
	...
	if (neutralBurgs.length)
		emblemStates.options.add(
			new Option(pack.states[0].name, "0", false, !state),
		);
	const stateList = pack.states.filter((s) => s.i && !s.removed);
	stateList.forEach((s) => {
		emblemStates.options.add(
			new Option(s.name, String(s.i), false, s.i === state),
		);
	});

	emblemProvinces.options.length = 0;
	emblemProvinces.options.add(new Option("", "0", false, !province));
	const provinceList = pack.provinces.filter(
		(p) => !p.removed && p.state === state,
	);
	provinceList.forEach((p) => {
		emblemProvinces.options.add(
			new Option(p.name, String(p.i), false, p.i === province),
		);
	});
	...
	burgList.forEach((b) => {
		emblemBurgs.options.add(
			new Option(
				b.capital ? `👑 ${b.name}` : b.name,
				String(b.i),
				false,
				b.i === burg,
			),
...
function updateEmblemData(): void {
	...
	const emblemShapeSelector = ensureEl<HTMLSelectElement>(
		"emblemShapeSelector",
	);
...
function selectState(): void {
	...
	if (state) {
		...
		const neutralBurgs = pack.burgs.filter(
			(b) => b.i && !b.removed && !b.state,
		);
...
function changeShape(): void {
	currentEl.coa.shield = ensureEl<HTMLSelectElement>(
		"emblemShapeSelector",
	).value;
...
function regenerate(): void {
	...
	const shield =
		el.coa.shield ||
		Emblems.getShield(el.culture || parent?.culture || 0, el.state);
	const { size, x, y } = el.coa;
	el.coa = {
		...Emblems.generate(parent ? parent.coa : null, 0.3, 0.1, undefined),
		shield,
		size,
		x,
		y,
...
function openInArmoria(): void {
	const coa =
		currentEl.coa && !currentEl.coa.custom ? currentEl.coa : { t1: "sable" };
...
function upload(type: "image" | "svg"): void {
	...
	const input =
		type === "image"
			? ensureEl<HTMLInputElement>("emblemImageToLoad")
			: ensureEl<HTMLInputElement>("emblemSVGToLoad");
	...
	reader.onload = (readerEvent) => {
		...
		if (type === "svg") {
			...
			if (!svgEl) {
				tip(
					"The file is not a valid SVG. Please use Armoria or other relevant tools",
					false,
					"error",
				);
				return;
...
async function downloadGallery(): Promise<void> {
	...
	const validProvinces = pack.provinces.filter(
		(p) => p.i && !p.removed && p.coa,
	);
	const validBurgs = pack.burgs.filter((b) => b.i && !b.removed && b.coa);
	...
	const burgSections = validStates
		.map((state) => {
			...
			let stateBurgSections = validProvinces
				...
				.map((province) => {
					const provinceBurgs = stateBurgs.filter(
						(b) => pack.cells.province[b.cell] === province.i,
					);
			...
			const stateBurgOutOfProvinces = stateBurgs.filter(
				(b) => !pack.cells.province[b.cell],
			);
...
async function renderAllEmblems(
	states: State[],
	provinces: Province[],
	burgs: Burg[],
): Promise<void> {
	...
	const burgPromises = burgs.map((burg) =>
...
function dragEmblem(this: SVGUseElement, event: EmblemDragEvent): void {
	...
	event.on("end", function (this: SVGUseElement, endEvent: EmblemDragEvent) {
		const categotySize = Number(
			(this.parentNode as SVGElement).getAttribute("font-size"),
		);
		...
		const entity =
			type && Number.isInteger(i) ? getEmblemEntity(type, i) : undefined;
