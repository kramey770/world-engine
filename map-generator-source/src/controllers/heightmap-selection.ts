...
function addListeners(): void {
	...
	ensureEl("heightmapSelectionRenderOcean").addEventListener(
		"change",
		redrawAll,
	);
	ensureEl("heightmapSelectionColorScheme").addEventListener(
		"change",
		redrawAll,
	);
	ensureEl("heightmapSelectionRedrawPreview").addEventListener(
		"click",
		redrawAll,
	);
	ensureEl("heightmapSelectionEditTemplates").addEventListener(
		"click",
		(event) => confirmHeightmapEdit(event.currentTarget as HTMLElement),
	);
	ensureEl("heightmapSelectionImportHeightmap").addEventListener(
		"click",
		(event) => confirmHeightmapEdit(event.currentTarget as HTMLElement),
	);
}
...
function getSelected(): string | undefined {
	return ensureEl("heightmapSelection").querySelector<HTMLElement>(".selected")
		?.dataset?.id;
}
...
function getName(id: string): string {
	...
	return isTemplate
		? heightmapTemplates[id].name
		: precreatedHeightmaps[id].name;
}
...
function drawTemplatePreview(id: string): void {
	...
	const article = ensureEl("heightmapSelection").querySelector(
		`[data-id="${id}"]`,
	);
...
function redrawAll(): void {
	graph = getGraph(graph);
	const articles =
		ensureEl("heightmapSelection").querySelectorAll<HTMLElement>("article");
...
function confirmHeightmapEdit(el: HTMLElement): void {
	...
	confirmationDialog({
		...
		message:
			"Opening the tool will erase the current map. Are you sure you want to proceed?",
		confirm: "Continue",
		onConfirm: () =>
			window.Controllers.HeightmapEditor.open({ mode: "erase", tool }),
	});
}

function getHeightmapPreview(heights: Uint8Array | null): string {
	const scheme = getColorScheme(
		ensureEl<HTMLSelectElement>("heightmapSelectionColorScheme").value,
	);
	const renderOcean = ensureEl<HTMLInputElement>(
		"heightmapSelectionRenderOcean",
	).checked;
