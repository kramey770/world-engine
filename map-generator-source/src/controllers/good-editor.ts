...
function open(editedGood?: Good, onUpdate?: () => void) {
	...
	const demandCoverageState: Partial<Record<DemandCategory, number>> = {
		...(editedGood?.demandCoverage || {}),
	};
	const biomeOutputState: Partial<Record<number, number>> = {
		...(editedGood?.biomeOutput || {}),
	};

	const demandCoverageSummary = (): string => {
		const entries = DEMAND_PRIORITY.map(
			(cat) => [cat, demandCoverageState[cat] ?? 0] as const,
		).filter(([, v]) => v > 0);
		if (!entries.length) return "none";
		return entries
			.map(
				([cat, v]) => `${DEMAND_CATEGORY_ICONS[cat]} ${capitalize(cat)}: ${v}`,
			)
			.join(", ");
	};

	const biomeOutputSummary = (): string => {
		const entries = Object.entries(biomeOutputState).filter(
			([, v]) => (v ?? 0) > 0,
		);
		if (!entries.length) return "none";
		return entries
			.map(([id, v]) => `${pack.biomes[Number(id)].name}: ${v}`)
			.join(", ");
	};

	const multipliers: {
		[K in MultiplierDimKey]?: Partial<Record<string, number>>;
	} = {
	...
	const multiplierSummary = (dim: MultiplierDimKey): string => {
		...
		if (!entries.length) return "none";
		return entries
			.map(([id, v]) => `${getMultiplierEntityName(dim, id)} ×${rn(v!, 2)}`)
			.join(", ");
	};
	...
	$(dialog!).dialog({
		...
		buttons: {
			...
			[editedGood ? "Apply" : "Add"]: () => {
				...
				const distribution =
					ensureEl("newGoodDistribution").textContent?.trim() ?? "";

				if (!name) errors.push("Name is required");
				if (!Number.isFinite(value) || value < 0)
					errors.push("Value must be a valid non-negative number");
				...
				if (distribution) {
					try {
						...
						errors.push(
							`Distribution function is invalid: ${(err as Error).message || err}`,
						);
					}
				}

				for (const recipe of recipes) {
					for (const [ingredientId, ingredientAmount] of Object.entries(
						recipe,
					)) {
					...
					if (!Object.keys(recipe).length)
						errors.push("Each recipe must have at least one ingredient");
				}
				...
				function buildFinalMultipliers(): Good["multipliers"] {
					...
					for (const [dimKey, vals] of Object.entries(multipliers) as [
						...
						if (Object.keys(nonDefault).length)
							(result as any)[dimKey] = nonDefault;
					}
				...
				if (editedGood) {
					...
					editedGood.biomeOutput = Object.keys(biomeOutputState).length
						? biomeOutputState
						: undefined;
					...
					pack.goods.push({
						...
						biomeOutput: Object.keys(biomeOutputState).length
							? biomeOutputState
							: undefined,
				...
				tip(
					editedGood ? "Good is updated" : "Good is added",
					false,
					"success",
					5000,
				);
    ...
    <div class="ge">
      ...
      <div id="newGoodError" class="ge-error"></div>
		...
		const sortedGoods = [...pack.goods].sort((a, b) =>
		...
		const updateTypeNotes = () => {
			...
			recipeNote.textContent =
				"This good is raw-only: gathered from the environment.";
			recipeNote.style.display = recipesEmpty && !rawEmpty ? "" : "none";

			const rawNote = ensureEl("newGoodRawNote");
			rawNote.textContent =
				"This good is manufactured-only: made from recipes in burgs.";
		...
		dialog
			.querySelector<HTMLButtonElement>(".dcEdit")!
			.addEventListener("click", () => {
				openDemandCoveragePopup({ ...demandCoverageState }, (values) => {
					(Object.keys(demandCoverageState) as DemandCategory[]).forEach(
						(k) => void delete demandCoverageState[k],
					);
		...
		dialog
			.querySelector<HTMLButtonElement>(".bpEdit")!
			.addEventListener("click", () => {
				openBiomeProductionPopup({ ...biomeOutputState }, (values) => {
					Object.keys(biomeOutputState).forEach(
						(k) => void delete biomeOutputState[+k],
					);
		...
		iconSelect.onchange = () =>
			ensureEl("newGoodIconPreview").setAttribute(
				"href",
				`#${iconSelect.value}`,
			);
...
function getMultiplierEntityName(dim: MultiplierDimKey, id: string): string {
	...
	if (dim === "zone")
		return pack.zones.find((z) => z.i === +id)?.name ?? `Zone ${id}`;
...
function uploadImage(
	...
	const input = ensureEl<HTMLInputElement>(
		type === "image" ? "imageToLoad" : "svgToLoad",
	);
		...
		buttons: {
			...
			Apply: function () {
				const inputs = Array.from(
					popupEl.querySelectorAll<HTMLInputElement>(".mPopupInput"),
				);
	...
	$(popupEl).dialog({
		...
		buttons: {
			...
			Apply: function () {
				...
				popupEl
					.querySelectorAll<HTMLInputElement>(".dcPopupInput")
					.forEach((input) => {
	...
	$(popupEl).dialog({
		...
		buttons: {
			...
			Apply: function () {
				...
				popupEl
					.querySelectorAll<HTMLInputElement>(".bpPopupInput")
					.forEach((input) => {
