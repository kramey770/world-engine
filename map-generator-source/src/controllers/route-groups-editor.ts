...
function onBodyClick(ev: Event): void {
	...
	if (target.classList.contains("editStyle") && group)
		editStyle("routes", group);
	else if (target.classList.contains("removeGroup") && group)
		removeGroup(group);
}
...
function addGroup(): void {
	prompt("Type group name", { default: "route-group-new" }, (v) => {
		...
		if (Number.isFinite(+group.charAt(0)))
			return tip("Group name should start with a letter", false, "error");
		...
		ensureEl<HTMLSelectElement>("routeGroup").options.add(
			new Option(group, group),
		);
		addLines();

		ensureEl<HTMLSelectElement>("routeCreatorGroupSelect").options.add(
			new Option(group, group),
		);
	});
}
