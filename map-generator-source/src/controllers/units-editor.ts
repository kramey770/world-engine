...
function open(): void {
	...
	ensureEl("distanceScaleInput").addEventListener(
		"change",
		changeDistanceScale,
	);
	ensureEl("heightUnit").addEventListener("change", changeHeightUnit);
	ensureEl("heightExponentInput").addEventListener(
		"input",
		changeHeightExponent,
	);
	ensureEl("temperatureScale").addEventListener(
		"change",
		changeTemperatureScale,
	);

	ensureEl("populationRateInput").addEventListener(
		"change",
		changePopulationRate,
	);
	ensureEl("urbanizationInput").addEventListener(
		"change",
		changeUrbanizationRate,
	);
...
function changeDistanceUnit(this: HTMLSelectElement): void {
	if (this.value === "custom_name") {
		prompt(
			...
			(custom) => {
				this.options.add(
					new Option(String(custom), String(custom), false, true),
				);
				lock("distanceUnit");
...
function restoreDefaultUnits(): void {
	distanceScale = 3;
	ensureEl<HTMLInputElement>("distanceScaleInput").value =
	...
	// population
	populationRate = 1000;
	ensureEl<HTMLInputElement>("populationRateInput").value =
