...
function open(element: SVGElement): void {
	...
	selectedLake = select<SVGElement, unknown>(
		element,
	) as unknown as typeof selectedLake;
	updateLakeValues();
	selectLakeGroup();
	drawLakeVertices();
...
function renderDialog(): void {
	...
	ensureEl("lakeNameSpeak").addEventListener("click", () =>
		speak(ensureEl<HTMLInputElement>("lakeName").value),
	);
...
function updateLakeValues(): void {
	...
	ensureEl<HTMLInputElement>("lakeArea").value =
		`${si(getArea(l.area))} ${getAreaUnit()}`;

	const length = polygonLength(
		l.vertices.map((v) => vertices.p[v] as [number, number]),
	);
	ensureEl<HTMLInputElement>("lakeShoreLength").value =
		`${si(length * distanceScale)} ${distanceUnitInput.value}`;
	...
	ensureEl<HTMLInputElement>("lakeAverageDepth").value = getHeight(
		mean(heights) ?? 0,
		true,
	);
	ensureEl<HTMLInputElement>("lakeMaxDepth").value = getHeight(
		min(heights) ?? 0,
		true,
	);
	...
	const inlets = l.inlets?.map(
		(inlet) => rivers.find((river) => river.i === inlet)?.name,
	);
	const outlet = l.outlet
		? rivers.find((river) => river.i === l.outlet)?.name
		: "no";
...
function drawLakeVertices(): void {
	...
	const neibCells: number[] = unique(
		vertices.flatMap((v) => pack.vertices.c[v]),
	);
	select("#debug")
		.select("#vertices")
	...
	select<SVGGElement, unknown>("#debug")
		...
		.call(
			drag<SVGCircleElement, number>()
				.on("drag", handleVertexDrag)
				.on("end", handleVertexDragEnd),
		)
		.on("mousemove", () =>
			tip(
				"Drag to move the vertex. Please use for fine-tuning only! Edit heightmap to change actual cell heights",
			),
		);
}
...
function handleVertexDrag(
	...
	ensureEl<HTMLInputElement>("lakeArea").value =
		`${si(getArea(feature.area))} ${getAreaUnit()}`;
...
function generateNameCulture(): void {
	const lake = getLake();
	lake.name = ensureEl<HTMLInputElement>("lakeName").value =
		Lakes.getName(lake);
}

function generateNameRandom(): void {
	const lake = getLake();
	lake.name = ensureEl<HTMLInputElement>("lakeName").value = Names.getBase(
		rand(Names.nameBases.length - 1),
	);
}

const isLakeType = (group: string) =>
	Layers.get("lakes").children.some((child) => child.id === group);
...
function selectLakeGroup(): void {
	...
	select<SVGGElement, unknown>("#lakes")
		...
		.each(function () {
			groupSelect.options.add(
				new Option(this.id, this.id, false, this.id === currentGroup),
			);
		});
}
...
function createNewGroup(this: HTMLInputElement): void {
	...
	if (!basic && oldGroup.childElementCount === 1) {
		...
		ensureEl<HTMLSelectElement>("lakeGroup").options.add(
			new Option(group, group, false, true),
		);
		oldGroup.id = group;
	...
	// create a new group
	const newGroup = (selectedLake.node()!.parentNode as SVGGElement).cloneNode(
		false,
	) as SVGGElement;
...
function removeLakeGroup(): void {
	...
	if (isLakeType(group)) {
		tip(
			"This is one of the default groups, it cannot be removed",
			false,
			"error",
		);
		return;
	...
	const count = (selectedLake.node()!.parentNode as SVGGElement)
		.childElementCount;
...
function editLakeLegend(): void {
	...
	void Controllers.NotesEditor.open(
		id,
		`${getLake().name} ${ensureEl<HTMLSelectElement>("lakeGroup").value} lake`,
	);
}
