...
function open(initial: string, callback: (value: string) => void): void {
	...
	table.onmouseover = (event) => {
		...
		if (target.tagName === "TD")
			tip(`Click to select ${target.textContent} icon`);
	};
	...
	addImageButton.onclick = () => {
		...
		if (!url.match(/^((http|https):\/\/)|data:image\//))
			return tip("Enter valid URL", false, "error", 4000);
	...
	for (const image of Array.from(
		ensureEl("addedIcons").querySelectorAll<HTMLElement>("div"),
	)) {
...
function renderIcons(table: HTMLTableElement): void {
	...
	ICONS.forEach((icon, i) => {
		if (i % ICONS_PER_ROW === 0)
			row = table.insertRow(Math.floor(i / ICONS_PER_ROW));
		row
			?.insertCell(i % ICONS_PER_ROW)
			.appendChild(document.createTextNode(icon));
	});
}
...
function getUsedImages(): Set<string> {
	...
	for (const unit of options.military)
		if (isExternal(unit.icon)) images.add(unit.icon);
	for (const state of pack.states) {
		for (const regiment of state?.military || [])
			if (isExternal(regiment.icon)) images.add(regiment.icon);
	}
	for (const marker of pack.markers || [])
		if (isExternal(marker.icon)) images.add(marker.icon);
...
function uploadIcon(
	...
	reader.onload = () => {
		...
		if (!svg)
			return void tip(
				"The file is not a valid SVG image",
				false,
				"error",
				4000,
			);
