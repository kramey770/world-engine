...
function open(element: SVGElement): void {
	...
	selectedCoastline = select<SVGElement, unknown>(
		element,
	) as unknown as typeof selectedCoastline;
...
function renderDialog(): void {
	...
	ensureEl("coastlineGroupRemove").addEventListener(
		"click",
		removeCoastlineGroup,
	);
...
function drawCoastlineVertices(): void {
	...
	).filter((cellId) => cellId < cellsNumber);
	select("#debug")
		.select("#vertices")
...
function handleVertexDrag(
	...
	ensureEl("coastlineArea").innerHTML =
...
function showGroupSection(): void {
	document
		.querySelectorAll<HTMLElement>("#coastlineEditor > button")
		.forEach((el) => {
...
function createNewGroup(this: HTMLInputElement): void {
	...
	if (!basic && oldGroup.childElementCount === 1) {
		...
		ensureEl<HTMLSelectElement>("coastlineGroup").options.add(
	...
	// create a new group
	const newGroup = (
		selectedCoastline.node()!.parentNode as SVGGElement
	).cloneNode(false) as SVGGElement;
...
function removeCoastlineGroup(): void {
	...
	const count = (selectedCoastline.node()!.parentNode as SVGGElement)
		.childElementCount;
	...
	$("#alert").dialog({
		...
		buttons: {
			Remove: function (this: HTMLElement) {
				...
				groupEl.remove();
				ensureEl<HTMLSelectElement>(
					"coastlineGroup",
				).selectedOptions[0].remove();
