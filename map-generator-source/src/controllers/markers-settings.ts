...
function open(): void {
	...
	ensureEl("dialogs").insertAdjacentHTML(
		"beforeend",
		`<div id="${DIALOG_ID}" class="dialog"></div>`,
	);
	drawConfigTable();
...
function applyChanges(): void {
	const rows =
		ensureEl(DIALOG_ID).querySelectorAll<HTMLTableRowElement>("tbody > tr");
	const rowsData = Array.from(rows).map((row) => {
		...
		if (!typeInput || !image || !emoji || !multiplierInput)
			throw new Error("Invalid markers configuration row");
	...
	Markers.setConfig(
		Markers.getConfig().map((markerType, index) => ({
			...markerType,
			...rowsData[index],
		})),
	);
...
function drawConfigTable(): void {
	...
	dialog
		.querySelectorAll<HTMLButtonElement>("button.changeIcon")
		.forEach((button) => {
			button.addEventListener("click", (event) => {
				...
				Controllers.IconSelector.open(
					image.getAttribute("src") || emoji.textContent || "",
					(value) => {
						const isExternal =
