...
import {
	clearMarkerRadius,
	drawMarkerRadius,
} from "@/renderers/draw-marker-radius";
...
function markerName(marker: Marker): string {
	return (
		notes.find((note) => note.id === `marker${marker.i}`)?.name ||
		marker.type ||
		"Marker"
	);
}
...
function applyRadius(distance: number): void {
	...
	const inRange = pack.markers.filter(
		(marker) =>
			Math.hypot(marker.x - center!.x, marker.y - center!.y) <= radiusPx,
	);
...
function onMarkerListClick(event: MouseEvent): void {
	...
	if (target.classList.contains("icon-pin"))
		return void togglePin(marker, target);
	if (target.classList.contains("locks"))
		return void toggleLock(marker, target);
	if (target.classList.contains("icon-trash-empty"))
		return void confirmRemove(marker);
...
function togglePin(marker: Marker, el: HTMLElement): void {
	...
	if (marker.pinned) {
		...
		if (!pack.markers.some((m) => m.pinned))
			markerGroup.removeAttribute("pinned");
	} else {
		marker.pinned = true;
...
function exportInRange(): void {
	if (!inRangeMarkers.length)
		return void tip("No markers in range to export", false, "error");
