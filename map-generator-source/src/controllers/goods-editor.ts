...
const columns: EditorColumn<Good>[] = [
	...
	},
	{
		...
		permanent: true,
		sortBy: (good) =>
			[good.recipes && "MFG", good.distribution && "RAW"]
				.filter(Boolean)
				.join(","),
		sortType: "alpha",
	},
	...
	},
	{
		...
		width: "6em",
		sortBy: (good) =>
			rn((production[good.i]?.burg ?? 0) + (production[good.i]?.cell ?? 0)),
		defaultSort: "desc",
...
const goodsTable = initEditorTable<Good>({
	getData: getGoodsData,
	onUpdate: renderGoodsPage,
});
...
function renderDialog(): void {
	...
	ensureEl("goodsTagsFilter").classList.toggle(
		"active",
		filterState.visibleTags.length > 0,
	);
	...
	ensureEl("goodsTagsFilter").addEventListener(
		"click",
		openTagsVisibilityDialog,
	);
	ensureEl("goodsAssign").addEventListener("click", enterResourceAssignMode);
	ensureEl("goodsAdd").addEventListener("click", () =>
		Controllers.GoodEditor.open(undefined, refreshEditor),
	);
	...
	ensureEl("goodsChains").addEventListener("click", () =>
		Controllers.ProductionChains.open(),
	);
	ensureEl("goodsRegenerateGoods").addEventListener(
		"click",
		requestGoodsRegeneration,
	);
	ensureEl("goodsRegenerateProduction").addEventListener(
		"click",
	...
	ensureEl("goodsBody").addEventListener("click", (ev) => {
		...
		if (!good) return;
		if (cl.contains("goodEdit"))
			return Controllers.GoodEditor.open(good, refreshEditor);
		if (cl.contains("goodDisplayed"))
			return toggleDisplayedGood(good, el as HTMLInputElement);
...
function getGoodsData(): Good[] {
	...
	const goods = hasFilter
		? pack.goods.filter((good) =>
				good.tags?.some((tag) => filterState.visibleTags.includes(tag)),
			)
		: [...pack.goods];
...
function renderGoodsPage(view: TableView<Good>) {
	...
	const totalProduced = Object.values(production).reduce(
		(sum, value) => sum + value.burg + value.cell,
		0,
	);
	...
	const lines = view.rows
		.map((good) => {
			const types = [good.recipes && "MFG", good.distribution && "RAW"].filter(
				Boolean,
			) as string[];
	...
	body
		...
		.forEach(
			(el) => void el.addEventListener("click", selectResourceOnLineClick),
		);
	body.querySelectorAll<HTMLButtonElement>(".goodProduced").forEach((el) => {
		el.addEventListener("click", (ev) => {
			ev.stopPropagation();
			openProducersDialog(
				Number(el.closest<HTMLElement>(".states")?.dataset.id),
			);
		});
	});
	...
	body.querySelectorAll<HTMLElement>(".goodStock").forEach((el) => {
		el.addEventListener("click", (ev) => {
			ev.stopPropagation();
			const goodId = Number(
				(el.closest<HTMLElement>(".states") as HTMLElement).dataset.id,
			);
...
function openProducersDialog(goodId: number) {
	...
	const producers = pack.burgs
		...
		.map((b) => ({
			burg: b,
			units: Production.getBurgProduction(b)[goodId] ?? 0,
		}))
...
function getAllStockData(): Record<
	number,
	{ total: number; sources: StockSource[] }
> {
	...
	for (const burg of pack.burgs) {
		...
		for (const record of burg.production) {
			if (isMfgRecord(record)) {
				netInventory[record.goodId] =
					(netInventory[record.goodId] || 0) + record.units;
				for (const item of record.recipe) {
					netInventory[item.goodId] =
						(netInventory[item.goodId] || 0) - item.units;
				}
...
function enterResourceAssignMode(this: HTMLElement) {
	...
	tip(
		"Select good line in editor, click on cells to remove or add a bonus resource",
		true,
	);
	select<SVGElement, unknown>("#viewbox").on(
		"click",
		changeResourceOnCellClick,
	);
}
...
function downloadGoodsData() {
	...
	let data =
		"Id,Good,Color,Type,Tags,Value,Unit,Demand Coverage,Chance,Model,Cells,Produced,Stock\n";

	for (const good of pack.goods) {
		const types = [good.recipes && "MFG", good.distribution && "RAW"]
			.filter(Boolean)
			.join(";");
