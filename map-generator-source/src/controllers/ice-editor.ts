...
function open(element: SVGElement): void {
	...
	if (document.getElementById("iceEditor") && element === selectedIce.node())
		return;
	...
	clearDragBehavior();
	selectedIce = select<SVGElement, unknown>(
		element,
	) as unknown as typeof selectedIce;
	...
	if (!isGlacier)
		sizeInput.value = String(
			iceElement && "size" in iceElement ? iceElement.size : "",
		);

	selectedIce
		.raise()
		.classed("draggable", true)
		.call(drag<SVGElement, unknown>().on("start", dragElement));
...
function addIcebergOnClick(event: PointerEvent): void {
	const [x, y] = getPointer(
		event,
		select<SVGElement, unknown>("#viewbox").node(),
	);
