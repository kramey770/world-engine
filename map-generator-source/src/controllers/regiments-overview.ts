...
const regimentsTable = initEditorTable<RegimentRow>({
	getData: getRegimentsData,
	onUpdate: renderRegimentsPage,
});
...
function renderDialog(): void {
	...
	ensureEl("regimentsOverviewRefresh").addEventListener(
		"click",
		refreshRegimentsOverview,
	);
	ensureEl("regimentsPercentage").addEventListener(
		"click",
...
function getRegimentColumns(): EditorColumn<RegimentRow>[] {
	const unitColumns: EditorColumn<RegimentRow>[] = options.military.map(
		(unit) => ({
...
function renderRegimentsPage(view: TableView<RegimentRow>): void {
	...
	const unitTotals = Object.fromEntries(
		options.military.map((unit) => [
			unit.name,
			sum(view.all.map((row) => row.regiment.u[unit.name] || 0)),
		]),
	);
	const total = sum(view.all.map((row) => row.regiment.a));
	const percent = (value: number, all: number) =>
		`${Math.round(all ? (value / all) * 100 : 0)}%`;
