...
const { getBurgLabelCandidates, isDrawnOn, optimizeLabelPlacements } =
	labelSpreadInternals;
type LabelPlacementCandidate = Parameters<
	typeof optimizeLabelPlacements
>[0][number]["candidates"][number];

function current(
	bounds = { x1: 40, y1: 40, x2: 60, y2: 44 },
...
describe("label spread Burg candidates", () => {
	test("creates six placements around the icon without covering it", () => {
		...
		for (const { bounds } of candidates) {
			const overlapsIcon =
				Math.min(bounds.x2, iconBounds.x2) >
					Math.max(bounds.x1, iconBounds.x1) &&
	...
	test("offers a centered slot above and below plus the four diagonals", () => {
		...
		const slots = candidates
			.slice(1)
			.map((candidate) => [candidate.bounds.x1, candidate.bounds.y1]);
	...
	test("keeps wide diagonal labels close to the icon center", () => {
		...
		expect(new Set(snapped.map((candidate) => candidate.bounds.x1))).toEqual(
			new Set([20, -17, 57]),
		);
	});
	...
	test("keeps a collision-free current placement", () => {
		...
		const solution = optimizeLabelPlacements(
			[
				...
				{
					id: "other",
					candidates: [current({ x1: 70, y1: 70, x2: 80, y2: 75 })],
				},
			],
			{ x1: 0, y1: 0, x2: 100, y2: 100 },
			"seed",
	...
	test("keeps the current placement first so the optimizer can judge the collision", () => {
		...
		expect(
			candidates.slice(1).every((candidate) => candidate.preference >= 120),
		).toBe(true);
	});
	...
	test("ranks Burg slots by screen-space distance", () => {
		...
		const bottomRight = candidates.find(
			(candidate) =>
				candidate.placement.dx === 17 && candidate.placement.dy === 10,
		);
