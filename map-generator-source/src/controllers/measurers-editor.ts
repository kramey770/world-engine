...
import {
	...
	Measurers,
	type MeasurerType,
} from "@/generators/measurers-generator";
...
type MeasurerDragEvent<E extends Element = SVGGElement> = D3DragEvent<
	E,
	unknown,
	unknown
>;
...
const rulerLayer = () =>
	document.getElementById("ruler") as unknown as SVGGElement;
...
function renderDialog(): void {
	...
	ensureEl("addRouteOpisometer").addEventListener(
		"click",
		toggleRouteOpisometerMode,
	);
...
function addRuler(): void {
	...
	const p = pt.matrixTransform(
		(select("#viewbox").node() as SVGGraphicsElement).getScreenCTM()!.inverse(),
	);
		...
		(event: any) => {
			...
			event.on("drag", (dragEvent: any) =>
				addPoint(
					...
					dragEvent.sourceEvent.shiftKey,
				),
			);
...
function toggleRouteOpisometerMode(this: HTMLElement): void {
	...
	startDrawingMode(this, tipText, (event: any) => {
		...
		const routeOpisometer = Measurers.create("RouteOpisometer", [
			getCellCoord(cell),
		]);
		redraw();
		event.on("drag", (dragEvent: any) => {
			const c = Pack.findCell(dragEvent.x, dragEvent.y)!;
			if (Routes.isConnected(c) || dragEvent.sourceEvent.shiftKey)
				trackCell(routeOpisometer, c, true);
		});
...
function trackCell(measurer: Measurer, cell: number, right: boolean): void {
	...
	if (right) {
		...
		if (cellStops.length > 1 && foundIndex !== -1)
			measurer.points.splice(foundIndex + 1);
		else measurer.points.push(getCellCoord(cell));
	} else {
		if (cellStops[0] === cell) return;
		if (cellStops.length > 1 && foundIndex !== -1)
			measurer.points.splice(0, foundIndex);
...
const WIRERS: Record<
	MeasurerType,
	(measurer: Measurer, el: MeasurerEl) => void
> = {
	Ruler: wireRuler,
...
function wireRuler(measurer: Measurer, el: MeasurerEl): void {
	...
	el.select<SVGPolylineElement>("polyline.white").call(
		measurerDrag<SVGPolylineElement>().on(
			"start",
			(event: MeasurerDragEvent<SVGPolylineElement>) => {
				const pointId = getSegmentId(measurer.points, [
					rn(event.x, 1),
					rn(event.y, 1),
				]);
			...
			select(this)
				...
				.call(
					measurerDrag<SVGCircleElement>()
						.clickDistance(3)
						.on("start", (event: MeasurerDragEvent<SVGCircleElement>) =>
							dragRulerPoint(measurer, event, pointId),
						),
				);
		});
...
type EndpointDragHandler = (
	measurer: Measurer,
	event: MeasurerDragEvent<SVGCircleElement>,
	right: boolean,
) => void;
...
function dragRulerPoint<E extends Element>(
	measurer: Measurer,
	event: MeasurerDragEvent<E>,
	pointId: number,
): void {
	...
	event.on("drag", (dragEvent: MeasurerDragEvent<E>) => {
		...
		if (shiftPressed && !axis)
			axis = Math.abs(dragEvent.dx) > Math.abs(dragEvent.dy) ? "x" : "y";
