...
const quantizationMap: Record<string, Metric> = {
	total_population: {
		...
		quantize: (cellId) =>
			getUrbanPopulation(cellId) + getRuralPopulation(cellId),
	...
	production_value: {
		...
		getContributions: (cellId, { biomeProduction }) => {
			...
			for (const [goodId, units] of Object.entries(produced)) {
				...
				if (good)
					contributions.push({ good: +goodId, value: units * good.value });
			}
	...
	production_units: {
		...
		getContributions: (cellId, { biomeProduction }) => {
			...
			const contributions: Contribution[] = [];
			for (const [goodId, units] of Object.entries(produced))
				contributions.push({ good: +goodId, value: units });
			return contributions;
		},
		aggregate: (values) => rn(sum(values)),
		formatTicks: (value) => si(value),
...
function renderDialog() {
	...
	const entities = Object.entries(entitiesMap).map(
		([entity, { label }]): [string, string] => [entity, label],
	);
	const plotBy = Object.entries(quantizationMap).map(
		([plotBy, { label }]): [string, string] => [plotBy, label],
	);

	const createOption = ([value, label]: [string, string]) =>
		`<option value="${value}">${label}</option>`;
	const createOptions = (values: [string, string][]) =>
		values.map(createOption).join("");
	...
	// set defaults
	ensureEl<HTMLSelectElement>("chartsOverview__entitiesSelect").value =
		"states";
	ensureEl<HTMLSelectElement>("chartsOverview__plotBySelect").value =
		"total_population";
	ensureEl<HTMLSelectElement>("chartsOverview__groupBySelect").value =
		"cultures";

	ensureEl("chartsOverview__form").addEventListener(
		"submit",
		addChart as EventListener,
	);
	ensureEl("chartsOverview__viewColumns").addEventListener(
		"change",
		changeViewColumns,
	);
	ensureEl("chartsOverview__plotBySelect").addEventListener(
		"change",
		updateMetricInfo,
	);
...
function updateMetricInfo() {
	const plotBy = ensureEl<HTMLSelectElement>(
...
function addChart(event?: Event) {
	...
	const entity = ensureEl<HTMLSelectElement>(
		"chartsOverview__entitiesSelect",
	).value;
	const plotBy = ensureEl<HTMLSelectElement>(
		"chartsOverview__plotBySelect",
	).value;
	let groupBy = ensureEl<HTMLSelectElement>(
		"chartsOverview__groupBySelect",
	).value;
	const sorting = ensureEl<HTMLSelectElement>(
		"chartsOverview__sortingSelect",
	).value;
	const type = ensureEl<HTMLSelectElement>("chartsOverview__chartType").value;
	const excludeNeutral = ensureEl<HTMLInputElement>(
		"chartsOverview__excludeNeutral",
	).checked;
...
function renderChart({
	...
	for (const cellId of pack.cells.i) {
		...
		for (const contribution of contributionsOf(cellId)) {
			...
			if (!dataCollection[entityId])
				dataCollection[entityId] = { [groupId]: [value] };
			else if (!dataCollection[entityId][groupId])
				dataCollection[entityId][groupId] = [value];
	...
	const chartData: ChartDatum[] = Object.entries(dataCollection).flatMap(
		([entityId, groupData]) => {
...
function createStackedBarChart(
	...
	const I = range(X.length).filter(
		(i) => yDomain.has(Y[i]) && zDomain.has(Z[i]),
	);
	...
	const series: StackSeries[] = stack<RolledEntry, string>()
		...
		.map((s) => {
			...
			const data: BarPoint[] = defined.map((d) =>
				Object.assign(d, { i: new Map(d.data[1]).get(s.key)! }),
			);
	...
	svg
		...
		.call((g) =>
			g.selectAll<SVGTextElement, number>("text").text((d) => formatX(d)),
		)
		.call((g) =>
			g
				.selectAll(".tick line")
	...
	const getTooltip = ({ i }: { i: number }) =>
		tooltip(Y[i], Z[i], X[i], X[i] / totalZ[Y[i]]);
	...
	const getLegendLabelX = (d: string, i: number) =>
		getLegendX(d, i) + LABEL_GAP;
	const getLegendY = (_d: string, i: number) =>
		Math.floor(i / rowElements) * ROW_HEIGHT;
  ...
  `;
	...
	const downloadChartData = () => {
		...
		const values = sortedData
			.map(({ name, group, value }) => `${name},${group},${value}`)
			.join("\n");
		...
		image.onload = () => {
			...
			if (context) {
				...
				canvas.toBlob(
					(blob) =>
						blob &&
						downloadFile(blob, `${getFileName(title)}.png`, "image/png"),
				);
			}
	...
	$figure
		.querySelector("button.icon-download")
		?.addEventListener("click", downloadChartData);
	$figure
		.querySelector("button.icon-export")
		?.addEventListener("click", downloadChartPng);
	$figure
		.querySelector("button.icon-chart-bar")
		?.addEventListener("click", downloadChartSvg);
	$figure
		.querySelector("button.icon-trash")
		?.addEventListener("click", removeChart);
}

function changeViewColumns() {
	const columns = ensureEl<HTMLSelectElement>(
...
function nameGetter(entity: CollectionKey) {
	return (i: string | number): string =>
		(pack[entity] as NamedColored[])[+i]?.name || EMPTY_NAME;
}

function colorsGetter(entity: CollectionKey) {
	return (): Record<string, string> =>
		Object.fromEntries(
			(pack[entity] as NamedColored[]).map((e) => [
				e.name || EMPTY_NAME,
				e.color || NEUTRAL_COLOR,
			]),
		);
}
...
function biomeColorsGetter(): Record<string, string> {
	return Object.fromEntries(
		pack.biomes.map(({ name, color }) => [name, color]),
	);
}
...
function marketNameGetter(i: string | number): string {
	...
	return (
		market.name || pack.burgs[market.centerBurgId]?.name || `Market ${market.i}`
	);
}
...
function goodColorsGetter(): Record<string, string> {
	return Object.fromEntries(
		(pack.goods || []).map((g) => [
			g.name || EMPTY_NAME,
			g.color || NEUTRAL_COLOR,
		]),
	);
}

function getCellProductionByGood(
	cellId: number,
	biomeProduction: BiomeProduction,
	...
	if (burgId) {
		...
		for (const [goodId, units] of Object.entries(urban))
