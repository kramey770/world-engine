...
import {
	getReliefIconId,
	type ReliefIcon,
} from "@/generators/relief-generator";
import {
	getSceneReliefIcon,
	redrawRelief,
} from "@/renderers/draw-relief-icons";
...
const iconsHtml = (): string =>
	Object.keys(RELIEF_SETS)
		.map(
			(set) =>
				`<div data-type="${set}" style="display: none">${setIconsHtml(set as ReliefSet)}</div>`,
		)
		.join("");
...
${iconsHtml()}
	...
	ensureEl("reliefEditStyle").addEventListener("click", () =>
		editStyle("terrain"),
	);
	ensureEl("reliefCopy").addEventListener("click", copyIcon);
	ensureEl("reliefMoveFront").addEventListener("click", () =>
		moveIcon("front"),
	);
...
function restoreEditMode(): void {
	if (!ensureEl("reliefTools").querySelector("button.pressed"))
		enterIndividualMode();
	else if (ensureEl("reliefBulkAdd").classList.contains("pressed"))
		enterBulkAddMode();
	else if (ensureEl("reliefBulkRemove").classList.contains("pressed"))
		enterBulkRemoveMode();
}
...
function updateReliefIconSelected(): void {
	...
	const button = reliefIconsDiv.querySelector(
		`svg[data-type='${selectedIcon.icon}']`,
	);
	if (!button) return;
...
function updateReliefSizeInput(): void {
	if (!selectedIcon) return;
	ensureEl<HTMLInputElement>("reliefSize").value = ensureEl<HTMLInputElement>(
		"reliefSizeNumber",
	).value = String(rn(selectedIcon.s));
}
...
function dragToAdd(this: SVGElement, event: any): void {
	const pressed =
		ensureEl("reliefIconsDiv").querySelector<SVGElement>("svg.pressed");
	if (!pressed) {
	...
	const tree = quadtree(
		pack.relief.map(
			({ x, y, s }) => [x + s / 2, y + s / 2] as [number, number],
		),
	);
...
function dragToRemove(this: SVGElement, event: any): void {
	...
	for (const reliefIcon of pack.relief) {
		...
		tree.add([
			reliefIcon.x + reliefIcon.s / 2,
			reliefIcon.y + reliefIcon.s / 2,
			reliefIcon,
		]);
	}
...
function changeIconSize(): void {
	if (
		!selectedIcon ||
		!ensureEl("reliefIndividual").classList.contains("pressed")
	)
		return;
...
function changeIconsSet(): void {
	...
	reliefIconsDiv.querySelector<HTMLElement>(
		`div[data-type='${set}']`,
	)!.style.display = "block";
}
...
function removeIcon(): void {
	const isIndividual =
		ensureEl("reliefTools").querySelector("button.pressed")?.id ===
		"reliefIndividual";
	const icon =
		ensureEl("reliefIconsDiv").querySelector<SVGElement>("svg.pressed")?.dataset
			.type;

	const doomed = isIndividual
		? new Set(selectedIcon ? [selectedIcon] : [])
		: new Set(
				pack.relief.filter((reliefIcon) => !icon || reliefIcon.icon === icon),
			);
	...
	$("#alert").dialog({
		...
		buttons: {
			Remove: function (this: HTMLElement) {
				pack.relief = pack.relief.filter(
					(reliefIcon) => !doomed.has(reliefIcon),
				);
				selectedIcon = null;
				redrawRelief();
				$(this).dialog("close");
