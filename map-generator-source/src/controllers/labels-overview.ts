...
import {
	calculateLabelSpread,
	type LabelSpreadPatch,
} from "@/controllers/label-spread";
import {
	LABEL_TYPES,
	type Label,
	type LabelType,
...
const spreadPreview: SpreadPreviewState = {
	phase: "idle",
	run: 0,
	snapshot: null,
};
...
function renderDialog(): void {
	...
	ensureEl("labelsGroupsConfig").addEventListener(
		"click",
...
function getFilteredLabels(): LabelData[] {
	...
	if (filterState.group !== ALL)
		labels = labels.filter(({ group }) => group === filterState.group);
	if (filterState.type !== ALL)
		labels = labels.filter(({ type }) => type === filterState.type);
	const search = filterState.search.trim().toLowerCase();
	if (search)
		labels = labels.filter(({ text }) =>
			text.replaceAll("|", "").toLowerCase().includes(search),
		);
...
function onBodyClick(event: Event): void {
	...
	if (element.classList.contains("labelsVisibility"))
		toggleLabelVisibility(element);
	else if (element.classList.contains("labelsReset")) resetLabel(element);
	else if (element.classList.contains("icon-target"))
		highlightLabel(element, id);
}

function onBodyChange(event: Event): void {
	const element = event.target as HTMLElement;
	if (element.classList.contains("labelsSelect"))
		return void updateSelectedCount();
	if (
		!(element instanceof HTMLSelectElement) ||
		!element.classList.contains("labelsGroup")
	)
		return;
...
function assignGroup(labels: LabelData[], groupName: string): void {
	...
	const apply = () => {
		for (const { type, entityId } of labels)
			Labels.setGroup({ type, entityId, group: groupName });
		Layers.draw("labels");
		refresh();
		tip(
			`${labels.length} label(s) assigned to the "${groupName}" group`,
			false,
			"success",
			4000,
		);
...
function getCheckboxes(): HTMLInputElement[] {
	return Array.from(
		ensureEl("labelsBody").querySelectorAll<HTMLInputElement>(
			"input.labelsSelect",
		),
	);
}
...
function applyBulkAssignment(): void {
	...
	if (!labels.length)
		return void tip("Select at least one label", false, "error");

	const group = ensureEl<HTMLSelectElement>("labelsBulkGroup").value;
	if (group === ALL)
		return void tip(
			"Define a label group to assign the labels to",
			false,
			"error",
		);
...
function takeLabelSnapshot(): LabelSnapshot[] {
	return getLabelsData().map(({ type, entityId }) => {
		...
		return {
			type,
			entityId,
			label: label === undefined ? undefined : structuredClone(label),
		};
	});
}
...
function syncSpreadControls(): void {
	...
	ensureEl<HTMLButtonElement>("labelsSpread").style.display = isReview
		? "none"
		: "";
	...
	>("button, input, select")) {
		if (
			control.id === "labelsSpreadApply" ||
			control.id === "labelsSpreadCancel"
		)
