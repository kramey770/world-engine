		...
		if (values === undefined) revertedCells.push(cell);
			...
			updates.push({
				...
				values: values.map((id) => ({
					id,
					color: itemsById.get(id)?.color ?? "#ffffff",
				})),
			});
	}
...
function addListeners(): void {
	ensureEl<HTMLSelectElement>("paintEditorSelect").addEventListener(
		"change",
		handleItemChange,
	);
...
function handleDragStart(
	this: SVGElement,
	event: D3DragEvent<SVGElement, unknown, unknown>,
): void {
...
function paintCells(
	...
	nextValue: number,
	historyEntry: PaintHistoryEntry,
): boolean {
	const { options, changes } = getState();
	const isErase = options.mode === "multiple" && nextValue === eraseAllValue;
	const landOnly =
		options.landOnlyControl &&
		ensureEl<HTMLInputElement>("paintEditorLandOnly").checked;
	const dontOverride =
		options.dontOverrideControl &&
		ensureEl<HTMLInputElement>("paintEditorDontOverride").checked &&
		!isErase;
	let changed = false;
...
function getNextValues(
	...
	if (options.mode === "multiple") {
		if (
			options.filterCell &&
			!options.filterCell(cell, currentValues, nextValue)
		)
			return null;
	...
	if (
		options.filterCell &&
		!options.filterCell(cell, currentValues[0], nextValue)
	)
		return null;
	return [nextValue];
}
...
function undo(): void {
	...
	ensureEl<HTMLButtonElement>("paintEditorUndo").disabled =
	...
	try {
		if (shouldApply) {
			...
			if (options.mode === "multiple") options.onApply(new Map(changes));
				...
				options.onApply(
					new Map([...changes].map(([cell, values]) => [cell, values[0]])),
				);
		}
	} finally {
