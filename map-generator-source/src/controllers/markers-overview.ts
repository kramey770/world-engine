...
function renderDialog(): void {
	...
	ensureEl("markersOverviewRefresh").addEventListener(
		"click",
		markersTable.refresh,
	);
	ensureEl("markersRegenerate").addEventListener("click", regenerateMarkers);
	ensureEl("markerTypeSelector").addEventListener(
		"click",
		toggleMarkerTypeMenu,
	);
	ensureEl("markersAddFromOverview").addEventListener(
		"click",
		() => void Controllers.MarkerCreator.toggle(),
	);
	ensureEl("markersGenerationConfig").addEventListener(
		"click",
...
function populateFilters(): void {
	const states = pack.states
		...
		.map((state) => ({
			...
			label: state.fullName || state.name,
		}));
...
function getFilteredMarkers(): Marker[] {
	...
	if (filterState.state !== "") {
		...
		markers = markers.filter(
			(marker) => pack.cells.state[marker.cell] === stateId,
		);
	}
	...
	if (filterState.culture !== "") {
		...
		markers = markers.filter(
			(marker) => pack.cells.culture[marker.cell] === cultureId,
		);
	}
...
function onFilterChange(): void {
	...
	filterState.culture = ensureEl<HTMLSelectElement>(
		"markersFilterCulture",
	).value;
...
function syncMapToFilter(
	...
	const ids = anyFilterActive
		? filteredMarkers.map((marker) => marker.i)
		: null;
...
function invertLock(): void {
	pack.markers = pack.markers.map((marker) => ({
		...marker,
		lock: !marker.lock,
	}));
	markersTable.refresh();
...
function changeMarkerType(): void {
	if (!ensureEl("markersAddFromOverview").classList.contains("pressed"))
		toggleAddMarker();
}
...
function triggerRemoveAll(): void {
	confirmationDialog({
		...
		message:
			"Are you sure you want to remove all non-locked markers? The action cannot be reverted",
		confirm: "Remove all",
...
function exportMarkers(): void {
	const headers =
		"Id,Type,Icon,Name,Note,State,Culture,X,Y,Latitude,Longitude\n";
