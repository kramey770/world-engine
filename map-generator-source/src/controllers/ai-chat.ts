...
import {
	...
	listModels,
	mergeModels,
} from "@/services/agent/providers-models";
...
function renderDialog(): void {
	...
	ensureEl("aiChatKeyHelp").addEventListener("click", () =>
		openURL(
			providerOf(ensureEl<HTMLSelectElement>("aiChatModel").value).keyLink,
		),
	);
...
function buildModelSelect(): void {
	...
	PROVIDERS.forEach((provider) => {
		...
		mergeModels(provider.models, cachedModels(provider.id)).forEach((model) => {
			group.append(
				new Option(model === LOCAL_MODEL ? "custom model…" : model, model),
			);
		});
		select.append(group);
	});
...
function loadKeyForModel(): void {
	...
	ensureEl("aiChatLocal").style.display =
		model === LOCAL_MODEL ? "flex" : "none";
	if (model === LOCAL_MODEL) {
		ensureEl<HTMLInputElement>("aiChatLocalUrl").value =
			localStorage.getItem(LOCAL_URL_STORAGE) ?? "";
		ensureEl<HTMLInputElement>("aiChatLocalModel").value =
...
function updateSendButton(): void {
	...
	button.disabled =
		!busy && !ensureEl<HTMLTextAreaElement>("aiChatInput").value.trim();
}
...
async function send(text?: string): Promise<void> {
	...
	if (model === LOCAL_MODEL) {
		const localModel =
			ensureEl<HTMLInputElement>("aiChatLocalModel").value.trim();
		if (!localModel) {
		...
		localStorage.setItem(
			LOCAL_URL_STORAGE,
			ensureEl<HTMLInputElement>("aiChatLocalUrl").value.trim(),
		);
	...
	try {
		await session.ask(conversation, question, {
			onText: (answer) =>
				addEntry({ kind: "message", role: "assistant", text: answer }),
...
function renderConversations(): void {
	...
	list().forEach((item) => {
		const label =
			item.mapId === mapId ? item.title : `${item.title} (other map)`;
...
function renderUsage(): void {
	...
	line.dataset.tip =
		"Tokens spent in this conversation. Cached tokens cost a tenth of the rest";
}
...
function setStepSummary(details: HTMLDetailsElement, result?: RunResult): void {
	...
	if (!result) summary.textContent = "Running a script…";
		...
		summary.textContent = result.ok
			? `Ran a script · ${result.ms} ms`
			: `Script failed · ${result.ms} ms`;
}
...
function emptyState(): HTMLElement {
	...
	hint.textContent =
		"I can read the current map and answer questions about it. I cannot change it yet.";
...
function showThinking(status: string): void {
	...
	if (!thinking) {
		...
		thinking.append(
			...
			...[0, 1, 2].map(() => document.createElement("i")),
		);
		log.append(thinking);
	}
