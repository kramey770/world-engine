	...
	$("#worldConfigurator").dialog({
        ...
        <label for="wcAutoChange" class="checkbox-label"><i>auto-apply changes</i></label>
			...
			button?.addEventListener("mousemove", () =>
				tip("Apply current settings to the map"),
			);
		},
...
function addListeners(): void {
	...
	ensureEl("temperatureEquatorInput").addEventListener(
		"input",
		changeTemperatureEquator,
	);
	ensureEl("temperatureEquatorOutput").addEventListener(
		"input",
		changeTemperatureEquator,
	);
	ensureEl("temperatureNorthPoleInput").addEventListener(
		"input",
		changeTemperatureNorthPole,
	);
	ensureEl("temperatureNorthPoleOutput").addEventListener(
		"input",
		changeTemperatureNorthPole,
	);
	ensureEl("temperatureSouthPoleInput").addEventListener(
		"input",
		changeTemperatureSouthPole,
	);
	ensureEl("temperatureSouthPoleOutput").addEventListener(
		"input",
		changeTemperatureSouthPole,
	);
	...
	ensureEl("wcWholeWorld").addEventListener("click", () =>
		applyWorldPreset(100, 50),
	);
	ensureEl("wcNorthern").addEventListener("click", () =>
		applyWorldPreset(33, 25),
	);
	ensureEl("wcTropical").addEventListener("click", () =>
		applyWorldPreset(33, 50),
	);
	ensureEl("wcSouthern").addEventListener("click", () =>
		applyWorldPreset(33, 75),
	);
...
function changeTemperatureEquator(this: HTMLInputElement): void {
	...
	ensureEl("temperatureEquatorConverted").innerText = convertedTemperature(
...
function changeTemperatureNorthPole(this: HTMLInputElement): void {
	...
	ensureEl("temperatureNorthPoleConverted").innerText = convertedTemperature(
...
function changeTemperatureSouthPole(this: HTMLInputElement): void {
	...
	ensureEl("temperatureSouthPoleConverted").innerText = convertedTemperature(
...
function updateInputValues(): void {
	ensureEl<HTMLInputElement>("temperatureEquatorInput").value = String(
		options.temperatureEquator,
	);
	ensureEl<HTMLInputElement>("temperatureEquatorOutput").value = String(
		options.temperatureEquator,
	);
	ensureEl<HTMLInputElement>("temperatureNorthPoleInput").value = String(
		options.temperatureNorthPole,
	);
	ensureEl<HTMLInputElement>("temperatureNorthPoleOutput").value = String(
		options.temperatureNorthPole,
	);
	ensureEl<HTMLInputElement>("temperatureSouthPoleInput").value = String(
		options.temperatureSouthPole,
	);
	ensureEl<HTMLInputElement>("temperatureSouthPoleOutput").value = String(
		options.temperatureSouthPole,
	);
	...
	ensureEl<HTMLInputElement>("longitudeInput").value = String(
		options.longitude,
	);
	ensureEl<HTMLInputElement>("longitudeOutput").value = String(
		options.longitude,
	);
...
function updateWorld(): void {
	...
	if (findEl("canvas3d"))
		setTimeout(() => window.Controllers.View3d.update(), 500);
}
...
function updateGlobePosition(): void {
	...
	ensureEl("meridianLengthFriendly").innerHTML =
		`${rn(eqD * 2 * distanceScale)} ${unit}`;
	ensureEl("meridianLengthEarth").innerHTML = meridian
		? ` = ${rn(meridian / 200)}%🌏`
		: "";
	...
	function lat(latitude: number): string {
		return latitude > 0
			? `${Math.abs(rn(latitude))}°N`
			: `${Math.abs(rn(latitude))}°S`;
	}
...
function updateWindDirections(): void {
	select("#globe")
		...
		.each(function (_d, i) {
			...
			this.setAttribute(
				"transform",
				`rotate(${options.winds[i]} ${tr[1]} ${tr[2]})`,
			);
		});
}
...
function handleWindChange(event: Event): void {
	...
	const arrow = (
		target.tagName === "path" ? target : target.nextElementSibling
	...
	arrow.setAttribute(
		"transform",
		`rotate(${options.winds[tier]} ${tr[1]} ${tr[2]})`,
	);
	localStorage.setItem("winds", String(options.winds));

	const mapTiers = range(
		mapCoordinates.latN ?? 0,
		mapCoordinates.latS ?? 0,
		-30,
	).map((c) => ((90 - c) / 30) | 0);
	if (
		ensureEl<HTMLInputElement>("wcAutoChange").checked &&
		mapTiers.includes(tier)
	)
		updateWorld();
}
