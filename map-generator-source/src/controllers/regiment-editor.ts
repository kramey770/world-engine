...
function editRegiment(selector: string): void {
	...
	armies
		...
		.call(drag<SVGGElement, unknown>().on("drag", dragRegiment));
...
function renderDialog(): void {
	...
	ensureEl("regimentRegenerateLegend").addEventListener(
		"click",
		regenerateLegend,
	);
...
function getRegiment(): Regiment | undefined {
	...
	return pack.states[+selectedRegiment.dataset.state!]?.military?.find(
		(r) => r.i === +selectedRegiment!.dataset.id!,
	);
}

function updateRegimentData(regiment: Regiment): void {
	ensureEl("regimentType").className = regiment.n
		? "icon-anchor"
		: "icon-users";
...
function rotateRegiment(
	...
	event.on(
		...
		function (
			this: SVGCircleElement,
			dragEvent: D3DragEvent<SVGCircleElement, unknown, unknown>,
		) {
			const { x, y } = dragEvent;
...
function changeType(): void {
	...
	selectedRegiment.querySelector("text")!.innerHTML = String(
...
function restoreName(): void {
	...
	selectedRegiment.dataset.name =
		reg.name =
		ensureEl<HTMLInputElement>("regimentName").value =
			name;
}
...
function changeEmblem(): void {
	...
	Controllers.IconSelector.open(regiment.icon ?? "", (value) => {
		...
		ensureEl("regimentEmblem").innerHTML = isExternal
			? `<img src="${value}" style="width: 1em; height: 1em;">`
			: value;
		selectedRegiment!.querySelector(".regimentIcon")!.innerHTML = isExternal
			? ""
			: value;
		selectedRegiment!
			.querySelector(".regimentImage")!
			.setAttribute("href", isExternal ? value : "");
	});
}
...
function toggleAttack(): void {
	...
	if (button.classList.contains("pressed")) {
		select<SVGGElement, unknown>("#viewbox")
			.style("cursor", "crosshair")
			.on("click", attackRegimentOnClick);
...
async function attackRegimentOnClick(
	this: SVGGElement,
	event: MouseEvent,
): Promise<void> {
	if (!selectedRegiment) return;
	...
	// draw battle icon
	const attackTransition = transition()
		.delay(300)
		.duration(700)
		.ease(easeSinInOut);
...
function toggleAttach(): void {
	...
	if (button.classList.contains("pressed")) {
		select<SVGGElement, unknown>("#viewbox")
			.style("cursor", "crosshair")
			.on("click", attachRegimentOnClick);
		tip(
			"Click on another regiment to unite both regiments. The current regiment will be removed",
			true,
		);
		select<SVGGElement, unknown>("#armies")
			.selectAll(":scope > g")
...
function attachRegimentOnClick(this: SVGGElement, event: MouseEvent): void {
	...
	if ((regSelected as Node) === (selectedRegiment as Node)) {
		tip(
			"Cannot attach regiment to itself. Please click on another regiment",
			false,
			"error",
		);
		return;
	...
	if (!reg) return;
	const sel = pack.states[newState].military!.find(
		(r) => r.i === +regSelected.dataset.id!,
	);
	if (!sel) return;
...
function removeRegiment(): void {
	ensureEl("alertMessage").innerHTML =
		"Are you sure you want to remove the regiment?";
	$("#alert").dialog({
		resizable: false,
		...
		buttons: {
			Remove: function () {
				...
				if (!selectedRegiment) return;
				const military =
					pack.states[+selectedRegiment.dataset.state!].military!;
				const reg = getRegiment();
...
function dragRegiment(
	...
	const reg = pack.states[+this.dataset.state!].military!.find(
	...
	const baseLine = select<SVGGElement, unknown>("#viewbox").select(
		"g#regimentBase > line",
	);
	const rotationControl = select<SVGGElement, unknown>("#debug").select(
		"#rotationControl",
	);
	...
	event.on(
		...
		function (
			this: SVGGElement,
			dragEvent: D3DragEvent<SVGGElement, unknown, unknown>,
		) {
			const { x, y } = dragEvent;
			reg.x = x;
			reg.y = y;
...
function closeEditor(): void {
	...
	armiesSel
		...
		.call(drag<SVGGElement, unknown>().on("drag", null));
