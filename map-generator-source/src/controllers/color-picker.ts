...
function renderPicker(): SVGSVGElement {
	...
	const colors = Array.from({ length: number }, (_, i) =>
		hsl((i / number) * 360, 0.7, 0.7).formatHex(),
	);
...
function addListeners(
	...
	getSvgElement("pickerControls").addEventListener("mousemove", (event) => {
		const control = (event.target as Element).closest<SVGGElement>(
			"g[data-tip]",
		);
	...
	container
		.querySelectorAll<SVGLineElement>("#pickerControls line")
		.forEach((line) => {
			line.addEventListener("click", (event) =>
				onControlClicked(event, callback),
			);
		});

	container
		.querySelectorAll<HTMLInputElement>("#pickerSpaces input")
		.forEach((input) => {
			input.addEventListener("change", (event) =>
				onSpaceChanged(event, callback),
			);
		});
...
const getControl = (id: (typeof CONTROLS)[number]) =>
	getSvgElement<SVGCircleElement>(id);

function setPickerControl(
	id: (typeof CONTROLS)[number],
	value: number,
	max: number,
): void {
...
const getHSL = () =>
	hsl(
		getPickerControl("pickerH"),
		getPickerControl("pickerS"),
		getPickerControl("pickerL"),
	);
...
function updatePickerColors(): void {
	const colors = Array.from(
		getSvgElement("pickerColors").querySelectorAll<SVGRectElement>("rect"),
	);
...
function onControlClicked(
	...
	(line.nextElementSibling as SVGCircleElement).setAttribute(
		"cx",
		String(event.x - min),
	);
	updateSpaces();
