...
function onBodyClick(ev: Event): void {
	...
	if (cl.contains("editFlux"))
		pack.cells.fl[cell] = +(el as HTMLInputElement).value;
...
function addRiver(): void {
	...
	const mouth =
		parent === riverId ? last(riverCells) : riverCells[riverCells.length - 2];
	...
	const length = Rivers.getApproximateLength(
		meanderedPoints as unknown as Point[],
	);
