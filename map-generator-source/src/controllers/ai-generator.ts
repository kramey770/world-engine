...
const PROVIDERS: Record<
	Provider,
	{ keyLink: string; generate: (options: GenerationOptions) => Promise<void> }
> = {
	openai: {
	...
	ollama: {
		keyLink:
			"https://github.com/Azgaar/Fantasy-Map-Generator/wiki/Ollama-text-generation",
...
async function handleStream(
	response: Response,
	getContent: (json: StreamChunk) => void,
): Promise<void> {
	if (!response.ok) {
		...
		try {
			...
			ERROR &&
				console.error("Failed to parse AI provider error response", error);
		}
	...
	while (true) {
		...
		for (let i = 0; i < lines.length - 1; i++) {
			...
			try {
				const parsed = line.startsWith("data: ")
...
function setInitialValues(defaultPrompt: string): void {
	...
	ensureEl<HTMLInputElement>("aiGeneratorTemperature").value =
	...
	ensureEl<HTMLInputElement>("aiGeneratorKey").value =
...
async function generate(button: HTMLButtonElement): Promise<void> {
	...
	const temperature = ensureEl<HTMLInputElement>(
		"aiGeneratorTemperature",
	).valueAsNumber;
	if (Number.isNaN(temperature))
		return tip("Temperature must be a number", true, "error", 4000);
