		...
		buttons: {
			...
			Add: () => {
				const maxOrder = Math.max(
					0,
					...options.burgs.groups.map(({ order }) => order),
				);
				const group: BurgGroup = {
					name: "",
					order: maxOrder + 1,
					active: true,
				};
...
function renderDialog(): void {
	...
	ensureEl("burgGroupsBody").addEventListener("click", (ev: Event) => {
		...
		if (el.getAttribute("name") === "states")
			return selectLimitation(el, pack.states);
		if (el.getAttribute("name") === "cultures")
			return selectLimitation(el, pack.cultures);
		if (el.getAttribute("name") === "religions")
			return selectLimitation(el, pack.religions);
		if (el.getAttribute("name") === "features")
			return selectFeaturesLimitation(el);
	...
	ensureEl("burgGroupsUnitsEditorLink").addEventListener("click", () =>
		Controllers.UnitsEditor.open(),
	);
	ensureEl("burgGroupsLabelGroupsLink").addEventListener("click", () =>
		Controllers.LabelGroupsConfigurator.open(),
	);
}
...
function createRow(group: BurgGroup): string {
	const count = pack.burgs.filter(
		(burg) => !burg.removed && burg.group === group.name,
	).length;
	// prettier-ignore
	...
	$("#alert").dialog({
		...
		buttons: {
			...
			Apply: function (this: HTMLElement) {
				...
				if (!selected.length)
					return tip("Select at least one element", false, "error");

				const allAreSelected = selected.length === inputs.length;
				(el.previousElementSibling as HTMLInputElement).value = allAreSelected
					? ""
					: selected.join(",");
	...
	$("#alert").dialog({
		...
		buttons: {
			Apply: function (this: HTMLElement) {
				...
				const values = features.reduce<Record<string, boolean>>(
					(acc, { name }) => {
						const featureValue = (form[name] as RadioNodeList).value;
						if (featureValue !== "undefined")
							acc[name] = featureValue === "true";
						return acc;
					},
					{},
				);
...
function validateForm(): boolean {
	...
	if (nameField.length) {
		const names = Array.from(nameField).map(
			(input) => (input as HTMLInputElement).value,
		);
		(nameField as unknown as NodeListOf<HTMLInputElement>).forEach(
			(nameInput) => {
	...
	const activeField = form.active as unknown as HTMLInputElement &
		RadioNodeList;
	if (activeField.length) {
		const active = Array.from(activeField).map(
		...
		activeField.setCustomValidity(
			activeField.checked ? "" : "At least one group should be active",
		);
	}

	const isDefaultField = form.isDefault as unknown as HTMLInputElement &
		RadioNodeList;
	if (isDefaultField.length) {
		const checked = Array.from(isDefaultField).map(
			(input) => (input as HTMLInputElement).checked,
		);
		...
		isDefaultField.setCustomValidity(
			isDefaultField.checked ? "" : "At least one group should be default",
		);
	}
...
function rowToGroup(row: Element): BurgGroup {
	...
	return {
		...
		preview:
			row.querySelector<HTMLSelectElement>('select[name="preview"]')!.value ||
			undefined,
...
function submitForm(event: Event): void {
	...
	validBurgs.forEach(
		(burg) => void Burgs.defineGroup(burg, popIndex, populations.length),
	);
