...
const columns: EditorColumn<River>[] = [
	...
	},
	{
		...
		width: "9em",
		sortBy: (river) =>
			pack.rivers.find(({ i }) => i === river.basin)?.name || "",
		sortType: "alpha",
	},
...
function getRiversById(): Map<number, River> {
	return new Map<number, River>(
		pack.rivers.map((river: River) => [river.i, river]),
	);
}
	...
	getData: () => {
		...
		const sortColumns = columns.map((column) =>
			column.key === "basin"
				? {
						...column,
						sortBy: (river: River) => riversById.get(river.basin)?.name || "",
					}
				: column,
		);
...
function renderDialog(): void {
	...
	applyLineHighlighting(dialogId, ({ target, cellId }) => {
		...
		return river && /^river\d+$/.test(river.id)
			? Number(river.id.slice(5))
			: undefined;
	});

	ensureEl("riversOverviewRefresh").addEventListener(
		"click",
		riversTable.refresh,
	);
	initColumnVisibility({
	...
	ensureEl("addNewRiver").addEventListener(
		"click",
		() => void Controllers.RiverAutoCreator.toggle(),
	);
	ensureEl("riverCreateNew").addEventListener("click", createNewRiver);
	ensureEl("riversBasinHighlight").addEventListener(
		"click",
		toggleBasinsHightlight,
	);
...
function renderRiversPage(view: TableView<River>): void {
	...
	ensureEl("riversFooterNumber").innerHTML =
		`${view.all.length} of ${pack.rivers.length}`;
	...
	ensureEl("riversFooterLength").innerHTML =
		`${averageLength * distanceScale} ${unit}`;
	const averageWidth = rn(mean(view.all.map((r) => r.width))!, 3) || 0;
	ensureEl("riversFooterWidth").innerHTML =
		`${rn(averageWidth * distanceScale, 3)} ${unit}`;
	...
	body
		...
		.forEach(
			(el) =>
				void el.addEventListener("mouseenter", (ev: Event) =>
					riverHighlightOn(ev),
				),
		);
	body
		...
		.forEach(
			(el) =>
				void el.addEventListener("mouseleave", (ev: Event) =>
					riverHighlightOff(ev),
				),
		);
	body
		.querySelectorAll("div > span.icon-target")
		.forEach((el) => void el.addEventListener("click", zoomToRiver));
	body
		.querySelectorAll("div > span.icon-pencil")
		.forEach((el) => void el.addEventListener("click", openRiverEditor));
	body
