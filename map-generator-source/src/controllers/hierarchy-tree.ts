...
// define svg elements
const svg = select<SVGSVGElement, unknown>("#hierarchyTree > svg").call(
	zoomBehavior,
);
...
function cleanupOrigins(elements: HierarchyElement[]): HierarchyElement[] {
	...
	return existingElements.map((d) => {
		...
		else if (!existingElements.find((el) => d.origins[0] === el.i))
			d.origins = [0];
		return d;
	});
}
...
function getRoot(): any {
	try {
		const root = stratify<HierarchyElement>()
			.id((d) => String(d.i))
			.parentId((d) => (d.origins[0] == null ? null : String(d.origins[0])))(
			validElements,
		);
...
const shapesMap: Record<string, string> = {
	...
	octagon:
		"M-4.97,-12.01 l9.95,0 l7.04,7.04 l0,9.95 l-7.04,7.04 l-9.95,0 l-7.04,-7.04 l0,-9.95Z",
	pentagon: "M0,-14l14,11l-6,14h-16l-6,-14Z",
};

const getSortIndex = (node: any): number => {
	const descendants = node.descendants();
	const secondaryOrigins = descendants.flatMap(({ data }: any) =>
		data.origins.slice(1),
	);
...
function mapCoords(newRoot: any, prevRoot: any): void {
	...
	for (const node of newRoot.descendants()) {
		const prevNode = prevRoot
			.descendants()
			.find((n: any) => n.data.i === node.data.i);
		if (prevNode) {
			node.x = prevNode.x;
			node.y = prevNode.y;
		}
...
function updateTree(): void {
	...
	const linkEnter = (enter: any) =>
		enter
			...
			.attr("opacity", 0)
			.call((enter: any) =>
				enter.transition().duration(linksUpdateDuration).attr("opacity", 1),
			);

	const linkUpdate = (update: any) =>
		update.call((update: any) =>
			update.transition().duration(linksUpdateDuration).attr("d", getLinkPath),
		);
...
function selectElement(d: any): void {
	...
	ensureEl<HTMLInputElement>("hierarchyTree_selectedCode").value =
		dataElement.code || "";

	ensureEl<HTMLInputElement>("hierarchyTree_selectedCode").onchange =
		function () {
			const input = this as HTMLInputElement;
			if (input.value.length > 3)
				return tip(
					"Abbreviation must be 3 characters or less",
					false,
					"error",
					3000,
				);
			if (!input.value.length)
				return tip("Abbreviation cannot be empty", false, "error", 3000);
	...
	const createOriginButtons = () => {
		ensureEl("hierarchyTree_selectedOrigins").innerHTML = dataElement.origins
			...
			.map((origin, index) => {
				const { name, code } =
					validElements.find((r) => r.i === origin) ||
					({} as Partial<HierarchyElement>);
		...
		ensureEl("hierarchyTree_selectedOrigins").onclick = (event: MouseEvent) => {
			...
			const filtered = dataElement.origins.filter(
				(elementOrigin) => elementOrigin !== origin,
			);
	...
	ensureEl("hierarchyTree_selectedSelectButton").onclick = () => {
		...
		const selectableElements = validElements.filter(
			({ i }) => !descendants.includes(i),
		);

		const selectableElementsHtml = selectableElements.map(
			({ i, name, code, color }) => {
		...
		$("#hierarchyTree_originSelector").dialog({
			...
			buttons: {
				Select: () => {
					...
					const selectedRadio = $selector.querySelector<HTMLInputElement>(
						"input[type='radio']:checked",
					);
					const selectedCheckboxes =
						$selector.querySelectorAll<HTMLInputElement>(
...
function dragToReorigin(
	...
	event.on("end", () => {
		...
		if (from.descendants().some((node: any) => node.data.i === newOrigin))
			return; // cannot be a child of its own child
