...
const columns: EditorColumn<Burg>[] = [
	...
	},
	{
		...
		sortBy: (b) =>
			rn(b.population! > 0 ? (b.product || 0) / b.population! : 0, 2),
	},
	{
		key: "treasury",
		label: "Treasury",
...
function open(filters: Filters = {}): void {
	...
	filterState = dialogState.get(dialogId, "filters", () => ({
		search: "",
		stateId: -1,
		cultureId: -1,
	}));
...
function renderDialog(): void {
	...
	applyLineHighlighting(dialogId, ({ target, cellId }) => {
		...
		const burg = target.closest<SVGElement>(
			"#labels [data-label-type='burg'][data-id], #burgIcons [data-id]",
		);
	...
	ensureEl("burgsOverviewRefresh").addEventListener(
		"click",
		refreshBurgsEditor,
	);
	ensureEl("burgsGroupsEditorButton").addEventListener("click", () =>
	...
	ensureEl("addNewBurg").addEventListener(
		"click",
		() => void Controllers.BurgCreator.toggle(),
	);
	ensureEl("addNewSkyBurg").addEventListener(
		"click",
		() => void Controllers.BurgCreator.toggleSky(),
	);
	ensureEl("burgsExport").addEventListener(
		"click",
		() => void downloadBurgsData(),
	);
	ensureEl("burgNamesImport").addEventListener("click", renameBurgsInBulk);
	ensureEl("burgsListToLoad").addEventListener(
		"change",
...
function closeBurgsOverview(): void {
	if (document.getElementById("addBurgTool")?.classList.contains("pressed"))
		void Controllers.BurgCreator.stop();
...
function updateFilter(): void {
	...
	const validStateIds = new Set(
		pack.states.filter((state) => !state.removed).map((state) => state.i),
	);
	if (!validStateIds.has(filterState.stateId)) filterState.stateId = -1;
	stateFilter.options.length = 0; // remove all options
	stateFilter.options.add(
		new Option("all", "-1", false, filterState.stateId === -1),
	);
	stateFilter.options.add(
		new Option(pack.states[0].name, "0", false, filterState.stateId === 0),
	);
	...
	statesSorted.forEach(
		(s) =>
			void stateFilter.options.add(
				new Option(s.name, String(s.i), false, s.i === filterState.stateId),
			),
	);

	const cultureFilter = ensureEl<HTMLSelectElement>("burgsFilterCulture");
	const validCultureIds = new Set(
		pack.cultures
	...
	cultureFilter.options.add(
		new Option(`all`, "-1", false, filterState.cultureId === -1),
	);
	cultureFilter.options.add(
		new Option(pack.cultures[0].name, "0", false, filterState.cultureId === 0),
	);
	const culturesSorted = pack.cultures
		.filter((c) => c.i && !c.removed)
		.sort((a, b) => (a.name > b.name ? 1 : -1));
	culturesSorted.forEach(
		(c) =>
			void cultureFilter.options.add(
				new Option(c.name, String(c.i), false, c.i === filterState.cultureId),
			),
	);
...
function onFilterChange(): void {
	...
	filterState.cultureId =
		+ensureEl<HTMLSelectElement>("burgsFilterCulture").value;
	dialogState.set(dialogId, "filters", filterState);
	burgsTable.reset();
}
...
function getFilteredBurgs(): Burg[] {
	...
	if (filterState.stateId !== -1)
		filtered = filtered.filter((b) => b.state === filterState.stateId); // filtered by state
	if (filterState.cultureId !== -1)
		filtered = filtered.filter((b) => b.culture === filterState.cultureId); // filtered by culture
	return filtered;
}
...
function renderBurgsPage(view: TableView<Burg>): void {
	...
	for (const b of view.all) {
		...
		const productPerCapita = rn(
			b.population! > 0 ? (b.product || 0) / b.population! : 0,
			2,
		);
	...
	ensureEl("burgsFooterBurgs").innerHTML =
		`${view.all.length} of ${validCount}`;
	ensureEl("burgsFooterPopulation").innerHTML = view.all.length
		? si(totalPopulation / view.all.length)
		: "0";
	ensureEl("burgsFooterGrossProduct").innerHTML = view.all.length
		? String(rn(totalProduct / view.all.length, 2))
		: "0";
	ensureEl("burgsFooterProductPerCapita").innerHTML = view.all.length
		? String(rn(totalProductPerCapita / view.all.length, 2))
		: "0";
	ensureEl("burgsFooterTreasury").innerHTML = view.all.length
		? String(rn(totalTreasury / view.all.length, 2))
		: "0";
	...
	body
		...
		.forEach(
			(el) =>
				void el.addEventListener("mouseenter", (ev) => burgHighlightOn(ev)),
		);
	body
		.querySelectorAll("div.states")
		.forEach(
			(el) => void el.addEventListener("mouseleave", () => burgHighlightOff()),
		);
	body
		.querySelectorAll("div > span.icon-dot-circled")
		.forEach((el) => void el.addEventListener("click", zoomIntoBurg));
	body
		.querySelectorAll("div > span.locks")
		.forEach((el) => void el.addEventListener("click", toggleBurgLockStatus));
	body
		.querySelectorAll("div > span.icon-pencil")
		.forEach((el) => void el.addEventListener("click", openBurgEditor));
	body
...
function burgHighlightOn(event: Event): void {
	...
	const label = select("#labels").select(
		`[data-label-type='burg'][data-id='${burg}']`,
	);
...
function burgHighlightOff(): void {
	select("#labels")
		.selectAll("text[data-label-type='burg'].drag")
		.classed("drag", false);
}
...
function triggerBurgRemove(this: HTMLElement): void {
	...
	if (pack.burgs[burgId].capital) {
		tip(
			"You cannot remove the capital. Please change the state capital first",
			false,
			"error",
		);
		return;
...
async function downloadBurgsData(): Promise<void> {
	...
	for (const b of valid) {
		...
		data += b.coa
			? `${JSON.stringify(b.coa).replace(/"/g, "").replace(/,/g, ";")},`
			: ",";
...
function importBurgNames(dataLoaded: string): void {
	...
	if (!change.length)
		message =
			"No changes found in the file. Please change some names to get a result";
...
function triggerAllBurgsRemove(): void {
	const number = pack.burgs.filter(
		(b) => b.i && !b.removed && !b.capital && !b.lock,
	).length;
	confirmationDialog({
...
function toggleLockAll(): void {
	...
	burgsTable.refresh();
	ensureEl("burgsLockAll").className = allLocked
		? "icon-lock"
		: "icon-lock-open";
}

function updateLockAllIcon(): void {
	const allLocked = pack.burgs.every(
		({ lock, i, removed }) => lock || !i || removed,
	);
	ensureEl("burgsLockAll").className = allLocked
		? "icon-lock-open"
		: "icon-lock";
