...
function getFilteredRoutes(): Route[] {
	...
	const routes = pack.routes.filter(
		(route: Route) => Boolean(route.points) && route.points.length >= 2,
	);
...
function renderDialog(): void {
	...
	ensureEl("routesOverviewRefresh").addEventListener(
		"click",
		routesTable.refresh,
	);
	initColumnVisibility({
...
function renderRoutesPage(view: TableView<Route>): void {
	...
	ensureEl("routesFooterNumber").innerHTML =
		`${view.all.length} of ${pack.routes.length}`;
	const averageLength = rn(mean(view.all.map((r) => r.length)) || 0) || 0;
	ensureEl("routesFooterLength").innerHTML =
		`${averageLength * distanceScale} ${distanceUnitInput.value}`;

	// add listeners
	body
		.querySelectorAll("div.states")
		.forEach((el) => void el.addEventListener("mouseenter", routeHighlightOn));
	body
		.querySelectorAll("div.states")
		.forEach((el) => void el.addEventListener("mouseleave", routeHighlightOff));
	body
		.querySelectorAll("div > span.icon-target")
		.forEach((el) => void el.addEventListener("click", zoomToRoute));
	body
		.querySelectorAll("div > span.icon-pencil")
		.forEach((el) => void el.addEventListener("click", openRouteEditor));
	body
		.querySelectorAll("div > span.locks")
		.forEach((el) => void el.addEventListener("click", toggleLockStatus));
	body
...
function toggleLockAll(): void {
	...
	routesTable.refresh();
	ensureEl("routesLockAll").className = allLocked
		? "icon-lock"
		: "icon-lock-open";
}
	...
	$("#alert").dialog({
		...
		buttons: {
			Remove: function (this: any) {
				const routesToRemove = pack.routes.filter(
					(route: Route) => !route.lock,
				);
				if (!routesToRemove.length) {
					if (!pack.routes.length) {
						...
						tip(
							"All routes are now locked; nothing was removed.",
							false,
							"error",
						);
					}
