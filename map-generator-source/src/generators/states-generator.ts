...
class StatesModule {
	...
	expandStates() {
		...
		while (queue.length) {
			...
			for (let ni = 0; ni < neighbors.length; ni++) {
				...
				const populationCost =
					hEn < 20 ? 0 : sEn ? (sEn < 20 ? 20 - sEn : 0) : 5000;

				// biomeCost
				let biomeCost: number;
				if (b === biomeEn) biomeCost = 10;
				else if (isHunting) biomeCost = biomeCostTable[biomeEn] * 2;
				else if (isNomadic && biomeEn > 4 && biomeEn < 10)
					biomeCost = biomeCostTable[biomeEn] * 3;
		...
		// per cluster.
		const skyState = states.find(
			(s) => s.i && !s.removed && burgs[s.capital]?.flying,
		);
