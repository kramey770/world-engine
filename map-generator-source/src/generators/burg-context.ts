...
export function scaledPopulation(
	population: number,
 ...
 */
export function effectiveWindowRadiusKm(
	radiusKm: number,
...
const SEA_TRADE_GROUPS = new Set<string>([
	"searoutes",
	"traderoutes",
...
export function readHydrology(input: {
	...
	const lakeside = win.cellIds.some(
		(id) => featureTypeById(Number(cellsF[id])) === "lake",
	);

	const haven = Number(cellsHaven[center] ?? 0);
	const havenPoint = haven ? cellsP[haven] : undefined;
	const [bx, by] = cellsP[center] ?? [0, 0];
	const oceanBearingDeg =
		coastal && havenPoint
			? compassBearing(havenPoint[0] - bx, havenPoint[1] - by)
			: undefined;

	const hasSeaTradeRoute = approaches.some((a) =>
		SEA_TRADE_GROUPS.has(a.group),
	);
	...
	for (const [riverId, cellId] of nearestRiverCell) {
		...
		const bearingDeg =
			cellId === center ? undefined : compassBearing(p[0] - bx, p[1] - by);
		rivers.push({
			id: riverId,
			name: river.name ?? "",
			type: river.type ?? "",
	...
	for (const id of win.cellIds) {
		...
		waterFeatures.push({
			featureId,
			type,
			name: featureNameById(featureId) ?? "",
		});
	}
...
export function readTerrain(input: {
	...
	const elevationM = elevationMetres(
		Number(cellsH[win.center] ?? 0),
		heightExponent,
	);
	const heights = win.cellIds.map((id) =>
		elevationMetres(Number(cellsH[id] ?? 0), heightExponent),
	);
	...
	const setting = ((): Terrain["setting"] => {
		...
		if (meanM - elevationM > RELIEF_TOLERANCE_M && reliefM >= HILLS_RELIEF_M)
			return "valley";
...
export function readEconomy(input: {
	...
	marketById: (
		id: number,
	) => { name?: string; centerBurgId?: number } | undefined;
	...
	const topGoods = [...unitsByGood.entries()]
		.map(([goodId, units]) => ({
			id: goodId,
			name: goodNameById(goodId),
			units,
		}))
		.filter((g): g is { id: number; name: string; units: number } =>
			Boolean(g.name),
		)
...
export function readCorridor(input: {
	...
	const heights = routeCells.map((id) =>
		elevationMetres(Number(cellsH[id] ?? 0), heightExponent),
	);
	...
	const relief = ((): Corridor["relief"] => {
		...
		if (
			first - midMin > RELIEF_TOLERANCE_M &&
			last - midMin > RELIEF_TOLERANCE_M
		)
			return "valley";
		if (
			midMax - first > RELIEF_TOLERANCE_M &&
			midMax - last > RELIEF_TOLERANCE_M
		)
			return "ridge";
		if (elevationDeltaM > RELIEF_TOLERANCE_M) return "ascent";
		if (elevationDeltaM < -RELIEF_TOLERANCE_M) return "descent";
 ...
 */
export function orderRouteCellsOutward(
	routeCells: number[],
	center: number,
): number[] {
...
const idIndexCache = new WeakMap<
	object,
	{ length: number; index: Map<number, unknown> }
>();
...
export function buildBurgContext(burg: Burg): BurgContext {
	...
	const population = scaledPopulation(
		burg.population ?? 0,
		populationRate,
		urbanization,
	);
	const radiusKm = effectiveWindowRadiusKm(
		DEFAULT_WINDOW_RADIUS_KM,
		distanceScale,
		Number(grid?.spacing ?? 0),
	);
	...
	const approaches = burg.flying
		? []
		: readApproaches(cell, cells.routes, cells.p, routeById);
	const windowCells = new Set(win.cellIds);
	const heightExponent = Number(heightExponentInput?.value ?? 1.8);
	for (const approach of approaches) {
		const outward = orderRouteCellsOutward(
			routeById.get(approach.routeId)?.cells ?? [],
			cell,
		);
	...
	const economy = readEconomy({
		...
		producedGoods: (burg.production ?? []).flatMap((record) =>
			"goodId" in record && "units" in record
	...
	return {
		burg: {
			...
			culture:
				burg.culture === undefined ? undefined : cultures[burg.culture]?.name,
		},
