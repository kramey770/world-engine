...
export class EmblemsGenerator {
	...
	generate(
		...
		if (ordinary) {
			emblem.ordinaries = [
				{ ordinary, t: this.getTincture("charge", usedTinctures, emblem.t1) },
			];
			if (linedOrdinary)
		...
		if (addCharge) {
			...
			if (ordinaryData?.positionsOn && P(0.8)) {
				...
				t =
					!usedPattern && P(0.3)
						? tOrdinary!
						: this.getTincture("charge", usedTinctures, emblem.t1);
			} else if (
			...
			if (p === "ABCDEFGHIJKL" && P(0.95)) {
				...
				} else if (allowCounter && P(0.4))
					emblem.charges[0].divided = "counter";
				// counterchanged, 40%
				else if (
					...
					emblem.charges[0].divided = "counter"; // counterchanged, 40%
			}
		...
		if (P(dominion as number) && parent?.charges) {
			...
			const t = invert
				? this.getTincture("division", usedTinctures, emblem.t1)
				: parent.t1;
	...
	private keepPlacement(
		previous: Emblem | undefined,
		emblem: HeraldicEmblem,
	): HeraldicEmblem {
	...
	regenerate(): void {
		pack.states.forEach((state) => {
			...
			state.coa = this.keepPlacement(
				state.coa,
				this.generate(null, null, null, cultureType),
			);
		...
		pack.burgs.forEach((burg) => {
			...
			const state =
				burg.state === undefined ? undefined : pack.states[burg.state];
			...
			burg.coa = this.keepPlacement(
				burg.coa,
				this.generate(state ? state.coa : null, kinship, null, burg.type),
			);
			burg.coa.shield = this.getShield(
				burg.culture ?? 0,
				state ? burg.state : 0,
			);
		});
		...
		pack.provinces.forEach((province) => {
			...
			const nameByBurg =
				province.burg &&
				province.name.slice(0, 3) === (parent.name ?? "").slice(0, 3);
			...
			province.coa = this.keepPlacement(
				province.coa,
				this.generate(parent.coa, kinship, Number(dominion), type),
			);
