...
function buildPack(
	routeCount: number,
	groupCycle: string[] = ["roads", "trails", "searoutes"],
) {
	const routes: any[] = [0];
...
describe("portImportance", () => {
	...
	it("scales with population within a role", () => {
		expect(portImportance(port(2, "marketTown"))).toBeGreaterThan(
			portImportance(port(1, "marketTown")),
		);
	});

	it("treats the capital flag as the capital role regardless of settlementType", () => {
		expect(portImportance(port(1, "hamlet", 1))).toBe(
			portImportance(port(1, "capital")),
		);
	});
...
describe("buildNavigableComponents", () => {
	...
	it("keeps each port-feature its own component on a non-360 map", () => {
		setup(180);
		const comp = (Routes as any).buildNavigableComponents() as Map<
			number,
			number
		>;
...
describe("generateSeaTradeNetwork hub dedup", () => {
	...
	const landmassOf: Record<number, number> = {
		[HUB]: 1,
		23: 2,
		25: 3,
		17: 4,
	...
	it("emits each undirected cell-edge at most once across all routes", () => {
		...
		const duplicated = [...edgeCounts.entries()].filter(
			([, count]) => count > 1,
		);
...
describe("generateSeaTradeNetwork feeder multi-target", () => {
	...
	it("lays feeder routes to land ports and draws the shared corridor only once", () => {
		const connections = new Set<number>();
		const burgIndex = {
			portsByFeature: { 1: [port(1, SOURCE), port(2, PL), port(3, PR)] },
		} as any;
		...
		for (const route of localRoutes) {
			...
			for (let k = 0; k < cs.length - 1; k++) {
				const key =
					Math.min(cs[k], cs[k + 1]) * N * N + Math.max(cs[k], cs[k + 1]);
...
describe("selectSeaTradeEdges", () => {
	...
	it("emits feeder+coastal tiers, links every port, dedupes pairs, and respects caps", () => {
		...
		expect(
			edges.every(
				(e) => e.tier !== "coastal" || (!isCapital(e.from) && !isCapital(e.to)),
			),
		).toBe(true);
		...
		edges.forEach((e) => {
			if (e.tier === "coastal")
				expect(km(ports[e.from], ports[e.to])).toBeLessThanOrEqual(120);
		});
	});
});
...
describe("generateTradeNetwork", () => {
	...
	it("produces trade routes connecting state hubs via waystations", () => {
		...
		const cells = new Set<number>(
			routes.flatMap((r: any) => r.points.map((pt: number[]) => pt[2])),
		);
...
describe("rebuildTradeRoutes", () => {
	...
	const setupPack = () => {
		...
		g.pack = {
			cells: buildGrid([1, 4, 7]),
			burgs: [{}, cap1, cap2, way],
			routes: [road, staleTrade],
		};
	...
	it("manual none on a hub excludes it and promotes the next-best port", () => {
		...
		const endpoints = trade.flatMap((r: any) => [
			r.points[0][2],
			r.points[r.points.length - 1][2],
		]);
	...
	it("is idempotent: consecutive rebuilds keep counts stable with no leftovers", () => {
		...
		(Routes as any).rebuildTradeRoutes();
		const pack = (globalThis as any).pack;
		const countAfterFirst = pack.routes.filter(
			(r: any) => r.group === "traderoutes",
		).length;
		const totalAfterFirst = pack.routes.length;

		(Routes as any).rebuildTradeRoutes();
		expect(
			pack.routes.filter((r: any) => r.group === "traderoutes").length,
