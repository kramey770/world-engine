...
export function cellSlotAfterRemoval(
	currentSlot: number,
	burg: Burg,
	burgs: Burg[],
): number {
	if (currentSlot !== burg.i) return currentSlot;
	const successor = burgs.find(
		(b) =>
			b.i && !b.removed && !b.flying && b.i !== burg.i && b.cell === burg.cell,
	);
...
export function groundSlotOnPlacement(
	currentSlot: number,
	burgId: number,
	flying: boolean,
): number {
...
export function transferTreasuryOnRemoval(
	burg: Burg,
	successorId: number,
	burgs: Burg[],
): void {
...
export function nearestBurgId(
	burgs: { x: number; y: number }[],
	ids: number[],
	ax: number,
	ay: number,
): number {
...
class BurgModule {
	...
	generate() {
		...
		const refillScore = (randomize: (s: number) => number) => {
			for (let i = 0; i < cells.s.length; i++)
				score[i] = cells.s[i] * randomize(1);
		};
		...
		const generateSkyBurgs = () => {
			...
			const anchorCell =
				coastalCells[Math.floor(Math.random() * coastalCells.length)];
			...
			const capacity = Math.floor(
				(Math.PI * maxRadius * maxRadius * 0.9) / (minSpacing * minSpacing),
			);
			...
			for (
				let attempts = 0;
				added < skyburgCount && attempts < maxAttempts;
				attempts++
			) {
			...
			const capitalId = nearestBurgId(
				burgs as unknown as { x: number; y: number }[],
				placedIds,
				ax,
				ay,
			);
	...
	private collectPortCandidates(burgs: Burg[]): Map<number, PortCandidate[]> {
		...
		const addCandidate = (candidate: PortCandidate) => {
			if (!byWater.has(candidate.portFeatureId))
				byWater.set(candidate.portFeatureId, []);
	...
	private selectPorts(candidates: PortCandidate[]): PortCandidate[] {
		...
		const rank = (candidate: PortCandidate) =>
			(candidate.burg.capital ? -1000 : 0) +
			(candidate.haven !== null ? cells.harbor[candidate.burg.cell] : 0);
		...
		for (const group of byLand.values()) {
			...
			promoted.add(
				group.reduce((best, c) => (rank(c) < rank(best) ? c : best)),
			);
		}

		if (promoted.size < 2) {
			const rest = candidates
				.filter((c) => !promoted.has(c))
	...
	private promoteToPort(
		candidate: PortCandidate,
		riversById: Map<number, River>,
	): void {
	...
	private getRiverTangent(
		...
		const from =
			prevCell !== undefined && prevCell >= 0
				? cells.p[prevCell]
				: cells.p[cellId];
		const to =
			nextCell !== undefined && nextCell >= 0
				? cells.p[nextCell]
				: cells.p[cellId];
	...
	private definePopulation(burg: Burg) {
		if (burg.flying) {
			...
			let population = burg.capital
				? gauss(3, 1.5, 2, 6)
				: gauss(0.6, 0.4, 0.2, 1.5);
	...
	defineGroup(burg: Burg, popIndex: Map<number, number>, popCount: number) {
		...
		if (burg.flying) {
			burg.group = burg.capital
				? "skyburg-capital"
				: skyburgGroupFromPopulation(burg.population as number);
			return;
		}
	...
	specify() {
		...
		pack.burgs.forEach((burg) => {
			...
			if (burg.flying)
				burg.altitude = skyburgAltitude(burg.population as number);
	...
	async getPreview(
		burg: Burg,
	): Promise<{ link: string | null; preview: string | null }> {
	...
	add(
		[x, y]: [number, number],
		options?: { flying?: boolean; altitude?: number },
	) {
		const { cells } = pack;
		...
		cells.burg[cellId as number] = groundSlotOnPlacement(
			cells.burg[cellId as number],
			burgId,
			flying,
		);
	...
	regenerate(): void {
		...
		const lockedBurgs = burgs.filter(
			(burg) => burg.i && !burg.removed && burg.lock,
		);
		for (const lockedBurg of lockedBurgs) {
			const newId = newBurgs.length;
			const noteIndex = notes.findIndex(
				(note) => note.id === `burg${lockedBurg.i}`,
			);
		...
		const marketCenterIds = new Set(
			pack.markets.map((market) => market.centerBurgId),
		);
		const unlockedMarketCenters = burgs.filter(
			(burg) =>
				burg.i && !burg.removed && !burg.lock && marketCenterIds.has(burg.i),
		);
		for (const centerBurg of unlockedMarketCenters) {
			...
			if (noteIndex !== -1) notes[noteIndex].id = `burg${newId}`;
			const market = pack.markets.find(
				(market) => market.centerBurgId === oldId,
			);
		...
		for (
			let index = 0;
			index < sorted.length && newBurgs.length < burgsCount;
			index++
		) {
	...
	remove(burgId: number) {
		...
		const newSlot = cellSlotAfterRemoval(
			pack.cells.burg[burg.cell],
			burg,
			pack.burgs,
		);
