...
class Resampler {
	...
	private restoreCellData(
		...
		for (const newPackCell of pack.cells.i) {
			...
			pack.cells.good[newPackCell] =
				parentMap.pack.cells.good?.[parentPackCell] || 0;
		}
	}
		...
		pack.states = pack.states.map((state) => {
			...
			state.center =
				!capital || capital.removed
					? Pack.findCell(poleX, poleY, Infinity)!
					: capital.cell;
				...
				(acc, regiment) => {
					...
					if (!this.isInMap(xPos, yPos)) {
						const noteIndex = notes.findIndex(
							(n) => n.id === `regiment${state.i}-${regiment.i}`,
						);
						if (noteIndex !== -1) notes.splice(noteIndex, 1);
						return acc;
					}

					const cellCoords = projection(
						...parentMap.pack.cells.p[regiment.cell],
					);
					const cell = this.isInMap(...cellCoords)
						? Pack.findCell(...cellCoords, Infinity)!
						: state.center;
		...
		pack.routes = parentMap.pack.routes
			.map((route) => {
				...
				const clipped = clippedSegments[0].map(
					([x, y]) =>
						[rn(x, 2), rn(y, 2), Pack.findCell(x, y, Infinity) as number] as [
							number,
							number,
							number,
						],
	...
	process(options: ResamplerProcessOptions): void {
		...
		pack.addedLabels = (parentMap.pack.addedLabels || []).map((addedLabel) => {
			...
			return {
				...
				label: {
					...addedLabel.label,
					pathPoints: addedLabel.label.pathPoints?.map(([x, y]) =>
						projection(x, y),
					),
				},
			};
