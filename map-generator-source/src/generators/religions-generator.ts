...
class ReligionsModule {
	...
	private generateOrganizedReligions(
		...
		const lockedReligionCount = lockedReligions.filter(
			({ type }) => type === "Organized" || type === "Cult",
		).length;
	...
	private combineReligions(
		...
		const maxIndex = Math.max(
			...
			namedReligions.length +
				lockedReligions.length +
				reservedOriginIds.size +
				1 -
				numberLockedFolk,
		);
		...
		function parseLockedReligions() {
			...
			const lockedReligionIds = new Set(
				lockedReligions.map((religion) => religion.i),
			);
			const reservedOriginIds = new Set(
				lockedReligions
					.flatMap((religion) => religion.origins ?? [])
					.filter(
						(originId) => originId > 0 && !lockedReligionIds.has(originId),
					),
			);
			const highestReservedIndex = Math.max(
				...lockedReligionIds,
				...reservedOriginIds,
				0,
			);
	...
	private defineOrigins(
		...
		for (const religion of indexedReligions) {
			...
			const folkReligion = religions.find(
				({ culture, type }) => type === "Folk" && culture === cultureId,
			);
			const isFolkBased =
				folkReligion && cultureId && expansion === "culture" && each(2)(center);
			if (isFolkBased) {
	...
	private generateHeresies(
		religions: Religion[],
		religionIds: Uint16Array,
	): Religion[] {
		const organizedReligions = religions.filter(
			(religion) =>
				religion.type === "Organized" &&
				!religion.lock &&
				!religion.removed &&
				(religion.expansionism ?? 0) >= 3,
		);
		if (!organizedReligions.length) return [];

		const organizedIds = new Set(
			organizedReligions.map((religion) => religion.i),
		);
		...
		for (const cellId of pack.cells.i) {
			...
			if (
				!pack.cells.c[cellId].some(
					(neighborId) => religionIds[neighborId] !== religionId,
				)
			)
				continue;
		...
		const occupiedCenters = new Set(
			religions
				.filter((religion) => !religion.removed)
				.map((religion) => religion.center),
		);
		const codes = religions.map((religion) => religion.code!);
		const heresies: Religion[] = [];

		for (const parent of organizedReligions) {
			const candidates = (boundaryCells.get(parent.i) ?? []).filter(
				(cellId) => !occupiedCenters.has(cellId),
			);
			...
			for (let index = 0; index < count && candidates.length; index++) {
				...
				const heresy = this.createHeresy(
					parent,
					center,
					religions.length + heresies.length,
					codes,
				);
	...
	private getReligionsInRadius(
		...
		return this.wouldCreateOriginCycle(religionId, fallbackOrigin, religions)
			? [0]
			: [fallbackOrigin];
	}

	private wouldCreateOriginCycle(
		religionId: number,
		originId: number,
		religions: Religion[],
	): boolean {
		...
		while (queue.length) {
			...
			queue.push(
				...(religions[currentId]?.origins ?? []).filter(
					(origin): origin is number => origin !== null,
				),
			);
		}
	...
	private expandHeresies(
		...
		const maxExpansionCost =
			(cells.i.length / 20) *
			(ensureEl("growthRate") as HTMLInputElement).valueAsNumber;
		const religionsMap = new Map(
			religions.map((religion) => [religion.i, religion]),
		);
		...
		for (const heresy of heresies) {
			...
			if (
				!baseReligionId ||
				religionsMap.get(baseReligionId)?.type !== "Organized"
			)
				continue;
		...
		while (queue.length) {
			...
			for (const nextCell of cells.c[cellId]) {
				...
				const religionCost =
					religionIds[nextCell] === baseReligionId ? 0 : 2000;
				const passageCost = this.getPassageCost(cellId, nextCell);
				const totalCost =
					p +
					10 +
					(religionCost + passageCost) / Math.max(religion.expansionism, 0.1);
				...
				if (!cost[nextCell] || totalCost < cost[nextCell]) {
					...
					cost[nextCell] = totalCost;
					queue.push(
						{ e: nextCell, p: totalCost, r, b: baseReligionId },
						totalCost,
					);
				}
			}
	...
	private normalizeHeresiesForExpansion(
		religions: Religion[],
		religionIds: Uint16Array,
	): Religion[] {
		const religionsMap = new Map(
		...
		const organizedReligions = religions.filter(
			(religion) =>
				religion.type === "Organized" && !religion.lock && !religion.removed,
		);

		return religions
			.filter(
				(religion) =>
					religion.type === "Heresy" && !religion.lock && !religion.removed,
			)
			.flatMap((heresy) => {
				const centerParent = religionsMap.get(religionIds[heresy.center]);
				const originParents = (heresy.origins ?? []).map((originId) =>
					religionsMap.get(originId),
				);
				const candidates = [
					...originParents,
					centerParent,
				...
				if (!parent) {
					...
					for (const religion of religions) {
						if (religion.lock || !religion.origins?.includes(heresy.i))
							continue;
						const origins = religion.origins.filter(
							(originId) => originId !== heresy.i,
						);
	...
	recalculate() {
		...
		const heresies = this.normalizeHeresiesForExpansion(
			pack.religions,
			newReligionIds,
		);
	...
	add(center: number) {
		...
		const color = missingFolk
			...
			: getMixedColor(parentReligion.color!, 0.3, 0);
		const form = rw(forms[type]);
		const deity: string | null =
			form === "Non-theism" || form === "Animism"
				? null
