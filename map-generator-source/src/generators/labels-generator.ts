...
export class LabelsModule {
	...
	getFallbackGroup(type: LabelType): LabelGroup {
		const fallbackGroup = this.getDefaultGroups().find(
			(group) => group.isDefault && group.type === type,
		);
		return (
			fallbackGroup ?? {
				name: type,
	...
	findGroup(groupName: string, type: LabelType): LabelGroup {
		const group = options.labels.groups.find(
			(group) => group.name === groupName && group.type === type,
		);
	...
	hasOverride(type: LabelType, id: number): boolean {
		...
		const { dx, dy, startOffset, fontSize, letterSpacing, pathPoints, hidden } =
			label;
		return [
			dx,
			dy,
