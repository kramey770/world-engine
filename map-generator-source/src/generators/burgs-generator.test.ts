...
describe("definePopulation for flying burgs", () => {
	...
	const makeFlying = (over: any = {}) =>
		({ i: 7, cell: 13, flying: 1, ...over }) as any;
...
describe("BurgsModule.assignPorts — open-lake port promotion", () => {
	...
	it.each([
		...
		"frozen",
		"lava",
	])("does not make ports on a %s lake (cannot be sailed)", (group) => {
		globalThis.pack = {
			burgs: makeBurgs(),
			cells: { ...BASE_CELLS },
			features: [
				null,
				{ i: 1, type: "lake", cells: 3, group },
				{ i: 2, type: "ocean" },
			],
			vertices: BASE_VERTICES,
			rivers: [],
	...
	it("promotes an exposed coastal burg so its island is not left portless", () => {
		globalThis.pack = {
			burgs: [
				0 as any,
				{ i: 1, cell: 1, x: 5, y: 5, capital: 0 },
				{ i: 2, cell: 2, x: 15, y: 5, capital: 0 },
			],
			cells: {
				haven: [0, 3, 3, 0],
...
describe("cellSlotAfterRemoval", () => {
	...
	it("promotes a co-located ground burg when the slot owner is removed", () => {
		...
		expect(
			cellSlotAfterRemoval(3, primary, [makeBurg(0, 0), primary, secondary]),
		).toBe(7);
	});
	...
	it("does NOT promote a flying co-resident", () => {
		...
		expect(
			cellSlotAfterRemoval(3, primary, [makeBurg(0, 0), primary, sky]),
		).toBe(0);
	});
	...
	it("does NOT promote a removed co-resident", () => {
		...
		expect(
			cellSlotAfterRemoval(3, primary, [makeBurg(0, 0), primary, dead]),
		).toBe(0);
	});
	...
	it("leaves the slot alone when removing a secondary (non-owner) burg", () => {
		const secondary = makeBurg(7, 10);
		expect(
			cellSlotAfterRemoval(3, secondary, [
				makeBurg(0, 0),
				makeBurg(3, 10),
				secondary,
			]),
	...
	it("leaves the slot alone when removing a flying burg above a ground burg (live wrong-clear bug)", () => {
		const sky = makeBurg(7, 10, { flying: 1 });
		expect(
			cellSlotAfterRemoval(3, sky, [makeBurg(0, 0), makeBurg(3, 10), sky]),
		).toBe(3);
	});
