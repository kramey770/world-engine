...
class ReliefModule {
	generate(): ReliefIcon[] {
		...
		for (const i of cells.i) {
			...
			const [minX, maxX] = extent(polygon, (p: number[]) => p[0]) as [
				number,
				number,
			];
			const [minY, maxY] = extent(polygon, (p: number[]) => p[1]) as [
				number,
				number,
			];
...
export const getReliefIconId = (
	type: string,
	variant: number,
	set: ReliefSet,
): string => `relief-${type}-${variant}${RELIEF_SETS[set].suffix}`;
...
function getTypeIcons(type: string, set: ReliefSet): ReliefTypeIcons | null {
	...
	for (
		let name: string | undefined = type;
		name;
		name = findType(name)?.fallback
	) {
		const icons = RELIEF_ICONS.find(
			(entry) => entry.set === base && entry.type === name,
		);
...
const findType = (type: string) =>
	RELIEF_ICONS.find((entry) => entry.type === type);
