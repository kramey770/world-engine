...
describe("effectiveWindowRadiusKm", () => {
	...
	it("keeps the window non-degenerate when spacing exceeds the metric radius", () => {
		...
		const floored = collectWindow(
			0,
			cellsC,
			cellsP,
			effectiveWindowRadiusKm(12, 3, 10),
			3,
		);
...
describe("readApproaches", () => {
	...
	const routeById = new Map<
		number,
		{ group: RouteGroup; type?: string; name?: string }
	>([
...
describe("readHydrology", () => {
	...
	const base = {
		...
		cellsP,
		cellsR: [0, 9, 0],
		featureTypeById: (id: number) =>
			({ 1: "island", 2: "ocean", 3: "lake" })[id],
		featureNameById: (id: number) =>
			({ 1: "Mainland", 2: "The Deep", 3: "Still Water" })[id],
		riverById: (id: number) =>
			id === 9
				? {
						name: "Greenwater",
	...
	it("calls a harbour large only with a sea/trade route AND enough population", () => {
		const seaRoute: Approach[] = [
			{ routeId: 1, group: "searoutes", bearingDeg: 90, through: false },
		];
		expect(
			readHydrology({
				...base,
				approaches: seaRoute,
				population: LARGE_HARBOUR_MIN_POPULATION,
			}).harbourSize,
		).toBe("large");
		// route but too small
		expect(
			readHydrology({ ...base, approaches: seaRoute, population: 100 })
				.harbourSize,
		).toBe("small");
		// big but no sea route
		expect(readHydrology({ ...base, population: 100000 }).harbourSize).toBe(
			"small",
		);
		// traderoutes with sufficient population also yields large
		const tradeRoute: Approach[] = [
			{ routeId: 2, group: "traderoutes", bearingDeg: 180, through: false },
		];
		expect(
			readHydrology({
				...base,
				approaches: tradeRoute,
				population: LARGE_HARBOUR_MIN_POPULATION,
			}).harbourSize,
		).toBe("large");
	});
...
describe("readTerrain statistics", () => {
	...
	it("calls a coastal burg coast regardless of relief", () => {
		expect(
			readTerrain({ ...base, cellsH: [20, 30, 40], coastal: true }).setting,
		).toBe("coast");
	});

	it("calls a very high burg mountain", () => {
		expect(readTerrain({ ...base, cellsH: [70, 75, 80] }).setting).toBe(
			"mountain",
		);
	});

	it("calls a low burg among high ground a valley", () => {
		expect(readTerrain({ ...base, cellsH: [60, 22, 60] }).setting).toBe(
			"valley",
		);
	});
	...
	it("calls a high but level inland burg a plateau", () => {
		...
		expect(readTerrain({ ...base, cellsH: [48, 50, 52] }).setting).toBe(
			"plateau",
		);
	});

	it("calls a flat low burg a plain", () => {
		expect(readTerrain({ ...base, cellsH: [21, 21, 21] }).setting).toBe(
			"plain",
		);
	});
...
describe("readClimate", () => {
	const base = {
		...
		biomeNameById: (id: number) =>
			({ 1: "Hot desert", 5: "Temperate deciduous forest" })[id],
	};
...
describe("readClimate biome mix", () => {
	...
	it("produces shares summing to 1", () => {
		const total = readClimate(base).biomeMix.reduce(
	...
	it("returns a single entry when the window is uniform", () => {
		expect(readClimate({ ...base, cellsBiome: [5, 5, 5, 5] }).biomeMix).toEqual(
			[{ name: "Grassland", share: 1 }],
		);
	});
...
describe("readCorridor", () => {
	...
	it("measures the sampled distance along the route", () => {
		expect(readCorridor({ ...base, cellsH: [20, 20, 20, 20] }).sampledKm).toBe(
			3,
		);
	});
	...
	it("recognises a valley: both ends above the middle", () => {
		expect(readCorridor({ ...base, cellsH: [45, 25, 25, 45] }).relief).toBe(
			"valley",
		);
	});

	it("recognises a ridge: both ends below the middle", () => {
		expect(readCorridor({ ...base, cellsH: [22, 48, 48, 22] }).relief).toBe(
			"ridge",
		);
	});
	...
	it("calls a level corridor flat", () => {
		expect(readCorridor({ ...base, cellsH: [30, 30, 30, 30] }).relief).toBe(
			"flat",
		);
	});

	it("flags a corridor following a river and names the river", () => {
		const c = readCorridor({
			...base,
			cellsH: [30, 30, 30, 30],
			cellsR: [0, 4, 4, 4],
		});
...
describe("readEconomy", () => {
	const base = {
		...
		treasury: 900,
		marketId: 2,
		marketById: (id: number) =>
			id === 2 ? { name: "Salt Exchange", centerBurgId: 7 } : undefined,
		producedGoods: [
			{ goodId: 1, units: 5 },
			{ goodId: 2, units: 30 },
			{ goodId: 3, units: 12 },
	...
	it("bands the treasury rather than exposing the raw figure", () => {
		...
		expect(readEconomy({ ...base, treasury: 900 }).treasuryBand).toBe(
			"prosperous",
		);
		expect(readEconomy({ ...base, treasury: 99999 }).treasuryBand).toBe("rich");
		expect(readEconomy({ ...base, treasury: undefined }).treasuryBand).toBe(
			"modest",
		);
	});
	...
	it("skips goods with no name", () => {
		const economy = readEconomy({
			...base,
			goodNameById: (id: number) => (id === 2 ? "Salt" : undefined),
		});
