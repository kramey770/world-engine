...
class GridModule {
	...
	shouldRegenerate(
		...
		return (
			graph.cellsX !== this.getCellsCount(spacing, width) ||
			graph.cellsY !== this.getCellsCount(spacing, height)
		);
	}
	...
	findCell(x: number, y: number, graph: GridGraph = grid): number {
		...
		return (
			Math.floor(Math.min(y / spacing, cellsY - 1)) * cellsX +
			Math.floor(Math.min(x / spacing, cellsX - 1))
		);
	}
	...
	private getJitteredPoints(
		...
		for (let y = radius; y < height; y += spacing) {
			for (let x = radius; x < width; x += spacing) {
				points.push([
					Math.min(rn(x + jitter(), 2), width),
					Math.min(rn(y + jitter(), 2), height),
				]);
			}
		}
		return points;
	...
	addDeepDepressionLakes(): void {
		const elevationLimit = +ensureEl<HTMLOutputElement>(
			"lakeElevationLimitOutput",
		).value;
		...
		const addLake = (lakeCells: number[]) => {
			...
			for (const i of lakeCells) {
				...
				for (const neibId of c[i]) {
					if (h[neibId] >= SEA_LEVEL && !lakeCells.includes(neibId))
						cells.t[neibId] = 1;
				}
			}
