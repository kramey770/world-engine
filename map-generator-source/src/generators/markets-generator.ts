...
export class MarketsModule {
	...
	private createMarkets(): Market[] {
		...
		let minSpacing =
			(((graphWidth + graphHeight) * 2) / pack.burgs.length ** 0.6) | 0;
	...
	private indexMarkets(markets: Market[] = pack.markets): void {
		...
		for (const market of markets)
			if (market) this.marketById[market.i] = market;
	}
	...
	private expandMarkets(markets: Market[]): Uint16Array {
		...
		while (queue.length) {
			...
			for (const neighborId of cells.c[cellId]) {
				...
				if (isWater) {
					cost += WATER_COST;
					if (burg.port !== cells.f[neighborId])
						cost += WATER_COST_FOR_NON_PORTS;
				} else {
					if (cells.f[burg.cell] !== cells.f[neighborId])
						cost += ISLAND_CHANGE_COST;
					if (cells.state[neighborId] && burg.state !== cells.state[neighborId])
		...
		const memberIds = groupedMemberIds(
			findMegalopolises(pack.burgs, pack.cells.burg),
		);
	...
	initializeMarketPrices(): void {
		...
		const avgIngredientsCostByGood = this.calculateAverageBaseCostByGood(
			pack.goods,
		);
		...
		for (const market of pack.markets) {
			...
			for (const good of pack.goods) {
				...
				const demandPrice =
					avgMarketCost + Math.max(0, good.value - avgBaseCost);
				marketGood.price = rn(
					minmax(
						demandPrice,
	...
	public getName(market: Market): string {
		return (
			market.name ||
			pack.burgs[market.centerBurgId]?.name ||
			`Market ${market.i}`
		);
	}
	...
	quoteMarket(
		market: Market,
		goodId: number,
	): { stock: number; buyPrice: number; sellPrice: number } {
		const good = Goods.get(goodId);
	...
	buy({
		...
		const actualUnits = rn(
			Math.min(units, marketGood.stock, budget / unitPrice),
			2,
		);
		...
		marketGood.price = rn(
			this.applyMarketPressure(good.value, marketGood.price, actualUnits),
			2,
		);
		return deal;
	}
	...
	sell({
		...
		marketGood.price = rn(
			this.applyMarketPressure(good.value, marketGood.price, -units),
			2,
		);
		return deal;
	}
	...
	runGlobalTrade(): void {
		...
		for (const m1 of pack.markets) {
			...
			for (const m2 of pack.markets) {
				...
				travelCost[m1.i][m2.i] =
					(distance / mapDiagonal) * DISTANCE_COST_FACTOR;
			}
		}
		...
		for (const good of pack.goods) {
			...
			for (const exporter of exporters) {
				...
				const exporterTaxPerUnit =
					States.getSalesTax(exporterCenter) * exporterGood.price;
				...
				for (const importer of importers) {
					...
					const unitProfit =
						importerGood.price -
						(exporterGood.price + transportCost + exporterTaxPerUnit);
			...
			opportunities.sort(
				(a, b) => b.totalProfit - a.totalProfit || b.units - a.units,
			);
			for (const opportunity of opportunities) {
				...
				const available = Math.max(
					0,
					exporterGood.stock - opportunity.reserveExporter,
				);
				const needed = Math.max(
					0,
					opportunity.reserveImporter - importerGood.stock,
				);
				const units = Math.min(available, needed);
				if (units < MIN_UNIT) continue;
				...
				exporterGood.price = rn(
					this.applyMarketPressure(good.value, exporterGood.price, units),
					2,
				);
				importerGood.price = rn(
					this.applyMarketPressure(good.value, importerGood.price, -units),
					2,
				);
	...
	private collectConsumerDemand(goods: Good[]): number[] {
		const totalCoverageByCategory = Object.fromEntries(
			DEMAND_PRIORITY.map((category) => [
				category,
				goods.reduce(
					(sum, g) => sum + (g?.demandCoverage?.[category] || 0),
					0,
				) || 1,
			]),
