...
class OceanModule {
	...
	generate(limits: number[]): OceanOutline[] {
		...
		const outlines = new Map<number, [number, number][][]>(
			limits.map((t) => [t, []]),
		);
		...
		for (const i of cells.i) {
			...
			const relaxed = chain.filter(
				(v, index) =>
					!(index % relax) || vertices.c[v].some((c: number) => c >= pointsN),
			);
		...
		function findStart(i: number, t: number): number | undefined {
			if (cells.b[i])
				return cells.v[i].find((v: number) =>
					vertices.c[v].some((c: number) => c >= pointsN),
				);
			return cells.v[i][
				cells.c[i].findIndex((c: number) => cells.t[c] < t || !cells.t[c])
			];
		}
		...
		function connectVertices(start: number, t: number): number[] {
			...
			for (
				...
				i === 0 || (current !== start && i < vertices.c.length);
				i++
			) {
