...
describe("settings", () => {
	...
	it("reuses the last values the user picked on the next map", () => {
		...
		expect(Coastline.settings).toEqual({
			...Coastline.getDefaultSettings(),
			baseAmplitude: 3,
			enabled: false,
		});
	});
...
describe("getFeaturePath", () => {
	...
	it("applies the settings of the loaded map", () => {
		...
		expect(Coastline.getFeaturePath(island)).toBe(
			"M10,50Q10,10 50,10Q90,10 90,50Q90,90 50,90Q10,90 10,50Z",
		);
	});
});
