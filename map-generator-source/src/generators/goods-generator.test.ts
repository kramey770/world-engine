...
describe("GoodsModule", () => {
	...
	it("restores the default catalogue when requested explicitly", () => {
		...
		expect(
			globalThis.pack.goods.some((good: any) => good.name === "Wood"),
		).toBe(true);
	...
	it("does not corrupt the default template when a restored good is edited", () => {
		goodsModule.restoreDefaults();
		const wood = globalThis.pack.goods.find(
			(good: any) => good.name === "Wood",
		)!;
		...
		expect(
			globalThis.pack.goods.find((good: any) => good.name === "Edited Wood"),
		).toBeUndefined();
		expect(
			globalThis.pack.goods.some((good: any) => good.name === "Wood"),
		).toBe(true);
