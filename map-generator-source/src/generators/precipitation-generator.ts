...
class PrecipitationModule {
	...
	generate(): void {
		...
		const passWind = (
			sources: (number | WindBand)[],
			initialMaxPrec: number,
			next: number,
			steps: number,
		) => {
		...
		if (northerly) {
			...
			const latModN =
				mapCoordinates.latT! > 60
					? (mean(LATITUDE_MODIFIER) as number)
	...
	getWinds(): Winds {
		...
		range(0, cells.i.length, cellsX).forEach((cellId, rowId) => {
			const lat =
				mapCoordinates.latN! - (rowId / cellsY) * mapCoordinates.latT!;
			...
			if (angle > 220 && angle < 320)
				easterly.push([cellId + cellsX - 1, latMod, tier]);
