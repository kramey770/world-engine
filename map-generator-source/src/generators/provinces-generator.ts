...
class ProvinceModule {
	...
	generate(regenerate = false, regenerateLockedStates = false) {
		...
		const baseMaxGrowth =
			provincesRatio === 100
				? 1000
				: gauss(20, 5, 5, 100) * provincesRatio ** 0.5;
					...
					cells.h[e] >= 70
						...
						: cells.h[e] >= 50
							...
							: cells.h[e] >= 20
								...
								: 100;
				const biomeCost =
					land && cells.biome[e] !== provinceBiome[province] ? 10 : 0;
		...
		states.forEach((s) => {
			...
			while (stateNoProvince.size) {
					...
					for (let n = 0; n < neighbors.length; n++) {
						...
						const ter = land
							...
							: cells.t[nextCellId]
								...
								: 30;
						const biomeCost =
							land && cells.biome[nextCellId] !== wildBiome ? 10 : 0;
				...
				const isleSubtype =
					!singleIsle &&
					!provCells.find((i) => pack.features[cells.f[i]].subtype !== "isle");
				const colony =
					!singleIsle &&
					!isleSubtype &&
					P(0.5) &&
