...
class MarkersModule {
	...
	add(marker: Marker) {
		...
		if (base) {
			...
			marker = this.addMarker(
				{ icon, type, dx, dy, px, size, pin, fill, stroke },
				marker,
			);
	...
	private addParty(id: string) {
		...
		notes.push({
			id,
			name: "The Party",
			legend: "Current location of the adventuring party.",
		});
	}
    ...
    */
	...
	private listRifts({ cells }: PackedGraph) {
		return cells.i.filter(
			(i) =>
				!this.occupied[i] &&
				cells.pop[i] <= 3 &&
				pack.biomes[cells.biome[i]].habitability,
		);
	}
	...
	private addEncounter(id: string, cell: number) {
		...
		const biomeName = (
			pack.biomes[cells.biome[cell]]?.name || "wilderness"
		).toLowerCase();
