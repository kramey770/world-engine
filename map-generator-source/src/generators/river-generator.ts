...
class RiverModule {
	...
	addDownhill(initialCell: number): { error?: string } {
		...
		while (cell) {
			...
			const nextCell: number = cells.c[cell].sort(
				(a, b) => heights[a] - heights[b],
			)[0];
			if (heights[cell] <= heights[nextCell]) {
				return {
					error: `Cell ${cell} is depressed, river cannot flow further`,
				};
			}
			...
			const oldRiverCells =
				oldRiver?.cells ||
				cells.i.filter((cellId) => cells.r[cellId] === oldRiverId);
			const oldRiverCellsUpper = oldRiverCells.filter(
				(cellId) => heights[cellId] > heights[nextCell],
			);
		...
		const widthFactor =
			...
			(!parent || parent === riverId
				? defaultWidthFactor * 1.2
				: defaultWidthFactor);
		const sourceWidth =
			river?.sourceWidth || this.getSourceWidth(cells.fl[source]);
