...
export class LakesModule {
	...
	defineClimateData(heights: number[] | Uint8Array) {
		...
		const getLowestShoreCell = (lake: Feature) => {
			return lake.shoreline.reduce((minCell, c) =>
				heights[c] < heights[minCell] ? c : minCell,
			);
		};
	...
	detectCloseLakes(h: number[] | Uint8Array) {
		...
		pack.features.forEach((feature) => {
			...
			let isDeep = true;
			const lowestShorelineCell = feature.shoreline.reduce((minCell, c) =>
				h[c] < h[minCell] ? c : minCell,
			);
	...
	defineShoreline(feature: Feature) {
		return unique(
			feature.vertices.flatMap((vertexIndex) =>
				pack.vertices.c[vertexIndex].filter((index) => isLand(index, pack)),
			),
		);
	}
}
