...
export function isWrapEnabled(): boolean {
	return (
		typeof mapCoordinates !== "undefined" &&
		!!mapCoordinates &&
		mapCoordinates.lonT === 360
	);
}
...
class RoutesModule {
	...
	private calculateUrquhartEdges(points: Point[], wrap = false, width = 0) {
		...
		if (wrap && width > 0)
			return this.calculateWrapUrquhartEdges(points, width);
	...
	private createCostEvaluator({
		...
		function getWaterPathCost(current: number, next: number) {
			...
			const distanceSq = wrapDistanceSquared(
				pack.cells.p[current],
				pack.cells.p[next],
				wrap,
				graphWidth,
			...
			if (tradeWater) {
				const depthModifier =
					TRADE_DEPTH_MODIFIERS[pack.cells.t[next]] ??
					TRADE_DEPTH_MODIFIERS.default;
				const reuse = connections.has(encodeConnection(current, next))
					? TRADE_LANE_REUSE
					: 1;
	...
	private buildNavigableComponents(): Map<number, number> {
		...
		const isRoutablePort = (burg: Burg) =>
			Boolean(burg.i) && !burg.removed && !burg.flying && Boolean(burg.port);
		...
		if (isWrapEnabled()) {
			for (const [w, e] of this.collectSeamLinks())
				union(cells.f[w], cells.f[e]);
		}
	...
	private findPathSegments({
		...
		const graph = wrap
			? { ...pack, cells: { ...pack.cells, c: seaAdjacency } }
			: pack;
	...
	private selectSeaTradeEdges(ports: Burg[]): SeaTradeEdge[] {
		...
		const gravity = (i: number, j: number) =>
			(imp[i] * imp[j]) / Math.max(d2(i, j), 1e-9);
		...
		const addEdge = (a: number, b: number, tier: SeaTradeTier) => {
			...
			if (current === undefined || tierRank[tier] > tierRank[current])
				edges.set(key, tier);
		};
	...
	private generateSeaTradeNetwork(
		...
		const crossesSeam = (cells: number[]) => {
			for (let i = 1; i < cells.length; i++) {
				if (
					Math.abs(pack.cells.p[cells[i]][0] - pack.cells.p[cells[i - 1]][0]) >
					seamThreshold
				)
					return true;
			}
		...
		for (const ports of Object.values(portsByComponent)) {
			...
			const mapScale = Math.sqrt((graphWidth * graphHeight) / 1_000_000);
			const feederMaxCost =
				SEA_FEEDER_CAP_KM * SEA_FEEDER_DETOUR_FACTOR * mapScale;
		...
		if (TIME && diagTrees.length) {
			...
			const pct = (n: number) =>
				sorted.slice(0, n).reduce((s, t) => s + t.ms, 0) /
				Math.max(totalMs, 1e-9);
			const unsettled = diagTrees.reduce(
				(s, t) => s + (t.targets - t.settled),
				0,
			);
			...
			console.log(
				...
				sorted
					...
					.map(
						(t) =>
							`${t.ms.toFixed(0)}ms/${t.expanded}exp/${t.settled}of${t.targets}`,
					)
					.join(" "),
			);
		}
	...
	private generateTradeNetwork(
		...
		for (const b of pack.burgs) {
			if (b.i && !b.removed && b.capital && b.state !== undefined)
				capitalByState.set(b.state, b);
		}
		...
		const megalopolisImportance = (b: Burg) => {
			...
			return pop === undefined
				? portImportance(b)
				: portImportance({ ...b, population: pop } as Burg);
		};
		...
		assignTradeRoles(
			pack.burgs.filter((b) => !b.i || !memberIds.has(b.i)),
			{
				...
				isLargePort: (b: Burg) =>
					Boolean(b.isLargePort) || b.settlementType === "largePort",
		...
		const adj = buildLegGraph(nodes, maxLegPx * maxLegPx, (a, b) =>
			dist2(a.x, a.y, b.x, b.y),
		);
		const { legs } = routeTradeNetwork(
			nodes.length,
			adj,
			hubIndices,
			TRADE_MAX_HOPS,
	...
	private mergeRoutes(routes: Route[]): Route[] {
		...
		for (let i = 0; i < routes.length; i++) {
			...
			while (true) {
				...
				for (let k = 1; k < nextCells.length; k++)
					route.cells.push(nextCells[k]);
	...
	private createRoutesData(routes: Route[], connections: Set<number>) {
		...
		const airUrquhart = this.calculateUrquhartEdges(
			airPoints,
			isWrapEnabled(),
			graphWidth,
		);
	...
	rebuildAirroutes(): void {
		...
		const urquhartEdges = this.calculateUrquhartEdges(
			points,
			isWrapEnabled(),
			graphWidth,
		);
	...
	private getRoutesIndex(): Map<number, Route> {
		...
		if (
			this.indexCache &&
			this.indexCache.ref === routes &&
			this.indexCache.len === routes.length
		) {
	...
	getConnectivityRate(cellId: number): number {
		...
		const connectivity = Object.values(connections).reduce((acc, routeId) => {
			...
			if (!route) return acc;
			const rate =
				CONNECTIVITY_RATE_BY_GROUP[route.group] ||
				CONNECTIVITY_RATE_BY_GROUP.default;
			return acc + rate;
		}, 0.8);
