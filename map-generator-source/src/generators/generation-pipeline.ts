...
const generationPipelineSteps = [
	{
		id: "grid",
		run: ({ seed: expectedSeed, graph }) => Grid.prepare(expectedSeed, graph),
	},
...
export const GenerationPipeline = new Pipeline<
	GenerationPipelineStepId,
	GenerationContext
>("Generation Pipeline", generationPipelineSteps);

const erasePipelineSteps = [
	{ id: "markupGrid", run: () => Features.markupGrid() },
	{
		id: "depressionLakes",
		run: ({ erosion }) => erosion && Grid.addDeepDepressionLakes(),
	},
	{
		id: "nearSeaLakes",
		run: ({ erosion }) => erosion && Grid.openNearSeaLakes(),
	},
	{ id: "temperatures", run: () => Temperature.generate() },
	{ id: "precipitation", run: () => Precipitation.generate() },
	{ id: "regraph", run: () => Pack.generate() },
...
export const ErasePipeline = new Pipeline<
	GenerationPipelineStepId,
	EraseContext
>("Erase Heightmap", erasePipelineSteps);
