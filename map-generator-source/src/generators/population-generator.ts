...
class PopulationModule {
	...
	rankCells(): void {
		...
		const getGoodValue = (i: number) =>
			cells.good?.[i] ? (Goods.get(cells.good[i])?.value ?? 0) : 0;
		...
		for (const i of cells.i) {
			...
			if (meanFlux)
				score +=
					normalize(cells.fl[i] + cells.conf[i], meanFlux, maxFlux) * 250; // big rivers and confluences are valued
			...
			if (
				cells.good &&
				(cells.good[i] || cells.c[i].some((c) => cells.good[c]))
			) {
			...
			cells.pop[i] =
				cells.s[i] > 0 ? (cells.s[i] * cells.area[i]) / meanArea : 0;
		}
	}
	...
	regenerate(): void {
		...
		pack.burgs.forEach((burg) => {
			...
			burg.population = rn(
				Math.max(
					pack.cells.s[cellId] / 8 + burg.i / 1000 + (cellId % 100) / 1000,
					0.1,
				),
				3,
			);
