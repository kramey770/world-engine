...
export class ProductionModule {
	...
	private fillBurgsDemand(sortedBurgs: Burg[], index: ProductionIndex): void {
		...
		for (const burg of sortedBurgs) {
			...
			this.fillDemandFromMarket({
				...
				demandTargets: getDemandTargets(
					pooled.get(burg.i) ?? burg.population ?? 0,
				),
	...
	private buildProductionIndex(goods: Good[]): ProductionIndex {
		...
		const demandGoodsByCategory = this.buildDemandGoodsByCategory(
			goods,
			demandCoverageByGood,
		);
		const recipes = this.buildRecipesArray(goods);
		const recipesByOutput = this.buildRecipesByOutput(recipes);
		const productiveGoods = goods.filter(
			(good) => recipesByOutput[good.i]?.length,
		);
	...
	private createBurgProductionState(
		...
		if (good) {
			...
			const bonus = minmax(
				population * BONUS_URBAN_PRODUCTION,
				MIN_BONUS_PRODUCTION,
				MAX_BONUS_PRODUCTION,
			);
	...
	private executeManufacture(
		...
		for (const ingredient of ingredients) {
			...
			const fromInventory = Math.min(
				state.inventory[ingredientId] || 0,
				amount,
			);
		...
		for (const { ingredientId, amount, fromInventory, deal } of plans) {
			...
			state.inventory[ingredientId] = Math.max(
				0,
				(state.inventory[ingredientId] || 0) - fromInventory,
			);
			this.addDemandCoverage(
				state.demandCoverage,
				ingredientId,
				-fromInventory,
				index.demandCoverageByGood,
	...
	private buildMinWorkersByGood(
		...
		for (const good of goods)
			minWorkersByGood[good.i] = recipesByOutput[good.i]?.length ? Infinity : 1;
	...
	private buildDemandCoverageByGood(goods: Good[]): number[][] {
		...
		for (const good of goods) {
			...
			for (let category = 0; category < DEMAND_PRIORITY.length; category++) {
				coverage[category] =
					good.demandCoverage?.[DEMAND_PRIORITY[category] as DemandCategory] ||
					0;
			}
	...
	private buildDemandGoodsByCategory(
		goods: Good[],
		demandCoverageByGood: number[][],
	): DemandGoodCandidate[][] {
		const demandGoodsByCategory: DemandGoodCandidate[][] = Array.from(
			{ length: DEMAND_PRIORITY.length },
			() => [],
		);
		...
		for (const good of goods) {
			...
			for (let category = 0; category < DEMAND_PRIORITY.length; category++) {
				...
				demandGoodsByCategory[category].push({
					good,
					goodId: good.i,
					coverageWeight,
				});
	...
	private fillDemandFromMarket({
		...
		for (
			...
			const sortedCandidates: {
				candidate: DemandGoodCandidate;
				costPerCoverage: number;
			}[] = [];
			...
			for (const { candidate } of sortedCandidates) {
				...
				const retainedCoverageByCategory =
					demandCoverageByGood[candidate.goodId];
				for (
					...
					const retainedCoverage =
						retainedCoverageByCategory[coverageCategoryIndex] || 0;
					if (!retainedCoverage) continue;
					demandCoverage[coverageCategoryIndex] +=
						deal.units * retainedCoverage;
				}
	...
	private getDemandEffect(
		...
		const coverageWeight =
			demandCoverageByGood[good.i]?.[demandFocus.categoryIndex] || 0;
	...
	private planGoodAction(
		...
		for (const recipe of recipeList) {
			...
			for (const ingredient of recipe.ingredients) {
				...
				const fromInventory = Math.min(
					remaining,
					state.inventory[ingredient.goodId] || 0,
				);
				...
				const lowerBoundWorkers =
					remaining * (index.minWorkersByGood[ingredient.goodId] ?? Infinity);
	...
	private makeProductionDecision(
		...
		for (const good of index.productiveGoods) {
			const demandEffect = this.getDemandEffect(
				good,
				demandFocus,
				index.demandCoverageByGood,
			);
		...
		if (activeGoalGoodId !== null && chosenGoal && !activeGoal) {
			...
			if (activeGood) {
				const activeDemand = this.getDemandEffect(
					activeGood,
					demandFocus,
					index.demandCoverageByGood,
				);
		...
		if (
			activeGoal &&
			chosenGoal &&
			activeGoal.normalizedGain >= chosenGoal.normalizedGain
		)
	...
	getBurgProduction(burg: Burg): Record<number, number> {
		...
		for (const record of burg.production || []) {
			...
			produced[record.goodId] = rn(
				(produced[record.goodId] || 0) + record.units,
				2,
			);
		}
		return produced;
	...
	getCellProduction(
		...
		const pop = isWater
			? sum(pack.cells.c[cellId].map((c) => pack.cells.pop[c])) || 0
			: pack.cells.pop[cellId];

		if (pop > 0) {
			for (const { goodId, production } of biomeProduction[
				pack.cells.biome[cellId]
			] || []) {
				const good = Goods.get(goodId);
	...
	private getModifiers(good: Good, cellId: number): number {
		...
		const cultureType =
			(burgId ? pack.burgs[burgId]?.type : pack.cultures[cultureId]?.type) ??
			DEFAULT_CULTURE_TYPE;

		let modifier =
			(mult.cultureType?.[cultureType as CultureType] ?? 1) *
	...
	private getZoneCellSets(): Map<number, Set<number>> {
		...
		if (this.zoneCellSets && this.zoneCellSetsSource === zones)
			return this.zoneCellSets;
...
export const isDealRecord = (record: ProductionRecord): record is DealRecord =>
	"dealId" in record;
export const isMfgRecord = (record: ProductionRecord): record is MfgRecord =>
	"recipe" in record;
