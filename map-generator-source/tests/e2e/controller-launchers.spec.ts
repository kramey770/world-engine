...
test.describe("controller launchers", () => {
	...
	test("opens Markers generation settings from Markers Overview", async ({
		page,
	}) => {
	...
	test("opens Goods Editor when some goods have no production", async ({
		page,
	}) => {
		await page.evaluate(() => {
		...
		await expect(
			page.locator("#goodsBody .goodName", { hasText: "No Production" }),
		).toBeVisible();
	});
	...
	test("opens Relief Editor by clicking a relief icon", async ({ page }) => {
		...
		if (
			await reliefToggle.evaluate((element) =>
				element.classList.contains("buttonoff"),
			)
		) {
			await reliefToggle.click();
		}

		const reliefIconIndex = await page
			.locator("#terrain > use")
