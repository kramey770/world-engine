...
async function closeIconSelector(page: Page): Promise<void> {
	...
	await page.waitForFunction(() => !document.getElementById("iconSelector"), {
		timeout: 5000,
	});
}
...
test.describe("Icon selector lifecycle", () => {
	...
	test("creates a fresh dialog on open and removes it on close", async ({
		page,
	}) => {
		await openIconSelector(page);

		await page.evaluate(() => {
			(window as any).__firstIconSelector =
