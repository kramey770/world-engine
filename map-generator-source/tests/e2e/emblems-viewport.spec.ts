...
const countUses = () =>
	document.querySelectorAll("#emblems use[data-i]").length;
const countStateUses = () =>
	document.querySelectorAll("#stateEmblems > use").length;
...
test.describe("emblems viewport rendering", () => {
	test.beforeEach(async ({ context, page }) => {
		...
		await page.waitForFunction(() => (window as Win).mapId !== undefined, {
			timeout: 60000,
		});
		await page.evaluate(() => (window as Win).Layers.show("emblems"));
		await expect
			.poll(async () => page.evaluate(countStateUses), { timeout: 30000 })
			.toBeGreaterThan(0);
	});

	test("zooming in drops off-screen emblems and zooming out brings them back", async ({
		page,
	}) => {
		...
		await page.evaluate(
			"options.emblems.showAll = true; invokeActiveZooming();",
		);
		...
		const pole = await page.evaluate(() => {
			const states = (window as Win).pack.states.filter(
				(s: any) => s.i && !s.removed && s.pole,
			);
			return states[Math.floor(states.length / 2)].pole as [number, number];
		});
		await page.evaluate(([x, y]) => (window as Win).zoomTo(x, y, 8, 0), pole);
		await expect
			.poll(async () => page.evaluate(countUses), { timeout: 15000 })
			.toBeLessThan(full);
	...
	test("full-map export emits every emblem with its definition, even while zoomed in", async ({
		page,
	}) => {
		...
		await expect
			.poll(async () => page.evaluate(countStateUses), { timeout: 15000 })
			.toBeLessThan(allStates);

		const report = await page.evaluate(async () => {
			const url: string = await (window as Win).Services.ExportMap.getMapURL(
				"svg",
				{ fullMap: true },
			);
			...
			return {
				...
				missing: uses
					...
					.filter((href) => !href || !exported.getElementById(href.slice(1)))
					.length,
			};
		});
	...
	test("keeps a hidden emblem's zero size when it is reselected in the editor", async ({
		page,
	}) => {
		await page.evaluate(() =>
			(window as Win).Controllers.EmblemsEditor.openDefault(),
		);
		...
		await expect(
			page.locator(`#stateEmblems > use[data-i="${first}"]`),
		).toHaveCount(0);
	});
});
