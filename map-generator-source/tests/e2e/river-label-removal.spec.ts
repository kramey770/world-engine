...
test.describe("removing a river removes its label", () => {
	test.beforeEach(async ({ page }) => {
		...
		await page.evaluate(() => {
			...
			const [x, y] =
				w.pack.cells.p[river.cells[Math.floor(river.cells.length / 2)]];
			w.zoomTo(x, y, 8, 0);
		});
		await page.waitForFunction(
			() => document.querySelector("#labels [id^=riverLabel]") !== null,
			{ timeout: 10000 },
		);
	});
	...
	const labelledRiverId = (page: any) =>
		page.evaluate(
			() =>
				+document
					.querySelector("#labels [id^=riverLabel]")!
					.id.replace("riverLabel", ""),
		);
	...
	test("through the river editor", async ({ page }) => {
		...
		await page.evaluate(
			(riverId) =>
				(window as any).Controllers.RiverEditor.open(`river${riverId}`),
			id,
		);
	...
	test("through remove all rivers", async ({ page }) => {
		await page.evaluate(() =>
			(window as any).Controllers.RiversOverview.open(),
		);
