...
declare const Services: {
	Save: { saveMap: (method: string) => Promise<void> };
};
...
test.describe("layers round-trip", () => {
	test("saved layer state and custom order survive a save and load", async ({
		page,
		context,
	}) => {
		...
		expect(before.order.indexOf("texture")).toBeGreaterThan(
			before.order.indexOf("rivers"),
		);
		...
		expect(svgOrder.indexOf("texture")).toBeGreaterThan(
			svgOrder.indexOf("rivers"),
		);
		...
		await page.evaluate(() =>
			history.replaceState(null, "", "?layers=scaleBar,burgIcons"),
		);
		await page.locator("#mapToLoad").setInputFiles([]);
		await page.locator("#mapToLoad").setInputFiles({
			name: "round-trip.map",
		...
		await page.waitForFunction(
			() =>
				JSON.stringify(Layers.state.active.slice().sort()) ===
				JSON.stringify(["burgIcons", "scaleBar"]),
		);

		expect(
			await page.evaluate(() => Layers.state.active.slice().sort()),
		).toEqual(["burgIcons", "scaleBar"]);
