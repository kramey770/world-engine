...
test("burg icons materialize only in-viewport and past their zoom gates", async ({
	page,
}) => {
	...
	const at = (zoom: number) =>
		page.evaluate(async (z) => {
			...
			return {
				...
				burgs: (window as any).pack.burgs.filter((b: any) => b.i && !b.removed)
					.length,
			};
		}, zoom);
	...
	const panned = await page.evaluate(async () => {
		const ids = () =>
			[...document.querySelectorAll("#burgIcons use")]
				.map((use) => use.id)
				.join(",");
		const before = ids();
