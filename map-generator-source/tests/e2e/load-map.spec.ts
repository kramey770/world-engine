...
declare const options: {
	labels: {
		...
		showAll: boolean;
		groups: { type: string; mode?: string }[];
	};
};
...
function buildLegacyReliefMap(hidden: boolean): Buffer {
	...
	const icons = LEGACY_RELIEF_ICONS.map(
		({ icon, x, y, s }) =>
			`<use href="#${icon}" x="${x}" y="${y}" width="${s}" height="${s}"/>`,
	).join("");
...
function buildLegacyMapWithAttributeHiddenBorders(): Buffer {
	...
	mapData[5] = mapData[5].replace(
		'<g id="borders" fill="none">',
		'<g id="borders" fill="none" display="none">',
	);
...
test.describe("Map loading", () => {
	...
	test("legacy fogging container should be unwrapped in place", async ({
		page,
	}) => {
		const fileInput = page.locator("#mapToLoad");
		const mapFilePath = path.join(__dirname, "../fixtures/1.112.1.map");
		...
		const fogging = await page.evaluate(() => ({
			...
			groups: Array.from(
				document.querySelectorAll("#viewbox > *"),
				(node) => node.id,
			),
		...
		expect(fogging.order.indexOf("fogging")).toBeLessThan(
			fogging.order.indexOf("rulers"),
		);
		expect(fogging.groups.indexOf("fogging")).toBeLessThan(
			fogging.groups.indexOf("ruler"),
		);
	...
	test("legacy added labels should migrate to pack.addedLabels", async ({
		page,
	}) => {
		const fileInput = page.locator("#mapToLoad");
		const mapFilePath = path.join(__dirname, "../fixtures/1.139.4.map");
		...
		const migrated = await page.evaluate(() => {
			...
			return {
				...
				entities: addedLabels.map((added: any) => ({
					...
					anchorOnPath: added.label.pathPoints.some(
						([x, y]: number[]) => x === added.x && y === added.y,
					),
				})),
				// every migrated label is rendered, and along its path
				rendered: addedLabels.map(
					(added: any) =>
						document.getElementById(`addedLabel${added.i}`)?.dataset
							.labelShape ?? "missing",
				),
				...
				orphanNotes: notes.filter(
					(note: any) =>
						...
						!addedLabels.some(
							(added: any) => `addedLabel${added.i}` === note.id,
						),
				).length,
			};
		});
	...
	test("legacy lakes without shoreline data should get it on load", async ({
		page,
	}) => {
		...
		expect(
			lakeShorelines.every(
				(shoreline: unknown) =>
					Array.isArray(shoreline) && shoreline.length > 0,
			),
		).toBe(true);
	});
	...
	test("a layer hidden by the display attribute should migrate as off, not as active and invisible", async ({
		...
		expect(
			await page.evaluate(() => (window as any).Layers.isOn("borders")),
		).toBe(false);
	...
	test("the legacy compass rose should be adopted rather than duplicated", async ({
		page,
	}) => {
		const mapFilePath = path.join(__dirname, "../fixtures/1.139.4.map");
	...
	test("legacy label settings should migrate without changing behavior", async ({
		page,
	}) => {
		const mapFilePath = path.join(__dirname, "../fixtures/1.139.4.map");
		...
		const migrated = await page.evaluate(() => {
			...
			return {
				...
				stateMode: labels.groups.find((group: any) => group.type === "state")
					?.mode,
			};
		});
	...
	test("legacy relief icons should migrate to pack.relief keeping the layer on", async ({
		page,
	}) => {
		await page.locator("#mapToLoad").setInputFiles({
	...
	test("hidden legacy relief layer should stay off after migration", async ({
		page,
	}) => {
		await page.locator("#mapToLoad").setInputFiles({
	...
	test("save data should preserve an active but empty label layer", async ({
		page,
	}) => {
		await page.waitForFunction(
			() => Boolean((window as any).pack?.cells?.i?.length),
			{ timeout: 120000 },
		);
	...
	test("an old map fixture should get its defs, geometry and layer styles back", async ({
		page,
	}) => {
		...
		const restored = await page.evaluate(() => {
			const features = (window as any).pack.features.filter(
				(f: any) => f && f.type !== "ocean",
			);
			const hrefs = (selector: string) =>
				Array.from(document.querySelectorAll(`${selector} use`), (use) =>
					use.getAttribute("href"),
				);
			const resolves = (href: string | null) =>
				Boolean(href && document.querySelector(`#featurePaths ${href}`));

			return {
				featurePaths: document.querySelectorAll("#featurePaths > path").length,
				expected: features.length,
				legacyMaskPaths: document.querySelectorAll("#land path, #water path")
					.length,
