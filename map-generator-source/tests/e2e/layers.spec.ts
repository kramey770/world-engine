...
declare const options: {
	labels: { groups: { name: string; active?: boolean }[] };
};
...
test.describe("map layers", () => {
	...
	test("labels use flat groups and entity-specific rendering", async () => {
		...
		const structure = await labels.evaluate((el) => {
			...
			return {
				...
				stateTextPaths: el.querySelectorAll(
					"text[data-label-type='state'] textPath",
				).length,
				burgLabels: el.querySelectorAll("text[data-label-type='burg']").length,
				burgTextPaths: el.querySelectorAll(
					"text[data-label-type='burg'] textPath",
				).length,
			};
		});

		expect(structure.groupIds).toEqual(
			expect.arrayContaining(["labels-state", "labels-town", "labels-added"]),
		);
	...
	test("deactivated labels group is not rendered", async () => {
		const counts = await sharedPage.evaluate(async () => {
			const stateGroup = options.labels.groups.find(
				(group) => group.name === "state",
			)!;
