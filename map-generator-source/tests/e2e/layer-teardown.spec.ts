...
test.describe("layer teardown keeps user data", () => {
	...
	test("routes keep custom groups and their styles across a hide/show round trip", async ({
		page,
	}) => {
		...
		await page.evaluate(() => {
			...
			routes.append(group);
			for (const route of (window as any).pack.routes.slice(0, 5))
				route.group = "caravans";
	...
	test("precipitation rebuilds the wind direction group with the layer", async ({
		page,
	}) => {
	...
	test("burg icons keep group styles edited while the layer is on", async ({
		page,
	}) => {
		...
		await page.evaluate(() =>
			document
				.querySelector("#burgIcons > #capital")!
				.setAttribute("fill", "#123456"),
		);
		...
		expect(await capitals.getAttribute("fill")).toBe("#123456");
		expect(
			await page.locator("#burgIcons > #capital use").count(),
		).toBeGreaterThan(0);
	});

	test("showing a layer that is already on does not redraw it", async ({
		page,
	}) => {
		// a redraw rebuilds every burg <use>, so a marker set on one of them would not survive it
		await page.evaluate(() => (window as any).Layers.show("burgIcons"));
		await page.evaluate(() =>
			document.querySelector("#burgIcons use")!.setAttribute("data-probe", "1"),
		);

		await page.evaluate(() =>
			(window as any).Layers.show("burgIcons", "labels"),
		);
	...
	test("the heightmap layer refuses to draw in the heightmap edit mode", async ({
		page,
	}) => {
		...
		await expect(page.locator("#tooltip")).toHaveText(
			/not available in the heightmap edit mode/,
		);
	...
	test("heightmap keep mode clears the landmass content and redraws it on exit", async ({
		page,
	}) => {
		await page.evaluate(() =>
			(window as any).Controllers.HeightmapEditor.open({ mode: "keep" }),
		);
		...
		const restored = await page.evaluate(() => ({
			...
			lakesMatchesState:
				(window as any).Layers.isOn("lakes") ===
				(document.getElementById("lakes")!.style.display !== "none"),
		}));

		expect(restored).toEqual({
			landmass: true,
			landmassDisplay: "",
			lakesMatchesState: true,
		});
	...
	test("ocean outlines follow the layers attribute and keep the base rect", async ({
		page,
	}) => {
		...
		await page.evaluate(() => {
			document
				.getElementById("oceanLayers")!
				.setAttribute("layers", "-6,-3,-1");
			(window as any).Layers.draw("ocean");
		});
	...
	test("the compass rose exists while the layer is off and outlives eraseAll", async ({
		page,
	}) => {
		const rose = page.locator("#compass > #compassRose");

		expect(
			await page.evaluate(() => (window as any).Layers.isOn("compass")),
		).toBe(false);
		...
		await page.evaluate(() =>
			document
				.querySelector("#compass > use")!
				.setAttribute("transform", "scale(2)"),
		);
	...
	test("reassigned arms replace the rendered shield, whether the layer was on or off", async ({
		page,
	}) => {
	...
	test("a style change does not render into a layer that is off", async ({
		page,
	}) => {
		expect(
			await page.evaluate(() => (window as any).Layers.isOn("goods")),
		).toBe(false);
