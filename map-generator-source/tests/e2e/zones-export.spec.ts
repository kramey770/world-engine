...
test.describe("Zone Export", () => {
	...
	async function createTestZone(page: any): Promise<number> {
		return await page.evaluate(() => {
			...
			if (zoneCells.length < 10) {
				throw new Error(
					`Not enough contiguous land cells found: ${zoneCells.length}`,
				);
			}
	...
	test("should export zone with valid GeoJSON root structure", async ({
		page,
	}) => {
		// Create a test zone
		...
		const testZoneFeature = geoJson.features.find(
	...
	test("should exclude hidden zones from GeoJSON export", async ({ page }) => {
		...
		const hiddenZoneId = await page.evaluate(() => {
			...
			for (let i = 1; i < totalCells; i++) {
				...
				const notInZone = !zones.some(
					(z: any) => z.cells && z.cells.includes(i),
				);
				if (isLand && notInZone) {
					startCell = i;
					break;
			...
			while (queue.length > 0 && zoneCells.length < 20) {
				...
				const notInZone = !zones.some(
					(z: any) => z.cells && z.cells.includes(current),
				);
				if (isLand && notInZone) {
					zoneCells.push(current);
				}
		...
		const regularZoneFeature = geoJson.features.find(
			(f: any) => f.properties.id === regularZoneId,
		);
		...
		const hiddenZoneFeature = geoJson.features.find(
			(f: any) => f.properties.id === hiddenZoneId,
		);
	...
	test("should exclude zones with empty cells array from GeoJSON export", async ({
		...
		const emptyZoneFeature = geoJson.features.find(
			(f: any) => f.properties.id === emptyZoneId,
		);
