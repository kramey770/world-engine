...
const waitForMap = (page: Page) =>
	page.waitForFunction(() => (window as any).mapId !== undefined, {
		timeout: 120000,
	});
...
function watchErrors(page: Page): string[] {
	...
	page.on("console", (msg) => {
		...
		const text = msg.text();
		if (
			/fonts\.googleapis|google-analytics|googletagmanager|Failed to load resource/.test(
				text,
			)
		)
			return;
	...
	page.evaluate(() =>
		Layers.all
			.map((layer) => {
				...
				return on === hidden
					? `${layer.id}: active=${on} but display=${element.style.display || "visible"}`
					: null;
			})
			.filter(Boolean),
	);
	...
	await page.evaluate(
		(config) => {
			(window as any).__regenerated = new Promise<void>((resolve) =>
				window.addEventListener("map:generated", () => resolve(), {
					once: true,
				}),
			);
...
test.describe("layer scenarios", () => {
	...
	for (const preset of PRESETS) {
		test(`the ${preset} preset applies without errors and matches the dom`, async ({
			page,
		}) => {
			...
			const active = await page.evaluate(() =>
				Layers.state.active.slice().sort(),
			);
	...
	test("switching to another preset and back restores identical content", async ({
		page,
	}) => {
		const errors = watchErrors(page);
	...
	test("a preset switch leaves no content behind in the layers it turns off", async ({
		page,
	}) => {
			...
			Layers.all
				...
				.map((layer) => {
					...
					const content = Array.from(element?.children ?? []).filter(
						(child) => child.innerHTML.length,
					);
					return content.length
						? `${layer.id}: ${content.map((c) => c.id || c.tagName).join(",")}`
						: null;
				})
				.filter(Boolean),
		);
	...
	test("regeneration keeps the active layer set and redraws its content", async ({
		page,
	}) => {
		const errors = watchErrors(page);
	...
	test("repeated regeneration does not accumulate content", async ({
		page,
	}) => {
		const errors = watchErrors(page);
	...
	test("a layer moved while it is off keeps its new slot when shown again", async ({
		page,
	}) => {
		const errors = watchErrors(page);
		...
		expect(orderWhileOff.indexOf("biomes")).toBeLessThan(
			orderWhileOff.indexOf("rivers"),
		);
		...
		expect(orderAfterShow.indexOf("biomes")).toBeLessThan(
			orderAfterShow.indexOf("rivers"),
		);

		// the registry order and the document order have to agree
		const registryOrder = await page.evaluate(() => Layers.state.order);
		expect(registryOrder.indexOf("biomes")).toBeLessThan(
			registryOrder.indexOf("rivers"),
		);

		const content = await page.evaluate(
			() => document.getElementById("biomes")!.innerHTML.length,
		);
	...
	test("hidden layers stay hidden in the exported svg", async ({ page }) => {
		...
		const svg = await page.evaluate(async () => {
			const url = await (window as any).Services.ExportMap.getMapURL("svg", {
				fullMap: true,
			});
	...
	test("a preset URL param applies that preset on load", async ({ page }) => {
		...
		expect(await page.evaluate(() => localStorage.getItem("preset"))).toBe(
			"religions",
		);
		expect(await visibilityDrift(page)).toEqual([]);
		expect(errors).toEqual([]);
	...
	test("a preset URL param matches the select display name", async ({
		page,
	}) => {
		await page.goto(
			"/?seed=url-preset-name&width=1280&height=720&preset=Religions%20map",
		);
		await waitForMap(page);
		await expect(page.locator("#layersPreset")).toHaveValue("religions");
	});
	...
	test("a layers URL param shows only the listed layers", async ({ page }) => {
		const errors = watchErrors(page);
		await page.goto(
			"/?seed=url-layers&width=1280&height=720&layers=provinces,borders,lakes,rivers",
		);
		await waitForMap(page);
	...
	test("layers wins over preset, ignores unknown ids and clears the set when none are known", async ({
		page,
	}) => {
		await page.goto(
