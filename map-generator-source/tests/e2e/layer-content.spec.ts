...
test.describe("layer content on hide and show", () => {
	...
	test("emblems render on show and again after a hide/show round trip", async ({
		page,
	}) => {
		...
		await expect
			.poll(async () => page.locator("#emblems use[href]").count(), {
				timeout: 10000,
			})
			.toBeGreaterThan(0);
	...
	test("ice drops its polygons when hidden and redraws them on show", async ({
		page,
	}) => {
	...
	test("fogging follows the fog mask and survives a preset change", async ({
		page,
	}) => {
		...
		await page.evaluate(() =>
			(window as any).Layers.set(["states", "labels", "rivers"]),
		);
		await expect(fogging).toBeVisible();
		expect(await rects.count()).toBe(2);
