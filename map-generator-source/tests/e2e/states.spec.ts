...
test.describe("States", () => {
	...
	test("removing a state via UI should allow military regeneration without errors", async ({
		...
		const neighborsBefore = await page.evaluate((id: number) => {
			...
			return states.filter(
				(s: any) =>
					s.i && !s.removed && s.neighbors && s.neighbors.includes(id),
			).length;
		}, stateId!);
		...
		await page.click(
			".ui-dialog:has(#alert) .ui-dialog-buttonpane button:first-child",
		); // "Remove" is first button
		await page.waitForTimeout(500);
		...
		const militaryResult = await page.evaluate(() => {
			...
			return {
				...
				statesWithMilitary: validStates.filter(
					(s: any) => s.military && s.military.length > 0,
				).length,
			};
		});
	...
	test("painting states applies after the States Editor closes", async ({
		page,
	}) => {
		...
		const target = await page.evaluate(() => {
			...
			const cell = cells.i.find(
				(cellId: number) =>
					cells.h[cellId] >= 20 &&
					cells.state[cellId] &&
					states[cells.state[cellId]].center !== cellId,
			);
			const [x, y] = cells.p[cell];
			const viewbox = document.getElementById(
				"viewbox",
		...
		await page.mouse.up();
		await expect(
			page.locator(`#temp polygon[data-cell="${target.cell}"]`),
		).toBeAttached();
		...
		await expect
			.poll(() =>
				page.evaluate(
					(cell) => (window as any).pack.cells.state[cell],
					target.cell,
				),
			)
			.toBe(0);
		expect(
			await page.evaluate(
				(state) => (window as any).pack.states[state].label,
				target.state,
			),
		).toBeUndefined();
	});
