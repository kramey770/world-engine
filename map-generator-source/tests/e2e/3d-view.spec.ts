...
test.describe("3D view with eroded terrain", () => {
	...
	test("bakes erosion detail and renders without errors", async ({ page }) => {
		...
		await page.evaluate(() =>
			(window as any).Controllers.View3d.open("viewMesh"),
		);
		...
		await page.waitForFunction(
			async () => (await (window as any).Controllers.View3d.isOn()) === true,
			{
				timeout: 60000,
			},
		);
		...
		await page.evaluate(() =>
			(document.getElementById("options3dErosion") as HTMLInputElement).click(),
		);

		// the bake must complete and cache the dense height field
		await page.waitForFunction(
			async () => await (window as any).Controllers.View3d.isCached(),
			{
				timeout: 60000,
			},
		);
