...
test.describe("Burgs.add", () => {
	...
	test("port toggle button should be inactive for non-coastal burg", async ({
		...
		await page.evaluate(() =>
			(window as any).Layers.draw("burgIcons", "labels"),
		);
	...
	test("repeated burg label click opens the label editor for the same burg", async ({
		page,
	}) => {
		...
		await expect(page.locator("#burgEditor")).toHaveAttribute(
			"data-burg-id",
			firstId!,
		);
