...
test("does route regeneration change geometry or only names?", async ({
	page,
}) => {
	...
	const out = await page.evaluate(async () => {
		...
		const geometry = () =>
			JSON.stringify(
				pack.routes
					.slice(0, 3)
					.map((r: any) => r.points.map((p: number[]) => p[2])),
			);
		const names = () =>
			JSON.stringify(pack.routes.slice(0, 3).map((r: any) => r.name));
