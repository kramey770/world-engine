...
const NOISE =
	/fonts.googleapis|google-analytics|googletagmanager|Failed to load resource|Name is too short|net::ERR/;
...
test("Q1: submap keeps icon stroke proportional", async ({ page }) => {
	...
	const out = await page.evaluate(async () => {
		...
		const read = () => {
			...
			return {
				fontSize: fs,
				strokeWidth: sw,
				visualStroke: +((sw * fs) / 10).toFixed(4),
			};
		};
		const before = read();
		...
		const buttons = [
			...document.querySelectorAll(
				".ui-dialog:has(#submapTool) .ui-dialog-buttonpane button",
			),
		] as HTMLElement[];
	...
	console.log(
		...
		JSON.stringify({
			...out,
			fontSizeFactor: ratio(out.after.fontSize, out.before.fontSize),
			strokeFactor: ratio(out.after.strokeWidth, out.before.strokeWidth),
			visualFactor: ratio(out.after.visualStroke, out.before.visualStroke),
		}),
	);
	// the map grew by usedScale: icon size scales with it, stroke-width stays, so the visual stroke grows once
	expect(ratio(out.after.fontSize, out.before.fontSize)).toBeCloseTo(
		out.usedScale,
		1,
	);
	expect(out.after.strokeWidth).toBeCloseTo(out.before.strokeWidth, 5);
	expect(ratio(out.after.visualStroke, out.before.visualStroke)).toBeCloseTo(
		out.usedScale,
		1,
	);
	...
	const fingerprint = async () => {
		...
		return page.evaluate(() => {
			...
			return {
				...
				nameableRoutes: pack.routes.filter((r: any) => r.points.length >= 4)
					.length,
				routeNames: pack.routes.slice(0, 5).map((r: any) => r.name),
				stateForms: states.slice(0, 4).map((s: any) => s.fullName),
				groupTally: burgs.reduce(
					(acc: any, b: any) => ((acc[b.group] = (acc[b.group] || 0) + 1), acc),
					{},
				),
...
test("Q2b: route regeneration is randomized and keeps names", async ({
	page,
}) => {
	const errors: string[] = [];
	watch(page, errors);
	...
	const out = await page.evaluate(async () => {
		...
		const snapshot = () => ({
			...
			pointsHash: pack.routes.reduce(
				(sum: number, r: any) => sum + r.points.length,
				0,
			),
		});
	...
	expect(JSON.stringify(out.secondRegen)).not.toBe(
		JSON.stringify(out.firstRegen),
	);
	expect(errors).toEqual([]);
});

test("Q2c: route generation no longer shifts the downstream stream", async ({
	page,
}) => {
	const errors: string[] = [];
	watch(page, errors);
	...
	const out = await page.evaluate(async () => {
		...
		return {
			...
			afterGenerate,
			afterSecondGenerate,
			streamStableAcrossCalls:
				JSON.stringify(afterGenerate) === JSON.stringify(afterSecondGenerate),
		};
	});
...
test("Q3: preset without font-size and with a legacy absolute size", async ({
	page,
}) => {
	const errors: string[] = [];
	watch(page, errors);
