import{A as e,C as t,Kt as n,M as r,Qt as i,S as a,dn as o,k as s,st as c}from"./utils-BIleyWmR.js";import{j as l,t as u}from"./layers-qPZxwHcp.js";import{r as d,t as f}from"./tooltips-zkair8Am.js";import{a as p,r as m,t as h}from"./dialog-helpers-50LJgx0U.js";import{b as g,j as _,y as v}from"./index-tuMZPduM.js";import{t as y}from"./highlighting-CqamXa3p.js";import{i as b,n as x,r as S,t as C}from"./table-oyVozzWP.js";var w={Ally:{inText:`is an ally of`,color:`#00b300`,tip:`Allies formed a defensive pact and protect each other in case of third party aggression`},Friendly:{inText:`is friendly to`,color:`#d4f8aa`,tip:`State is friendly to anouther state when they share some common interests`},Neutral:{inText:`is neutral to`,color:`#edeee8`,tip:`Neutral means states relations are neither positive nor negative`},Suspicion:{inText:`is suspicious of`,color:`#eeafaa`,tip:`Suspicion means state has a cautious distrust of another state`},Enemy:{inText:`is at war with`,color:`#e64b40`,tip:`Enemies are states at war with each other`},Unknown:{inText:`does not know about`,color:`#a9a9a9`,tip:`Relations are unknown if states do not have enough information about each other`},Rival:{inText:`is a rival of`,color:`#ad5a1f`,tip:`Rivalry is a state of competing for dominance in the region`},Vassal:{inText:`is a vassal of`,color:`#87CEFA`,tip:`Vassal is a state having obligation to its suzerain`},Suzerain:{inText:`is suzerain to`,color:`#00008B`,tip:`Suzerain is a state having some control over its vassals`}},T=`diplomacyEditor`,E={my:`right top`,at:`right-10 top+10`,of:`svg`,collision:`fit`},D=0,O=[{key:`name`,label:`State`,width:`15em`,permanent:!0,sortBy:e=>e.fullName||e.name,sortType:`alpha`},{key:`relations`,label:`Relations`,width:`7em`,sortBy:e=>e.diplomacy?.[D]??``,sortType:`alpha`},{key:`actions`,width:`1.4em`,permanent:!0}],k=x({getData:()=>g(T,pack.states.filter(e=>e.i&&!e.removed&&e.i!==D),O),onUpdate:P}),A=()=>pack.states[0].diplomacy;function j(){if(!customization){if(pack.states.filter(e=>e.i&&!e.removed).length<2){d(`There should be at least 2 states to edit the diplomacy`,!1,`error`);return}(!D||!pack.states[D]||pack.states[D].removed)&&(D=pack.states.find(e=>e.i&&!e.removed).i),h(`#${T}, .stable`),u.show(`states`,`borders`),u.hide(`provinces`,`cultures`),u.hide(`biomes`,`religions`),M(),N(),o(`#viewbox`).style(`cursor`,`crosshair`).on(`click`,R),$(`#${T}`).dialog({title:`Diplomacy Editor`,resizable:!1,width:fitContent(),close:Y,position:E})}}function M(){m(T);let e=`<div id="${T}" class="dialog stable editorDialog">
  ...
  </div>`;s(`dialogs`).insertAdjacentHTML(`beforeend`,e),v(T,k.reset),y(T,({cellId:e})=>pack.cells.state[e]),C({dialogId:T,columns:O,onUpdate:()=>p(T,{width:`fit-content`,position:E})}),s(`diplomacyEditorRefresh`).addEventListener(`click`,N),s(`diplomacyEditStyle`).addEventListener(`click`,()=>editStyle(`regions`)),s(`diplomacyRegenerate`).addEventListener(`click`,V),s(`diplomacyReset`).addEventListener(`click`,H),s(`diplomacyShowMatrix`).addEventListener(`click`,G),s(`diplomacyHistory`).addEventListener(`click`,U),s(`diplomacyExport`).addEventListener(`click`,J),s(`diplomacyBodySection`).addEventListener(`click`,e=>{let t=e.target,n=t.closest(`.states`);if(n&&!n.classList.contains(`Self`)){if(t.closest(`.changeRelations`)){let e=+n.dataset.id,t=+s(`diplomacyBodySection`).querySelector(`div.Self`).dataset.id,r=n.dataset.relations;z(e,t,r);return}D=+n.dataset.id,N()}})}function N(){k.reset(),L()}function P(e){let t=s(`diplomacyBodySection`),n=pack.states,r=D,i=n[r].name;l.trigger(`stateCOA${r}`,n[r].coa);let a=`<div class="states Self" data-id=${r} data-tip="List below shows relations to ${i}">
    ...
    <div data-col="actions"></div>
      ...
      <div data-col="actions"></div>
    </div>`}t.innerHTML=a,t.querySelectorAll(`div.states`).forEach(e=>{e.addEventListener(`mouseenter`,F)}),t.querySelectorAll(`div.states`).forEach(e=>{e.addEventListener(`mouseleave`,I)}),s(`diplomacyFooterStates`).textContent=String(e.all.length+1),b(s(`diplomacyFooter`),e,k.goto),p(T,{width:`fit-content`,position:E})}function F(e){if(!u.isOn(`states`))return;let t=+e.target.dataset.id;if(customization||!t)return;let r=o(`#regions`).select(`#state${t}`).attr(`d`),i=o(`#debug`).append(`path`).attr(`class`,`highlight`).attr(`d`,r).attr(`fill`,`none`).attr(`stroke`,`red`).attr(`stroke-width`,1).attr(`opacity`,1).attr(`filter`,`url(#blur1)`),a=i.node().getTotalLength(),s=(a+5e3)/2,c=n(`0,${a}`,`${a},${a}`);i.transition().duration(s).attrTween(`stroke-dasharray`,()=>e=>c(e))}function I(){o(`#debug`).selectAll(`.highlight`).each(function(){o(this).transition().duration(1e3).attr(`opacity`,0).remove()})}function L(){let e=s(`diplomacyBodySection`).querySelector(`div.Self`),t=e?+e.dataset.id:pack.states.find(e=>e.i&&!e.removed).i;t&&(u.show(`states`),o(`#statesBody`).selectAll(`path`).each(function(){if(this.id.slice(0,9)===`state-gap`)return;let e=+this.id.slice(5),n=w[pack.states[e].diplomacy[t]]?.color||`#4682b4`;this.setAttribute(`fill`,n),o(`#statesBody`).select(`#state-gap${e}`).attr(`stroke`,n),o(`#statesHalo`).select(`#state-border${e}`).attr(`stroke`,i(n).darker().hex())}))}function R(e){let t=r(e,this),n=Pack.findCell(t[0],t[1]),i=pack.cells.state[n];i&&pack.states[i]&&!pack.states[i].removed&&D!==i&&(D=i,N())}function z(e,t,n){let r=pack.states,i=r[e],a=Object.entries(w).map(([e,{color:t,inText:r,tip:i}])=>`
        <div data-tip="${i}">
          <label class="pointer">
  ...
  `,$(`#alert`).dialog({width:fitContent(),title:`Change relations`,buttons:{Apply:function(){let t=new FormData(s(`relationsForm`)),r=t.get(`relationSelect`),i=[...t.getAll(`objectSelect`)].map(Number);for(let t of i)B(e,t,n,r);$(this).dialog(`close`)},Cancel:function(){$(this).dialog(`close`)}}});let c=s(`selectAllNoneBtn`),l=()=>document.querySelectorAll(`#stateSelectionContainer input[name='objectSelect']`);function u(){let e=l();Array.from(e).every(e=>e.checked)&&e.length>0?c.classList.add(`pressed`):c.classList.remove(`pressed`)}function d(){let e=l(),t=!Array.from(e).every(e=>e.checked);e.forEach(e=>{e.checked=t}),u()}c.addEventListener(`click`,e=>{e.preventDefault(),d()}),u()}function B(t,n,r,i){if(i===r)return;let a=pack.states,o=A(),s=a[t].name,l=a[n].name;a[t].diplomacy[n]=i,a[n].diplomacy[t]=i===`Vassal`?`Suzerain`:i===`Suzerain`?`Vassal`:i;let u=()=>[`Relations change`,`${s}-${c(l)} relations changed to ${i.toLowerCase()}`],d=()=>[`Defence pact`,`${s} entered into defensive pact with ${l}`],f=()=>[`Vassalization`,`${s} became a vassal of ${l}`],p=()=>[`Vassalization`,`${s} vassalized ${l}`],m=()=>[`Rivalization`,`${s} and ${l} became rivals`],h=()=>[`Relations severance`,`${s} recalled their ambassadors and wiped all the records about ${l}`];r===`Enemy`?o.push([`War termination`,`${s} and ${l} agreed to cease fire and signed a peace treaty`,(i===`Ally`?d():i===`Vassal`?f():i===`Suzerain`?p():i===`Unknown`?h():u())[1]]):i===`Enemy`?o.push([`War declaration`,`${s} declared a war on its enemy ${l}`]):i===`Vassal`?o.push(f()):i===`Suzerain`?o.push(p()):i===`Ally`?o.push(d()):i===`Unknown`?o.push(h()):i===`Rival`?o.push(m()):o.push(u()),N(),e(`diplomacyMatrix`)&&G()}function V(){States.generateDiplomacy(),N()}function H(){let e=+s(`diplomacyBodySection`).querySelector(`div.Self`).dataset.id;if(!e)return;let t=pack.states;t[e].diplomacy.forEach((n,r)=>{n!==`x`&&(t[e].diplomacy[r]=`Neutral`,t[r].diplomacy[e]=`Neutral`)}),N()}function U(){let e=A(),n=`<div autocorrect="off" spellcheck="false">`;e.forEach((e,t)=>{n+=`<div>`,e.forEach((e,r)=>{n+=`<div contenteditable="true" data-id="${t}-${r}"
