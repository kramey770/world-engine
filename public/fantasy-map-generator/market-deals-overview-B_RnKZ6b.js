import{C as e,S as t,Tt as n,k as r,n as i}from"./utils-omhVlsRt.js";import{r as a}from"./tooltips-BQ3W48fB.js";import{a as o}from"./dialog-helpers-CdcIkxap.js";import{t as s}from"./state-CHxunpWe.js";import{b as c,y as l}from"./index-DWZM8gkY.js";import{i as u,n as d,r as f,t as p}from"./table-DM0fGDYA.js";var m=0,h,g=`marketDeals`,_={my:`right top`,at:`right bottom+10`,of:`#marketOverview`,collision:`fit`},v=[{key:`icon`,width:`2em`,permanent:!0},{key:`good`,label:`Good`,width:`6.8em`,permanent:!0,sortBy:e=>Goods.get(e.good)?.name??``,sortType:`alpha`},{key:`direction`,label:`Type`,width:`5em`,sortBy:e=>D(e,m),sortType:`alpha`},{key:`counterparty`,label:`Counterparty`,width:`8em`,sortBy:e=>A(e)?.name??``,sortType:`alpha`},{key:`units`,label:`Units`,width:`5em`,sortBy:e=>e.units},{key:`income`,label:`Income`,width:`5em`,sortBy:e=>j(e,m)},{key:`actions`,width:`1.2em`,permanent:!0}],y=d({getData:C,onUpdate:w});function b(e){let t=Markets.get(e);if(!t){a(`Invalid market. The selected market does not exist`,!0,`error`,5e3);return}h=s.get(g,`filters`,()=>({scope:`all`})),[`all`,`local`,`global`].includes(h.scope)||(h.scope=`all`),s.set(g,`filters`,h),m=e,x(),r(`marketDealsFilter`).value=h.scope,y.reset(),$(`#${g}`).dialog({title:`${Markets.getName(t)} Market Deals`,position:_,close:S})}function x(){document.getElementById(g)?.remove();let e=`<div id="${g}" class="dialog stable editorDialog">
      ...
      <div data-col="units" class="marketDealUnits">${n(e.units,2)}</div>
      <div data-col="income" class="marketDealIncome" style="color:${c}">${i(r)}</div>
    </div>`}function A(e){let t=O(e,m),n=t.type===`burg`?t.id:Markets.get(t.id)?.centerBurgId;return n&&pack.burgs[n]||null}function j(e,t){let r=n(e.units*e.price,2);return E(e,t)?r:-r}function M(){if(!Markets.get(m))return;let r=T(pack.deals,m),i=`Id,Good,Type,Client,Units,Price,Net
`;for(let e of r){let t=Goods.get(e.good);t&&(i+=[e.i,t.name,D(e,m),A(e)?.name??``,n(e.units,2),n(e.price,2),n(j(e,m),2)].join(`,`),i+=`
`)}t(i,`${e(`Market_${m}_Deals`)}.csv`)}var N={open:b};export{N as MarketDealsOverview};