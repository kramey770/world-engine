import{Et as e,N as t,at as n,k as r,w as i,wn as a}from"./utils-BIleyWmR.js";import{t as o}from"./layers-qPZxwHcp.js";import{r as s}from"./tooltips-zkair8Am.js";import{i as c,r as l}from"./dialog-helpers-50LJgx0U.js";import{t as u}from"./cultures-generator-DSVAHC-N.js";import{n as d,t as f}from"./goods-generator-C2_UidL2.js";import{P as p}from"./index-tuMZPduM.js";function m(n,i){let u=Array.from(r(`good-icons`).querySelectorAll(`symbol`)).map(e=>e.id),m={...n?.demandCoverage||{}},b={...n?.biomeOutput||{}},x=()=>{let e=d.map(e=>[e,m[e]??0]).filter(([,e])=>e>0);return e.length?e.map(([e,n])=>`${f[e]} ${t(e)}: ${n}`).join(`, `):`none`},S=()=>{let e=Object.entries(b).filter(([,e])=>(e??0)>0);return e.length?e.map(([e,t])=>`${pack.biomes[Number(e)].name}: ${t}`).join(`, `):`none`},C={cultureType:{...n?.multipliers?.cultureType??{}},culture:{...n?.multipliers?.culture??{}},state:{...n?.multipliers?.state??{}},religion:{...n?.multipliers?.religion??{}},biome:{...n?.multipliers?.biome??{}},zone:{...n?.multipliers?.zone??{}}},w=t=>{let n=C[t]??{},r=Object.entries(n).filter(([,e])=>e!==1);return r.length?r.map(([n,r])=>`${h(t,n)} ×${e(r,2)}`).join(`, `):`none`},T=(e,t)=>`
      ...
      <div class="ge-edit-row">
      ...
      </div>`,E=n?.recipes||[],D;O(),$(D).dialog({width:`30em`,resizable:!1,title:n?`Edit good`:`Add new good`,open:function(){n&&(this.parentElement?.querySelector(`.ui-dialog-buttonpane`))?.insertAdjacentHTML(`afterbegin`,`<div class="dontAsk" data-tip="Re-place this good and recompute production, trade and taxes. Uncheck to update the good only, without disturbing the current economy.">
        ...
        </div>`)},close:()=>{l(`goodEditor`)},buttons:{Cancel:function(){$(this).dialog(`close`)},[n?`Apply`:`Add`]:()=>{let e=[],t=r(`newGoodName`).value.trim(),l=r(`newGoodTags`).value.trim(),u=a(l.split(`,`).map(e=>e.trim().toLocaleLowerCase())),d=+r(`newGoodValue`).value,f=+r(`newGoodChance`).value,p=r(`newGoodUnit`).value.trim(),h=r(`newGoodIcon`).value,g=r(`newGoodColor`).value,_=r(`newGoodDistribution`).textContent?.trim()??``;if(t||e.push(`Name is required`),(!Number.isFinite(d)||d<0)&&e.push(`Value must be a valid non-negative number`),(!Number.isFinite(f)||f<0||f>100)&&e.push(`Chance must be between 0 and 100`),_)try{let e=Goods.getMethods(),t=`{${Object.keys(e).join(`, `)}}`;Function(t,`return ${_}`)(e)}catch(t){e.push(`Distribution function is invalid: ${t.message||t}`)}for(let t of E){for(let[n,r]of Object.entries(t)){let t=Number(n),i=Goods.get(t);i||e.push(`Recipe references unknown good id: ${t}`);let a=Number(r);(Number.isNaN(a)||!Number.isFinite(a)||a<=0)&&e.push(`Invalid recipe amount for good ${i?.name}`)}Object.keys(t).length||e.push(`Each recipe must have at least one ingredient`)}if(r(`newGoodError`).textContent=e.join(`. `),e.length)return;function v(){let e={};for(let[t,n]of Object.entries(C)){let r=Object.fromEntries(Object.entries(n??{}).filter(([,e])=>e!==void 0&&e!==1));Object.keys(r).length&&(e[t]=r)}return Object.keys(e).length?e:void 0}n?(n.name=t,n.tags=u,n.icon=h,n.color=g,n.value=d,n.chance=f,n.unit=p,n.demandCoverage=m,n.multipliers=v(),n.distribution=_||void 0,n.biomeOutput=Object.keys(b).length?b:void 0,n.recipes=E.length?E:void 0,r(`goodRegenerateEconomy`).checked?(Goods.regeneratePlacement(n.i),Production.regenerateEconomy(),o.draw(`markets`,`goods`),o.draw(`trade`),c()):Goods.sync()):(pack.goods.push({i:(()=>{let e=pack.goods?.at(-1)?.i??1;for(;Goods.get(e);)e++;return e})(),name:t,tags:u,icon:h,color:g,value:d,chance:f,unit:p,demandCoverage:m,multipliers:v(),distribution:_||void 0,biomeOutput:Object.keys(b).length?b:void 0,recipes:E.length?E:void 0}),Goods.sync()),s(n?`Good is updated`:`Good is added`,!1,`success`,5e3),i?.(),$(D).dialog(`close`)}}});function O(){l(`goodEditor`),r(`dialogs`).insertAdjacentHTML(`beforeend`,`<div id="goodEditor" class="dialog">
    <style>
    ...
    <div class="ge">
      <div>
        ...
        <div class="ge-grid">
          ...
          <input id="newGoodName" class="ge-field" value="${n?.name||``}" />

          <label for="newGoodTags">Tags</label>
          <input id="newGoodTags" class="ge-field" value="${n?.tags.join(`, `)||``}" placeholder="comma separated" />

          <label for="newGoodValue">Base Price*</label>
          <span class="ge-inline"><input id="newGoodValue" class="ge-num" type="number" min="0" step="1" value="${n?.value??1}" /> 🟡</span>

          <label for="newGoodChance">Chance</label>
          <input id="newGoodChance" class="ge-num" type="number" min="0" max="100" step="0.1" value="${n?.chance??1}" />

          <label for="newGoodUnit">Unit</label>
          <input id="newGoodUnit" class="ge-field" placeholder="e.g. wagon, barrel" value="${n?.unit||``}" />

          <label for="newGoodIcon">Icon*</label>
          <div class="ge-inline">
            <select id="newGoodIcon" class="ge-icon-select">${u.map(e=>`<option value="${e}" ${n?.icon===e?`selected`:``}>${e}</option>`).join(``)}</select>
            <svg class="ge-icon-preview" width="2em" height="2em">
              <circle id="newGoodIconCircle" cx="50%" cy="50%" r="42%" fill="${n?.color||`#ff5959`}" stroke="${Goods.getStroke(n?.color||`#ff5959`)}"/>
              <use id="newGoodIconPreview" href="#${n?.icon||`good-unknown`}" x="10%" y="10%" width="80%" height="80%"/>
            </svg>
            ...
            <input id="newGoodColor" class="ge-color" type="color" data-tip="Set a stroke color" value="${n?.color||`#ff5959`}" />
          </div>
      ...
      <div>
        ...
        <div class="ge-grid ge-grid--top">
          ...
          <div class="ge-edit-row">
            <div id="newGoodDistribution" class="ge-dist">${n?.distribution||``}</div>
  ...
  </div>`),D=r(`goodEditor`);let e=r(`newGoodRecipeList`),t=pack.goods[0]?.i??0,i=[...pack.goods].sort((e,t)=>e.name.localeCompare(t.name)),a=()=>!Object.values(b).some(e=>(e??0)>0)&&!document.getElementById(`newGoodDistribution`)?.textContent?.trim(),o=()=>{let e=a(),t=E.length===0,n=r(`newGoodRecipeNote`);n.textContent=`This good is raw-only: gathered from the environment.`,n.style.display=t&&!e?``:`none`;let i=r(`newGoodRawNote`);i.textContent=`This good is manufactured-only: made from recipes in burgs.`,i.style.display=e&&!t?``:`none`},s=()=>{e.innerHTML=E.map((e,t)=>`
