	...
	class o extends THREE.EventDispatcher {
		constructor(o, a) {
			super(),
				void 0 === a &&
					console.warn(
						'THREE.MapControls: The second parameter "domElement" is now mandatory.',
					),
				a === document &&
					console.error(
						'THREE.MapControls: "document" should not be used as the target "domElement". Please use "renderer.domElement" instead.',
					),
				(this.object = o),
				(this.domElement = a),
				...
				(this.update = (() => {
					...
					return () => {
						...
						t.copy(o).sub(i.target),
							...
							i.enableDamping
								? ((l.theta += p.theta * i.dampingFactor),
									(l.phi += p.phi * i.dampingFactor))
								: ((l.theta += p.theta), (l.phi += p.phi));
						...
						return (
							isFinite(T) &&
								...
								g < -Math.PI ? (g += b) : g > Math.PI && (g -= b),
								(l.theta =
									T <= g
										? Math.max(T, Math.min(g, l.theta))
										: l.theta > (T + g) / 2
											? Math.max(T, l.theta)
											: Math.min(g, l.theta))),
							(l.phi = Math.max(
							...
							!0 === i.enableDamping
								? ((p.theta *= 1 - i.dampingFactor),
									(p.phi *= 1 - i.dampingFactor),
									E.multiplyScalar(1 - i.dampingFactor))
								: (p.set(0, 0, 0), E.set(0, 0, 0)),
							(m = 1),
							!!(
								u ||
								h.distanceToSquared(i.object.position) > c ||
								8 * (1 - d.dot(i.object.quaternion)) > c
							) &&
								(i.dispatchEvent(e),
					...
					return (t, n) => {
						...
						if (i.object.isPerspectiveCamera) {
							...
							M(
								(2 *
									t *
									(r *= Math.tan(((i.object.fov / 2) * Math.PI) / 180))) /
									o.clientHeight,
								i.object.matrix,
							),
								N((2 * n * r) / o.clientHeight, i.object.matrix);
						} else
			...
			function I(e) {
				...
				A((2 * Math.PI * b.x) / t.clientHeight),
			...
			function U(e) {
				...
				O.subVectors(g, T).multiplyScalar(i.panSpeed), w(O.x, O.y), T.copy(g);
			}
			function V(e) {
				const t = q(e),
					n = e.pageX - t.x,
					o = e.pageY - t.y;
				R.set(0, Math.sqrt(n * n + o * o)),
					H.set(0, (R.y / f.y) ** i.zoomSpeed),
					j(H.y),
					f.copy(R);
			}
			function _(e) {
						...
						: ((e) => {
								...
								switch (n) {
									case THREE.MOUSE.DOLLY: {
										...
										(o = e), f.set(o.clientX, o.clientY), (s = r.DOLLY);
										break;
									}
									...
									switch (s) {
										...
										case r.PAN:
											...
											(n = e),
												...
												O.subVectors(g, T).multiplyScalar(i.panSpeed),
												w(O.x, O.y),
												T.copy(g),
												i.update();
									}
			...
			function Z(e) {
				...
				!1 !== i.enabled &&
					...
					(e.preventDefault(),
					i.dispatchEvent(t),
					(o = e).deltaY < 0 ? S(v()) : o.deltaY > 0 && j(v()),
					i.update(),
					i.dispatchEvent(n));
			}
			function K(e) {
				!1 !== i.enabled &&
					...
					((e) => {
						...
						switch (e.code) {
							case i.keys.UP:
								w(0, i.keyPanSpeed), (t = !0);
								break;
							case i.keys.BOTTOM:
								w(0, -i.keyPanSpeed), (t = !0);
								break;
							case i.keys.LEFT:
								w(i.keyPanSpeed, 0), (t = !0);
								break;
							case i.keys.RIGHT:
								w(-i.keyPanSpeed, 0), (t = !0);
						}
