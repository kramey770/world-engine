import{C as e,Gt as t,L as n,N as r,Q as i,S as a,Tt as o,d as s,k as c,un as l}from"./utils-omhVlsRt.js";import{t as u}from"./layers-DHltDypB.js";import{t as d}from"./sum-BpJJqxFM.js";import{r as f}from"./tooltips-BQ3W48fB.js";import{a as p,t as m}from"./dialog-helpers-CdcIkxap.js";import{P as h,b as g,y as _}from"./index-DWZM8gkY.js";import{t as v}from"./highlighting-D_bHr-1Y.js";import{i as y,n as b,r as x,t as S}from"./table-DM0fGDYA.js";var C=`militaryOverview`,w={my:`right top`,at:`right-10 top+10`,of:`svg`,collision:`fit`},T=[],E=b({getData:P,onUpdate:I});function D(){customization||(m(`#militaryOverview, .stable`),u.show(`states`,`borders`,`military`),O(),E.reset(),$(`#militaryOverview`).dialog({title:`Military Overview`,resizable:!1,width:fitContent(),close:k,position:w}))}function O(){T=j(),document.getElementById(`militaryOverview`)?.remove();let e=`<div id="${C}" class="dialog stable editorDialog">
      ...
      <div id="militaryBottom" class="editorToolbar">
    ...
    </div>`;c(`dialogs`).insertAdjacentHTML(`beforeend`,e),M(),v(`militaryOverview`,({cellId:e})=>pack.cells.state[e]);let t=c(`militaryBody`);c(`militaryOverviewRefresh`).addEventListener(`click`,F),c(`militaryPercentage`).addEventListener(`click`,V),c(`militaryOptionsButton`).addEventListener(`click`,H),c(`militaryRegimentsList`).addEventListener(`click`,()=>A(-1)),c(`militaryOverviewRecalculate`).addEventListener(`click`,G),c(`militaryExport`).addEventListener(`click`,K),c(`militaryWiki`).addEventListener(`click`,()=>i(`Military-Forces`)),t.addEventListener(`change`,e=>{let t=e.target,n=t.closest(`.states`);n&&L(+n.dataset.id,+t.value)}),t.addEventListener(`click`,e=>{let t=e.target,n=t.closest(`.states`);if(!n)return;let r=+n.dataset.id;t.tagName===`SPAN`&&A(r)})}function k(){$(`#militaryOverview`).dialog(`destroy`),c(`militaryOverview`).remove()}async function A(e){h.RegimentsOverview.open(e)}function j(){return[{key:`color`,width:`1.2em`,permanent:!0},{key:`state`,label:`State`,width:`7em`,permanent:!0,sortBy:e=>e.state.name||``,sortType:`alpha`},...options.military.map(e=>({key:`unit:${e.name}`,label:r(e.name.replace(/_/g,` `)),width:`5em`,mobileHidden:!0,tip:`State ${e.name} units number. Click to sort`,sortBy:t=>t.forces[e.name]||0})),{key:`total`,label:`Total`,width:`5em`,defaultSort:`desc`,sortBy:e=>e.total,tip:`Total military personnel (considering crew). Click to sort`},{key:`population`,label:`Population`,width:`6.5em`,mobileHidden:!0,sortBy:e=>e.population},{key:`rate`,label:`Rate`,width:`5em`,sortBy:e=>e.rate,tip:`Military personnel rate (% of state population). Depends on war alert. Click to sort`},{key:`alert`,label:`War Alert`,width:`5.5em`,sortBy:e=>e.alert,tip:`War Alert. Modifier to military forces number, depends on political situation. Click to sort`},{key:`actions`,width:`1.4em`,permanent:!0,align:`right`}]}function M(){_(C,E.reset),S({dialogId:C,columns:T,onUpdate:()=>p(C,{width:`fit-content`,position:w})})}function N(){T=j(),c(`${C}Header`).outerHTML=x({dialogId:C,columns:T}),M(),E.reset()}function P(){let e=pack.states.filter(e=>e.i&&!e.removed).map(e=>{let t=Object.fromEntries(options.military.map(t=>[t.name,(e.military||[]).reduce((e,n)=>e+(n.u[t.name]||0),0)])),n=o(((e.rural||0)+(e.urban||0)*urbanization)*populationRate),r=options.military.reduce((e,n)=>e+(t[n.name]||0)*n.crew,0);return{state:e,forces:t,total:r,population:n,rate:n?r/n*100:0,alert:e.alert??0}});return g(C,e,T)}function F(){E.refresh()}function I(e){let t=c(`militaryBody`),n=t.dataset.type===`percentage`,r=e.all.reduce((e,t)=>{e.total+=t.total,e.population+=t.population;for(let n of options.military)e.units[n.name]=(e.units[n.name]||0)+t.forces[n.name];return e},{total:0,population:0,units:{}}),i=(e,t)=>`${o(t?e/t*100:0)}%`,a=e.rows.map(e=>{let t=options.military.map(t=>{let a=e.forces[t.name]||0;return`<div data-col="${`unit:${t.name}`}" data-tip="State ${t.name} units number">${n?i(a,r.units[t.name]||0):a}</div>`}).join(``);return`<div class="states" data-id="${e.state.i}">
        ...
        ${t}
        <div data-col="total" data-tip="Total state military personnel (considering crew)" style="font-weight:bold">${n?i(e.total,r.total):s(e.total)}</div>
        <div data-col="population" data-tip="State population">${n?i(e.population,r.population):s(e.population)}</div>
        <div data-col="rate" data-tip="Military personnel rate (% of state population). Depends on war alert">${o(e.rate,2)}%</div>
        <input data-col="alert" data-tip="War Alert. Editable modifier to military forces number, depends on political situation" type="number" min="0" step=".01" value="${o(e.alert,2)}" />
        <div data-col="actions"><span data-tip="Show regiments list" class="icon-list-bullet pointer"></span></div>
      </div>`}).join(``);t.querySelectorAll(`:scope > .states`).forEach(e=>{e.remove()}),t.insertAdjacentHTML(`beforeend`,a),R(e),y(c(`militaryFooter`),e,E.goto),t.querySelectorAll(`:scope > .states`).forEach(e=>{e.addEventListener(`mouseenter`,z),e.addEventListener(`mouseleave`,B)}),p(C,{width:`fit-content`,position:w})}function L(e,t){let n=pack.states[e],r=n.alert??1,i=r?t/r:0;n.alert=t,(n.military||[]).forEach(e=>{Object.keys(e.u).forEach(t=>{e.u[t]=o(e.u[t]*i)}),e.a=d(Object.values(e.u)),l(`#armies > g > g#regiment${n.i}-${e.i} > text`).text(Military.getTotal(e))}),E.refresh()}function R(e){let t=e.all.length,n=d(e.all.map(e=>e.total));c(`militaryFooterStates`).innerHTML=String(t),c(`militaryFooterForcesTotal`).innerHTML=s(n),c(`militaryFooterForces`).innerHTML=s(t?n/t:0),c(`militaryFooterRate`).innerHTML=`${o(t?d(e.all.map(e=>e.rate))/t:0,2)}%`,c(`militaryFooterAlert`).innerHTML=String(o(t?d(e.all.map(e=>e.alert))/t:0,2))}function z(e){let n=+e.target.dataset.id;if(customization||!n||(l(`#armies > g > g#army${n}`).transition().duration(2e3).style(`fill`,`#ff0000`),!u.isOn(`states`)))return;let r=l(`#regions`).select(`#state${n}`).attr(`d`),i=l(`#debug`).append(`path`).attr(`class`,`highlight`).attr(`d`,r).attr(`fill`,`none`).attr(`stroke`,`red`).attr(`stroke-width`,1).attr(`opacity`,1).attr(`filter`,`url(#blur1)`),a=i.node().getTotalLength(),o=(a+5e3)/2,s=t(`0,${a}`,`${a},${a}`);i.transition().duration(o).attrTween(`stroke-dasharray`,()=>e=>s(e))}function B(e){l(`#debug`).selectAll(`.highlight`).each(function(){l(this).transition().duration(1e3).attr(`opacity`,0).remove()});let t=+e.target.dataset.id;l(`#armies > g > g#army${t}`).transition().duration(1e3).style(`fill`,null)}function V(){let e=c(`militaryBody`);e.dataset.type=e.dataset.type===`absolute`?`percentage`:`absolute`,E.refresh()}function H(){U();let e=[`melee`,`ranged`,`mounted`,`machinery`,`naval`,`armored`,`aviation`,`magical`],t=c(`militaryOptions`).querySelector(`tbody`);if(r(),options.military.map(e=>s(e)),$(`#militaryOptions`).dialog({title:`Edit Military Units`,resizable:!1,width:fitContent(),position:{my:`center`,at:`center`,of:`svg`},close:W,buttons:{Apply:d,Add:()=>s({icon:`🛡️`,name:`custom${c(`militaryOptionsTable`).rows.length}`,rural:.2,urban:.5,crew:1,power:1,type:`melee`,separate:0}),Restore:l,Cancel:function(){$(this).dialog(`close`)}},open:function(){let e=$(this).dialog(`widget`).find(`.ui-dialog-buttonset > button`);e[0].addEventListener(`mousemove`,()=>f(`Apply military units settings. <span style='color:#cb5858'>All forces will be recalculated!</span>`)),e[1].addEventListener(`mousemove`,()=>f(`Add new military unit to the table`)),e[2].addEventListener(`mousemove`,()=>f(`Restore default military units and settings`)),e[3].addEventListener(`mousemove`,()=>f(`Close the window without saving the changes`))}}),modules.overviewMilitaryCustomize)return;modules.overviewMilitaryCustomize=!0,t.addEventListener(`click`,e=>{let t=e.target;if(t.tagName!==`BUTTON`)return;let n=t.dataset.type;if(n===`icon`){h.IconSelector.open(t.textContent||``,e=>{t.innerHTML=e.startsWith(`http`)||e.startsWith(`data:image`)?`<img src="${e}" style="width:1.2em;height:1.2em;pointer-events:none;">`:e});return}if(n===`biomes`){u(t,pack.biomes.filter(e=>!e.removed).map(({i:e,name:t,color:n})=>({i:e,name:t,color:n})));return}if(n===`states`)return u(t,pack.states);if(n===`cultures`)return u(t,pack.cultures);if(n===`religions`)return u(t,pack.religions)});function r(){t.querySelectorAll(`tr`).forEach(e=>{e.remove()})}function i(e){return e?.join(`,`)||``}function a(e){return e?.length?`some`:`all`}function o(e,t){return e?.length?e.map(e=>t?.[e]?.name||``).join(`, `):``}function s(n){let{type:r,icon:s,name:c,rural:l,urban:u,power:d,crew:f,separate:p}=n,m=document.createElement(`tr`),h=e.map(e=>`<option ${r===e?`selected`:``} value="${e}">${e}</option>`).join(` `),g=e=>{let t=e===`biomes`?[]:pack[e];return`<button
        ...
        </td>`,t.appendChild(m)}function l(){r(),Military.getDefaultOptions().map(e=>s(e))}function u(e,t){let n=e.dataset.type,r=e.dataset.value,i=r?r.split(`,`).map(e=>+e):[],a=t.filter(e=>e.i&&!e.removed).map(({i:e,name:t,fullName:n,color:r})=>`
          <tr data-tip="${t}">
    ...
    </div>`)}function W(){$(`#militaryOptions`).dialog(`destroy`),c(`militaryOptions`).remove()}function G(){c(`alertMessage`).innerHTML=`Are you sure you want to recalculate military forces for all states?<br>Regiments for all states will be regenerated`,$(`#alert`).dialog({resizable:!1,title:`Recalculate military`,buttons:{Recalculate:function(){$(this).dialog(`close`),Military.generate(),u.draw(`military`),F()},Cancel:function(){$(this).dialog(`close`)}}})}function K(){let t=options.military.map(e=>e.name),n=`Id,State,${t.map(e=>r(e)).join(`,`)},Total,Population,Rate,War Alert\n`;for(let e of P())n+=`${e.state.i},${e.state.name},${t.map(t=>e.forces[t]||0).join(`,`)},${e.total},${e.population},${o(e.rate,2)}%,${e.alert}\n`;let i=`${e(`Military`)}.csv`;a(n,i)}var q={open:D};export{q as MilitaryOverview};