...
{
	const systemOptions = systemPresets.map(
		(styleName) => `<option value="${styleName}">${styleName}</option>`,
	);
	const storedStyles = Object.keys(localStorage).filter((key) =>
		key.startsWith(customPresetPrefix),
	);
	const customOptions = storedStyles.map(
		(styleName) =>
			`<option value="${styleName}">${styleName.replace(customPresetPrefix, "")} [custom]</option>`,
	);
...
async function getStylePreset(desiredPreset) {
	...
	if (isCustom) {
		...
		if (!storedStyleJSON) {
			ERROR &&
				console.error(
					`Custom style ${desiredPreset} in not found in localStorage. Applying default style`,
				);
			...
			ERROR &&
				console.error(
					`Custom style ${desiredPreset} stored in localStorage is not valid. Applying default style`,
				);
			presetToLoad = "default";
		}
	}
...
function applyStylePreset(presetJson) {
	for (const selector in presetJson) {
		...
		if (selector.startsWith("#labels > #")) {
			...
			style.labels.groups[labelGroup] = getStyleAttributes(
				presetJson[selector],
			);
		}
		...
		const el = labelGroup
			? document.querySelector(
					`#labels > [data-group="${CSS.escape(labelGroup)}"]`,
				)
		...
		for (const attribute in presetJson[selector]) {
			...
			if (
				selector === "#terrain" &&
				RELIEF_STYLE_ATTRIBUTES.includes(attribute)
			)
				continue; // stored in style.relief
			...
			if (
				selector === "#terrs" &&
				attribute === "scheme" &&
				!(value in heightmapColorSchemes)
			) {
	...
	for (const group of options.labels.groups) {
		...
		const defaultGroupStyle =
			style.labels.groups[Labels.getFallbackGroup(group.type).name];
		if (defaultGroupStyle)
			style.labels.groups[group.name] = { ...defaultGroupStyle };
	}
...
function addStylePreset() {
	...
	document
		...
		.addEventListener("change", loadStyleFile);
	...
	function collectStyleData() {
		...
		for (const selector in attributes) {
			...
			for (const attr of attributes[selector]) {
				...
				// markets use data-size for the marker circle, so keep its real font-size
				if (
					attr === "font-size" &&
					selector !== "#markets" &&
		...
		if (presetStyle["#terrain"])
			Object.assign(presetStyle["#terrain"], style.relief);
	...
	function checkName() {
		...
		const isSystem =
			systemPresets.includes(styleName) ||
			systemPresets.includes(styleSaverName.value);
		if (isSystem) return (styleSaverTip.innerHTML = "default");

		const isExisting = Array.from(stylePreset.options).some(
	...
	function loadStyleFile() {
		...
		function styleUpload(dataLoaded) {
			...
			if (!isValid)
				return tip(
					"Loaded data is not a valid JSON, please check the format",
					false,
					"error",
				);
...
function requestRemoveStylePreset() {
	...
	confirmationDialog({
		...
		message:
			"Are you sure you want to remove the style preset? This action cannot be undone.",
		confirm: "Remove",
